...
rollback;
commit;
select *  from ttkd_bsc.dm_daily_khdn where thang=202401 and ma_daily in ('GTGT00181', 'GTGT00182', 'GTGT00186', 'GTGT00194', 'GTGT00185', 'GTGT00128');
   and ma_nguoigt in ('GTGT00165','GTGT00166','GTGT00182','DL_HOANGKIM','GTGT00173','GTGT00164','GTGT00083','GTGT00125', 'GTGT00194');
...
desc ttkd_bsc.dm_daily_khdn;
		...
		insert into ttkd_bsc.dm_daily_khdn (THANG, MA_DAILY, TEN_DAILY, MANV_QLDAILY, THANG_KYHD, LOAI_HOPDONG)		
					select 202404 thang, ma_nv  MA_DAILY, ten_nv TEN_DAILY, 'CTV077029' MANV_QLDAILY, 202404 THANG_KYHD, 'DAI LY MOI' LOAI_HOPDONG
					from admin_hcm.nhanvien where ma_nv = 'GTGT00195'
		;
		----Dai ly moi
		update  ttkd_bsc.dm_daily_khdn 
			set thang_kyhd = thang, MANV_QLDAILY= 'CTV084801'
			where thang=202404 and ma_daily in ('GTGT00193')
		;
		---Dai  ly hien huu co ban moi
		update  ttkd_bsc.dm_daily_khdn 
			set thang_kyhd = thang, loai_hopdong = 'PHU LUC MOI', MANV_QLDAILY= 'CTV084801'
			where thang=202404 and ma_daily in ('GTGT00193')
		;
		---update maTo, PB, VTCV
		update ttkd_bsc.dm_daily_khdn 
			set (ma_to, ma_pb, ma_vtcv, ten_vtcv)= (select ma_to, ma_pb, ma_vtcv, ten_vtcv from ttkd_bsc.nhanvien_202404 where ma_nv = MANV_QLDAILY)
			where thang=202404;
...
delete from ttkd_bsc.dm_daily_khdn where thang=202404  --thang n
	;
			insert into ttkd_bsc.dm_daily_khdn (thang, ma_daily,ten_daily,manv_qldaily,ma_pb, thang_kyhd,ma_to)
			    select 202404 thang, ma_daily,ten_daily,manv_qldaily,ma_pb,thang_kyhd,ma_to		---thang n
			    from ttkd_bsc.dm_daily_khdn a
			    where thang=202403 		---thang n-1
						and not exists(select 1 from ttkd_bsc.dm_daily_khdn 
											where thang=202404		---thang n 
...
-- Dai ly moi:
		insert into ttkd_bsc.dm_daily_khdn (ma_daily,ten_daily,ma_pb, ma_to, manv_qldaily, thang, thang_kyhd, loai_hopdong )
		    select distinct ma_daily, (select upper(ten_nv) from admin_hcm.nhanvien_onebss where ma_nv=a.ma_daily) ten_daily,
					 ma_pb, ma_to, ma_nv, 202404, 202404, 'DAI LY MOI'	---thang n
		    from 
					   (select distinct
						...
						union all
								...
								from ttkd_bsc.dt_ptm_vnp_202404 		---thang n, file anh Tuyen tao
...
-- chi tiet tb ptm trong thang:
	...
	select * from ttkd_bsc.ct_ptm_daily_khdn where thang=202404;

			delete from ttkd_bsc.ct_ptm_daily_khdn where thang=202404		---thang n
				;
		insert into ttkd_bsc.ct_ptm_daily_khdn 
				...
				select thang_ptm thang, dich_vu, tenkieu_ld, ma_gd, ma_tb, thuebao_id, chuquan_id, 
										  ...
										  (select ma_to from ttkd_bsc.nhanvien_202404 where ma_nv=a.ma_nv) ma_to,
										  (select ma_pb from ttkd_bsc.nhanvien_202404 where ma_nv=a.ma_nv) ma_pb, ma_daily
				from (select  thang_ptm, dich_vu, tenkieu_ld, a.ma_gd, a.ma_tb, a.thuebao_id, a.chuquan_id
						  ...
						  from ttkd_bsc.ct_bsc_ptm a
						  where a.thang_ptm = 202404 ---THANG N
					   ...
					   from ttkd_bsc.ct_bsc_ptm a
					   where loaihd_id = 41 
									  and (ma_tiepthi in (select ma_daily from ttkd_bsc.dm_daily_khdn where thang = 202404 )
														or ma_nguoigt in (select ma_daily from ttkd_bsc.dm_daily_khdn where thang= 202404) 
														or nguoi_gt in (select ma_daily from ttkd_bsc.dm_daily_khdn where thang= 202404)
											)
									  and dthu_ps is null and dthu_ps_n1 is not null
									  and exists (select 1 from css_hcm.db_datcoc dc
														 where thang_bd = 202404 and cuoc_dc>0 and hieuluc=1 and ttdc_id = 0 --tien_thoai is null 
		...
		----
			update ttkd_bsc.ct_ptm_daily_khdn a
			    set (ma_nv, ma_to, ma_pb) = (select nv.ma_nv, nv.ma_to, nv.ma_pb 
																	from ttkd_bsc.dm_daily_khdn dm, ttkd_bsc.nhanvien_202404 nv 
																	where dm.manv_qldaily=nv.ma_nv and dm.thang = a.thang and dm.ma_daily=a.ma_tiepthi)
			    where thang=202404 and ma_tiepthi = ma_nguoigt
...
-- Xac dinh AM QLDL doi voi VNPTS
		update ttkd_bsc.ct_ptm_daily_khdn a
		    set (ma_nv, ma_to, ma_pb) = (select nv.ma_nv, nv.ma_to, nv.ma_pb from ttkd_bsc.dm_daily_khdn dm, ttkd_bsc.nhanvien_202404 nv 
															where dm.manv_qldaily=nv.ma_nv and dm.thang=202404 and dm.ma_daily=a.ma_nguoigt)
		    where thang=202404 and dich_vu in ('VNPTS','VCC') and ma_tiepthi is null and ma_nv is null
...
--											and ma_vtcv = a.ma_vtcv)
			...
			update ttkd_bsc.bangluong_kpi_202404 a
			   set HCM_DT_DAILY_003=(select round(sum(dthu_kpi)/1000000, 3) from ttkd_bsc.ct_ptm_daily_khdn
													where thang =202404 and ma_nv = a.ma_nv)
			...
			update ttkd_bsc.bangluong_kpi_202404 a
				   set HCM_DT_DAILY_003=(select round(sum(dthu_kpi)/1000000, 3) from ttkd_bsc.ct_ptm_daily_khdn
													  where thang=202404 and ma_to=a.ma_to)
			    where exists (select * from ttkd_bsc.blkpi_danhmuc_kpi_vtcv
...
--										  where ma_to=a.ma_to and ma_vtcv='VNP-HNHCM_KHDN_3.1')
			...
			commit;
				...
				from ttkd_bsc.bangluong_kpi_202404 a where HCM_DT_DAILY_003 is not null
				;
...
-- 2/ HCM_SL_DAILY_001:  So luong Dai ly hien huu co phat trien them dich vu moi va co phat sinh doanh thu trong thang
			...
			select distinct a.*, b.*, b.ten_vtcv from ttkd_bsc.blkpi_danhmuc_kpi a, ttkd_bsc.blkpi_danhmuc_kpi_vtcv b, ttkd_bsc.nhanvien_202403 c  
				   where a.ma_kpi=b.ma_kpi and b.ma_vtcv=c.ma_vtcv       
							and a.thang_kt is null and b.thang_kt is null 
...
--										  where thang=202403 and ma_nv=a.ma_nv_hrm)
				...
				update ttkd_bsc.bangluong_kpi_202404 a
				   set HCM_SL_DAILY_001=(select count(distinct ma_daily) 
															    ...
															    where thang=202404 and dthu_kpi>0
...
-- Truong Line:                                    
			...
			update ttkd_bsc.bangluong_kpi_202404 a
			    set HCM_SL_DAILY_001=(select count(distinct ma_daily) 
																from ttkd_bsc.ct_ptm_daily_khdn b
																    where b.thang=202404 and dthu_kpi>0
																		 and exists (select 1 from ttkd_bsc.dm_daily_khdn
																								 where thang_kyhd= b.thang and loai_hopdong='PHU LUC MOI' 
																											and ma_daily=b.ma_daily
																						)   
...
--										  where ma_to=a.ma_to and ma_vtcv='VNP-HNHCM_KHDN_3.1')
				...
				from ttkd_bsc.bangluong_kpi_202404 a where HCM_SL_DAILY_001 is not null
				;
...
--- 3/ HCM_SL_DAILY_002: So luong Dai ly moi phat trien va co phat sinh doanh thu trong thang
			...
			select distinct a.*, b.*, ten_vtcv from ttkd_bsc.,blkpi_danhmuc_kpi a, ttkd_bsc..blkpi_danhmuc_kpi_vtcv b, ttkd_bsc..nhanvien_202403 c  
				   where a.ma_kpi=b.ma_kpi and b.ma_vtcv=c.ma_vtcv       
							and a.thang_kt is null and b.thang_kt is null 
