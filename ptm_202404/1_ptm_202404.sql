select * from ptm_codinh_202404;  
drop table ptm_codinh_202404 purge;
create table ptm_codinh_202404 as
		...
		, km_sd as (select a.thuebao_id, max(a.tyle_sd) tyle_sd 
								from css.v_khuyenmai_dbtb a, css.v_ct_khuyenmai b
								  where a.chitietkm_id = b.chitietkm_id and a.thang_bd >= 202404		---thang n
		...
		, km_tb as (select a.thuebao_id, max(a.tyle_tb) tyle_tb 
						  from css.v_khuyenmai_dbtb a, css.v_ct_khuyenmai b
						  where a.chitietkm_id = b.chitietkm_id and a.thang_bd >= 202404
											and a.hieuluc=1 and ttdc_id = 0
		...
		, datcoc_goc as (select hd.hdtb_id, dc.thang_bd thang_bddc, dc.thang_kt thang_ktdc, dc.CUOC_DC DATCOC_CSD
            ...
            --                                                                , sum(cuoc_dc) datcoc_csd, sum(tien_td) tien_td 
                                                            ...
                                                            where hd.hdtb_id=dc.hdtb_id and hd.tthd_id = 6 and to_number(to_char(hd.ngay_ht,'yyyymm')) = 202404
                                                                            and dc.cuoc_dc>0 and (dc.nhom_datcoc_id not in (15,19, 20, 22, 24, 40) or dc.nhom_datcoc_id is null)
                                                                            and kmb.khuyenmai_id in (1977, 2056, 2998, 2999) and kmb.chitietkm_id = dc.chitietkm_id-- and dc.nhom_datcoc_id =  ndc.nhom_datcoc_id (+)
            ...
            --                                                    , sum(a.datcoc_csd) datcoc_csd,sum(a.tien_td) tien_td
                                                     ...
                                                     where c.hdtb_id=a.hdtb_id and a.chitietkm_id = b.chitietkm_id-- and dc.nhom_datcoc_id =  ndc.nhom_datcoc_id (+)
                                                                        ...
                                                                        and to_number(to_char(c.ngay_ht,'yyyymm')) = 202404
...
--                , round(datcoc.datcoc_csd/1.1,0) tien_td
   ...
   from  css.v_hd_khachhang a, css.v_hd_thuebao b, v_db, v_hdtb, tinhcuoc.v_dbtb db_old
         ...
         where a.hdkh_id = b.hdkh_id and b.thuebao_id = v_db.thuebao_id  and db_old.ky_cuoc (+) = 20240301 and b.thuebao_id = db_old.thuebao_id (+)
                ...
                and ( (a.loaihd_id=1 and b.kieuld_id not in (540,541,557,249,13130,13222,13224,71,280,550,551,13235, 14069) )  
                 ...
                 and trunc(b.ngay_ht) >= to_date('01/04/2024','dd/mm/yyyy') 
                and trunc(b.ngay_ht) <= to_date('30/04/2024','dd/mm/yyyy')
--                and b.ma_tb = 'LL001075589'
             ...
             select * from ptm_codinh_202404
                where (ma_tb, ma_gd, hdtb_id) in 
                                                ...
                                                from ptm_codinh_202404 a 
                                                group by ma_tb, ma_gd, hdtb_id having count(*)>1 )
            ;
--Them cot------ tien_dnhm, tien_camket, tien_sodep:
                    ALTER TABLE ptm_codinh_202404 ADD (tien_dnhm NUMBER, tien_sodep NUMBER, tien_camket NUMBER)
				;
                    UPDATE ptm_codinh_202404 a 
                ...
                commit;
                    ...
                    MERGE INTO ptm_codinh_202404 a
                    USING (with dnhm as (SELECT dnhm1.khoanmuctt_id loai,dnhm1.hdtb_id, dnhm1.phieutt_id
                                                                          ...
                                                                          WHERE hd.hdtb_id=dnhm1.hdtb_id AND dnhm1.khoanmuctt_id IN(1,2,13,17)
                                                                                AND TO_CHAR(hd.ngay_ht,'yyyymm') = '202404'
                                                                       )
                                    ...
                                    FROM css.v_phieutt_hd hd1,       
                                                (SELECT decode(ct.khoanmuctt_id,19,dnhm.loai,20, thietbi.loai,ct.khoanmuctt_id) loai,     
                                                    ...
                                                    WHERE EXISTS(SELECT 1 FROM css.v_hd_thuebao WHERE TO_CHAR(ngay_ht,'yyyymm') ='202404' AND hdtb_id=ct.hdtb_id)
                                                ) ct      
...
-- Mucuoc_tb:
	   ...
	   ;
	   select * from temp_muccuoctb_thuebao where thuebao_id = 9038880;
        create table temp_muccuoctb_thuebao as
                with km_dbtb as (select thuebao_id, tyle_tb, cuoc_tb, tien_tb , ngay_sd
												from css.v_khuyenmai_dbtb
                                                          where phanvung_id=28 and hieuluc=1 and ttdc_id = 0 and (tyle_tb>0 or cuoc_tb>0 or tien_tb>0) and thang_bd >=202404 
														--and to_number(to_char(ngay_sd, 'yyyymm')) = 202404
                                                       ) 
					, hdtb as (select hdtb.thuebao_id, row_number() over (partition by hdtb.thuebao_id order by hdtb_id desc) rnk
									...
									where hdkh.loaihd_id in (1,6,31,41,49) and hdtb.dichvuvt_id in (13,14,15,16) and hdtb.tthd_id = 6
													 and hdtb.loaitb_id = 39 --and hdtb.thuebao_id = 12193228
													and to_number(to_char(hdtb.ngay_ht,'yyyymm')) = 202404
									)
            ...
            from hdtb
                      ,css.v_db_thuebao db, css.v_db_cntt dbc, css.v_muccuoc_tb mc
                      , km_dbtb
            ...
            where dbc.phanvung_id=28 and mc.phanvung_id=28 
			 ...
			 and db.loaitb_id = 39 --and db.thuebao_id = 12193228
			 and hdtb.rnk = 1
                ;
			 ...
			 ;
                create table temp_muccuoctb_hdtb as
                    with km_hdtb as (select hdtb1.hdtb_id, km_hdtb1.tyle_tb, km_hdtb1.cuoc_tb,  km_hdtb1.tien_tb
                                                            ...
                                                            where hdtb1.hdtb_id = km_hdtb1.hdtb_id
                                                                and hdtb1.tthd_id = 6 and to_number(to_char(hdtb1.ngay_ht,'yyyymm')) =202404
                ...
                from css.v_hd_khachhang hdkh, css.v_hd_thuebao hdtb, css.v_hdtb_cntt hdtbc, css.v_muccuoc_tb mc
                    ...
                    where hdkh.phanvung_id=28 and hdtb.phanvung_id=28 and hdtbc.phanvung_id=28 and mc.phanvung_id=28
                        ...
                        and hdtb.tthd_id=6 and hdkh.loaihd_id in (1,6,31,41,49) and to_number(to_char(hdtb.ngay_ht,'yyyymm')) =202404
			 ...
			 select hdtb_id,   count(*) from temp_muccuoctb_hdtb group by hdtb_id having count(*)>1;
                ...
                update ptm_codinh_202404 a set muccuoc_tb ='';
                ...
                update ptm_codinh_202404 a 
                    ...
                    commit;
--                        
				select * from ptm_codinh_202404 where thuebao_id = 8278567;
...
-- tien_dvgt theo hdtb_id lay thoi gian n-1
            ...
            create table temp_tien_dvgt_theohdtb as
                with b as (select hd.hdtb_id, km.chitietkm_id, km.cuoc_sd, km.tyle_sd, km.tien_sd, ctkm.dichvugt_id 
                                                ...
                                                where hd.hdtb_id=km.hdtb_id and km.chitietkm_id=ctkm.chitietkm_id and km.hieuluc=1 and ttdc_id = 0 --and thang_bd>=202403
                                                    and nvl(km.tyle_tb,0)=0 and nvl(km.cuoc_tb,0)=0 and nvl(km.tien_tb,0)=0
                                                    and to_number(to_char(hd.ngay_ht,'yyyymm')) >=202403 
									)
                    select hdtb_id, sum(cuoc_sd) cuoc_sd
                        from (select a.hdtb_id, a.ma_tb, b.chitietkm_id, d.noidung soluong, d.cuoc_sd cuoc_sd_goc, b.cuoc_sd cuoc_giam_1donvi, b.tyle_sd, b.tien_sd                        
						    ...
						    where e.phanvung_id=28 and a.hdtb_id=d.hdtb_id and d.dichvugt_id=e.dichvugt_id                                              
								  ...
								  and to_number(to_char(a.ngay_ht,'yyyymm')) >=202403
						    ) group by hdtb_id
                ;     
            ...
            drop table temp_tien_dvgt_theothuebao purge;
                ...
                ;
                    ...
                    update ptm_codinh_202404 set tien_dvgt='';
                    update ptm_codinh_202404 a 
                        ...
                        -- select tien_dvgt from ptm_codinh_202402 a
...
-- tien_tbi: 
                    UPDATE ptm_codinh_202404 SET tien_tbi='';
                    MERGE INTO ptm_codinh_202404 a
                    USING (SELECT cttbi.hdtb_id, tbi_lhtb.loaitb_id, SUM(CASE WHEN tien_tratruoc>0 THEN cttbi.soluong*cttbi.tien_tratruoc ELSE cttbi.soluong*cttbi.tien END) tien_tbi
                                    ...
                                    WHERE cttbi.loaitbi_id=tbi.loaitbi_id AND tbi.loaitbi_id=tbi_lhtb.loaitbi_id AND lhtb.loaitb_id=tbi_lhtb.loaitb_id
                                                        ...
                                                        AND EXISTS(SELECT 1 FROM ptm_codinh_202404 WHERE hdtb_id=cttbi.hdtb_id AND loaitb_id=tbi_lhtb.loaitb_id)
		     ...
		     with datcoc_goc as (select b.hdtb_id, a.thuebao_id , a.thang_bd thang_bddc, a.thang_kt thang_ktdc, a.cuoc_dc datcoc_csd, a.tien_td, ctkm.huong_dc
                                                                   from css.v_db_datcoc a, css.v_hdtb_datcoc b, css.v_ct_khuyenmai ctkm
                                                                    where a.thuebao_dc_id=b.thuebao_dc_id and a.chitietkm_id = ctkm.chitietkm_id and ctkm.khuyenmai_id not in (1977, 2056, 2998, 2999)
                                                                                and a.cuoc_dc > 0 and a.hieuluc = 1 and a.ttdc_id = 0 and b.thang_bd >= 202404			--thang n
...
--                                                                    group by a.hdtb_id, a.thuebao_id, a.thang_bddc,a.thang_ktdc     
                                ...
                                , datcoc as (select max(hdtb_id) hdtb_id, thuebao_id, min(thang_bddc) thang_bddc, max(thang_ktdc) thang_ktdc, max(huong_dc) huong_dc, sum(datcoc_csd) datcoc_csd, sum(tien_td) tien_td
                                                        from datcoc_goc where datcoc_csd > 0 and thang_bddc >= 202404
                                                        group by  thuebao_id
                                                        )
...
--                                , round(datcoc.tien_td/1.1,0) tien_td
                ...
                where a.hdkh_id=b.hdkh_id and a.ctv_id=c.nhanvien_id(+) and b.hdtb_id = datcoc.hdtb_id
                        ...
                        and to_number(to_char(b.ngay_ht,'yyyymm')) = 202404		---thang n
                        and exists(select 1 from ptm_codinh_202404 where datcoc_csd is null and thuebao_id=b.thuebao_id and ma_tiepthi=c.ma_nv )
				--and b.ma_tb = 'hcm_ca_00067906'
				    ;
                         
       select * from css.v_hd_thuebao where hdtb_id = 24483191;
	  select * from css.kieu_ld;
   -- Kiem tra trung truoc khi update bs datcoc:
            select * from temp_datcoc
            where thuebao_id in 
                (select thuebao_id from temp_datcoc  a
                where exists(select 1 from ptm_codinh_202404 where datcoc_csd is null and thuebao_id=a.thuebao_id and ma_tiepthi=a.ma_nv)  
                group by  thuebao_id having count(*)>1)
                ;
 
            --them cot hdtb_id_datcoc
            alter table ptm_codinh_202404 add hdtb_id_datcoc number
		  ;
		  
            update ptm_codinh_202404 a set (hdtb_id_datcoc, datcoc_csd, tien_td, thang_bddc, thang_ktdc, sothang_dc)=
                ...
                where datcoc_csd is null 
                    ...
                    and ngay_bbbg=(select max(ngay_bbbg) from ptm_codinh_202404 where thuebao_id=a.thuebao_id )
                    ;
                    
            ---recover datcoc_csd updated
                update ptm_codinh_202404 set datcoc_csd = null where hdtb_id_datcoc is not null;
            commit;
            
            select a.*, b.ma_tiepthi , b.DATCOC_CSD, b.SOTHANG_DC, b.hdtb_id_datcoc
            from temp_datcoc a, ptm_codinh_202404 b
            where a.thuebao_id=b.thuebao_id
            ;
            
------ thoihan_id
            update ptm_codinh_202404 set thoihan_id=1 
                where (thoihan_id<>1 or thoihan_id is null) and (tg_thue_tu is not null and tg_thue_den is not null )
                        and thoihan_id is null
            ;
		  ---move ve TTKDDB
		  create table ttkd_bct.ptm_codinh_202404 as select * from ttkd_hcm.ptm_codinh_202404@dataguard;
		  -- lydo_khongtinh_luong: Update ky luong T3/24
						alter table ptm_codinh_202404 add lydo_khongtinh_luong varchar2(200)
						;
						update ptm_codinh_202404 a
									set lydo_khongtinh_luong = (
														   with x as (
															  select hdtb_id
																		   ...
																		   ,case when (upper(ma_tiepthi)='DL_CNT' or upper(ma_tiepthi)like'GTGT_%' 
															  ...
															  from ptm_codinh_202404 b
--															  where (thuebao_id, hdtb_id) in (select thuebao_id, hdtb_id from hocnq_ttkd.ptm_xuly_khieunai_202404)
															)                                        
...
--khong chay nay----- lydo_khongtinh_luong:
      /*          alter table ptm_codinh_202404 add lydo_khongtinh_luong varchar2(200);
                update ptm_codinh_202404 a set lydo_khongtinh_luong=''
                ;
			 
                update ptm_codinh_202404 a 
                        set lydo_khongtinh_luong= lydo_khongtinh_luong||'; Chu quan khong thuoc TTKD-HCM'
                    where chuquan_id not in (145,266,264) ;
                    
                update ptm_codinh_202404 a 
                        set lydo_khongtinh_luong= lydo_khongtinh_luong||'; Thue bao khong co chu quan'
                    where chuquan_id is null;
                
                update ptm_codinh_202404 a 
                        set lydo_khongtinh_luong= lydo_khongtinh_luong||'; Hop dong GTGT/CNTT da thanh ly'
                    where dichvuvt_id in (13,14,15,16) and loaitb_id not in (90,146) and ngay_cat is not null;
                    
                update ptm_codinh_202404 a 
                        set lydo_khongtinh_luong=lydo_khongtinh_luong||'; Doi tuong nghiep vu'
                    where a.doituong_id in (14,15,16,17,19,32,33,34,35,36,48,51,52,54,55,56,57,66); 
                    
                update ptm_codinh_202404 a 
                        set lydo_khongtinh_luong= lydo_khongtinh_luong||'; TSL doi tuong VMS, STTTT, Bo CA'
                    where a.doituong_id in (47,49,50) and a.dichvuvt_id in (7,8,9);
                        
                update ptm_codinh_202404 a 
                        set lydo_khongtinh_luong=lydo_khongtinh_luong||'; Thue bao dung thu'
                    where a.doituong_id in (190)
				; 
                update ptm_codinh_202404 a 
                        set lydo_khongtinh_luong=lydo_khongtinh_luong||'; Dich vu khong tinh luong'
                    where exists(select 1 from ttkd_bsc.dm_loaihinh_hsqd where loaitru_tinhluong=1 and loaitb_id=a.loaitb_id)
                    ;
                        
                        ----Dai ly Cao  Nhat Thanh tinh Phong, To, con cac dai ly khac tinh Phong
                update ptm_codinh_202404 a 
                        set lydo_khongtinh_luong = lydo_khongtinh_luong||'; Phat trien qua Dai ly, chi tinh bsc cho Phong'
                    where (upper(ma_tiepthi)='DL_CNT' or upper(ma_tiepthi)like'GTGT_%' 
                                          or upper(ma_tiepthi)like'DAILY_%' or upper(ma_tiepthi)like'DL_%'
                                          or upper(ma_nguoigt)like'DL_CNT' or upper(ma_nguoigt)like'GTGT_%' 
                                          or upper(ma_nguoigt)like'DAILY_%' or upper(ma_nguoigt)like'DL_%')
								  ; 
                ...
                update ptm_codinh_202404 a 
                        set lydo_khongtinh_luong = lydo_khongtinh_luong||'; TB ngan han nhung khong thu tien DNHM dung qui dinh'
                    where thoihan_id=1 
                        and ( (loaitb_id in (58,59) and tien_dnhm<2000000) or (dichvuvt_id in (7,8,9) and tien_dnhm<3000000) ) 
				    ;
                
                update ptm_codinh_202404 a          -- Gia han VNPT SmartCloud theo VB 167/TTr-NS-DH 23/05/2022: chi ghi nhan khi gia han goi 6 thang tro len
                        set lydo_khongtinh_luong = lydo_khongtinh_luong||'; Ghi nhan dthu khi gia han goi>=6 thang' 
                        --select loaitb_id, sothang_dc, lydo_khongtinh_luong from ttkd_bct.ptm_codinh_202404
                    where loaihd_id=41 and loaitb_id=153 and sothang_dc<6 or sothang_dc is null
                    ;
             */       
                    rollback;
                    commit;
				...
				-- select loaitb_id thang, hdtb_id, thuebao_id, loaitb_id, ma_tiepthi, pbh_nhan_id, nhanvien_id, datcoc_csd, ghi_chu  from hocnq_ttkd.ptm_xuly_khieunai_BHOL_202404
						 select 202404 thang, hdtb_id, thuebao_id, loaitb_id, ma_tiepthi, pbh_nhan_id
				 ...
				 ;
				update ttkd_bct.ptm_codinh_202404 a 
				    set ma_tiepthi =(select ma_nv from admin_hcm.nhanvien where nhanvien_id=a.nhanvien_id)  --manv_ptm='VNP017772'
				    -- select * from ttkd_bct.ptm_codinh_202404 a
				    where loaitb_id=288 and ma_tiepthi is null 
					   and pbh_nhan_id=2941 and nhanvien_id=1077 and datcoc_csd>0
					   ; 
				...
				update ttkd_bct.ptm_codinh_202404 a 
				    set (manv_ptm,tennv_ptm, ma_to,ten_to,ma_pb,ten_pb,ma_vtcv,loai_ld, nhom_tiepthi)=
						(select b.manv_hrm,b.ten_nv,b.ma_to,b.ten_to,b.ma_pb,b.ten_pb,b.ma_vtcv,b.loai_ld,b.nhomld_id
						  from ttkd_bsc.nhanvien_202404 b, ttkd_bsc.dm_phongban pb 
						  where b.ma_pb=pb.ma_pb and b.manv_hrm=a.ma_tiepthi) 
				    where chuquan_id in (145,266,264) 
					   and exists (select 1 from ttkd_bsc.nhanvien_202404 b where b.manv_hrm=a.ma_tiepthi) 
				...
				update ptm_codinh_202404 a
						   set (manv_ptm,tennv_ptm, ma_to,ten_to,ma_pb,ten_pb,ma_vtcv, loai_ld, nhom_tiepthi)
								= (select b.ma_nv, b.ten_nv, b.ma_to, b.ten_to, b.ma_pb, b.ten_pb, b.ma_vtcv, b.loai_ld, 2
									 from ttkd_bsc.nhanvien_vttp b 
									 where thang=(select max(thang) from ttkd_bsc.nhanvien_vttp where ma_nv=b.ma_nv)
											   and ma_nv = a.ma_tiepthi)
				   -- select pbh_ptm_id,manv_ptm,tennv_ptm, ma_to,ten_to,ma_pb,ten_pb,ma_vtcv,loai_ld,nhomld_id from ptm_codinh_202403 a
				   where exists(select 1 from ttkd_bsc.nhanvien_vttp b
									   where thang=(select max(thang) from ttkd_bsc.nhanvien_vttp where ma_nv=b.ma_nv)
												and ma_nv=a.ma_tiepthi)
				;
...
-- Tao ds dai ly thang va ins dai ly moi:
			--****-- Dai ly hien huu:
			delete from dm_daily_khdn where thang='202403' ;
			
			insert into ttkd_bsc.dm_daily_khdn (ma_daily,ten_daily,manv_qldaily,ma_pb,thang,thang_kyhd,ma_to)
			    select ma_daily,ten_daily,manv_qldaily,ma_pb,'202404',thang_kyhd,ma_to
			    from ttkd_bsc.dm_daily_khdn
			    where thang='202403'
						 and (select count(*) from ttkd_bsc.dm_daily_khdn where thang='202404')=0
			 ;
			  --****-- Dai ly moi:
					insert into ttkd_bsc.dm_daily_khdn (ma_daily,ten_daily,ma_pb, ma_to, manv_qldaily, thang, thang_kyhd, loai_hopdong )
					    select distinct ma_tiepthi ma_daily, (select upper(ten_nv) from admin_hcm.nhanvien_onebss where ma_nv = a.ma_tiepthi) ten_daily,
								 ma_pb, ma_to, ma_nv, '202404', '202404', 'DAI LY MOI'
					    from 
						   (
				...
				-- Gan ma pb doi voi thue bao phat trien qua Dai ly :
					/*update ptm_codinh_202403 a 
					    set (pbh_ptm_id, ma_pb, ten_pb)=(select (select pbh_id from ttkd_bsc.dm_phongban pb where active=1 and ma_pb=dl.ma_pb) pbh_ptm_id, ma_pb,
																	    (select ten_pb from ttkd_bsc.dm_phongban where active=1 and ma_pb=dl.ma_pb)
															  from ttkd_bsc.dm_daily_khdn dl
															  where thang='202403' and ma_daily=a.ma_tiepthi_new)
					    where chuquan_id in (145,266) and ma_pb is null  
								 and exists (select ma_daily from ttkd_bsc.dm_daily_khdn 
													where thang='202403' and ma_daily=a.ma_tiepthi_new);
										*/
					...
					;
						   ...
						   select * from admin_Hcm.donvi where donvi_id in (2948,11563,11564);
			...
			-- PTTT:
				update ptm_codinh_202404 a 
				    set  ma_pb=(select ma_pb from ttkd_bsc.dm_phongban where active=1 and pbh_id=16)
				...
				update ptm_codinh_202404 
				    ...
				    ;
				    select manv_hotro, ma_duan_banhang, tyle_hotro, hdtb_id, thuebao_id from ptm_codinh_202404 where ma_duan_banhang is not null and manv_hotro is not null ;
...
-- manv_hotro:  chua chay
				...
				*/
				...
				update ptm_codinh_202404 a set manv_hotro='', tyle_hotro='', tyle_am='' where ma_duan_banhang is not null
				;
								
--				update ptm_codinh_202404 a 
--					set (manv_hotro, tyle_hotro, tyle_am) =
--							 
--											(with 
...
--																		)
--														, t as	 (select c.manv_presale_hrm, c.tyle/100 tyle_hotro, decode(tyle_am,0,1,c.tyle_am/100) tyle_am, d.loaitb_id_obss, b.ma_yeucau--, b.ma_dichvu, c.id_ycdv
...
--																	)
--											select distinct t.manv_presale_hrm, t.tyle_hotro, t.tyle_am
--											from t 
--											where t.ma_yeucau = to_number(regexp_replace (a.ma_duan_banhang, '\D', ''))
--																	  and t.loaitb_id_obss = a.loaitb_id 
--											)
--								
--				    -- select ma_gd, ma_tb, loaitb_id, ma_duan_banhang, manv_hotro, tyle_hotro,tyle_am from ptm_codinh_202404 a
...
--					;
				...
				--  select thang_luong, ma_duan_banhang from ttkd_bsc.ct_bsc_ptm a where thang_ptm = 202404 and  ma_duan_banhang is not null and manv_hotro is null ;
					MERGE into ttkd_bct.ptm_codinh_202404 a
							using (with 
													yc_dv as (select ma_yeucau, id_ycdv, ma_dichvu, row_number() over(partition by MA_YEUCAU, MA_DICHVU order by NGAYCAPNHAT desc) rnk
																	from ttkdhcm_ktnv.amas_yeucau_dichvu
																	)
												...
												select distinct t.manv_presale_hrm, t.tyle_hotro, t.tyle_am, ma_yeucau, loaitb_id_obss
												from t 
										) b
							...
							---  select manv_hotro, tyle_hotro, tyle_am from ttkd_bct.ptm_codinh_202404 a
							where ma_duan_banhang is not null and manv_hotro is null 
							 and exists (select distinct c.manv_presale_hrm, c.tyle/100
												from ttkdhcm_ktnv.amas_yeucau_dichvu b, ttkdhcm_ktnv.amas_booking_presale c, ttkdhcm_ktnv.amas_loaihinh_tb d
												where b.ma_yeucau=c.ma_yeucau and b.id_ycdv=c.id_ycdv and b.ma_dichvu=d.loaitb_id
					...
					;
			--		update ptm_codinh_202404 a set ma_duan_banhang = '223791' where ma_gd = '00798153';
			--		select manv_hotro,  tyle_hotro, tyle_am from ptm_codinh_202404 where ma_duan_banhang is not null and ma_gd = '00798153' ;
			--		desc ptm_codinh_202404
					commit;
					rollback;
-- Cuoc phat sinh:    ---chuyen len dataguard
				...
				select distinct a.* from bcss.v_ct_no a
				    where phanvung_id = 28 and ky_cuoc = 20240401 
					   and khoanmuctt_id not in (441,520,521,527,3126,3127,3421,3953) 
					   and exists(select 1 from ptm_codinh_202404 where thuebao_id=a.thuebao_id)
					   ;
				create table temp_ps as select * from ttkd_hcm.temp_ps@dataguard;
				create index temp_ps_tbid on temp_ps (thuebao_id);
				
				
				alter table ptm_codinh_202404 add dthu_ps number(12);
				update ptm_codinh_202404 a set dthu_ps=''
				;
				update ptm_codinh_202404 a 
	...
	/* khong thuc hien			
				update ptm_codinh_202404 a 
				    set doituong_id=doituong_ct_id, lydo_khongtinh_luong=''
				    -- select ma_gd, thuebao_id, ma_tb, doituong_id, doituong_ct_id, dthu_ps, lydo_khongtinh_luong from ptm_codinh_202404 
				    where chuquan_id in (145, 264, 266) and (doituong_id = 190 and dthu_ps > 0) 
	*/		
			----Update Ky luong T3/2024
				    update ptm_codinh_202404 a 
							   set doituong_id=doituong_ct_id
						    where chuquan_id in (145,264,266) and (doituong_id=190 and dthu_ps>0) ;
				;
				commit;
				---select dthu_ps from ptm_codinh_202404 a
-- Dthu goi:
				alter table ptm_codinh_202404 
				    add (dthu_goi_goc number(12), dthu_goi number(12), dthu_goi_ngoaimang number(12))
				;
				update ptm_codinh_202404 bc set dthu_goi_goc='', dthu_goi='', dthu_goi_ngoaimang=''
				
				;
				--  select * from ttkd_bct.ptm_codinh_202404;
				
				update ptm_codinh_202404 bc set dthu_goi_goc=
				    (case
						...
						-- CA-TAX,CA-IVAN,VNPT CA Sign Server,IVAN: tong tien hop dong tru tien token
					    ...
					    when bc.loaitb_id in (80) and bc.thang_bddc is null then (select distinct mc.cuoc_tg from css_hcm.muccuoc_tb mc where mc.mucuoctb_id=bc.mucuoctb_id) 
				...
				/*   
					 ...
					 */  
						...
						when bc.dichvuvt_id=4 and bc.loaitb_id=18 
							 then (select cuoc_tb from css_hcm.muccuoc_tb mc where bc.dichvuvt_id=mc.dichvuvt_id and bc.tocdo_id=mc.tocdo_id)
				
					    -- Mega, Fiber tham gia goi tich hop:
						when bc.loaitb_id in (58,11) and bc.goi_dadv_id in (select goi_id from css.v_goi_dadv_lhtb@dataguard b 
																										where phanvung_id=28 and b.goi_id=bc.goi_dadv_id and b.loaitb_id=bc.loaitb_id and b.tocdo_id=bc.tocdo_id and (giamcuoc_tb=100 or giamcuoc_sd=100) ) 
							 then
								    case when (select soluong_ip from css.tocdo_adsl@dataguard where soluong_ip>0 and (sl_ip_mp is null or sl_ip_mp<soluong_ip) and tocdo_id=bc.tocdo_id)>=8                                                
												then (select distinct tien_goi from css.v_goi_dadv_lhtb@dataguard b where phanvung_id=28 and b.goi_id=bc.goi_dadv_id and b.loaitb_id=bc.loaitb_id and b.tocdo_id=bc.tocdo_id)+500000
										   when (select soluong_ip from css.tocdo_adsl@dataguard where soluong_ip>0 and (sl_ip_mp is null or sl_ip_mp<soluong_ip) and tocdo_id=bc.tocdo_id)=1
												then (select distinct tien_goi from css.v_goi_dadv_lhtb@dataguard b where phanvung_id=28 and b.goi_id=bc.goi_dadv_id and b.loaitb_id=bc.loaitb_id and b.tocdo_id=bc.tocdo_id )+200000 
										   else (select distinct tien_goi from css.v_goi_dadv_lhtb@dataguard b where phanvung_id=28 and b.goi_id=bc.goi_dadv_id and b.loaitb_id=bc.loaitb_id and b.tocdo_id=bc.tocdo_id)
								    end  
						    
						-- Mega, Fiber khong tham gia goi tich hop
						when bc.loaitb_id in (58,11) and (bc.goi_dadv_id is null or bc.goi_dadv_id not in (select goi_id from css.v_goi_dadv_lhtb@dataguard b where phanvung_id=28 and b.goi_id=bc.goi_dadv_id and b.loaitb_id=bc.loaitb_id and b.tocdo_id=bc.tocdo_id and (giamcuoc_tb=100 or giamcuoc_sd=100) ) ) then
							 case when tocdo_id in (44171, 44140) then 250000    -- Fiber VT công ích gói FV60
									when (select soluong_ip from css.tocdo_adsl@dataguard where soluong_ip>0 and (sl_ip_mp is null or sl_ip_mp<soluong_ip) and tocdo_id=bc.tocdo_id)>=8   
										  then (select cuoc_tg from css_hcm.muccuoc_tb mc where mc.mucuoctb_id=bc.mucuoctb_id)+500000
									when (select soluong_ip from css.tocdo_adsl@dataguard where soluong_ip=1 and sl_ip_mp is null and tocdo_id=bc.tocdo_id)=1
										  then (select cuoc_tg from css_hcm.muccuoc_tb mc where mc.mucuoctb_id=bc.mucuoctb_id)+200000 
								    else (select cuoc_tg from css_hcm.muccuoc_tb mc where mc.dichvuvt_id=bc.dichvuvt_id and mc.mucuoctb_id=bc.mucuoctb_id)
							 end  
						...
						when bc.loaitb_id in (61,171) then
							 case 
								...
								when bc.goi_dadv_id in (select goi_id from css.v_goi_dadv_lhtb@dataguard b where phanvung_id=28 and b.goi_id=bc.goi_dadv_id and b.loaitb_id=bc.loaitb_id and b.tocdo_id=bc.tocdo_id and (giamcuoc_tb=100 or giamcuoc_sd=100) ) 
									   then (select tien_goi from css.v_goi_dadv_lhtb@dataguard b where phanvung_id=28 and b.goi_id=bc.goi_dadv_id and b.loaitb_id=bc.loaitb_id and b.tocdo_id=bc.tocdo_id)
								-- MyTV khong tham gia goi tich hop
								when (bc.goi_dadv_id is null or bc.goi_dadv_id not in (select goi_id from css.v_goi_dadv_lhtb@dataguard b where phanvung_id=28 and b.goi_id=bc.goi_dadv_id and b.loaitb_id=bc.loaitb_id and b.tocdo_id=bc.tocdo_id and (giamcuoc_tb=100 or giamcuoc_sd=100) ) )            
									   then (select cuoc_tb from css_hcm.muccuoc_tb mc where mc.mucuoctb_id=bc.mucuoctb_id)                     
							 end
						...
						when bc.dichvuvt_id in (1,10,11) then 
							 case 
								    ...
								    when loaitb_id=63 and bc.goi_dadv_id in (15648,15589,15590,15649,15591,15650,15592,15651,15593,15652,15594,15653,15595,15654)
											 then (select tien_goi from css.v_goi_dadv_lhtb@dataguard where phanvung_id=28 and goi_id=bc.goi_dadv_id and loaitb_id=bc.loaitb_id)
...
------- Tinh lai dthu_goi_goc cho thue bao vua lap moi co dinh vua chuyen sang Siptrunk trong thang:
			...
			update ptm_codinh_202404 a 
					set (dthu_goi_goc, ghi_chu)=
									   (select 250000-dthu_goi_goc, 'dthu goi=250000-dthu goi cua hs lap moi (lap moi, chuyen doi cung thang)' from ptm_codinh_202404 b
										  where loaihd_id=1 and thuebao_id=a.thuebao_id                 
											  and exists (select 1 from ptm_codinh_202404 where thuebao_id=b.thuebao_id and loaihd_id=6)
											  )
		    where loaitb_id=77 and loaihd_id=6            
					  and exists (select 1 from ptm_codinh_202404 where thuebao_id=a.thuebao_id and loaihd_id<>6)
								
				;
			
			update ptm_codinh_202404 set dthu_goi=dthu_goi_goc 
			
			;
			update ptm_codinh_202404 a 
					    set dthu_goi_ngoaimang =
											 (select sum(nogoc) from bcss.v_tonghop@dataguard
												where ky_cuoc=20240401 and khoanmuctc_id in (37,38,39,40,935,936,938,939,943,944,945,4097) and ma_tb=a.ma_tb)
							, dthu_goi = (select sum(nogoc) from bcss.v_tonghop@dataguard
													   where ky_cuoc=20240401 and khoanmuctc_id not in (37,38,39,40,935,936,938,939,943,944,945,4097) and ma_tb=a.ma_tb) 
					where loaitb_id = 131 
...
-- ISDN 30B+D lon hon hoac bang 30 kenh/1 luong tu thang truoc => dthu goi =10K
			update ptm_codinh_202404 a 
			    ...
			    where loaitb_id in (15,17) 
				   and exists (select 1 from (select thuebao_cha_id, count(*) sl_socon from tinhcuoc_hcm.dbtb a
																 where ky_cuoc = 20240301 ---thang n-1
																				and exists (select 1 from ptm_codinh_202404
			...
			update ptm_codinh_202404 a 
				   set dthu_goi_goc = (select cuoc_tggoc + cuoc_ipgoc from bcss.v_thftth@dataguard where phanvung_id=28 and ky_cuoc = 20240401 and ma_tb = a.ma_tb)
					    , dthu_goi = (select cuoc_tggoc + cuoc_ipgoc from bcss.v_thftth@dataguard where phanvung_id=28 and ky_cuoc = 20240401 and ma_tb = a.ma_tb)
			...
			update ptm_codinh_202404
				   set dthu_goi_goc = muccuoc_tb, dthu_goi = muccuoc_tb
			    where thoihan_id = 2 and chuquan_id in (145,266) and loaitb_id = 58 
					  and dthu_goi is null ;
			   
			update ptm_codinh_202404 
				   set dthu_goi = dthu_goi_goc
			    where chuquan_id in (1 , 9, 264) and loaitb_id = 277 and dthu_goi is null 
...
---- kiem tra trung
			    ...
			    thang_bddc, datcoc_csd, tien_tt, goikhoan, phongbh_nhan, loaitb_id,
...
-- Kiem tra dthu goi:
				select a.ma_gd, ma_tb, datcoc_csd, dthu_goi, dthu_ps, bs
				...
				select  chuquan_id, tenkieu_ld, manv_ptm, dich_vu, thuebao_id, ma_gd, ma_tb, loaitb_id, doituong_id, ten_tb, diachi_ld, ma_td, ngaycn_bbbg, ngay_bbbg, 
						  ngay_tt, soseri, tien_tt,thoihan_id,ma_tiepthi,
						  muccuoc_tb, sl_mailing, goi_id,
					   ...
					   from ptm_codinh_202403 a
					   where chuquan_id in (1,9,264) and loaitb_id not in (61,210,222,224) and (thoihan_id=2 or thoihan_id is null)
							    -- and loaitb_id = 279 
								and dthu_goi is null ; 
...
-- Tai lap:
				update ttkd_bct.ptm_codinh_202404 a
				    set ngay_td = (select ngay_td from tinhcuoc.v_dbtb@dataguard where ky_cuoc=20240301 and ngay_td is not null and thuebao_id=a.thuebao_id)
				    where loaihd_id=7 and kieuld_id in (96,13089) and ngay_td is null;
				commit;
				
				
				update ptm_codinh_202404 a set songay_tamngung = '' 
				;
				update ptm_codinh_202404 a
				...
				drop table tailap_202404 purge;
				create table ttkd_bct.tailap_202404 as 
				    select a.* from ptm_codinh_202404 a
					   where a.kieuld_id in (96,13089) and duoctinh_ptm=1
						  and hdtb_id=(select max(hdtb_id) from ptm_codinh_202404 
											  where kieuld_id in (96,13089) and thuebao_id=a.thuebao_id);
												 
												    
				create index tailap_202404_ttid on tailap_202404 (thanhtoan_id);
				create index tailap_202404_tbid on tailap_202404 (thuebao_id);
				create index tailap_202404_hdtbid on tailap_202404 (hdtb_id)
				;
				
			-----	
			xong ptm_codinh_202404
						 tailap_202404
			xong file 2
