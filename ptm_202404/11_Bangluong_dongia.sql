create table ttkd_bsc.bangluong_dongia_202403_l3 as select * from ttkd_bsc.bangluong_dongia_202404; 


---lan dau tien chay doan Frame nay, lan sau khong chay doan FRAME nay
------BEGIN dot 1---khong chay cho dot 2
drop table ttkd_bsc.bangluong_dongia_202404;
				create table ttkd_bsc.bangluong_dongia_202404 (
				ma_nv               varchar(10),
				ma_nv_hrm      varchar(10),
				ten_nv               varchar(50),
				ma_vtcv            varchar(20),
	...
	;
			...
			insert into ttkd_bsc.bangluong_dongia_202404
				 (ma_nv,ma_nv_hrm,ten_nv,ma_vtcv,ten_vtcv,ma_donvi,ten_donvi,ma_to,ten_to,loai_ld)
			SELECT ma_nv,manv_hrm,ten_nv,ma_vtcv,ten_vtcv,ma_pb,ten_pb,ma_to,ten_to,loai_ld
			  FROM ttkd_bsc.nhanvien_202404 a
			  ; 
...
-- Ins tam danh muc dinh muc dthu tu ky truoc de tinh cho dot 1:  
			insert into ttkd_bsc.dinhmuc_dthu_ptm 
							(thang, ma_nv, ten_nv, ma_vtcv, ten_vtcv, ten_to, ma_pb, ten_pb, muc_quy_dinh, dt_giao_bsc, dt_didong_giao, ghi_chu) 
			    
			    select 202404, ma_nv, ten_nv, ma_vtcv, ten_vtcv, ten_to, ma_pb, ten_pb,muc_quy_dinh, dt_giao_bsc, dt_didong_giao, ghi_chu
...
-- dtptm_quydinh, dtptm_muctieu:
			alter table ttkd_bsc.bangluong_dongia_202404 add dtptm_quydinh_g number
			;
			update ttkd_bsc.bangluong_dongia_202404 a set dtptm_quydinh ='', dtptm_muctieu=''
			;
			select* from ttkd_bsc.dinhmuc_dthu_ptm 
			where thang=202404 and dtptm_muctieu is null
			;
			rollback;
			commit;
			alter table ttkd_bsc.dinhmuc_dthu_ptm add dtptm_muctieu number
			;'VNP-HNHCM_KHDN_18' 'VNP0702500'
			update ttkd_bsc.dinhmuc_dthu_ptm a
				   ...
				   where muc_quy_dinh>0 and thang = 202404 and ma_nv in ('CTV084000','CTV084314','CTV085604','CTV085605','CTV085665','CTV085345');
	   ;
			update ttkd_bsc.dinhmuc_dthu_ptm a 
						set dtptm_muctieu = (select distinct dtptm_muctieu 
																	from ttkd_bsc.dinhmuc_dthu_ptm 
																	where dtptm_muctieu > 0 and thang = 202404 and ma_pb = a.ma_pb and ma_vtcv = a.ma_vtcv)
		--	select * from ttkd_bsc.dinhmuc_dthu_ptm a
						where dtptm_muctieu is null and thang = 202404
			;
			update ttkd_bsc.dinhmuc_dthu_ptm a 
						set dtptm_muctieu = 40800000
			where dtptm_muctieu is null and thang = 202404 and ma_vtcv = 'VNP-HNHCM_KHDN_18'
			;
			
			update ttkd_bsc.bangluong_dongia_202404 a
					  set (dtptm_quydinh_g, dtptm_quydinh, dtptm_muctieu)=(select muc_quy_dinh, dt_giao_bsc, dtptm_muctieu from ttkd_bsc.dinhmuc_dthu_ptm where thang=202404 and ma_nv=a.ma_nv)
			    -- select ma_nv, ten_vtcv, ten_to, ten_donvi, dtptm_quydinh_g, dtptm_quydinh, dtptm_muctieu, (select dt_giao_bsc from ttkd_bsc.dinhmuc_dthu_ptm where thang=202403 and ma_nv=a.ma_nv) dieuchinh_dthu_quydinh from bangluong_dongia_202403 a
			    where exists(select dt_giao_bsc from ttkd_bsc.dinhmuc_dthu_ptm where thang=202404 and ma_nv=a.ma_nv)
			    ;
--update bangluong_dongia_202404 a
--            set (dtptm_quydinh_g, dtptm_quydinh)=(select muc_quy_dinh, dt_giao_bsc from ttkd_bsc.dinhmuc_dthu_ptm where thang=202404 and ma_nv=a.ma_nv)
    select ma_nv, ten_vtcv, ten_to, ten_donvi, dtptm_quydinh_g, dtptm_quydinh, dtptm_muctieu, (select dt_giao_bsc from ttkd_bsc.dinhmuc_dthu_ptm where thang=202404 and ma_nv=a.ma_nv) dieuchinh_dthu_quydinh from ttkd_bsc.bangluong_dongia_202404 a
    where ma_nv in ('CTV084000','CTV084314','CTV085604','CTV085605','CTV085665','CTV085345')
		and exists(select dt_giao_bsc from ttkd_bsc.dinhmuc_dthu_ptm where thang=202404 and ma_nv=a.ma_nv)
	   ;

--			update ttkd_bsc.bangluong_dongia_202404 a
--					  set dtptm_muctieu=
--							    (case dtptm_quydinh_g 
--								   when 52500000 then 68000000
--								   when 35000000 then 70200000 -- Nhân viên qu?n lý ?i?m bán: ap theo VB la 70200000, da hoi Xuan Vinh PNS 05/05/2023
--								   when 31500000 then 40800000
--								   when 16000000 then 20800000
...
-- khong chay cho dot 2----Doanh thu binh quan 3 thang truoc lien ke: khong bao gom VNPtt 
			...
			---thang n-1, n-2, n-3
				create table ttkd_bsc.temp_dtbq as
							...
							from (select 202403 thang, ma_nv, DTPTM_DONGIA_CDBR, DTPTM_DONGIA_CNTT DTPTM_DONGIA_CNTT, DTPTM_DONGIA_VNPTS,
										   ...
										   union all
														   select 202401 thang, ma_nv, dtptm_dongia_cdbr, dtptm_dongia_cntt dtptm_dongia_cntt, dtptm_dongia_vnpts,
																	nvl(dtptm_dongia_cdbr ,0) + nvl(dtptm_dongia_cntt ,0) + nvl(dtptm_dongia_vnpts ,0)  tong_dtptm
														   from ttkd_bsc.bangluong_dongia_202401
								   ) group by ma_nv
						;

			update ttkd_bsc.bangluong_dongia_202404 a
	---select * from ttkd_bsc.bangluong_dongia_202404 a
				set dtptm_bq3t='' where dtptm_bq3t is not null
				;
 
     
			update ttkd_bsc.bangluong_dongia_202404 a
				   set dtptm_bq3t =(select dtbq from ttkd_bsc.temp_dtbq where ma_nv=a.ma_nv)
		---select dtptm_bq3t from ttkd_bsc.bangluong_dongia_202404 a
...
------END dot 1---khong chay cho dot 2
		...
		---tao bang tong hop
drop table ttkd_bsc.tonghop_dtdongia_ptm_202404 purge
;
		create table ttkd_bsc.tonghop_dtdongia_ptm_202404 as
					select manv_ptm ma_nv, 
							  (select ma_to from ttkd_bsc.nhanvien_202404 where manv_hrm=a.manv_ptm) ma_to,
							  (select ma_pb from ttkd_bsc.nhanvien_202404 where manv_hrm=a.manv_ptm) ma_pb,
		...
		union all     
					    ...
					    select  manv_tt_dai,
							...
							,'nvtt dai' nguon
					    from ttkd_bsc.ct_bsc_ptm
					    where thang_tldg_dt_dai = 202404
					    group by manv_tt_dai
		...
		union all     
					    ...
					    where thang_tldg_dt_nvhotro = 202404 
					    group by manv_hotro   
			    ) a
			...
			update ttkd_bsc.bangluong_dongia_202404 a
				   set (dtptm_dongia_cdbr,dtptm_dongia_vnpts , dtptm_dongia_cntt, tong_dtptm) =
					    (select dthu_ptm_cdbr, dthu_ptm_vnpts, dthu_ptm_cntt, nvl(dthu_ptm_cdbr,0)+nvl(dthu_ptm_vnpts,0)+nvl(dthu_ptm_cntt,0)
						 from ttkd_bsc.tonghop_dtdongia_ptm_202404
						 where ma_nv=a.ma_nv)
			    where exists (select * from ttkd_bsc.tonghop_dtdongia_ptm_202404 where ma_nv=a.ma_nv)
					;
		---3

-- he so quy doi cua tong dthu:     
			update ttkd_bsc.bangluong_dongia_202404 a set heso_qd_tong=''
					;
			--4
			
			update ttkd_bsc.bangluong_dongia_202404 a
			    set heso_qd_tong = 
				    case when tong_dtptm>0 and dtptm_quydinh>0
		...
		---5
			...
			update ttkd_bsc.bangluong_dongia_202404 a set luong_dongia_cdbr='', luong_dongia_vnpts='', luong_dongia_cntt=''
			;
			--6
			update ttkd_bsc.bangluong_dongia_202404 a 
			...
			update ttkd_bsc.bangluong_dongia_202404 a
			    set (ctvxhh_qly_ptr_ctv,luong_dongia_dnhm_vnptt, luong_dongia_goi_kpbdb, luong_dongia_goi_hcm, luong_dongia_goi_qldb,luong_dongia_vnptt)  =             
								  (select sum(luong_dongia_ctv_xhh), sum(luong_dongia_dnhm), sum(luong_dongia_goi_kpbdb), sum(luong_dongia_goi_hcm), sum(luong_dongia_goi_qldb),
								   ...
								   from ttkd_bsc.bangluong_dongia_202404_vnptt 
								   where ma_nv=a.ma_nv)
			    where exists (select * from ttkd_bsc.bangluong_dongia_202404_vnptt where ma_nv=a.ma_nv)
			;
commit;
...
-- Luong don gia ghtt + Dong luc ghtt : Nhu Y
 create table ttkd_bsc.bangluong_dongia_202404_l3 as select * from  ttkd_bsc.bangluong_dongia_202404;
...
create index ttkd_bsc.temp_ghtt_makpi on ttkd_bsc.temp_ghtt (ma_kpi,loai_tinh,ma_nv);
		...
		---KIEMTRA
				select sum(luong_dongia_ghtt) luong_dongia_ghtt, sum(giamtru_ghtt_cntt) giamtru_ghtt_cntt, sum(tong_luong_dongia), sum(tong_luong_thuhoi)
				from ttkd_bsc.bangluong_dongia_202404 a 
				union all
				select sum(tien) luong_dongia_ghtt, 0,0,0
					from ttkd_bsc.temp_ghtt
					where ma_kpi = 'DONGIA' and loai_tinh in ('DONGIATRA_OB' ,'DONGIA_TS_TP_TT')
					and  tien <> 0
				union all
				select 0, -sum(tien) giamtru_ghtt_cntt,0,0
						from ttkd_bsc.temp_ghtt
	...
	-----
update ttkd_bsc.bangluong_dongia_202404 a 
    set luong_dongia_ghtt=null, giamtru_ghtt_cntt =null;
     
            
update ttkd_bsc.bangluong_dongia_202404 a 
    set 
		luong_dongia_ghtt = (select sum(tien) from ttkd_bsc.temp_ghtt
                                                where ma_kpi = 'DONGIA' and loai_tinh in ('DONGIATRA_OB','DONGIA_TS_TP_TT') ---chi thang sau
																										) and ma_nv = a.ma_nv 
		...
		, giamtru_ghtt_cntt = (select -sum(tien) from ttkd_bsc.temp_ghtt
                                                            where ma_kpi = 'DONGIA' and loai_tinh = 'DONGIATRU_CA' and ma_nv = a.MA_NV_HRM
                                                            group by ma_nv 
                                                            having  sum(tien)  <> 0)
...
-- Luong khac: 
update ttkd_bsc.bangluong_dongia_202404 set luong_khac='', ghichu=' ' 
    where luong_khac is not null;

    
 
-- Don gia gia han VNPTS: Chi Nguyen
update ttkd_bsc.bangluong_dongia_202404 a set ghtt_vnpts=''
;
update ttkd_bsc.bangluong_dongia_202404 a
    set ghtt_vnpts=(select round(sum(luong_dongia),0) from ttkd_bsc.ghtt_vnpts 
                                    where thang=202404 and thang_giao is not null and ma_nv is not null
                                            and ma_nv=a.ma_nv)
    -- select ma_nv, ten_vtcv, ten_donvi, ghtt_vnpts from ttkd_bsc.bangluong_dongia_202404 a
    where exists (select 1 from ttkd_bsc.ghtt_vnpts 
                                where thang=202404 and thang_giao is not null and ma_nv is not null and ma_nv=a.ma_nv);
            
 commit; 
-- Tong luong don gia:  chay dot 1, dot 2
--		update ttkd_bsc.bangluong_dongia_202404 set tong_luong_dongia='' where tong_luong_dongia is not null
		;
		update ttkd_bsc.bangluong_dongia_202404
		    set tong_luong_dongia= nvl(luong_dongia_cdbr,0)+nvl(luong_dongia_vnpts,0)+nvl(luong_dongia_cntt,0)
		...
		update ttkd_bsc.bangluong_dongia_202404
		    ...
		    ;
     --    select tong_luong_dongia, ma_nv from ttkd_bsc.bangluong_dongia_202404
...
/* -- Kiem tra tong luong don gia: 
			...
			union all
			select sum(tong_luong_dongia) from ttkd_bsc.bangluong_dongia_202404;

*/

select * from ttkd_bsc.bangluong_dongia_202404;
            
-- THU HOI + PHAT HUY NVPTM: 
			update ttkd_bsc.bangluong_dongia_202404 a
						 set  
--								dtptm_dongia_cdbr_thuhoi='', luong_dongia_cdbr_thuhoi='',
--							    dtptm_dongia_vnpts_thuhoi='', luong_dongia_vnpts_thuhoi='',
--							    dtptm_dongia_vnptt_thuhoi='', luong_dongia_vnptt_thuhoi=''
								luong_dongia_khac_thuhoi='', luong_dongia_ptm_thuhoi=''
				;
			select luong_dongia_ptm_thuhoi, giamtru_ghtt_cntt, luong_dongia_khac_thuhoi  from ttkd_bsc.bangluong_dongia_202404
			;
...
update ttkd_bsc.bangluong_dongia_202404 a  
   set luong_dongia_ptm_thuhoi = (select sum(tien_thuhoi) from ttkd_bsc.ct_thuhoi where thang=202404 and ma_nv=a.ma_nv)
   where exists(select 1 from ttkd_bsc.ct_thuhoi where thang=202404 and ma_nv=a.ma_nv);

 
-- giam tru do thu ho so tai nha: 
update ttkd_bsc.bangluong_dongia_202404 a set giamtru_hosotainha=''
;
update ttkd_bsc.bangluong_dongia_202404 a
        set giamtru_hosotainha=
             (select round(sum(tien_giam) ,0) from ttkd_bsc.ct_giamtru where thang=202404 and ma_nv=a.ma_nv)
    --  select giamtru_hosotainha from ttkd_bsc.bangluong_dongia_202403  a
    where exists (select 1 from ttkd_bsc.ct_giamtru where thang=202404 and ma_nv=a.ma_nv);



-- giam tru phat huy qldb: 
drop table ttkd_bsc.temp_phathuy;
create table ttkd_bsc.temp_phathuy as select * from ttkd_bsc.ct_bsc_phathuy
;
create index temp_phathuy_thang on temp_phathuy (thang);
create index temp_phathuy_manvcskh on temp_phathuy (manv_cskh);

update ttkd_bsc.bangluong_dongia_202404 a set giamtru_phathuy_qldb='' where giamtru_phathuy_qldb is not null
;
update ttkd_bsc.bangluong_dongia_202404 a 
        set giamtru_phathuy_qldb = (select round(sum(tien_nvcskh) ,0) from ttkd_bsc.temp_phathuy
                                                            where thang = 202404 and manv_cskh = a.ma_nv
                                                            group by manv_cskh)
    where exists (select 1 from ttkd_bsc.temp_phathuy where thang = 202404 and manv_cskh = a.ma_nv);
commit;                                                                                                                                                                        



-- tong_luong_thuhoi:
update ttkd_bsc.bangluong_dongia_202404 a set tong_luong_thuhoi='' where tong_luong_thuhoi is not null
;
update ttkd_bsc.bangluong_dongia_202404 a
...
commit;
			...
			from ttkd_bsc.bangluong_dongia_202404
			union all
			select 'tien_luong_thuhoi', sum(tong_luong_thuhoi) from ttkd_bsc.bangluong_dongia_202404;
			*/


-- XUAT GUI BANG LUONG DON GIA: gui NSU
	select sum(luong_dongia_ghtt_bst4), sum(tong_luong_dongia), sum(tong_luong_thuhoi) from ttkd_bsc.bangluong_dongia_202404; 
	   ...
	   --174593646	5582764781	257085459
	----NSu tren 100tr gui NSU
		select ma_nv, ten_nv, ten_donvi, tong_luong_dongia from ttkd_bsc.bangluong_dongia_202404 where tong_luong_dongia>100000000
		;
		select * from ttkd_bsc.ct_bsc_ptm where thang_tldg_dt=202404 and manv_ptm='VNP027927';

	---gui chi tiet NSU
		select ma_nv, ten_nv,ma_vtcv,ten_vtcv,	ma_donvi,	ten_donvi,	ma_to,ten_to,	loai_ld,
				  ...
				  luong_dongia_ghtt, tong_luong_dongia,	ghichu	,
				  luong_dongia_ptm_thuhoi,	luong_dongia_khac_thuhoi,	giamtru_hosotainha,	giamtru_phathuy_qldb, giamtru_ghtt_cntt, tong_luong_thuhoi	
		from ttkd_bsc.bangluong_dongia_202404;

select * from ttkd_bsc.bangluong_dongia_202404;
select * from ttkd_bsc.bangluong_kpi_202404;
...
from ttkd_bsc.bangluong_kpi_202404;
			...
			from (
					select ma_nv, dthu_ptm_cdbr, dthu_ptm_vnpts, dthu_ptm_vnptt, dthu_ptm_cntt, luong_ptm_cdbr, luong_ptm_vnpts, luong_ptm_vnptt, luong_ptm_cntt 
					  from ttkd_bsc.tonghop_dtdongia_ptm_202404 a 
					  where 
...
--								 and not exists(select * from ttkd_bsc.nhanvien where donvi = 'VTTP' and thang >= 202301 and ma_nv=a.ma_nv)
								-- and 
								  not exists(select 1 from ttkd_bsc.nhanvien where thang = 202404 and ma_nv = a.ma_nv)
								 and exists (select 1 from ttkd_bsc.nhanvien where thang <= 202403 and donvi = 'TTKD' and ma_nv = a.ma_nv)
					...
					union all
							select ma_nv, 0,0,0,0,0,0,  sum(nvl(luong_dongia_ctv_xhh,0)) + sum(nvl(luong_dongia_vnptt_tong,0))  ,0
							  from ttkd_bsc.bangluong_dongia_202404_vnptt a
							  where 
...
-- chi tiet gui anh Nghia:
			select * from ttkd_bsc.ct_bsc_ptm 
			    where (thang_ptm>=202401 and (thang_tldg_dt=202404 or thang_tldg_dt is null)) or (thang_ptm<202401 and thang_tldg_dt=202404); 
			
			select * from ttkd_bsc.ct_bsc_ptm_pgp 
			    where (thang_ptm>=202401 and (thang_tldg_dt_nvhotro=202404 or thang_tldg_dt_nvhotro is null)) 
					  or (thang_ptm<202401 and (thang_tlkpi_hotro=202404 or thang_tldg_dt_nvhotro=202404)  ); 
...
from ttkd_bsc.bangluong_dongia_202404 a, ttkd_bsc.bangluong_dongia_202404_l3 b 
