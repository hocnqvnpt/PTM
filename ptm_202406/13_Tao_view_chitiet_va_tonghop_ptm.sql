-- nv ptm:
select * from ttkd_bsc.v_ct_bsc_ptm_202406;
select * from ttkd_bsc.v_bangluong_dongia_ptm_202406;
drop view ttkd_bsc.v_ct_bsc_ptm_202406;
create OR REPLACE VIEW ttkd_bsc.v_ct_bsc_ptm_202406 as
...
from ttkd_bsc.ct_bsc_ptm a
 where (loaitb_id not in (20,21) or (loaitb_id=20 and goi_luongtinh is null) or loaitb_id is null)
              and   (thang_tldg_dt=202406 or thang_tlkpi=202406 or thang_tlkpi_dnhm=202406
                              or (thang_ptm between 202403 and 202406 and thang_tldg_dt is null) 
                              )
  ;          
union all  -- VNPTT
;
 select thang_ptm, tenkieu_ld,ma_tb,dich_vu, datcoc_csd, thang_bddc, thang_ktdc, sothang_dc,
       ...
       ,dthu_goi_goc,dthu_goi
        ...
        doanhthu_kpi_dnhm, thang_tlkpi_dnhm
 from ttkd_bsc.ct_bsc_ptm a
 where loaitb_id=21 and thang_ptm=202406
 ;
                    
select * from ttkd_bsc.v_ct_bsc_ptm_dai_202406;
 
-- CT Thuê bao PTM (vnpts do PBHOL ptm + thuê bao h? tr? kh?i gián ti?p)
 drop view ttkd_bsc.v_ct_bsc_ptm_dai_202406;
 create OR REPLACE VIEW ttkd_bsc.v_ct_bsc_ptm_dai_202406 as
 select 
 thang_ptm,ma_gd,ma_gd_gt,ma_tb,dich_vu,tenkieu_ld,ten_tb,diachi_ld,sothang_dc,ma_da,
        ...
        (select ma_to from ttkd_bsc.nhanvien where thang = 202406 and  ma_nv = a.manv_tt_dai) ma_to,
        (select ten_to from ttkd_bsc.nhanvien where thang = 202406 and  ma_nv=a.manv_tt_dai) ten_to,
        (select ma_pb from ttkd_bsc.nhanvien where thang = 202406 and  ma_nv=a.manv_tt_dai) ma_pb,
        (select ma_pb from ttkd_bsc.nhanvien where thang = 202406 and  ma_nv=a.manv_tt_dai) ten_pb,
 ...
 from ttkd_bsc.ct_bsc_ptm a
 where  thang_ptm between 202403 and 202406 
                and (thang_tldg_dt=202406 or thang_tldg_dt is null or thang_tldg_dt_dai=202406 )
                and manv_tt_dai is not null;

  -- Ho tro cua Phong Giai phap:
 drop view ttkd_bsc.v_ct_bsc_ptm_phongGP_202406;
 create OR REPLACE VIEW ttkd_bsc.v_ct_bsc_ptm_phongGP_202406 as
 select 
			   ...
			   (select ma_to from ttkd_bsc.nhanvien where thang = 202406 and manv_hrm=a.manv_hotro) ma_to,
			   (select ten_to from ttkd_bsc.nhanvien where thang = 202406 and  manv_hrm=a.manv_hotro) ten_to,
			   (select ma_pb from ttkd_bsc.nhanvien where thang = 202406 and  manv_hrm=a.manv_hotro) ma_pb,
			   (select ma_pb from ttkd_bsc.nhanvien where thang = 202406 and  manv_hrm=a.manv_hotro) ten_pb,
 ...
 where (thang_ptm=202406 
              or ((thang_ptm between 202403 and 202406) and (thang_tldg_dt_nvhotro is null or thang_tldg_dt_nvhotro = 202406 ))
              or thang_tlkpi_hotro = 202406) 
              and exists (select 1 from ttkd_bsc.nhanvien where thang = 202406 and  ma_pb='VNP0702600' and ma_nv = a.manv_hotro)
		    ;
              
 --view chi tieu 044
 -- nv ptm:
drop view ttkd_bsc.v_ct_bsc_cntt_202406;

select * from ttkd_bsc.v_ct_bsc_cntt_202406
;
create OR REPLACE VIEW ttkd_bsc.v_ct_bsc_cntt_202406 as
			 select thang_ptm,ma_gd, ma_gd_gt, tenkieu_ld,ma_tb,dich_vu,ten_tb,diachi_ld--,sdt_lh, email_lh
							,sothang_dc,ma_da,
							   to_char(ngay_bbbg,'dd/mm/yyyy') ngay_bbbg, to_char(ngay_luuhs_ttkd,'dd/mm/yyyy') ngay_luuhs_ttkd, to_char(ngay_luuhs_ttvt,'dd/mm/yyyy') ngay_luuhs_ttvt,
							   to_char(ngay_scan,'dd/mm/yyyy') ngay_scan,goi_cuoc
...
drop view ttkd_bsc.v_bangluong_dongia_ptm_202406;
create OR REPLACE VIEW ttkd_bsc.v_bangluong_dongia_ptm_202406 as
select ma_nv,ten_nv,ma_vtcv,ten_vtcv, ma_pb, ten_pb, ma_to,ten_to,dtptm_dongia_cdbr,dtptm_dongia_cntt,
            ...
            luong_dongia_cdbr,luong_dongia_cntt,luong_dongia_vnpts, luong_dongia_vnptt,ctvxhh_qly_ptr_ctv,
             luong_dongia_ghtt, luong_dongia_nghiepvu, GHTT_VNPTS
