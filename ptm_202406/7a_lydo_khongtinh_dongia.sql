...
--		   select thang_luong, thang_ptm, ma_tb, lydo_khongtinh_dongia, thang_tldg_dt, thang_tldg_dt_nvhotro, thang_tlkpi_phong from ttkd_bsc.ct_bsc_ptm a
		    where thang_ptm>=202403 and lydo_khongtinh_dongia is not null---thang n-3
...
--			    select * from ttkd_bsc.ct_bsc_ptm a 
			    where thang_ptm>=202403 and thang_ptm <= 202404 and ungdung_id=17
			    ;
...
--				select thang_ptm, nocuoc_ptm from ttkd_bsc.ct_bsc_ptm a 
				    where ma_kh<>'GTGT rieng' and loaitb_id<>21 and thang_tldg_dt is null 
								   ...
								   and ((thang_ptm = 202406 and nocuoc_ptm is not null)           ---thang n     
											 or (thang_ptm = 202405 and nocuoc_n1 is not null)				---thang n-1
											 or (thang_ptm = 202404 and nocuoc_n2 is not null)				--thang n -2
											 or (thang_ptm = 202403 and nocuoc_n3 is not null))			---thang n-3
							 ;
				commit;                 
...
--			     select thang_ptm, ma_tb, thang_tldg_dt, lydo_khongtinh_luong, lydo_khongtinh_dongia, nguon from ttkd_bsc.ct_bsc_ptm a
			    where thang_ptm >= 202403 and loaitb_id=358 and nvl(dthu_goi,0)=0
...
--				select thang_ptm, ma_tb, thang_tldg_dt, lydo_khongtinh_luong, lydo_khongtinh_dongia, nguon from ttkd_bsc.ct_bsc_ptm a 
			    where thoihan_id=1 and loaitb_id not in (90,146) 
					  ...
					  and ((thang_ptm = 202403 and trangthaitb_id_n3 not in (7,9)) 
							    or (thang_ptm = 202404 and trangthaitb_id_n2 not in (7,9)) 
							    or (thang_ptm = 202405 and trangthaitb_id_n1 not in (7,9))
							    or (thang_ptm = 202406 and trangthaitb_id not in (7,9))
							)
...
-- 99 tb vnpts ptm 03/2024 co psc bat thuong
		...
		---   select thang_tldg_dt, lydo_khongtinh_dongia, dthu_goi from ttkd_bsc.ct_bsc_ptm a 
...
-- Thue bao chua phat sinh cuoc, ly do: thang_bddc lon hon thang hien tai, nen chua tinh cuoc phan ky vao cuoc ps
		update ttkd_bsc.ct_bsc_ptm a 
				  set thang_tldg_dt         = case when thang_tldg_dt=202406 then null end
					   ,thang_tlkpi             = case when thang_tlkpi=202406 then null end
					   ,thang_tlkpi_to        = case when thang_tlkpi_to=202406 then null end
					   ,thang_tlkpi_phong = case when thang_tlkpi_phong=202406 then null end
					   ,thang_tldg_dnhm  = case when thang_tldg_dnhm=202406 then null end
					   ,thang_tlkpi_dnhm = case when thang_tlkpi_dnhm=202406 then null end
					   ,thang_tlkpi_dnhm_to = case when thang_tlkpi_dnhm_to=202406 then null end
					   ,thang_tlkpi_dnhm_phong = case when thang_tlkpi_dnhm_phong=202406 then null end
					   ,thang_tldg_dt_nvhotro = case when thang_tldg_dt_nvhotro=202406 then null end
					   ,thang_tlkpi_hotro = case when thang_tlkpi_hotro=202406 then null end
...
-- 			select id, thang_luong, thang_ptm, ma_gd, ma_tb, loaitb_id, nguon, thang_tldg_dt, thang_bddc, lydo_khongtinh_luong, lydo_khongtinh_dongia, chuquan_id, dthu_goi from ttkd_bsc.ct_bsc_ptm a
		    where thang_ptm >= 202403 and (loaitb_id<>21 or ma_kh = 'GTGT rieng') 
...
--		     select thang_luong, thang_ptm, thang_tldg_dt, ma_gd, ma_tb, trangthai_tt_id, lydo_khongtinh_dongia from ttkd_bsc.ct_bsc_ptm a
		    where thang_ptm >= 202403 and chuquan_id<>264 and thang_tldg_dt is null and loaitb_id not in (20,21,149) 
				  and not exists(select 1 from ttkd_bsc.dm_loaihinh_hsqd where loaitru_tinhluong=1 and loaitb_id=a.loaitb_id)
...
--		     select thang_ptm, thang_tldg_dt, lydo_khongtinh_dongia from ttkd_bsc.ct_bsc_ptm a
		    where thang_ptm >= 202403 and thang_tldg_dt is null and loaitb_id not in (20,21,149) 
				  and not exists(select 1 from ttkd_bsc.dm_loaihinh_hsqd where loaitru_tinhluong=1 and loaitb_id=a.loaitb_id)
