select * from ptm_codinh_202406;  
drop table ptm_codinh_202406 purge;
create table ptm_codinh_202406 as
...
--                , round(datcoc.datcoc_csd/1.1,0) tien_td
   ...
   from  css.v_hd_khachhang a, css.v_hd_thuebao b, v_db, v_hdtb, tinhcuoc.v_dbtb db_old
         ...
         where a.hdkh_id = b.hdkh_id and b.thuebao_id = v_db.thuebao_id  and db_old.ky_cuoc (+) = 20240501 	---thang n-1
...
--                and b.ma_tb = 'LL001075589'
             ...
             select * from ptm_codinh_202406
                where (ma_tb, ma_gd, hdtb_id) in 
                                                ...
                                                from ptm_codinh_202406 a 
                                                group by ma_tb, ma_gd, hdtb_id having count(*)>1 )
            ;
--Them cot------ tien_dnhm, tien_camket, tien_sodep:
                    ALTER TABLE ptm_codinh_202406 ADD (tien_dnhm NUMBER, tien_sodep NUMBER, tien_camket NUMBER)
				;
                    UPDATE ptm_codinh_202406 a 
                ...
                commit;
                    ...
                    MERGE INTO ptm_codinh_202406 a
                    ...
                    MERGE INTO ptm_codinh_202406 a
                                ...
                                UPDATE ptm_codinh_202406 a
                                    ...
                                    -- SELECT ma_tb, tien_sodep FROM ttkd_bct.ptm_codinh_202406 a
        ...
        create table temp_muccuoctb_thuebao as
            ...
            where dbc.phanvung_id=28 and mc.phanvung_id=28 
			 ...
			 select hdtb_id,   count(*) from temp_muccuoctb_hdtb group by hdtb_id having count(*)>1;
                ...
                update ptm_codinh_202406 a set muccuoc_tb ='';
                ...
                update ptm_codinh_202406 a 
                    ...
                    commit;
--                        
				select * from ptm_codinh_202406 where thuebao_id = 8278567;
     where dichvuvt_id in (13,14,15,16);
...
--                    update ptm_codinh_202406 set tien_dvgt=''
				;
                    update ptm_codinh_202406 a 
                        ...
                        -- select tien_dvgt from ptm_codinh_202406 a
...
-- tien_tbi: 
--                    UPDATE ptm_codinh_202406 SET tien_tbi=''
				;
                    MERGE INTO ptm_codinh_202406 a
                    USING (SELECT cttbi.hdtb_id, tbi_lhtb.loaitb_id, SUM(CASE WHEN tien_tratruoc>0 THEN cttbi.soluong*cttbi.tien_tratruoc ELSE cttbi.soluong*cttbi.tien END) tien_tbi
                                    ...
                                    WHERE cttbi.loaitbi_id=tbi.loaitbi_id AND tbi.loaitbi_id=tbi_lhtb.loaitbi_id AND lhtb.loaitb_id=tbi_lhtb.loaitb_id
                                                        ...
                                                        AND EXISTS(SELECT 1 FROM ptm_codinh_202406 
...
--                where exists(select 1 from ptm_codinh_202406 where datcoc_csd is null and thuebao_id=a.thuebao_id and ma_tiepthi=a.ma_nv)  
--                group by  thuebao_id having count(*)>1)
--                ;
 
            --them cot hdtb_id_datcoc ---> rui ro --> thu check theo tien hop dong khoan muc tra truoc
            alter table ptm_codinh_202406 add hdtb_id_datcoc number
		  ;
            ...
            update ptm_codinh_202406 a 
                ...
                -- select thuebao_id, ma_tb, sothang_dc, datcoc_csd, tien_td from ptm_codinh_202406 a
                where datcoc_csd is null 
                    and exists(select 1 from temp_datcoc b
                                        where datcoc_csd>0 and thuebao_id=a.thuebao_id and ma_nv=a.ma_tiepthi)
                    and ngay_bbbg=(select max(ngay_bbbg) from ptm_codinh_202406 where thuebao_id=a.thuebao_id )
                    ;
                    
            ---recover datcoc_csd updated
--                update ptm_codinh_202406 set datcoc_csd = null 
----				select * from ttkd_bct.ptm_codinh_202406
--				where hdtb_id_datcoc is not null;
            commit;
            
--            select a.*, b.ma_tiepthi , b.DATCOC_CSD, b.SOTHANG_DC, b.hdtb_id_datcoc
--            from temp_datcoc a, ptm_codinh_202406 b
--            where a.thuebao_id=b.thuebao_id
            ;
            
------ thoihan_id
            update ptm_codinh_202406 set thoihan_id=1 
...
----da xong 1/6
	...
	-- Cuoc phat sinh:    ---chuyen len dataguard
				...
				select distinct a.* from bcss.v_ct_no a
				    where phanvung_id = 28 and ky_cuoc = 20240601 		--thang n
					   and khoanmuctt_id not in (441,520,521,527,3126,3127,3421,3953) 
					   and exists(select 1 from ptm_codinh_202406 where thuebao_id=a.thuebao_id)
					   ;
--				create index temp_ps_tbid on temp_ps (thuebao_id);
				
				alter table ptm_codinh_202406 add dthu_ps number(12);
--				update ptm_codinh_202406 a set dthu_ps=''
				;
				update ptm_codinh_202406 a 
...
-- Dthu goi:
				alter table ptm_codinh_202406 
				    add (dthu_goi_goc number(12), dthu_goi number(12), dthu_goi_ngoaimang number(12))
				;
				update ptm_codinh_202406 bc set dthu_goi_goc='', dthu_goi='', dthu_goi_ngoaimang=''
				
				;
				--  select * from ttkd_bct.ptm_codinh_202406;
				
				update ptm_codinh_202406 bc set dthu_goi_goc=
				    (case
					 ...
					 */  
						...
						when bc.dichvuvt_id in (1,10,11) then 
							 case 
								    ...
								    -- Siptrunking:
								    when bc.loaitb_id=77 and goi_id = 15587 then 355000
...
-------- Tinh lai dthu_goi_goc cho thue bao vua lap moi co dinh vua chuyen sang Siptrunk trong thang:
			...
			update ptm_codinh_202406 a 
					set (dthu_goi_goc, ghi_chu)=
									   (select case when goi_id = 15587 then 355000 - dthu_goi_goc
															else 250000 - dthu_goi_goc, 'dthu goi=250000-dthu goi cua hs lap moi (lap moi, chuyen doi cung thang)' 
                                            from ptm_codinh_202406 b
										  where loaihd_id=1 and thuebao_id=a.thuebao_id                 
											  and exists (select 1 from ptm_codinh_202406 where thuebao_id=b.thuebao_id and loaihd_id=6)
											  )
		    where loaitb_id=77 and loaihd_id=6
					  and exists (select 1 from ptm_codinh_202406 where thuebao_id=a.thuebao_id and loaihd_id<>6)
								
				;
			
			update ptm_codinh_202406 set dthu_goi=dthu_goi_goc
			...
			update ptm_codinh_202406 a 
					    set dthu_goi_ngoaimang =
											 (select sum(nogoc) from bcss.v_tonghop
												where ky_cuoc = 20240601 --thang n
...
-- ISDN 30B+D lon hon hoac bang 30 kenh/1 luong tu thang truoc => dthu goi =10K
			update ptm_codinh_202406 a 
			    ...
			    where loaitb_id in (15,17) 
				   and exists (select 1 from (select thuebao_cha_id, count(*) sl_socon from tinhcuoc.v_dbtb a
																 where ky_cuoc = 20240501 ---thang n-1
																				and exists (select 1 from ptm_codinh_202406
 ...
 ---move ve TTKDDB
		  create table ttkd_bct.ptm_codinh_202406 as select * from ttkd_hcm.ptm_codinh_202406@dataguard;
		  SELECT * FROM ttkd_bct.ptm_codinh_202406;
		  
		  ----Update ma_nguoigt la daily
		  update ttkd_bct.ptm_codinh_202406 a
						set ma_nguoigt = upper(a.ma_tiepthi)
								, NGUOI_GT =  a.ten_tiepthi
								, NHOM_GT = a.to_tt
--					select * from ttkd_bct.ptm_codinh_202406 a
		  ...
		  create index ttkd_bct.idx_ptm_hdtbid on ttkd_bct.ptm_codinh_202406 (hdtb_id);
		  -- lydo_khongtinh_luong: Update ky luong T3/24
						alter table ttkd_bct.ptm_codinh_202406 add lydo_khongtinh_luong varchar2(200)
						;
						update ttkd_bct.ptm_codinh_202406 a
									set lydo_khongtinh_luong = (
														   with x as (
																			  ...
																			  from ttkd_bct.ptm_codinh_202406 b
																			)                                        
                    ...
                    -- nv ptm:   
                    alter table ttkd_bct.ptm_codinh_202406
				...
				insert into ttkd_bsc.ptm_xuly_50_BHOL 
						 select 202406 thang, hdtb_id, thuebao_id, loaitb_id, ma_tiepthi, pbh_nhan_id
...
--				     select ma_tiepthi from ttkd_bct.ptm_codinh_202406 a
				    where loaitb_id = 288 and ma_tiepthi is null 
					   and pbh_nhan_id=2941 and nhanvien_id=1077 and datcoc_csd>0
					   ; 
...
-- Tao ds dai ly thang va ins dai ly moi:
			--****-- Dai ly hien huu:
--			delete from dm_daily_khdn where thang='202406' ;
			...
			insert into ttkd_bsc.dm_daily_khdn --(ma_daily,ten_daily,manv_qldaily,ma_pb,thang,thang_kyhd,ma_to)
			    ...
			    select 202406, ma_daily, ten_daily, manv_qldaily, ma_to, ma_pb, ma_vtcv, ten_vtcv, thang_kyhd, ghichu
								
			    from ttkd_bsc.dm_daily_khdn a
			    where a.thang = 202405
			  ...
			  --****-- Dai ly moi:
					insert into ttkd_bsc.dm_daily_khdn (ma_daily,ten_daily,ma_pb, ma_to, manv_qldaily, thang, thang_kyhd, loai_hopdong )
					    select distinct ma_tiepthi ma_daily, (select upper(ten_nv) from admin_hcm.nhanvien_onebss where ma_nv = a.ma_tiepthi) ten_daily,
								 ma_pb, ma_to, ma_nv, 202406, 202406, 'DAI LY MOI' loai_hopdong
					    from 
						   (
					...
					update ttkd_bct.ptm_codinh_202406 a
					    ...
					    --- select MA_GD, pbh_nhan_id, pbh_nhan_goc_id, donvi_tt_id, ma_pb, ten_pb from ttkd_bct.ptm_codinh_202406 a
					...
					update ttkd_bct.ptm_codinh_202406 a
					    ...
					    --- select ma_gd, ma_nguoigt, (select ma_dv from admin_Hcm.donvi where donvi_id = a.pbh_nhan_id), ma_pb, ten_pb from ttkd_bct.ptm_codinh_202406 a
...
--				    select (select ten_dv from admin_Hcm.donvi where donvi_id = 11298) from ttkd_bct.ptm_codinh_202406 a
			...
			commit;
				...
				update ttkd_bct.ptm_codinh_202406 a 
				    ...
				    ;
				    select manv_hotro, ma_duan_banhang, tyle_hotro, tyle_am, hdtb_id, thuebao_id from ttkd_bct.ptm_codinh_202406 a where ma_duan_banhang is not null and manv_hotro is not null 
			;
				-- TTVT ptm nhung ko co ma_tiepthi hoac ma_tiepthi sai/nghi viec: 
				update ttkd_bct.ptm_codinh_202406 a
				    set ma_pb = 'HCM15'
--				     select ma_tiepthi, ma_pb, pbh_nhan_id, donvi_tt_id, donviql_tt_id from ttkd_bct.ptm_codinh_202406 a
					...
					update ttkd_bct.ptm_codinh_202406 a
					    set  
							  ma_pb = (select ma_dv from admin_Hcm.donvi where donvi_id = a.pbh_nhan_id)
							  , ten_pb = (select ten_dv from admin_Hcm.donvi where donvi_id = a.pbh_nhan_id)
--					      select ma_tiepthi, ma_pb, pbh_nhan_id, donvi_tt_id, donviql_tt_id, (select ten_dv from admin_Hcm.donvi where donvi_id = a.pbh_nhan_id) from ttkd_bct.ptm_codinh_202406 a
...
-- manv_hotro:  chua chay
				...
				*/
				...
				update ttkd_bct.ptm_codinh_202406 a set manv_hotro='', tyle_hotro='', tyle_am='' where ma_duan_banhang is not null
				;	
...
---Test ma_da --> book 1 PS lam nhieu cong doan
				...
				--select * from ttkdhcm_ktnv.amas_booking_presale where ma_yeucau = 228705;
					MERGE into ttkd_bct.ptm_codinh_202406 a
							using (
												with 
												...
												select MANV_PRESALE_HRM, LOAITB_ID_OBSS, MA_YEUCAU, TYLE_AM, sum(tyle_hotro) tyle_hotro, sum(TYLE_NHOM) TYLE_NHOM --, MA_DICHVU
															from t
--															where ma_yeucau not in (267460, 177676)
...
--					select manv_hotro, tyle_hotro, tyle_am, ma_duan_banhang, loaitb_id, ma_gd, to_number(regexp_replace (a.ma_duan_banhang, '\D', '')) mada from ttkd_bct.ptm_codinh_202406 a
...
--							and ma_duan_banhang   in ('259121', '242802')
...
---- kiem tra trung
			    ...
			    thang_bddc, datcoc_csd, phongbh_nhan, loaitb_id,
					  ...
					  from ttkd_bct.ptm_codinh_202406
					  where (ma_gd, thuebao_id) in (select ma_gd, thuebao_id from ttkd_bct.ptm_codinh_202406 group by ma_gd, thuebao_id having count(*)>1) 
...
-- Kiem tra dthu goi:
				select a.ma_gd, ma_tb, datcoc_csd, dthu_goi, dthu_ps, manv_ptm
				    from ttkd_bct.ptm_codinh_202406 a
				    where (thuebao_id, ma_tb, kieuld_id) in (select thuebao_id, ma_tb, kieuld_id from ttkd_bct.ptm_codinh_202406 group by thuebao_id, ma_tb, kieuld_id having count(*)>1) 
				    order by thuebao_id, ngay_bbbg;
				
				select * from ttkd_bct.ptm_codinh_202406 
--				 delete from ttkd_bct.ptm_codinh_202406
...
-- Tai lap:
--				update ttkd_bct.ptm_codinh_202406 a
--				    set ngay_td_kytruoc = (select ngay_td from tinhcuoc.v_dbtb@dataguard where ky_cuoc=20240601 and ngay_td is not null and thuebao_id=a.thuebao_id)
--				    where loaihd_id=7 and kieuld_id in (96,13089) and ngay_td_kytruoc is null;
--				commit;
--				
--				
--				update ptm_codinh_202406 a set songay_tamngung = '' 
--				;
--				update ptm_codinh_202406 a
...
--				commit;
				...
				drop table ttkd_bct.tailap_202406 purge
				;
				select * from ttkd_bct.tailap_202406;
				create table ttkd_bct.tailap_202406 as 
				    select a.* from ttkd_bct.ptm_codinh_202406 a
					   where a.kieuld_id in (96,13089) and duoctinh_ptm=1
						  and hdtb_id=(select max(hdtb_id) from ttkd_bct.ptm_codinh_202406 
											  where kieuld_id in (96,13089) and thuebao_id=a.thuebao_id)
								;
												 
												    
				create index tailap_202406_ttid on tailap_202406 (thanhtoan_id);
				create index tailap_202406_tbid on tailap_202406 (thuebao_id);
				create index tailap_202406_hdtbid on tailap_202406 (hdtb_id)
				;
