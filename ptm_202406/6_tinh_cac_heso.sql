...
--thang_luong = 4 --> dung thu chuyen dung that file 5, file 6 update thong tin duan de tinh heso & thay doi cac heso, file 5 update DIGISHOP
...
-- Update lai thang_luong: khi nao can
...
--		insert into ttkd_hcm.temp_tt
				select kh.loaihd_id, a.ma_gd, a.hdkh_id, b.hdtb_id,a.ngay_cn, a.ngay_tt ngay_tt, a.soseri, sum(b.tien) tien, a.trangthai trangthai--, a.ht_tra_id
				    ...
				    where kh.hdkh_id = tb.hdkh_id and tb.hdtb_id = b.hdtb_id and a.phieutt_id = b.phieutt_id and a.trangthai = 1 --and b.tien >0 
										and tb.tthd_id = 6
								and to_number(to_char(a.ngay_cn,'yyyymm'))>=202403					--thang n -3
...
---			select ma_tb, thuebao_id, ngay_tt, soseri, tien_tt, trangthai_tt_id, hdtb_id from ttkd_bsc.ct_bsc_ptm a
			WHERE thang_ptm >= 202403 and chuquan_id <> 264 and thang_tldg_dt is null and nvl(trangthai_tt_id,0)=0			--thang n -3
	...
	;
		create table ttkd_bsc.tmp_ct_no as 
				select thuebao_id, khoanmuctt_id, nogoc 
						...
						where phanvung_id=28 and ky_cuoc=20240601
					;
...
-- dthu_ps thang n:
		update ttkd_bsc.ct_bsc_ptm a 
		    ...
		    where thang_ptm = 202406 and loaitb_id<>21 and dthu_ps is null 
...
-- Dot 2: 
		update ttkd_bsc.ct_bsc_ptm a 
		    ...
		    where thang_ptm=202406 and loaitb_id<>21 and nvl(dthu_ps, 0) = 0 
...
--		    select * from ttkd_bsc.ct_bsc_ptm a
		    where thang_ptm=202405 and loaitb_id<>21 and nvl(dthu_ps_n1, 0) = 0
			   and exists(select 1 from ttkd_bct.cuoc_thuebao_ttkd where dthu>0 and ma_tb=a.ma_tb and loaitb_id=a.loaitb_id)
			   ;
...
--		    select * from ttkd_bsc.ct_bsc_ptm a		    
		    where thang_ptm=202404 and loaitb_id<>21 and nvl(dthu_ps_n2, 0) = 0
			   and exists(select 1 from ttkd_bct.cuoc_thuebao_ttkd where dthu>0 and ma_tb=a.ma_tb and loaitb_id=a.loaitb_id)
			   ;
...
--		    select * from ttkd_bsc.ct_bsc_ptm a		    
			    where thang_ptm=202403 and loaitb_id<>21 and nvl(dthu_ps_n3, 0) = 0
				   and exists(select 1 from ttkd_bct.cuoc_thuebao_ttkd where dthu>0 and ma_tb=a.ma_tb and loaitb_id=a.loaitb_id)
				   ;
commit;
...
--		    select thang_luong, thang_ptm, ma_tb, trangthaitb_id, ngay_bbbg, ngay_cat, (select ngay_cat from css_hcm.db_thuebao where thuebao_id=a.thuebao_id) ngay_cat_dbonline, dthu_ps, dthu_ps_n1, dthu_ps_n2, dthu_ps_n3, dthu_goi, nvl(a.dthu_ps,0)+nvl(a.dthu_ps_n1,0)+nvl(a.dthu_ps_n2,0)+nvl(a.dthu_ps_n3,0) dthu_goi_new, ghi_chu from ttkd_bsc.ct_bsc_ptm a
		    where thang_ptm >= 202403 and thoihan_id=1 and dichvuvt_id in (1, 10, 11, 4, 7, 8, 9) ---thang n-3
...
--insert into hocnq_ttkd.temp_ps
						with 
								   ...
								   , datcoc as (select thuebao_id, sum(round(cuoc_dc/1.1,0)) datcoc_csd_new
										 ...
										 where hieuluc=1 and ttdc_id = 0 and to_number(to_char(ngay_cn,'yyyymm'))>=202403 
										 group by thuebao_id
										 )
							select thang_ptm, a.thuebao_id, ma_gd, ma_tb, dthu_goi, dthu_ps, dthu_ps_n1, dthu_ps_n2, dthu_ps_n3
								...
								where thang_ptm >= 202403 and nvl(dthu_goi, 0) = 0 --dthu_goi is null		---thang n -3
										  and (thang_tldg_dt is null 
																or thang_tldg_dt = to_number(to_char(trunc(sysdate, 'month') - 1, 'yyyymm'))											---thang n
													)
...
-- SMS Brandname: Tinh lai dthu goi, dthu_ps thang n va dthu_ps thang n+1
		...
		create table ttkd_bsc.tmp_bsc_smsbrn as
		    ...
		    from ttkd_bsc.ct_bsc_ptm a
		    where thang_ptm = 202405 and loaitb_id=131 ---thang n -1
		    ;
...
--			    	    select thang_tldg_dt, thang_ptm, khachhang_id, ma_tb, doituong_kh, dich_vu, mst from ttkd_bsc.ct_bsc_ptm a
    ...
    -- Update doi voi ds tbao VNPts, hoac ktra cac th ngoai le                  
				update ttkd_bsc.ct_bsc_ptm a
					   set doituong_kh = (case when (loaitb_id not in (20,149) and doituong_id in (374, 387, 361, 362))
																    or (loaitb_id in (20,149) and doituong_id in (1))
																				then 'KHCN' 
																when loaitb_id in (20,149) and doituong_id in (2, 4, 19, 21) then 'KHDN'
																--when length(mst) >= 10 then 'KHDN'
														else null end)
--				  select thang_tldg_dt, thang_ptm, khachhang_id, ma_kh, ma_tb, ten_tb, doituong_id, doituong_kh, dich_vu, mst, loaitb_id from ttkd_bsc.ct_bsc_ptm a
				    where thang_ptm =202406   --- thang n
							and nvl(ma_kh, 'abv') <> 'GTGT rieng' and doituong_kh is null
...
--				select thang_luong, thang_ptm, ma_duan_banhang, hdkh_id from ttkd_bsc.ct_bsc_ptm a
					where exists (select 1 from css_hcm.hd_khachhang where MA_DUAN is not null and hdkh_id = a.hdkh_id and nvl(a.ma_duan_banhang, 'r') <> replace(ma_duan, ' ', ''))
								and thang_ptm >= 202403 
								and nvl(thang_tldg_dt, 999999) >= 202406
								and nvl(thang_tldg_dt_nvhotro, 999999) >= 202406			--- chua tinh luong
...
--									and ma_duan_banhang   in ('259121')
--										and thang_luong = 86
								;
			-----END bo sung thong tin QLDA

-- QLDA: xoa , chay lai
delete from ttkd_bsc.ct_bsc_ptm_kiemtraduan where thang=202406 and ma_duan_banhang = '227812'; and ma_tb = 'hcm_ca_00103739';and ma_tb = 'hcm_colo_00009265';
		;
		select * from ttkd_bsc.ct_bsc_ptm_kiemtraduan a where thang = 202406 and ma_duan_banhang = '227812'; and ma_tb = 'hcm_ca_00103739'; and ma_gd = 'HCM-TD/00690780';hcm_hddt_inbot_00001044
...
--		select ma_tb, kiemtra_duan, doituong_kh from ttkd_bsc.ct_bsc_ptm a 
			    where thang_ptm>=202403 and doituong_kh='KHDN' and kiemtra_duan is not null
			...
			update ttkd_bsc.ct_bsc_ptm a 
				   set kiemtra_duan = (select nvl(kt_nghiemthu, 0) || kiemtra_duan from ttkd_bsc.ct_bsc_ptm_kiemtraduan 
																							where thang = 202406 and ptm_id=a.id)
...
--		 select thang_luong, thang_ptm, kiemtra_duan, thang_tldg_dt, thang_tlkpi, nguon, ma_duan_banhang, ma_gd,  ma_tb, mst from ttkd_bsc.ct_bsc_ptm a 
			    where exists(select 1 from ttkd_bsc.ct_bsc_ptm_kiemtraduan where thang = 202406 and ptm_id=a.id)
--			    and ma_duan_banhang = '235405'
...
-- KH hien huu - KH moi:   
	...
	--   select khhh_khm from ttkd_bsc.ct_bsc_ptm
			    where thang_ptm = 202406 and khhh_khm is not null
					...
					and doituong_kh='KHDN' and mst is not null --thang n
					  and exists (select mst_chuan from ttkd_bct.db_thuebao_ttkd
												   where ma_dt_kh='dn' and cvnv is null and tb_dacbiet is null 
																  and trangthaitb_id not in (7,8,9) and to_number(to_char(ngay_sd,'yyyymm')) < 202406			--- thang n
...
--					      select khhh_khm, doituong_kh from ttkd_bsc.ct_bsc_ptm a
				    where thang_ptm = 202406 and doituong_kh='KHDN'
...
--												  )
			---hoac 2
					...
					update ttkd_bsc.ct_bsc_ptm a 
					   set khhh_khm =  'KHM'
--					      select khhh_khm, doituong_kh, ma_tb, nguon, ghi_chu, tenkieu_ld from ttkd_bsc.ct_bsc_ptm a
				    where thang_ptm = 202406 	--thang n
					...
					;
				update ttkd_bsc.ct_bsc_ptm set khhh_khm='KHM' 
--		   select khhh_khm, dichvuvt_id, doituong_kh, loaitb_id, nguon from ttkd_bsc.ct_bsc_ptm
			    where thang_ptm = 202406 --- thang n
							and khhh_khm is null 			--loai tru da gan roi, khong gan lai
...
-- Dia ban:        vi tri ban hang truc tiep moi xet trong/ngoai dia ban
	...
	---	select diaban from ttkd_bsc.ct_bsc_ptm a
			    where thang_ptm = 202406 and (loaitb_id<>21 or ma_kh='GTGT rieng') and diaban is not null
...
--											when nhom_tiepthi=2 or thuebao_id is null then 'Khong xet trong/ngoai CT ban hang'
											...
											when loaitb_id = 149 then 'Khong xet trong/ngoai CT ban hang'				----VCC tren CCBS khong co nhap dc QLDA
...
--							select thang_luong, thuebao_id, ma_tb, dichvuvt_id, loaitb_id, dich_vu, mst, diaban, nhom_tiepthi, ma_vtcv, doituong_kh, kiemtra_duan, ma_duan_banhang, nguon, thang_tldg_dt from ttkd_bsc.ct_bsc_ptm a
				    where (a.thang_ptm = 202406 --- thang n
					   ...
					   and nvl(thang_tldg_dt, 999999) >= 202406 --and nvl(thang_tlkpi_phong, 999999) >= 202406  --lay cac tbao chua chi dektra duan
...
--		 select * from ttkd_bsc.ct_bsc_ptm
			    where thang_ptm = 202406 --- thang n
						   ...
						   and ( (dichvuvt_id not in (2,13,14,15,16) and (loaihd_id is null or chuquan_id in (266,264))  ---Newlife, VTH1 hoac hok co hop dong --> mien hso goc
											...
											or ( thang_ptm = 202406 and loaitb_id in (15,17) and thuebao_cha_id is not null)       -- isdn, thue bao luong (so con)
...
---da xong 14/07/24 15g
-- He so dich vu:    
		---tao bang TEMP bat buoc
		create table ttkd_bsc.x_temp as select * from ttkd_bct.ptm_codinh_202406
		;
...
--										when loaitb_id = 20 and VN_MOI = 'MNP' then 1.5	---thue bao MNP
...
--										when loaitb_id = 20 and kieuld_id = 27 then 0.8		---Card to Post
...
-- He so khu vuc dac thu: danh muc quan ly thay doi from PDH.
			...
			;
			select * from ttkd_bsc.dm_duan_dacthu where thang=202406
			;
			desc ttkd_bsc.dm_duan_dacthu;
			
			insert into ttkd_bsc.dm_duan_dacthu 
					    select 202406 thang, ma_da, ten_duan, heso_kvdacthu, toanha_id, ghichu
					    from ttkd_bsc.dm_duan_dacthu a
					    where not exists (select 1 from ttkd_bsc.dm_duan_dacthu where thang = 202406 and ma_da=a.ma_da)    
							  and thang = 202405
							  ;
...
-- He so dai ly :  VB 353/TTr-DH-NS - 12/2023:    AM ban hang thong qua Dai ly tinh 5%; nguoc lai AM QLDL ban hang thong qua DAI LY 0%
    ...
    select *  from ttkd_bsc.dm_daily_khdn where thang = 202406;
    
			update ttkd_bsc.ct_bsc_ptm a 
			    set heso_daily = 
											case when exists (select 1  from ttkd_bsc.dm_daily_khdn where thang = 202406 and thang_kyhd < a.thang_ptm and ma_daily = a.ma_nguoigt) ---Dai ly hien huu
																then 0
													when exists (select 1  from ttkd_bsc.dm_daily_khdn where thang = 202406 and  thang_kyhd = a.thang_ptm and ma_daily = a.ma_nguoigt) ---Dai ly moi
...
--						, thang_luong = 25
--		 select thang_luong, thang_ptm, manv_ptm, ma_vtcv, ma_nguoigt, heso_daily, thang_tldg_dt from ttkd_bsc.ct_bsc_ptm a
			    where thang_ptm = 202406 and manv_ptm is not null-- and heso_daily is null
...
-- he so vtcv: cac vtcv duoc tinh = 1, ngoai tru ca th dac biet duyet qua Mail se dieu chinh lai heso (vi du duyet chi phi TTKD tu mail Chu Minh KHKT)
				...
				update ttkd_bsc.ct_bsc_ptm a 
					   set   heso_vtcv_nvptm = case when chiphi_ttkd > 0 and thang_xn_chiphi_ttkd is not null then 0.5 else 1 end
							 ,heso_vtcv_nvhotro = case when chiphi_ttkd > 0 and thang_xn_chiphi_ttkd is not null then 0.5 else 1 end
							 ,heso_vtcv_dai = case when chiphi_ttkd > 0 and thang_xn_chiphi_ttkd is not null then 0.5 else 1 end
--				    select thang_luong, thang_ptm, chiphi_ttkd, thang_xn_chiphi_ttkd, heso_vtcv_nvptm, heso_vtcv_nvhotro, heso_vtcv_dai, manv_hotro, thang_tldg_dt from ttkd_bsc.ct_bsc_ptm a 
				    where (thang_ptm = 202406 		--- thang n
				...
				update ttkd_bsc.ct_bsc_ptm a
				    set heso_hotro_nvptm  = case 
																	when a.loaitb_id = 20 and exists (select sdt_datmua, ma_gioithieu, tennv_gioithieu 
																																...
																																where lower(trangthai_shop) like 'th_nh c_ng' and sdt_datmua = a.ma_tb and thang >= 202403)
																	...
																	when a.dichvuvt_id not in (2, 14, 15, 16) 
																											and exists (select ma_dhsx, ma_gioithieu, tennv_gioithieu 
																																from ttkd_bsc.digishop 
																																where lower(trangthai_shop) like 'th_nh c_ng' and ma_dhsx = a.ma_gd_gt and thang >= 202403)
...
--																	when manv_hotro is not null and tyle_am is null then 0.5			ap dung toi uu, dang nghien cuu--50% cho cong doan nguoi xu ly, case hok khong PS
																	   when exists (select 1 from ttkd_bct.ptm_codinh_202406
...
--		     select thang_luong, ma_tb, tenkieu_ld, ngay_bbbg, ngay_luuhs_ttkd, ngay_luuhs_ttvt, nop_du, mien_hsgoc, bs_luukho , heso_hoso from ttkd_bsc.ct_bsc_ptm a
		    where ((thang_ptm = 202403 and (to_number(to_char(ngay_luuhs_ttkd,'yyyymmdd'))>20240513 or to_number(to_char(ngay_luuhs_ttvt,'yyyymmdd'))>20240513))
...
--							or (thang_ptm = 202404 and (to_number(to_char(ngay_luuhs_ttkd,'yyyymmdd'))>20240613 or to_number(to_char(ngay_luuhs_ttvt,'yyyymmdd'))>20240613))
					  ...
					  and mien_hsgoc is null and nop_du = 1 and substr(bs_luukho,1,6)= '202407' --and heso_hoso is null		---thang n+1
					  and nvl(thang_tldg_dt, 999999) >= 202406
--					  and thang_luong<>'3'
			  ;
...
select ma_gd, ma_tb, nguon, loaihd_id, dthu_goi, kieuld_id,lydo_khongtinh_luong, thang_tldg_dt, lydo_khongtinh_dongia
    ...
    where thang_ptm = 202406 and loaitb_id<>21 and
                (ma_gd, ma_tb, loaitb_id) in 
                          (select ma_gd, ma_tb, loaitb_id from ttkd_bsc.ct_bsc_ptm 
                            where thang_ptm = 202406 and loaitb_id!=20 group by ma_gd, ma_tb, loaitb_id having count(*)>1
					   )
				;
...
--		    select * from ttkd_bsc.ct_bsc_ptm a
		    where thang_ptm = 202406 and nguon like 'thaydoitocdo%' and lydo_khongtinh_luong not like '%Tinh luong theo hs tai lap%'
...
-- cac dv ngoai tru VNPTT, SMS Brandname, Voice Brandnanme
			...
			update ttkd_bsc.ct_bsc_ptm a 
			    set doanhthu_dongia_nvptm   = case when heso_daily =  0.05 then round(dthu_goi * nvl(tyle_huongdt,1)* heso_daily * heso_hotro_nvptm, 0)
																			when heso_daily =  0 then round(dthu_goi * nvl(tyle_huongdt,1)* heso_daily * heso_hotro_nvptm, 0)		---AM QLDL, AM va Daily hien huu = 0
					...
					,doanhthu_dongia_nvhotro = case when heso_daily =  0.05 then round(dthu_goi * nvl(tyle_huongdt,1)* heso_daily * heso_hotro_nvhotro, 0)
																			when heso_daily =  0 and exists (select 1  from ttkd_bsc.dm_daily_khdn 
																																		where thang = 202406 and thang_kyhd = a.thang_ptm and ma_daily = a.ma_nguoigt) ---Dai ly moi
																								then round(dthu_goi * nvl(tyle_huongdt,1)* heso_daily * heso_hotro_nvhotro, 0)
																			else round(dthu_goi*nvl(tyle_huongdt,1)*heso_dichvu*nvl(heso_khuyenkhich,1)*nvl(heso_tratruoc,1)
					...
					,doanhthu_dongia_dai          = case when heso_daily =  0.05 then round(dthu_goi * nvl(tyle_huongdt,1)* heso_daily * heso_hotro_dai, 0)
																			when heso_daily =  0 and exists (select 1  from ttkd_bsc.dm_daily_khdn 
																																	where thang = 202406 and  thang_kyhd = a.thang_ptm and ma_daily = a.ma_nguoigt) ---Dai ly moi
																								then round(dthu_goi * nvl(tyle_huongdt,1)* heso_daily * heso_hotro_dai, 0)
																			else round(dthu_goi*nvl(tyle_huongdt,1)*heso_dichvu*nvl(heso_khuyenkhich,1)*nvl(heso_tratruoc,1)
																																*heso_quydinh_dai * heso_vtcv_dai * nvl(heso_kvdacthu,1)
																																* heso_hotro_dai * heso_khachhang * nvl(heso_tbnganhan,1)
																																*nvl(heso_hoso,1)*nvl(heso_diaban_tinhkhac,1)
																								, 0) end
--		    select nguon, sothang_dc, thang_luong, thang_ptm, ma_gd, ma_tb, dichvuvt_id, manv_hotro, heso_daily, dthu_goi, doanhthu_dongia_nvptm, doanhthu_dongia_nvhotro, doanhthu_dongia_dai, ma_kh, loaitb_id, thang_tldg_dt from ttkd_bsc.ct_bsc_ptm a
			    where (thang_ptm = 202406 --- thang n
...
-- SMS Brandname thang n-1: 
			update ttkd_bsc.ct_bsc_ptm a 
			    set doanhthu_dongia_nvptm = case when heso_daily =  0.05 then round((nvl(dthu_goi,0) + nvl(dthu_goi_ngoaimang,0)) * nvl(tyle_huongdt,1)* heso_daily * heso_hotro_nvptm, 0)
																			else round(	
																								((nvl(dthu_goi,0)*heso_dichvu) +(nvl(dthu_goi_ngoaimang,0)*heso_dichvu_1)
																									...
																									* heso_quydinh_nvptm * heso_vtcv_nvptm * heso_hotro_nvptm *nvl(heso_hoso,1)
																									*heso_khachhang * nvl(heso_tbnganhan,1)*nvl(heso_diaban_tinhkhac,1)*nvl(heso_daily,1) 
																							,0) end
					,doanhthu_dongia_nvhotro = case when heso_daily =  0.05 then round((nvl(dthu_goi,0) + nvl(dthu_goi_ngoaimang,0)) * nvl(tyle_huongdt,1)* heso_daily * heso_hotro_nvhotro, 0)
																			else round(
																								((nvl(dthu_goi,0)*heso_dichvu)+(nvl(dthu_goi_ngoaimang,0)*heso_dichvu_1) 
																								) *nvl(tyle_huongdt,1)
																									*nvl(heso_khuyenkhich,1)*nvl(heso_tratruoc,1) *nvl(heso_kvdacthu,1) 
																									*heso_quydinh_nvhotro * heso_vtcv_nvhotro * heso_hotro_nvhotro * nvl(heso_hoso,1) 
																									* heso_khachhang * nvl(heso_tbnganhan,1) * nvl(heso_diaban_tinhkhac,1) * nvl(heso_daily,1)
																					, 0) end
					,doanhthu_dongia_dai = null
---	   select heso_tratruoc, heso_quydinh_nvptm, heso_vtcv_nvptm, heso_hotro_nvptm, heso_khachhang, doanhthu_dongia_nvptm  from ttkd_bsc.ct_bsc_ptm a
...
-- don gia dnhm: 
			...
			update ttkd_bsc.ct_bsc_ptm a 
			    set doanhthu_dongia_dnhm = case when heso_daily =  0.05 then round((nvl(tien_dnhm,0)+nvl(tien_sodep,0)) * nvl(tyle_huongdt,1)* heso_daily, 0)
																		when heso_daily =  0 then round((nvl(tien_dnhm,0)+nvl(tien_sodep,0)) * nvl(tyle_huongdt,1)* heso_daily, 0)
...
--		  select thang_luong, ma_tb, luong_dongia_nvptm, doanhthu_dongia_dnhm, tien_dnhm, tien_sodep, heso_daily, nguon, thang_tldg_dnhm from ttkd_bsc.ct_bsc_ptm a
				...
				update ttkd_bsc.ct_bsc_ptm a 
							set luong_dongia_dnhm_nvptm = case when heso_daily in (0, 0.05) then doanhthu_dongia_dnhm
...
--		 select thang_luong, thang_ptm, doanhthu_dongia_nvptm, doanhthu_dongia_nvhotro, doanhthu_kpi_nvptm, doanhthu_kpi_nvdai, doanhthu_kpi_nvhotro, doanhthu_kpi_dnhm, doanhthu_dongia_nvhotro, thang_tlkpi from ttkd_bsc.ct_bsc_ptm a
				    where (thang_ptm = 202406 --- thang n
								or thang_luong in (1, 2, 3, 4, 87)
								)			---flag 4 file so 5 import dung thu chuyen dung that
									and dich_vu not like 'Thi_t b_ gi_i ph_p%' and (loaitb_id not in (21,131) or ma_kh='GTGT rieng') 
									and nvl(thang_tlkpi, 999999) >= 202406
--							  and  thang_luong = 86
...
-- SMS Brandname thang n-1:
			update ttkd_bsc.ct_bsc_ptm
			    set  doanhthu_kpi_nvptm = doanhthu_dongia_nvptm,
					  doanhthu_kpi_nvhotro = doanhthu_dongia_nvhotro
--			    select doanhthu_dongia_nvptm, doanhthu_dongia_nvhotro from ttkd_bsc.ct_bsc_ptm
			    where (thang_ptm = 202405 --- thang n-1
								or thang_luong in (1, 2, 3, 4))			---flag 4 file so 5 import dung thu chuyen dung that
							 and loaitb_id=131 
				...
				update ttkd_bsc.ct_bsc_ptm a
					   set doanhthu_kpi_phong = (case when dich_vu like 'Thi_t b_ gi_i ph_p%' 
																		  then round(dthu_goi_goc * nvl(tyle_huongdt,1) * heso_dichvu * nvl(heso_khuyenkhich,1) * nvl(heso_tratruoc,1)
																									   ...
																									   * nvl(heso_kvdacthu,1) * heso_khachhang * nvl(heso_tbnganhan,1)
 ...
 --  SMS Brandname thang n-1
				update ttkd_bsc.ct_bsc_ptm
				    set doanhthu_kpi_phong= round(((nvl(dthu_goi,0)*nvl(heso_dichvu,1))+(nvl(dthu_goi_ngoaimang,0)*nvl(heso_dichvu_1,1))  
														    ...
														    *nvl(heso_khuyenkhich,1)*nvl(heso_tratruoc,1)*nvl(heso_kvdacthu,1)
														    * heso_vtcv_nvptm * heso_hotro_nvptm
														    * heso_khachhang *nvl(heso_tbnganhan,1)
														    *nvl(heso_hoso,1)*nvl(heso_diaban_tinhkhac,1) ,0)
...
-- Kpi dnhm phong:
		...
		update ttkd_bsc.ct_bsc_ptm a
		    set doanhthu_kpi_dnhm_phong = round( (nvl(tien_dnhm,0)+nvl(tien_sodep,0)) *nvl(tyle_huongdt,1) * heso_dichvu_dnhm
																	* heso_vtcv_nvptm * heso_hotro_nvptm
																	* nvl(heso_kvdacthu,1) * nvl(heso_diaban_tinhkhac,1) ,0)
...
-- TB ngan han:
select thang_ptm, ma_pb,manv_ptm,(select ten_nv from ttkd_bsc.nhanvien_202406 where manv_hrm=a.manv_ptm) ten_nv,
    ...
    from ttkd_bsc.ct_bsc_ptm a
    where thang_ptm = 202406 and thoihan_id=1
