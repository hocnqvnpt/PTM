...
create table ttkd_bsc.ct_bsc_ptm_pgp as 
			...
			, HESO_DIABAN_TINHKHAC, nvl(HESO_DIABAN_TINHKHAC, null) heso_daily, DOANHTHU_KPI_NVPTM, DOANHTHU_KPI_NVHOTRO, DOANHTHU_DONGIA_NVHOTRO, LUONG_DONGIA_NVHOTRO
...
drop table x_ct_bsc_ptm_pgp;

rollback;
commit;
select * from ttkd_bsc.ct_bsc_ptm_pgp where thang_tldg_dt_nvhotro = 202406; and heso_daily = 0.05;
select * from ttkd_bsc.ct_bsc_ptm_pgp where thang_ptm = 202406; and heso_daily = 0.05; and ma_tb in ('hcm_hddt_00013102', 'hcm_hddt_00023066', 'hcm_hddt_00007104')
							and thang_ptm = 202406; 
select * from ttkd_bsc.ct_bsc_ptm where ma_gd = 'HCM-LD/01616773';

delete from ttkd_bsc.ct_bsc_ptm_pgp where thang_ptm=202406; 
...
insert into ttkd_bsc.ct_bsc_ptm_pgp a
							  (ptm_id, thang_ptm, nguon, ma_duan, ma_gd, ma_tb, dich_vu, dichvuvt_id, loaitb_id, tenkieu_ld, kieuld_id, khachhang_id, thanhtoan_id, thuebao_id
							   ...
							   ,heso_vtcv_nvhotro, heso_hotro_nvptm, heso_hotro_nvhotro,heso_quydinh_nvhotro, heso_diaban_tinhkhac, heso_daily, doanhthu_kpi_nvptm
...
--		select*
--		from tt where ma_yeucau in ('236617', '183656', '196901', '261122', '245883', '158573')
...
--				   , b.manv_presale_hrm
    ...
    where thang_ptm = 202406 and manv_hotro is not null and (loaitb_id is null or loaitb_id<>21)			--thang n
...
--		select * from ttkd_bsc.ct_bsc_ptm_pgp a
			where exists (select * from ttkd_bsc.ct_bsc_ptm where ma_duan_banhang is not null and thang_tldg_dt_nvhotro = 202406 and a.ptm_id = id )
...
--											select * from ta where ma_yeucau in ('236617', '183656', '207081')
					...
					select --a.loaitb_id
					    ...
					    from ttkd_bsc.ct_bsc_ptm a
									left join tt b on to_number(regexp_replace (a.ma_duan_banhang, '\D', ''))  = b.ma_yeucau and a.loaitb_id = b.loaitb_id_obss
					    where thang_tldg_dt_nvhotro = 202406  and manv_hotro is not null and (loaitb_id is null or loaitb_id<>21)			--thang n
