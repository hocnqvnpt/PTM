...
----imp ds anh Nghia trong/ngoai ctr
				select * from ttkd_bsc.ct_ptm_ngoaictr_imp where thang = 202406; and ma_tb = 'hcm_colo_00010493'; and dichvu_vt = 'VNP tra sau' and TENKIEU_LD != 'Gia han dich vu';
...
--				select * from ttkd_bsc.ct_ptm_ngoaictr_imp 
				where thang = 202406 --and dichvuvt_id in (14, 15, 16)
--							ma_tb = 'hcm_vbn_00001181'
--							and DICHVUVT_id is null
--							and thuebao_id = 12215982 --chu upda, ?ang kiem tra vi sao ti?n hok có trong ctr
							and thuebao_id in (8592878)
				;		
			
				;
		
				--ktra ma_gd da tinh tien
				select * from ttkd_bsc.ct_bsc_ptm where thang_ptm >= 202312 and  thuebao_id in (8592878);
				and thuebao_id in (select thuebao_id from ttkd_bsc.ct_bsc_ptm where thang_ptm <= 202405 and replace(ma_gd, ' ', '') = 'HCM-LD/01617924');
				
				select dthu_goi, luong_dongia_nvptm, thang_tldg_dt, ma_tb, thuebao_id from ttkd_bsc.ct_bsc_ptm where thang_ptm <= 202406 and replace(ma_gd, ' ', '') = '000809631';

				select * from ttkd_bsc.ct_ptm_ngoaictr_imp where thang = 202406 and nvl(dichvuvt_id, 0)  in (14, 15, 16, 0) and command is not null ;and ghichu is not null;= 'update'; and ma_tb= 'hcm_vbn_00001065';
				...
				insert into ttkd_bsc.ct_bsc_ptm (thang_luong, thang_ptm, ten_pb, ma_pb, ten_to, ma_to, manv_ptm, tennv_ptm, ma_vtcv, loai_ld, NHOM_TIEPTHI, TENKIEU_LD, ma_gd, ma_tb, ma_kh
							...
							select 86, a.THANG, b.ten_pb, b.ma_pb, b.ten_to, b.ma_to, a.MANV_PTM, a.TENNV_PTM, b.ma_vtcv, b.loai_ld, b.NHOMLD_ID NHOM_TIEPTHI
											, nvl(kld.TEN_KIEULD, TENKIEU_LD) TEN_KIEULD , a.MA_GD, nvl(c.MA_TB, 'khongco'||rownum) ma_tb, nvl(kh.MA_KH, ) MA_KH, a.SO_HD
											...
											, 'ct_ptm_ngoaictr_imp_'||command as nguon, plk.MA_PLKH, nvl(kh.mst, 1) mst, nvl(db.dthu_ps, a.DTHU_GOI) dthu_ps, nvl(c.trangthaitb_id, 1) trangthaitb_id, 145 chuquan_id, null dongia, nvl(lh.loaihinh_tb, dichvu_vt) dich_vu
...
--											, null nop_du, null mien_hsgoc		--co ma_gd
								...
								where a.thang = 202406 and a.dichvuvt_id in (14, 15, 16)-- not in ('VNP tra sau', 'Internet truc tiep')
												and command = 'insert'
...
--												, a.tyle_hotro = nvl(t.heso_hotro_nvhotro, a.heso_hotro_nvhotro)
						...
						WHERE a.thuebao_id in (select thuebao_id from ttkd_bsc.ct_ptm_ngoaictr_imp where thang = 202406 and command = 'update')
					;
					commit;
					rollback;
					---check cot de UPDATE
