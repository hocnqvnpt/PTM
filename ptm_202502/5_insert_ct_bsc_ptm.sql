...
create table ttkd_bsc.ct_bsc_ptm_202502 as
	select * from ttkd_bsc.ct_bsc_ptm where thang_ptm >= 202401
	;
alter table ttkd_bsc.ct_bsc_ptm_202502 read only;
update ttkd_bsc.ct_bsc_ptm set thang_luong = thang_ptm
    where thang_luong<>thang_ptm and thang_ptm < 202502
    ;
...
insert into ttkd_bsc.ct_bsc_ptm 
                    ...
                    ngay_bbbg, ngay_cat, thoihan_id, tg_thue_tu, tg_thue_den,songay_sd,
...
--				,ma_tiepthi_new
				,to_tt_id,donvi_tt_id,donvi_tt,
                 ...
                 --   pbh_ptm_id
				,ma_pb,ten_pb,ma_to,ten_to,ma_vtcv, loainv_id, loai_ld,manv_hotro,tyle_hotro, tyle_am,
        ...
        select  202502 thang_ins, 202502 thang_ptm, ma_gd, ma_gd_gt,ma_kh,ma_tb,dich_vu, ten_kieuld,ten_tb,diachi_ld,so_gt,mst,mst_tt,
		...
		--			 doituong_id <> 190
					 ...
					 and not exists(select 1 from ttkd_bsc.ct_bsc_ptm where thang_ptm > = 202411 and hdtb_id=a.hdtb_id)
			 ;
commit
    ;
-- thue bao cdbr dung thu da chuyen sang dung that:  KIEMTRA SUBQURRY lai trong T6
	select * from ttkd_bsc.ct_bsc_ptm  where thang_luong = 49
	;
insert into ttkd_bsc.ct_bsc_ptm 
...
--			    ngay_tt,soseri,tien_tt, ht_tra_id
      ...
      ---thang n-3
	  select  49 thang_luong, 202502 thang_ptm, ma_gd,ma_kh,ma_tb,dich_vu,ten_kieuld,ten_tb,diachi_ld,so_gt,mst,mst_tt,
					    ...
					    union all
					    ----thang n-2
					    select  49 thang_ins, 202502 thang_ptm, ma_gd,ma_kh,ma_tb,dich_vu,ten_kieuld,ten_tb,diachi_ld,so_gt,mst,mst_tt,
									   tien_dnhm,tien_sodep,thang_bddc,thang_ktdc,sothang_dc, datcoc_csd,ma_duan,ma_duan_banhang, 
									   (select ngay_sd from css_hcm.db_thuebao where thuebao_id=a.thuebao_id) ngay_bbbg,
									   (select ngay_bg from ttkdhcm_ktnv.v_bangiao_hoso_new where ma_gd=a.ma_gd and ma_tb=a.ma_tb and loaitb_id=a.loaitb_id) ngay_luuhs_ttkd
	   ...
	   USING (with datcoc_goc as (select b.hdtb_id, a.thuebao_id , a.thang_bd thang_bddc, a.thang_kt thang_ktdc, a.cuoc_dc datcoc_csd, a.tien_td, ctkm.huong_dc
                                                                   from css_hcm.db_datcoc a, css_hcm.hdtb_datcoc b,css_hcm.ct_khuyenmai ctkm
                                                                    where a.thuebao_dc_id=b.thuebao_dc_id and a.chitietkm_id = ctkm.chitietkm_id and ctkm.khuyenmai_id not in (1977, 2056, 2998, 2999)
                                                                                and a.cuoc_dc > 0 and a.hieuluc = 1 and a.ttdc_id = 0 and b.thang_bd >= 202412			--thang n -2
...
--                                                                    group by a.hdtb_id, a.thuebao_id, a.thang_bddc,a.thang_ktdc     
											...
											select max(hdtb_id) hdtb_id, thuebao_id, min(thang_bddc) thang_bddc, min(thang_ktdc) thang_ktdc
                                                        ...
                                                        from datcoc_goc 
											 where datcoc_csd > 0 and thang_bddc >= 202412 --and thuebao_id = 12211057
                                                        group by  thuebao_id
						) b
...
-- thay doi toc do trong thang: a Tuyen da import
				select 'ct_bsc_ptm', count(*) from ttkd_bsc.ct_bsc_ptm where thang_ptm = 202502 and nguon='thaydoitocdo'
				union all
				select 'thaydoitocdo', count(*) from ttkd_bct.thaydoitocdo_202502;
...
-- tai lap theo vb 458/TTr-NS-DH 06/11/2020 tu ky 11/2020:
			select count(*) from ttkd_bsc.ct_bsc_ptm  
					where thang_ptm = '202502' and nguon='tailap'
					;
		---thang n
			insert into ttkd_bsc.ct_bsc_ptm 
							...
							,lydo_khongtinh_luong,manv_ptm,tennv_ptm
							,ma_pb,ten_pb,ma_to,ten_to,ma_vtcv,loainv_id,loai_ld, nhom_tiepthi,manv_hotro,tyle_hotro
							,dthu_ps, dthu_goi_goc,dthu_goi, tinh_id , nguon, nhanvien_nhan_id,chuquan_id, tocdo_id)
			
				   select  
							202502 thang_ins, 202502 thang_ptm, ma_gd,ma_kh,ma_tb,dich_vu,ten_kieuld tenkieu_ld,ten_tb,diachi_ld
							...
							, manv_ptm, tennv_ptm, a.ma_pb, a.ten_pb, a.ma_to, a.ten_to, a.ma_vtcv, a.loainv_id, a.loai_ld, a.nhom_tiepthi
							,manv_hotro,tyle_hotro
							,dthu_ps,dthu_goi_goc,dthu_goi, tinh_id , 'tailap', a.nhanvien_id
							,chuquan_id, tocdo_id
				   
				   from ttkd_bct.tailap_202502 a
								--	left join ttkd_bsc.nhanvien nv on nv.ma_nv = a.manv_ptm and thang = 202502
				   where a.duoctinh_ptm = 1 and a.manv_ptm is not null
							and not exists (select 1 from ttkd_bsc.ct_bsc_ptm where thang_ptm = 202502 and loaihd_id=7 and hdtb_id=a.hdtb_id)
				;
...
--	 select * from ttkd_bsc.ct_bsc_ptm
	where thang_ptm=202502 and dich_vu='MyTV MOBILE'
;      
...
--		select manv_ptm,tennv_ptm, ma_to,ten_to,ma_pb,ten_pb,ma_vtcv, loainv_id, ten_loainv, loai_ld, nhom_tiepthi from ttkd_bsc.ct_bsc_ptm a 
			    where thang_ptm=202502 and loaitb_id = 271
...
--					select thang_tldg_dt, thang_luong, thang_ptm, ma_gd_gt, ma_tb,  ghi_chu, ungdung_id, nguon, manv_ptm, ma_nguoigt, nguoi_gt, manv_hotro, heso_hotro_nvhotro, luong_dongia_nvptm, luong_dongia_nvhotro from ttkd_bsc.ct_bsc_ptm a
					WHERE thang_ptm = 202502 and loaitb_id = 20 and ma_tb is not null and manv_hotro is null
...
--										select thang_tldg_dt, thang_luong, ngay_bbbg, thang_ptm, ma_gd_gt, ma_tb,  ghi_chu, ungdung_id, nguon, manv_ptm, ma_nguoigt, nguoi_gt, manv_hotro, heso_hotro_nvhotro, luong_dongia_nvptm, luong_dongia_nvhotro from ttkd_bsc.ct_bsc_ptm a
										WHERE thang_ptm = 202502 and ma_gd_gt is not null and a.manv_hotro is null
...
---SHOP_CTV Fiber, MyTV, Mesh
				---theo ma_dhsx = ma_gd_gt, trangthai_shop = thanh cong
				Select * From khanhtdt_ttkd.IMP_SHOPCTV_DH where thang = 202502 and VAITRO_CTV = 'CTV liên kết' and MA_DHSXKD is not null
				;
					MERGE INTO ttkd_bsc.ct_bsc_ptm a
									USING (select ma_dhsxkd, ma_nvkd, ten_nvkd, sodt_nvkd 
																from khanhtdt_ttkd.IMP_SHOPCTV_DH 
																where thang = 202502 and VAITRO_CTV = 'CTV liên kết' and MA_DHSXKD is not null) b
									...
									WHEN MATCHED THEN
												...
												SET a.manv_hotro = b.ma_nvkd
														...
														, thang_luong = 4
--														, thang_tldg_dt_nvhotro = 202502, thang_tlkpi_hotro = 202502
													--	, a.nguoi_gt = b.tennv_gioithieu
--										select thang_tldg_dt, thang_luong, ngay_bbbg, thang_ptm, ma_gd_gt, ma_tb,  ghi_chu, ungdung_id, nguon, manv_ptm, ma_nguoigt, nguoi_gt, heso_hotro_nvptm, manv_hotro, heso_hotro_nvhotro, luong_dongia_nvptm, luong_dongia_nvhotro from ttkd_bsc.ct_bsc_ptm a
										WHERE thang_ptm = 202502 and ma_gd_gt is not null and a.manv_hotro is null
														and ma_gd_gt  in (select ma_dhsxkd from khanhtdt_ttkd.IMP_SHOPCTV_DH
...
rollback;
				...
				from ttkd_bsc.ct_bsc_ptm a
								left join (select sdt_datmua, ma_gioithieu from ttkd_bsc.digishop where sdt_datmua is not null and lower(trangthai_shop) like 'th_nh c_ng') b
											on a.ma_tb = b.sdt_datmua
				WHERE a.thang_ptm = 202412 and thang_luong = 4 and a.loaitb_id = 20 and a.ma_tb is not null
				...
				from ttkd_bsc.ct_bsc_ptm a
									 left join (select ma_dhsx, ma_gioithieu from ttkd_bsc.digishop where ma_dhsx is not null and lower(trangthai_shop) like 'th_nh c_ng') b
											on a.ma_gd_gt = b.ma_dhsx
				WHERE thang_ptm = 202412 and thang_luong = 4 and dichvuvt_id not in (7,8,9, 14,15,16, 2)
							
			;				
		----
		select * 
		from ttkd_bsc.ct_bsc_ptm a
				WHERE a.thang_ptm = 202411 and a.GHI_CHU = 'donghang_digi'
