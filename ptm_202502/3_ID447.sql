							...
							, d as (select ptm.id, ptm.ma_yeucau, ptm.ngaycapnhat, ptm.ngaycapnhat_tl, ptm.ma_gd, ptm.ma_tb, ptm.sohopdong
										...
										from ttkdhcm_ktnv.amas_duan_ngoai_doanhthu ptm
												 ...
												 where ptm.trangthai_tl=10 and ptm.dexuat=7 and ptm.ma_dichvu not in (20,149) 
															    ...
															    and trunc(ngaycapnhat_tl) <= to_date('07/03/2025','dd/mm/yyyy') 			---ngay 7 thang n+ 1
															    and trunc(ngaycapnhat_khkt) <= to_date('07/03/2025','dd/mm/yyyy') 			---ngay 7 thang n+ 1
															    and not exists(select * from ttkd_bct.ptm_gtgt where id=ptm.id)                 
												 )
--												 	select * from d where id in (7301445)
--SELECT trunc(sysdate, 'month') FROM DUAL
			    select 202502 thang, d.id, d.ma_yeucau, d.ngaycapnhat, d.ngaycapnhat_tl, d.loaihinh_tb, d.dichvuvt_id, d.loaitb_id_obss, d.ma_gd
			    ...
			    from d
							left join ttkd_bsc.nhanvien e on d.manv_ptm = e.ma_nv and e.thang = 202502
--				where ma_yeucau = '10972465'
			where not exists (select * from ttkd_bct.ptm_gtgt where id = d.id)
			    order by id
			    ;
...
-- and ma_kh='GTGT rieng';
			...
			insert into ttkd_bsc.ct_bsc_ptm 
			    ...
			    select 202502 thang_ins, 202502 thang_ptm, id, case when ma_gd is not null then ma_gd else sohopdong end ma_gd, 'GTGT rieng' ma_kh, ma_tb, 1 loaihd_id
			    ...
			    where thang = 202502 and not exists(select 1 from ttkd_bsc.ct_bsc_ptm where id_447 = a.id) 
			    ;
