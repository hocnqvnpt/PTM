create table ptm_codinh_202502 as
select * from ptm_codinh_202502_cntt	--48s
union all
select * from ptm_codinh_202502_tsl	--39s
union all
select * from ptm_codinh_202502_cd --93s đk dichvt_id tren schema tinhcuoc
union all
select * from ptm_codinh_202502_br	---329s
;
drop table x_v_db purge
;

select * from ptm_codinh_202502;
create table ptm_codinh_202502_tsl as
...
;
             ...
             select * from ptm_codinh_202502
                where (ma_tb, ma_gd, hdtb_id) in 
                                                ...
                                                from ptm_codinh_202502 a 
                                                group by ma_tb, ma_gd, hdtb_id having count(*)>1 )
            ;
--Them cot------ tien_dnhm, tien_camket, tien_sodep:
                    ALTER TABLE ptm_codinh_202502 ADD (tien_dnhm NUMBER, tien_sodep NUMBER, tien_camket NUMBER)
				;
                    UPDATE ptm_codinh_202502 a 
                ...
                commit;
                    ...
                    MERGE INTO ptm_codinh_202502 a
                    ...
                    MERGE INTO ptm_codinh_202502 a
                                ...
                                UPDATE ptm_codinh_202502 a
                                    ...
                                    -- SELECT ma_tb, tien_sodep FROM ttkd_bct.ptm_codinh_202502 a
...
--                    update ptm_codinh_202502 set tien_dvgt='';
                    ...
                    update ptm_codinh_202502 a 
                        ...
                        -- select tien_dvgt from ptm_codinh_202502 a
...
-- tien_tbi: 
--                    UPDATE ptm_codinh_202502 SET tien_tbi=''
				;
                    MERGE INTO ptm_codinh_202502 a
                    USING (SELECT cttbi.hdtb_id, tbi_lhtb.loaitb_id, SUM(cttbi.soluong*cttbi.tien) tien_tbi
                                    ...
                                    WHERE cttbi.loaitbi_id=tbi.loaitbi_id AND tbi.loaitbi_id=tbi_lhtb.loaitbi_id AND lhtb.loaitb_id=tbi_lhtb.loaitb_id
                                                        ...
                                                        AND EXISTS(SELECT 1 FROM ptm_codinh_202502 
...
--                                , round(datcoc.tien_td/1.1,0) tien_td
		...
		;
            alter table ptm_codinh_202502 add hdtb_id_datcoc number
		  ;
            ...
            update ptm_codinh_202502 a 
                ...
                -- select thuebao_id, ma_tb, sothang_dc, datcoc_csd, tien_td from ptm_codinh_202502 a
                where datcoc_csd is null 
                    and exists(select 1 from temp_datcoc b
                                        where datcoc_csd>0 and thuebao_id=a.thuebao_id and ma_nv=a.ma_tiepthi)
                    and ngay_bbbg=(select max(ngay_bbbg) from ptm_codinh_202502 where thuebao_id=a.thuebao_id )
                    ;
                    
            ---recover datcoc_csd updated
--                update ptm_codinh_202502 set datcoc_csd = null 
----				select * from ttkd_bct.ptm_codinh_202502
--				where hdtb_id_datcoc is not null;
            commit;
            
--            select a.*, b.ma_tiepthi , b.DATCOC_CSD, b.SOTHANG_DC, b.hdtb_id_datcoc
--            from temp_datcoc a, ptm_codinh_202502 b
--            where a.thuebao_id=b.thuebao_id
            ;
            
------ thoihan_id
            update ptm_codinh_202502 set thoihan_id=1 
	...
	-- Cuoc phat sinh:    ---chuyen len dataguard
				...
				select distinct a.* from bcss.v_ct_no a
				    where phanvung_id = 28 and ky_cuoc = 20250201 		--thang n
					   and khoanmuctt_id not in (441,520,521,527,3126,3127,3421,3953) 
					   and exists(select 1 from ptm_codinh_202502 where thuebao_id=a.thuebao_id)
					   ;
				create index tmp_ct_no_tbid on tmp_ct_no (thuebao_id);
				
				alter table ptm_codinh_202502 add dthu_ps number(12)
--				update ptm_codinh_202502 a set dthu_ps=''
				;
				update ptm_codinh_202502 a 
		...
		commit;
				...
				---select dthu_ps from ptm_codinh_202502 a
-- Dthu goi:
				alter table ptm_codinh_202502 
				    add (dthu_goi_goc number(12), dthu_goi number(12), dthu_goi_ngoaimang number(12))
				;
				update ptm_codinh_202502 bc set dthu_goi_goc='', dthu_goi='', dthu_goi_ngoaimang=''
				
				;
				--  select * from ttkd_bct.ptm_codinh_202502;
				
				update ptm_codinh_202502 bc set dthu_goi_goc=
				    (case
...
--															where soluong_ip=1 and sl_ip_mp is null and tocdo_id=bc.tocdo_id)=1
			...
			;
			commit;
select * from ptm_codinh_202502 where tyle_sd >= 50;
...
--															where tien_camket > 250000 then 0
                                            ...
                                            from ptm_codinh_202502 b
										  where loaihd_id=1 and thuebao_id=a.thuebao_id                 
											  and exists (select 1 from ptm_codinh_202502 where thuebao_id=b.thuebao_id and loaihd_id=6)
											  )
		    where loaitb_id=77 and loaihd_id=6
					  and exists (select 1 from ptm_codinh_202502 where thuebao_id=a.thuebao_id and loaihd_id<>6)
								
				;
			
			update ptm_codinh_202502 set dthu_goi=dthu_goi_goc
			...
			update ptm_codinh_202502 a 
					    set dthu_goi_ngoaimang =
											 (select sum(nogoc) from bcss.v_tonghop
												where ky_cuoc = 20250201 --thang n
...
--			    select * from ptm_codinh_202502 a
 ...
 ---move ve TTKDDB
		  create table ttkd_bct.ptm_codinh_202502 as select * from ttkd_hcm.ptm_codinh_202502@dataguard;
		  SELECT * FROM ttkd_bct.ptm_codinh_202502;

--- Rycle Bin DATAGUARD
		drop table ptm_codinh_202502_bs purge;
		drop table ptm_codinh_202502_cd purge;
		drop table ptm_codinh_202502_tsl purge;
		drop table ptm_codinh_202502_cntt purge;
		drop table ptm_codinh_202502_br purge;
		  
		  ----Update ma_nguoigt la daily
		  update ttkd_bct.ptm_codinh_202502 a
						set NHANVIENGT_ID= a.ctv_id
								, ma_nguoigt = upper(a.ma_tiepthi)
								, NGUOI_GT =  a.ten_tiepthi
								, NHOM_GT = a.donvi_tt
--					select * from ttkd_bct.ptm_codinh_202502 a
		  ...
		  create index ttkd_bct.idx_ptm_hdtbid on ttkd_bct.ptm_codinh_202502 (hdtb_id);
		  
		  -- lydo_khongtinh_luong: Update ky luong T3/24						
						alter table ttkd_bct.ptm_codinh_202502 add lydo_khongtinh_luong varchar2(200)
						;
						update ttkd_bct.ptm_codinh_202502 a
									set lydo_khongtinh_luong = (
														   with x as (
																			  select hdtb_id
																						   ...
																						   ,case when ( upper(ma_tiepthi)like'GTGT_%' 
																													...
																													or upper(ma_nguoigt)like'DAILY_%' or upper(ma_nguoigt)like'DL_%')
																															then ';kq5 Phat trien qua Dai ly, chi tinh bsc cho To, Phong' 
																								end kq5
																			  ...
																			  from ttkd_bct.ptm_codinh_202502 b
																			)                                        
                    ...
                    -- nv ptm:   
                    alter table ttkd_bct.ptm_codinh_202502
				...
				insert into ttkd_bsc.ptm_xuly_50_BHOL 
						 select 202502 thang, hdtb_id, thuebao_id, loaitb_id, ma_tiepthi, pbh_nhan_id
						 ...
						 from ttkd_bct.ptm_codinh_202502 a
...
--				     select ma_tiepthi, manv_ptm from ttkd_bct.ptm_codinh_202502 a
				    where loaitb_id = 288 and ma_tiepthi is null 
					   and pbh_nhan_id=2941 and nhanvien_id=1077 and datcoc_csd>0
					   ; 
...
-- Tao ds dai ly thang va ins dai ly moi:
			--****-- Dai ly hien huu:
--			delete from dm_daily_khdn where thang='202502' ;
			...
			insert into ttkd_bsc.dm_daily_khdn --(ma_daily,ten_daily,manv_qldaily,ma_pb,thang,thang_kyhd,ma_to)
			    ...
			    select 202502, ma_daily, ten_daily, manv_qldaily, ma_to, ma_pb, ma_vtcv, ten_vtcv, thang_kyhd, ghichu
			  ...
			  --****-- Dai ly moi:
					...
					;
					    select distinct ma_tiepthi ma_daily, (select upper(ten_nv) from admin_hcm.nhanvien_onebss where ma_nv = a.ma_tiepthi) ten_daily,
								 ma_pb, ma_to, ma_nv, 202502, 202502, 'DAI LY MOI' loai_hopdong
					    from 
						   (
...
--					     select MA_GD, pbh_nhan_id, pbh_nhan_goc_id, donvi_tt_id, ma_pb, ten_pb from ttkd_bct.ptm_codinh_202502 a
...
--					     select ma_tiepthi, ma_gd, ma_nguoigt, (select ma_dv from admin_Hcm.donvi where donvi_id = a.pbh_nhan_id)ma_dv, ma_pb, ten_pb, pbh_nhan_id from ttkd_bct.ptm_codinh_202502 a
...
--				    select ma_gd, (select ten_dv from admin_Hcm.donvi where donvi_id = 11298) ten_dv, pbh_nhan_id, pbh_nhan_goc_id from ttkd_bct.ptm_codinh_202502 a
...
--				      select ma_gd, (select ten_dv from admin_Hcm.donvi where donvi_id = 11298) ten_dv, pbh_nhan_id, pbh_nhan_goc_id, ten_tb, ma_pb, manv_ptm from ttkd_bct.ptm_codinh_202502 a
...
--				     select ma_tiepthi, ma_pb, pbh_nhan_id, donvi_tt_id, donviql_tt_id, (select ma_dv from admin_hcm.donvi where donvi_id = a.donvi_tt_id) ma_pb_new from ttkd_bct.ptm_codinh_202502 a
...
--					      select ma_gd, ma_tiepthi, ma_pb, pbh_nhan_id, donvi_tt_id, donviql_tt_id, (select ma_dv ||ten_dv from admin_Hcm.donvi where donvi_id = a.DONVI_TT_ID) from ttkd_bct.ptm_codinh_202502 a
					...
					update ttkd_bct.ptm_codinh_202502 a
					    set  
							  ma_pb = (select ma_dv from admin_Hcm.donvi where donvi_id = a.pbh_nhan_id)
							  , ten_pb = (select ten_dv from admin_Hcm.donvi where donvi_id = a.pbh_nhan_id)
--					      select ma_gd, ma_tiepthi, manv_ptm, ma_pb, pbh_nhan_id, donvi_tt_id, donviql_tt_id, (select ma_dv ||ten_dv from admin_Hcm.donvi where donvi_id = a.pbh_nhan_id) from ttkd_bct.ptm_codinh_202502 a
					    where ma_pb is null and chuquan_id in (145,266,264)
				...
				update ttkd_bct.ptm_codinh_202502 a set manv_hotro='', tyle_hotro='', tyle_am='' where ma_duan_banhang is not null
				;	

				--select * from ttkdhcm_ktnv.amas_booking_presale where ma_yeucau = 228705;
					MERGE into ttkd_bct.ptm_codinh_202502 a
							using (
												with 
...
--					select manv_hotro, tyle_hotro, tyle_am, ma_duan_banhang, loaitb_id, ma_gd, to_number(regexp_replace (a.ma_duan_banhang, '\D', '')) mada from ttkd_bct.ptm_codinh_202502 a
...
---- kiem tra trung
			    ...
			    thang_bddc, datcoc_csd, phongbh_nhan, loaitb_id,
					  ...
					  from ttkd_bct.ptm_codinh_202502
					  where (ma_gd, thuebao_id) in (select ma_gd, thuebao_id from ttkd_bct.ptm_codinh_202502 group by ma_gd, thuebao_id having count(*)>1) 
...
-- Kiem tra dthu goi:
				select a.ma_gd, ma_tb, datcoc_csd, dthu_goi, dthu_ps, manv_ptm
				    from ttkd_bct.ptm_codinh_202502 a
				    where (ma_gd, thuebao_id, ma_tb, kieuld_id) in (select ma_gd, thuebao_id, ma_tb, kieuld_id from ttkd_bct.ptm_codinh_202502 group by ma_gd, thuebao_id, ma_tb, kieuld_id having count(*)>1) 
				    order by thuebao_id, ngay_bbbg;
				
				select * from ttkd_bct.ptm_codinh_202502 
--				 delete from ttkd_bct.ptm_codinh_202502
				...
				select * from bcss.v_thftth@dataguard a 
				    where phanvung_id=28 and ky_cuoc=20250201
					   and exists(select 1 from ttkd_bct.ptm_codinh_202502 
...
-- Tai lap:
--				update ttkd_bct.ptm_codinh_202502 a
--				    set ngay_td_kytruoc = (select ngay_td from tinhcuoc.v_dbtb@dataguard where ky_cuoc=20250201 and ngay_td is not null and thuebao_id=a.thuebao_id)
--				    where loaihd_id=7 and kieuld_id in (96,13089) and ngay_td_kytruoc is null;
--				commit;
--				
--				
--				update ptm_codinh_202502 a set songay_tamngung = '' 
--				;
--				update ptm_codinh_202502 a
...
--				commit;
				...
				drop table ttkd_bct.tailap_202502 purge
				;
				select * from ttkd_bct.tailap_202502;
				create table ttkd_bct.tailap_202502 as 
				    select a.* from ttkd_bct.ptm_codinh_202502 a
					   where a.kieuld_id in (96,13089) and duoctinh_ptm=1
						  and hdtb_id=(select max(hdtb_id) from ttkd_bct.ptm_codinh_202502 
											  where kieuld_id in (96,13089) and thuebao_id=a.thuebao_id)
								;
												 
												    
--				create index ttkd_bct.tailap_202502_ttid on ttkd_bct.tailap_202502 (thanhtoan_id);
--				create index ttkd_bct.tailap_202502_tbid on ttkd_bct.tailap_202502 (thuebao_id);
--				create index ttkd_bct.tailap_202502_hdtbid on ttkd_bct.tailap_202502 (hdtb_id)
				;
