...
create table ttkd_bsc.ct_bsc_ptm_202406 as
	select * from ttkd_bsc.ct_bsc_ptm where thang_ptm >= 202306
	;
update ttkd_bsc.ct_bsc_ptm set thang_luong=thang_ptm
    where thang_luong<>thang_ptm and thang_ptm < 202407
    ;
    commit;
    rollback;
...
--				,ma_tiepthi_new
        ...
        select  202407 thang_ins, 202407 thang_ptm, ma_gd, ma_gd_gt,ma_kh,ma_tb,dich_vu, ten_kieuld,ten_tb,diachi_ld,so_gt,mst,mst_tt,
...
--			 doituong_id <> 190
			 ...
			 and dichvuvt_id <> 2 --- khong import dvu VNP tren Onebss
                and not exists(select 1 from ttkd_bsc.ct_bsc_ptm where thang_ptm > = 202401 and hdtb_id=a.hdtb_id)
			 ;
select * from css_hcm.chuquan where chuquan_id in (145, 264, 266);
...
--			    ngay_tt,soseri,tien_tt, ht_tra_id
      ...
      ---thang n-3
	  select  49 thang_luong, 202407 thang_ptm, ma_gd,ma_kh,ma_tb,dich_vu,ten_kieuld,ten_tb,diachi_ld,so_gt,mst,mst_tt,
					    ...
					    union all
					    ----thang n-2
					    select  49 thang_ins, 202407 thang_ptm, ma_gd,ma_kh,ma_tb,dich_vu,ten_kieuld,ten_tb,diachi_ld,so_gt,mst,mst_tt,
									   tien_dnhm,tien_sodep,thang_bddc,thang_ktdc,sothang_dc, datcoc_csd,ma_duan,ma_duan_banhang, 
									   (select ngay_sd from css_hcm.db_thuebao where thuebao_id=a.thuebao_id) ngay_bbbg,
									   (select ngay_bg from ttkdhcm_ktnv.v_bangiao_hoso_new where ma_gd=a.ma_gd and ma_tb=a.ma_tb and loaitb_id=a.loaitb_id) ngay_luuhs_ttkd
...
--                                                                    group by a.hdtb_id, a.thuebao_id, a.thang_bddc,a.thang_ktdc     
											...
											select max(hdtb_id) hdtb_id, thuebao_id, min(thang_bddc) thang_bddc, min(thang_ktdc) thang_ktdc
                                                        ...
                                                        from datcoc_goc 
											 where datcoc_csd > 0 and thang_bddc >= 202404 --and thuebao_id = 12211057
                                                        group by  thuebao_id
						) b
...
-- thay doi toc do trong thang: a Tuyen da import
				select 'ct_bsc_ptm', count(*) from ttkd_bsc.ct_bsc_ptm where thang_ptm = 202407 and nguon='thaydoitocdo'
				union all
				select 'thaydoitocdo', count(*) from ttkd_bct.thaydoitocdo_202407;
...
-- tai lap theo vb 458/TTr-NS-DH 06/11/2020 tu ky 11/2020:
			select count(*) from ttkd_bsc.ct_bsc_ptm  
					where thang_ptm='202407' and nguon='tailap'
					;
		---thang n
			insert into ttkd_bsc.ct_bsc_ptm 
				   ...
				   select  
							202407 thang_ins, 202407 thang_ptm, ma_gd,ma_kh,ma_tb,dich_vu,ten_kieuld tenkieu_ld,ten_tb,diachi_ld
				   ...
				   from ttkd_bct.tailap_202407 a
								--	left join ttkd_bsc.nhanvien nv on nv.ma_nv = a.manv_ptm and thang = 202405
				   where a.duoctinh_ptm = 1 and a.manv_ptm is not null
							and not exists (select 1 from ttkd_bsc.ct_bsc_ptm where thang_ptm = 202407 and loaihd_id=7 and hdtb_id=a.hdtb_id)
				;
...
--	 select * from ttkd_bsc.ct_bsc_ptm
	where thang_ptm=202407 and dich_vu='MyTV MOBILE'
;      
...
--		select manv_ptm,tennv_ptm, ma_to,ten_to,ma_pb,ten_pb,ma_vtcv, loainv_id, ten_loainv, loai_ld, nhom_tiepthi from ttkd_bsc.ct_bsc_ptm a 
			    where thang_ptm=202407 and loaitb_id = 271
...
--					select thang_tldg_dt, thang_luong, thang_ptm, ma_gd_gt, ma_tb,  ghi_chu, ungdung_id, nguon, manv_ptm, ma_nguoigt, nguoi_gt, manv_hotro, heso_hotro_nvhotro, luong_dongia_nvptm, luong_dongia_nvhotro from ttkd_bsc.ct_bsc_ptm a
					WHERE thang_ptm >= 202406 and loaitb_id = 20 and ma_tb is not null and manv_hotro is null
...
--									and ma_tb = '84911180458'
					...
					rollback;
					commit;
					select * from ttkd_bsc.digishop where ma_dhsx = 'HCM-GT/00163907';
...
--										select thang_tldg_dt, thang_luong, ngay_bbbg, thang_ptm, ma_gd_gt, ma_tb,  ghi_chu, ungdung_id, nguon, manv_ptm, ma_nguoigt, nguoi_gt, manv_hotro, heso_hotro_nvhotro, luong_dongia_nvptm, luong_dongia_nvhotro from ttkd_bsc.ct_bsc_ptm a
										WHERE thang_ptm >= 202406 and ma_gd_gt is not null and a.manv_hotro is null
