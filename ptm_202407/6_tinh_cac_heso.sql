...
;
create table ttkd_bsc.ct_bsc_ptm_202406_l2 as
	select * from ttkd_bsc.ct_bsc_ptm where thang_ptm >= 202306 		--thang n-24
;
	   	...
	   	----BSUNG ma nhan vien tiep thi
			UPDATE ttkd_bsc.ct_bsc_ptm a
										set manv_ptm = (select y.ma_nv 
												...
												, thang_luong = 4
--					select thang_luong, thang_ptm, thang_tldg_dt, THANG_TLKPI_PHONG, thuebao_id from ttkd_bsc.ct_bsc_ptm a
									...
									= (select TEN_NV, MA_TO, TEN_TO, MA_PB, TEN_PB, MA_VTCV, LOAI_LD, NHOMLD_ID
											from 
													(select ma_nv, b.ten_nv, b.ma_to, b.ten_to, b.ma_pb, b.ten_pb, b.ma_vtcv, b.loai_ld, b.nhomld_id
													  ...
													  where b.thang >= 202404 --thang n-3
																	
													  ) where ma_nv = a.manv_ptm and thang = a.thang_ptm
											) 
--			select * from ttkd_bsc.ct_bsc_ptm a
							    where  a.thang_ptm >= 202404 and thang_luong = 4 and tennv_ptm is null and ma_vtcv is null 
											and nvl(thang_tldg_dt, 999999) >= 202407 and nvl(loaitb_id, 0) <> 21
											and exists (select 1	from ttkd_bsc.nhanvien b
																		where b.thang >= 202404 --thang n-3
																							and b.ma_nv = a.manv_ptm and b.thang = a.thang_ptm)
						;
				----END
...
---			select ma_tb, thuebao_id, ngay_tt, soseri, tien_tt, trangthai_tt_id, hdtb_id from ttkd_bsc.ct_bsc_ptm a
			WHERE thang_ptm >= 202404 and chuquan_id <> 264 and thang_tldg_dt is null and nvl(trangthai_tt_id,0) = 0			--thang n -3
...
-- DTHU_PS:
-- Dot 1: da update ps 1 lan 4/8/24
	...
	;
		create table ttkd_bsc.tmp_ct_no as 
				select thuebao_id, khoanmuctt_id, nogoc 
						...
						where phanvung_id=28 and ky_cuoc=20240701
					;
...
-- dthu_ps thang n:
		update ttkd_bsc.ct_bsc_ptm a 
		    ...
		    where thang_ptm = 202407 and loaitb_id<>21 and dthu_ps is null 
		...
		update ttkd_bsc.ct_bsc_ptm a 
			   set dthu_ps_n1 = (select sum(nogoc) from ttkd_bsc.tmp_ct_no
										 where khoanmuctt_id not in (441,520,521,527,3126,3127,3421,3953) and thuebao_id=a.thuebao_id)
		    where thang_ptm = 202406 and loaitb_id not in (20,21,131)
			...
			update ttkd_bsc.ct_bsc_ptm a 
				   set dthu_ps_n2 = (select sum(nogoc) from ttkd_bsc.tmp_ct_no
											 where khoanmuctt_id not in (441,520,521,527,3126,3127,3421,3953) and thuebao_id=a.thuebao_id)
			    where thang_ptm = 202405 and loaitb_id not in (20,21,131)
...
-- Dot 2: vi co cuoc VNP, CNTT, GTGT vi sau ngay 5 moi tinh cuoc xong
		update ttkd_bsc.ct_bsc_ptm a 
		    ...
		    -- select * from ttkd_bsc.ct_bsc_ptm a
		    where thang_ptm = 202407 and loaitb_id<>21 and nvl(dthu_ps, 0) = 0 
...
--		    select * from ttkd_bsc.ct_bsc_ptm a
		    where thang_ptm = 202406 and loaitb_id<>21 and nvl(dthu_ps_n1, 0) = 0
			   and exists(select 1 from ttkd_bct.cuoc_thuebao_ttkd where dthu>0 and ma_tb=a.ma_tb and loaitb_id=a.loaitb_id)
			   ;
...
--		    select * from ttkd_bsc.ct_bsc_ptm a		    
		    where thang_ptm = 202405 and loaitb_id<>21 and nvl(dthu_ps_n2, 0) = 0
			   and exists(select 1 from ttkd_bct.cuoc_thuebao_ttkd where dthu>0 and ma_tb=a.ma_tb and loaitb_id=a.loaitb_id)
			   ;
...
--		    select * from ttkd_bsc.ct_bsc_ptm a		    
			    where thang_ptm = 202404 and loaitb_id<>21 and nvl(dthu_ps_n3, 0) = 0
				   and exists(select 1 from ttkd_bct.cuoc_thuebao_ttkd where dthu>0 and ma_tb=a.ma_tb and loaitb_id=a.loaitb_id)
				   ;
commit;
...
--		    select thang_luong, thang_ptm, ma_tb, trangthaitb_id, ngay_bbbg, ngay_cat, (select ngay_cat from css_hcm.db_thuebao where thuebao_id=a.thuebao_id) ngay_cat_dbonline, dthu_ps, dthu_ps_n1, dthu_ps_n2, dthu_ps_n3, dthu_goi, nvl(a.dthu_ps,0)+nvl(a.dthu_ps_n1,0)+nvl(a.dthu_ps_n2,0)+nvl(a.dthu_ps_n3,0) dthu_goi_new, ghi_chu from ttkd_bsc.ct_bsc_ptm a
		    where thang_ptm >= 202404 and thoihan_id=1 and dichvuvt_id in (1, 10, 11, 4, 7, 8, 9) ---thang n-3
					   ...
					   and exists(select 1 from css_hcm.db_thuebao 
												where trangthaitb_id = 7 and to_number(to_char(ngay_cat,'yyyymm')) = to_number(to_char(trunc(sysdate, 'month') - 1, 'yyyymm')) --thang n
...
--insert into hocnq_ttkd.temp_ps
						with 
								   ...
								   , datcoc as (select thuebao_id, sum(round(cuoc_dc/1.1,0)) datcoc_csd_new
										 ...
										 where hieuluc=1 and ttdc_id = 0 and to_number(to_char(ngay_cn,'yyyymm')) >= 202404
										 group by thuebao_id
										 )
							select thang_ptm, a.thuebao_id, ma_gd, ma_tb, dthu_goi, dthu_ps, dthu_ps_n1, dthu_ps_n2, dthu_ps_n3
								...
								from ttkd_bsc.ct_bsc_ptm a
										    ...
										    left join ps on a.thuebao_id = ps.thuebao_id
								where thang_ptm >= 202404 and nvl(dthu_goi, 0) = 0 --dthu_goi is null		---thang n -3
										  and (thang_tldg_dt is null 
																or thang_tldg_dt = to_number(to_char(trunc(sysdate, 'month') - 1, 'yyyymm'))											---thang n
													)
...
--										  and ma_tb = 'hcm_ca_00067906'
			...
			update ttkd_bsc.ct_bsc_ptm a 
							set   thang_luong='2' , 
--									, DTHU_PS = case when thang_ptm = 202406 then 1 end
--									, DTHU_PS_N1 = case when thang_ptm = 202405 then 1 end
--									, DTHU_PS_N2 = case when thang_ptm = 202404 then 1 end
--									, DTHU_PS_N3 = case when thang_ptm = 202403 then 1 end
...
--				 select thang_luong, thang_ptm, hdkh_id, thuebao_id, ma_tb,  dich_vu, tenkieu_ld, dthu_goi_goc, dthu_goi, thang_ptm, dthu_ps, dthu_ps_n1, dthu_ps_n2, dthu_ps_n3 , lydo_khongtinh_dongia, doanhthu_dongia_nvptm, luong_dongia_nvptm, thang_tldg_dt, thang_tlkpi, thang_tlkpi_to from ttkd_bsc.ct_bsc_ptm a
				where 
							exists  (select dthu_tong from hocnq_ttkd.temp_ps where 
														ma_gd=a.ma_gd and ma_tb =a.ma_tb and thang_ptm=a.thang_ptm) 
							and 
							nvl(dthu_goi, 0) = 0
--						and ma_tb = 'hcm_ca_00067906'
...
-- SMS Brandname: Tinh lai dthu goi, dthu_ps thang n va dthu_ps thang n+1
		...
		create table ttkd_bsc.tmp_bsc_smsbrn as
		    ...
		    from ttkd_bsc.ct_bsc_ptm a
		    where thang_ptm = 202406 and loaitb_id=131 ---thang n -1
		    ;


		update ttkd_bsc.tmp_bsc_smsbrn a 
			    set dthu_goi_ngoaimang_n = (select sum(nogoc) from bcss.v_tonghop@dataguard  --thang n- 1
															where phanvung_id=28 and ky_cuoc=20240601 and khoanmuctc_id in (37,38,39,40,935,936,938,939,943,944,945,4097) and ma_tb=a.ma_tb)
				   ,dthu_goi_ngoaimang_n1= (select sum(nogoc) from bcss.v_tonghop@dataguard    ---thang n
                                                            where phanvung_id=28 and ky_cuoc=20240701 and khoanmuctc_id in (37,38,39,40,935,936,938,939,943,944,945,4097) and ma_tb=a.ma_tb)
			;
...
update ttkd_bsc.ct_bsc_ptm a set khachhang_id = 9441989 where ma_tb = 'hcm_edu_00009976' and thang_ptm = 202403;
...
--			    	    select thang_tldg_dt, thang_luong, thang_ptm, khachhang_id, ma_tb, doituong_kh, dich_vu, mst from ttkd_bsc.ct_bsc_ptm a
				...
				update ttkd_bsc.ct_bsc_ptm a
					   set doituong_kh = (case when-- (loaitb_id not in (20,149) and doituong_id in (374, 387, 361, 362))
																    ...
																    (loaitb_id in (20,149) and doituong_id in (1, 25))
																				then 'KHCN' 
																when loaitb_id in (20,149) and doituong_id in (2, 3, 4, 5, 6, 7, 10, 16, 18, 19, 21) then 'KHDN'
--																when length(mst) >= 10 then 'KHDN'
														else null end)
...
--																	and nvl(a.ma_duan_banhang, 'r') <> replace(ma_duan, ' ', '')
								...
								and thang_ptm >= 202404
								and nvl(thang_tldg_dt, 999999) >= 202407
								and nvl(thang_tldg_dt_nvhotro, 999999) >= 202407			--- chua tinh luong
...
--					  select thang_luong, thang_ptm, manv_hotro, tyle_hotro, tyle_am, ma_duan_banhang, loaitb_id, ma_gd from ttkd_bsc.ct_bsc_ptm a
							where ma_duan_banhang is not null and manv_hotro is  null
										and thang_tldg_dt_nvhotro is null and thang_tldg_dt is null
										and (thang_ptm = 202407		---thang n - 3
...
-- QLDA: xoa , chay lai
delete from ttkd_bsc.ct_bsc_ptm_kiemtraduan where thang=202407; and ma_duan_banhang = '227812'; and ma_tb = 'hcm_ca_00103739';and ma_tb = 'hcm_colo_00009265';
		;
		select * from ttkd_bsc.ct_bsc_ptm_kiemtraduan a where thang = 202405 and ma_duan_banhang = '219097';227188; and ma_duan_banhang = '227812'; and ma_tb = 'hcm_ca_00103739'; and ma_gd = 'HCM-TD/00690780';hcm_hddt_inbot_00001044
...
--						and a.thang_luong in (70, 71)
--			and thang_luong = 86 and  not exists (select * from ttkd_bsc.ct_bsc_ptm_kiemtraduan where thang=202406 and ma_tb = a.ma_tb and nguon like 'ct_%')
...
--						and ma_duan_banhang = '227188'
			...
			update ttkd_bsc.ct_bsc_ptm a 
				   set kiemtra_duan = '' 
--		select ma_tb, kiemtra_duan, doituong_kh from ttkd_bsc.ct_bsc_ptm a 
			...
			update ttkd_bsc.ct_bsc_ptm a 
				   set kiemtra_duan = (select nvl(kt_nghiemthu, 0) || kiemtra_duan from ttkd_bsc.ct_bsc_ptm_kiemtraduan 
																							where thang = 202407 and ptm_id=a.id)
...
--		 select thang_luong, thang_ptm, kiemtra_duan, thang_tldg_dt, thang_tlkpi, nguon, ma_duan_banhang, ma_gd,  ma_tb, mst from ttkd_bsc.ct_bsc_ptm a 
			    where exists(select 1 from ttkd_bsc.ct_bsc_ptm_kiemtraduan where thang = 202407 and ptm_id=a.id)
...
-- KH hien huu - KH moi:   
	...
	--   select khhh_khm from ttkd_bsc.ct_bsc_ptm
			    where thang_ptm = 202407 and khhh_khm is not null
...
--					  and ma_tb = '84916665480'
					...
					update ttkd_bsc.ct_bsc_ptm a 
					   set khhh_khm =  'KHHH'
--					      select khhh_khm, doituong_kh from ttkd_bsc.ct_bsc_ptm a
				    where a.thang_ptm = 202407 and doituong_kh='KHDN'
...
--												  )
			---hoac 2
					...
					update ttkd_bsc.ct_bsc_ptm a 
					   set khhh_khm =  'KHM'
--					      select khhh_khm, doituong_kh, ma_tb, ten_tb, mst, nguon, ghi_chu, tenkieu_ld from ttkd_bsc.ct_bsc_ptm a
				    where thang_ptm = 202407 	--thang n
...
-- Dia ban:        vi tri ban hang truc tiep moi xet trong/ngoai dia ban
			---VTTP, BHOL khong xet trong/ngoai chuong trinh
			update ttkd_bsc.ct_bsc_ptm a 
						set diaban =null
	---	select diaban from ttkd_bsc.ct_bsc_ptm a
			    where thang_ptm = 202407 and (loaitb_id<>21 or ma_kh='GTGT rieng') and diaban is not null
...
--											when nhom_tiepthi=2 or thuebao_id is null then 'Khong xet trong/ngoai CT ban hang'
											when nhom_tiepthi in (2, 11) then 'Khong xet trong/ngoai CT ban hang'			---TTVT, Potmasco
...
--							select thang_luong, thang_ptm, thuebao_id, ma_tb, dichvuvt_id, loaitb_id, dich_vu, mst, diaban, nhom_tiepthi, ma_vtcv, doituong_kh, kiemtra_duan, ma_duan_banhang, manv_ptm, nguon, thang_tldg_dt from ttkd_bsc.ct_bsc_ptm a
				    where (a.thang_ptm = 202407 --- thang n
								or a.thang_luong in (4, 44, 70, 71, 86, 87))			---flag 4, 44 file 6 kiem tra duan sau khi da update QLDA va ctr ngoai tinh bsung
						and (loaitb_id<>21 or ma_kh='GTGT rieng')
				
					   and nvl(thang_tldg_dt, 999999) >= 202407 --and nvl(thang_tlkpi_phong, 999999) >= 202407  --lay cac tbao chua chi dektra duan
--					   and  exists (select * from ttkd_bsc.nhanvien where thang = 202407 and donvi = 'VTTP' and a.manv_ptm = ma_nv)
...
--		select * from ttkd_bsc.ct_bsc_ptm
			    where thang_ptm = 202406 and dichvuvt_id not in (2,13,14,15,16) and loaitb_id not in (21,172) and mien_hsgoc is not null
			    ;
...
--		 select * from ttkd_bsc.ct_bsc_ptm
			    where thang_ptm = 202407 --- thang n
						   ...
						   and ( (dichvuvt_id not in (2,13,14,15,16) and (loaihd_id is null or chuquan_id in (266, 264))  ---Newlife, VTH1 hoac hok co hop dong --> mien hso goc
											...
											or ( thang_ptm = 202407 and loaitb_id in (15,17) and thuebao_cha_id is not null)       -- isdn, thue bao luong (so con)
...
-- He so dich vu:    
		...
		---thang n
		create table ttkd_bsc.x_temp as select * from ttkd_bct.ptm_codinh_202407
		;
...
--																		    then 0.4  -- dthu tron hop dong 
                                                ...
                                                when loaitb_id in (279, 317, 287) then                                         -- VNPT AntiDDoS: theo thang =1, theo thuê dich vu 72 gio = 0.3, eoffice 718660 
														  ...
														  case when exists (select muccuoc_tb, datcoc_csd from ttkd_bsc.x_temp
                                                ...
                                                when loaitb_id in (317,287,285,279) then                                            --  VNPT AntiDDoS ,VNPT IOC, VNPT eDIG, VNPT AI Camera
												   case when not exists (select 1 from ttkd_bsc.x_temp
...
--										when loaitb_id = 20 and kieuld_id = 27 ---khong ap dung tu thang 9/2024
--													and exists (select 1 from ttkdhcm_ktnv.input_goira where kichban_id = 54 and kycuoc in ('202406', '202407') and a.ma_tb = ma_tb)
--													then 0.8		---Card to Post trong tap giao n-1, n
--										when loaitb_id = 20 and kieuld_id = 27 then 0.4		---Card to Post ngoai tap giao
...
-- He so khu vuc dac thu: danh muc quan ly thay doi from PDH.
			...
			;
			select * from ttkd_bsc.dm_duan_dacthu where thang=202407
			;
			desc ttkd_bsc.dm_duan_dacthu;
			
			insert into ttkd_bsc.dm_duan_dacthu 
					    select 202407 thang, ma_da, ten_duan, heso_kvdacthu, toanha_id, ghichu
					    from ttkd_bsc.dm_duan_dacthu a
					    where not exists (select 1 from ttkd_bsc.dm_duan_dacthu where thang = 202407 and ma_da=a.ma_da)    
							  and thang = 202406
							  ;
			...
			---	select heso_kvdacthu from ttkd_bsc.ct_bsc_ptm a
				    where thang_ptm = 202406 and dichvuvt_id!=2
					   and ma_da is not null and heso_kvdacthu is not null
					   and exists (select 1 from ttkd_bsc.dm_duan_dacthu where thang = 202406 and ma_da=a.ma_da)
					   ;
...
-- He so tra truoc: 
	...
	---	select nguon, heso_tratruoc from ttkd_bsc.ct_bsc_ptm a
		    where thang_ptm = 202406 and loaitb_id!=20 and nguon not like 'ct_ptm_ngoaictr%'  --loai tru cac file tu excel A Nghia
...
--				select thang_luong, thang_ptm, ma_tb, thang_tldg_dt, sothang_dc, heso_tratruoc, dich_vu, dichvuvt_id, loaitb_id from ttkd_bsc.ct_bsc_ptm a
			    where 
							(thang_ptm = 202407 --- thang n
								or thang_luong in (2))			---flag 2 update Dthu_goi = datcoc_csd file 6
							and loaitb_id != 20 	---VNPts anh Tuyen da tinh
							and nvl(thang_tldg_dt, 999999) >= 202407
--							thang_luong = 89
...
-- He so dai ly :  VB 353/TTr-DH-NS - 12/2023:    AM ban hang thong qua Dai ly tinh 5%; nguoc lai AM QLDL ban hang thong qua DAI LY 0%
			...
			update ttkd_bsc.ct_bsc_ptm a 
			    set heso_daily = ''
			    where thang_ptm = 202406 and heso_daily is not null
			    ;
   
    select *  from ttkd_bsc.dm_daily_khdn where thang = 202407;
    
			update ttkd_bsc.ct_bsc_ptm a 
			    set heso_daily = 
											case when exists (select 1  from ttkd_bsc.dm_daily_khdn where thang = 202407 and thang_kyhd < a.thang_ptm and ma_daily = a.ma_nguoigt) ---Dai ly hien huu
																then 0
													when exists (select 1  from ttkd_bsc.dm_daily_khdn where thang = 202407 and  thang_kyhd = a.thang_ptm and ma_daily = a.ma_nguoigt) ---Dai ly moi
																	and ma_vtcv in ('VNP-HNHCM_KHDN_3.1')		---AM QLDL ho tro
																then 0
													when exists (select 1  from ttkd_bsc.dm_daily_khdn where thang = 202407 and  thang_kyhd = a.thang_ptm and ma_daily = a.ma_nguoigt) ---Dai ly moi
																	and ma_vtcv not in ('VNP-HNHCM_KHDN_3.1')		---AM thuong ho tro
																then 0.05
													else null
											end
										
--						, thang_luong = 25
--		 select thang_luong, thang_ptm, manv_ptm, ma_vtcv, ma_nguoigt, heso_daily, thang_tldg_dt, thang_tlkpi_phong from ttkd_bsc.ct_bsc_ptm a
			    where (thang_ptm = 202407 --and manv_ptm is not null-- 
								or thang_luong in (4)		---file 6 update ma_vtcv
								)
			   -- and heso_daily is null
							  ...
							  -- and ma_vtcv  not in ('VNP-HNHCM_KHDN_3.1')
							   and nvl(thang_tlkpi_phong, 999999) >= 202407
							  -- and ma_nguoigt = 'GTGT00111'
...
-- HE SO QUY DINH: ban hang ngoai dia ban 
		...
		-- Gia han GTGT/CNTT xet QLDA
			update ttkd_bsc.ct_bsc_ptm 
				...
				update ttkd_bsc.ct_bsc_ptm a
				    set heso_quydinh_nvptm = case when chuquan_id = 264 or ma_kh='GTGT rieng' or nhom_tiepthi in (2, 11) then 1
						...
						, heso_quydinh_nvhotro = case when chuquan_id = 264 or ma_kh='GTGT rieng' or nhom_tiepthi in (2, 11) then 1
																			  else case when diaban in ('Trong CT ban hang','Khong xet trong/ngoai CT ban hang') then 1
																							when diaban='Ngoai CT ban hang' then 0.5 else 1 end
																	end
...
--				  select nguon, thang_luong, thang_ptm, ma_kh, ma_tb, heso_quydinh_nvptm, heso_quydinh_nvhotro, heso_hotro_nvhotro, manv_hotro, manv_ptm, heso_quydinh_dai, manv_tt_dai, diaban, chuquan_id, nhom_tiepthi, thang_tldg_dt from ttkd_bsc.ct_bsc_ptm a    
				    where 
								(thang_ptm = 202407 --- thang n
										or thang_luong in (4, 87))			---flag 4 file so 6 sau khi update QLDA
...
--				    select thang_luong, thang_ptm, chiphi_ttkd, thang_xn_chiphi_ttkd, heso_vtcv_nvptm, heso_vtcv_nvhotro, heso_vtcv_dai, manv_hotro, manv_tt_dai, thang_tldg_dt from ttkd_bsc.ct_bsc_ptm a 
				    where (thang_ptm = 202407		--- thang n
									or thang_luong in (44, 87))		---flag 87 file Update anh Nghia
					and (loaitb_id<>21 or ma_kh='GTGT rieng') 
					and nvl(thang_tldg_dt, 999999) >= 202407
--					and thang_luong in (71)
...
-- He so khach hang: 
	...
	---  select phanloai_kh from ttkd_bsc.ct_bsc_ptm a 
			    where  thang_ptm = 202406 and phanloai_kh is not null-- and thang_luong = 86
			    ; 
         
           
-- Da co file giao:        khong chay dot 1               
			update ttkd_bsc.ct_bsc_ptm a
			    set phanloai_kh = case when ma_kh='GTGT rieng' 
																		then
...
--			 select thang_luong, thang_ptm, phanloai_kh, chuquan_id ,ma_kh, ma_tb, nguon, thang_tldg_dt, thuebao_id from ttkd_bsc.ct_bsc_ptm a 
			    where thang_ptm = 202407 --- thang n
...
--				 select thang_luong, mst, heso_khachhang, heso_kvdacthu, doituong_kh, khhh_khm, phanloai_kh, nguon, thuebao_id, ma_duan_banhang, khachhang_id, thang_tldg_dt from ttkd_bsc.ct_bsc_ptm a
				    where a.thang_ptm = 202407 and nvl(thang_tldg_dt, 999999) >= a.thang_ptm
...
-- He so thue bao ngan han: 
			update ttkd_bsc.ct_bsc_ptm a 
					set heso_tbnganhan = case when thoihan_id=1 and dichvuvt_id in (1,10,11,4,7,8,9) then 0.3
																else 1 end
...
-- he so ho tro:
				...
				create table hocnq_ttkd.temp_hesohotro as 
						select hdtb_id from ttkd_bct.ptm_codinh_202407
				...
				update ttkd_bsc.ct_bsc_ptm a
				    set heso_hotro_nvptm  = case 
																	when a.loaitb_id = 20 and exists (select sdt_datmua, ma_gioithieu, tennv_gioithieu 
																																...
																																where lower(trangthai_shop) like 'th_nh c_ng' and sdt_datmua = a.ma_tb and thang >= 202404)
																	...
																	when a.dichvuvt_id not in (2, 14, 15, 16) 
																											and exists (select ma_dhsx, ma_gioithieu, tennv_gioithieu 
																																from ttkd_bsc.digishop 
																																where lower(trangthai_shop) like 'th_nh c_ng' and ma_dhsx = a.ma_gd_gt and thang >= 202404)
...
--																		when manv_hotro is not null then 0.5  		--50% cho cong doan nguoi gioi thieu, case hok khong PS
																		...
																		when manv_ptm='VNP017772' and loaitb_id=288 
																				 and (hdtb_id, thuebao_id) in (select hdtb_id, thuebao_id from ttkd_bsc.ptm_xuly_50_BHOL where thang = a.thang_ptm)
																							then 0.5   -- SmartCA tinh cho Huong BHOL 50%
																		when thang_luong = 71 then 0.8		--ap dung chi duy nhat 1 thang 202407 theo vanban_id, sau thang 7 bo 
																		else 0 end
						,heso_hotro_dai         = case when thang_luong = 71 then 0.2 else 0 end		--ap dung chi duy nhat 1 thang 202407 theo vanban_id, sau thang 7 bo  else 0 end
						--, thang_luong = 11
...
--		select thang_luong, thang_ptm, ma_tb, dich_vu, manv_hotro, tyle_hotro, tyle_am,  heso_hotro_nvptm, heso_hotro_nvhotro, heso_hotro_dai, ma_duan_banhang from ttkd_bsc.ct_bsc_ptm a    
				where (thang_ptm = 202407 --- thang n
								or thang_luong in (4, 44, 87))			---flag 4 file so 6 sau khi update ma QLDA, so 5 update DIGISHOP, file Update anh Nghia
					 and (loaitb_id<>21 or ma_kh='GTGT rieng') 
					 and nvl(thang_tldg_dt, 999999) >= 202407
--					 and thang_luong = 71
...
-- He so dia ban tinh khac: ngay 4/8/24 co sua dk ban CNTT tinh LAN va BDG
	...
	---		select ma_gd, ma_tb, heso_diaban_tinhkhac, dichvuvt_id, kieuld_id, tinh_id, thuebao_id from ttkd_bsc.ct_bsc_ptm a
			    where thang_ptm = 202407 --- thang n
				--	and dichvuvt_id in (13,14,15,16) and tinh_id  in (8, 36)
...
--		     select thang_luong, ma_tb, tenkieu_ld, ngay_bbbg, ngay_luuhs_ttkd, ngay_luuhs_ttvt, nop_du, mien_hsgoc, bs_luukho , heso_hoso, thang_tldg_dt from ttkd_bsc.ct_bsc_ptm a
		    where ((thang_ptm = 202404 and (to_number(to_char(ngay_luuhs_ttkd,'yyyymmdd')) > 20240613 or to_number(to_char(ngay_luuhs_ttvt,'yyyymmdd')) > 20240613))
							or (thang_ptm = 202405 and (to_number(to_char(ngay_luuhs_ttkd,'yyyymmdd')) > 20240713 or to_number(to_char(ngay_luuhs_ttvt,'yyyymmdd')) > 20240713))
--							or (thang_ptm = 202404 and (to_number(to_char(ngay_luuhs_ttkd,'yyyymmdd'))>20240613 or to_number(to_char(ngay_luuhs_ttvt,'yyyymmdd'))>20240613))
					  ...
					  and (loaitb_id not in (21) or ma_kh='GTGT rieng') and heso_hoso is null
					  and mien_hsgoc is null and nop_du = 1 --and substr(bs_luukho,1,6)= '202408' --and heso_hoso is null		---thang n+1
					  and nvl(thang_tldg_dt, 999999) >= 202407
--					  and thang_luong<>'3'
			  ;
           rollback;
...
select ma_gd, ma_tb, nguon, loaihd_id, dthu_goi, kieuld_id,lydo_khongtinh_luong, thang_tldg_dt, lydo_khongtinh_dongia
    ...
    where thang_ptm = 202407 and loaitb_id<>21 and
                (ma_gd, ma_tb, loaitb_id) in 
                          (select ma_gd, ma_tb, loaitb_id from ttkd_bsc.ct_bsc_ptm 
                            where thang_ptm = 202407 and loaitb_id!=20 group by ma_gd, ma_tb, loaitb_id having count(*)>1
					   )
				;
...
--		    select * from ttkd_bsc.ct_bsc_ptm a
		    where thang_ptm = 202407 and nguon like 'thaydoitocdo%' and lydo_khongtinh_luong not like '%Tinh luong theo hs tai lap%'
...
-- DTHU DON GIA 
			----858 vnd --> 800 vnd theo vb 322, eO 5696552_1488	--> TTKD
								---640 vnd theo vb 1485/TTKD HCM -NS 15/08/24 --> VTTP
			update ttkd_bsc.ct_bsc_ptm a 
						set dongia = 800
...
-- cac dv ngoai tru VNPTT, SMS Brandname, Voice Brandnanme
			...
			update ttkd_bsc.ct_bsc_ptm a 
			    set doanhthu_dongia_nvptm   = case when heso_daily =  0.05 then round(dthu_goi * nvl(tyle_huongdt,1)* heso_daily * heso_hotro_nvptm, 0)
																			...
																			else	round(dthu_goi*nvl(tyle_huongdt,1) *heso_dichvu*nvl(heso_khuyenkhich,1)*nvl(heso_tratruoc,1)
					...
					,doanhthu_dongia_nvhotro = case when heso_daily =  0.05 then round(dthu_goi * nvl(tyle_huongdt,1)* heso_daily * heso_hotro_nvhotro, 0)
																			when heso_daily =  0 and exists (select 1  from ttkd_bsc.dm_daily_khdn 
																																		where thang = 202407 and thang_kyhd = a.thang_ptm and ma_daily = a.ma_nguoigt) ---Dai ly moi
																								then 0
																			else round(dthu_goi*nvl(tyle_huongdt,1)*heso_dichvu*nvl(heso_khuyenkhich,1)*nvl(heso_tratruoc,1)
					...
					,doanhthu_dongia_dai          = case when heso_daily =  0.05 then round(dthu_goi * nvl(tyle_huongdt,1)* heso_daily * heso_hotro_dai, 0)
																			when heso_daily =  0 and exists (select 1  from ttkd_bsc.dm_daily_khdn 
																								...
																								then 0
																																...
																																*nvl(heso_hoso,1)*nvl(heso_diaban_tinhkhac,1)
																								, 0) end
--		    select nguon, sothang_dc, thang_luong, thang_ptm, ma_gd, ma_tb, dichvuvt_id, manv_hotro, heso_daily, dthu_goi, doanhthu_dongia_nvptm, doanhthu_dongia_nvhotro, doanhthu_dongia_dai, ma_kh, loaitb_id, thang_tldg_dt, lydo_khongtinh_luong from ttkd_bsc.ct_bsc_ptm a
			    where (thang_ptm = 202407 --- thang n
							...
							and nvl(thang_tldg_dt, 999999)>=202407 
							and not (loaitb_id = 358 and thang_luong = 202407) --- voice Brandname chua dc tinh thang n 
			    ...
			    set doanhthu_dongia_nvptm = 
																	case when heso_daily =  0.05 then round((nvl(dthu_goi,0) + nvl(dthu_goi_ngoaimang,0)) * nvl(tyle_huongdt,1)* heso_daily * heso_hotro_nvptm, 0)
																			when heso_daily =  0 then 0		---AM QLDL, AM va Daily hien huu = 0
																									...
																									*nvl(tyle_huongdt,1) *nvl(heso_khuyenkhich,1)*nvl(heso_tratruoc,1)
																									* heso_quydinh_nvptm * heso_vtcv_nvptm * nvl(heso_kvdacthu,1)
																									* heso_hotro_nvptm * heso_khachhang * nvl(heso_tbnganhan,1)
																									* nvl(heso_hoso,1)*nvl(heso_diaban_tinhkhac,1)
																								,0) end 
...
---	   select heso_daily, heso_tratruoc, heso_quydinh_nvptm, heso_vtcv_nvptm, heso_hotro_nvptm, heso_khachhang, dthu_goi, dthu_goi_ngoaimang, doanhthu_dongia_nvptm  from ttkd_bsc.ct_bsc_ptm a
			    where (thang_ptm = 202406 --- thang n -1
								or thang_luong in (1, 2, 3, 4))			---flag 4 file so 5 import dung thu chuyen dung that
					and loaitb_id=131
...
--		  select thang_luong, ma_tb, luong_dongia_nvptm, doanhthu_dongia_dnhm, tien_dnhm, tien_sodep, heso_daily, nguon, thang_tldg_dnhm from ttkd_bsc.ct_bsc_ptm a
			    where (thang_ptm = 202407 --- thang n
							   ...
							   and nvl(thang_tldg_dnhm, 999999) >= 202407
				...
				update ttkd_bsc.ct_bsc_ptm a
				    set  doanhthu_kpi_nvptm = case when heso_daily =  0.05 then doanhthu_dongia_nvptm
																			when heso_daily =  0 then 0		---AM QLDL, AM va Daily hien huu = 0
																									...
																									* heso_quydinh_nvptm * heso_vtcv_nvptm
																									* heso_hotro_nvptm * nvl(heso_tbnganhan,1)
																									* nvl(heso_diaban_tinhkhac,1)
																								,0) end
						 , doanhthu_kpi_nvdai   = 
																	case when heso_daily =  0.05 then doanhthu_dongia_dai
																			when heso_daily =  0 and exists (select 1  from ttkd_bsc.dm_daily_khdn 
																																		where thang = 202407 and thang_kyhd = a.thang_ptm and ma_daily = a.ma_nguoigt) ---Dai ly moi
																								then 0
																			else round(dthu_goi*nvl(tyle_huongdt,1)*heso_dichvu*nvl(heso_khuyenkhich,1)*nvl(heso_tratruoc,1)
																								* heso_quydinh_dai * heso_vtcv_dai
						 ...
						 , doanhthu_kpi_nvhotro = case when heso_daily =  0.05 then doanhthu_dongia_nvhotro
																			when heso_daily =  0 and exists (select 1  from ttkd_bsc.dm_daily_khdn 
																								...
																								* heso_quydinh_nvhotro * heso_vtcv_nvhotro
																								* heso_hotro_nvhotro * nvl(heso_tbnganhan,1)
																								* nvl(heso_diaban_tinhkhac,1) 
																						,0) end
						 , doanhthu_kpi_dnhm    = case when heso_daily =  0.05 and heso_dichvu_dnhm > 0 then doanhthu_dongia_dnhm
																				when heso_daily =  0 then 0
																					else round((nvl(tien_dnhm,0)+nvl(tien_sodep,0)) *nvl(tyle_huongdt,1) *heso_dichvu_dnhm
																											 * heso_quydinh_nvptm * heso_vtcv_nvptm
																											 * nvl(heso_diaban_tinhkhac,1)
																										,0) end
--		 select thang_luong, thang_ptm, doanhthu_dongia_nvptm, doanhthu_dongia_nvhotro, doanhthu_kpi_nvptm, doanhthu_kpi_nvdai, doanhthu_kpi_nvhotro, doanhthu_kpi_dnhm, thang_tlkpi from ttkd_bsc.ct_bsc_ptm a
				    where (thang_ptm = 202407 --- thang n
								or thang_luong in (1, 2, 3, 4, 87)
								)			---flag 4 file so 5 import dung thu chuyen dung that
...
-- dtkpi cua dich vu Thiet bi giai phap:  la dthu hop dong x cac he so tinh bsc (ko tinh tren chenh lech thu chi).
					update ttkd_bsc.ct_bsc_ptm a
						   set doanhthu_kpi_nvptm  = case when heso_daily =  0.05
																				...
																				when heso_daily =  0 then 0
																					else round(dthu_goi_goc * nvl(tyle_huongdt,1) * heso_dichvu * nvl(heso_khuyenkhich,1)*nvl(heso_tratruoc,1)
																														* heso_quydinh_nvptm * heso_vtcv_nvptm
																														* heso_hotro_nvptm * nvl(heso_tbnganhan,1)
...
--				select thang_luong, ma_duan_banhang, nguon, dthu_goi_goc, dthu_goi, doanhthu_kpi_nvptm, doanhthu_kpi_nvhotro, heso_khachhang from 	ttkd_bsc.ct_bsc_ptm a
					    where (thang_ptm = 202407 --- thang n
			...
			update ttkd_bsc.ct_bsc_ptm a
			    set  doanhthu_kpi_nvptm = 
																	case when heso_daily =  0.05 then doanhthu_dongia_nvptm
																			when heso_daily =  0 then 0		---Daily moi va Daily hien huu = 0
																			else	round( ( (nvl(dthu_goi,0)*heso_dichvu) +(nvl(dthu_goi_ngoaimang,0) * heso_dichvu_1) )																									
																									*nvl(tyle_huongdt,1) *nvl(heso_khuyenkhich,1)*nvl(heso_tratruoc,1)
																									* heso_quydinh_nvptm * heso_vtcv_nvptm
																									* heso_hotro_nvptm * nvl(heso_tbnganhan,1)
					  ...
					  , doanhthu_kpi_nvhotro = case when heso_daily =  0.05 then doanhthu_dongia_nvhotro
																			when heso_daily =  0 and exists (select 1  from ttkd_bsc.dm_daily_khdn 
																								...
																								* nvl(heso_diaban_tinhkhac,1)
																						,0) end
--			    select doanhthu_dongia_nvptm, doanhthu_kpi_nvptm, doanhthu_dongia_nvhotro, doanhthu_kpi_nvhotro from ttkd_bsc.ct_bsc_ptm
			    where (thang_ptm = 202406 --- thang n-1
								or thang_luong in (1, 2, 3, 4))			---flag 4 file so 5 import dung thu chuyen dung that
							 and loaitb_id = 131 
		...
		--Khong ap dung heso_quydinh_nvptm, ghi nhan 100%, khong ap dung 5% heso_daily
				update ttkd_bsc.ct_bsc_ptm a
					   set doanhthu_kpi_phong = (case when dich_vu like 'Thi_t b_ gi_i ph_p%' 
																		  then round(dthu_goi_goc * nvl(tyle_huongdt,1) * heso_dichvu * nvl(heso_khuyenkhich,1) * nvl(heso_tratruoc,1)
...
--		   select heso_khachhang, heso_kvdacthu, doanhthu_kpi_phong, ma_tb from ttkd_bsc.ct_bsc_ptm a
				    where (thang_ptm = 202407 --- thang n
								or thang_luong in (1, 2, 3, 4, 87)
								)			---flag 4 file so 5 import dung thu chuyen dung that
...
-- Kpi dnhm phong:
		...
		update ttkd_bsc.ct_bsc_ptm a
		    set doanhthu_kpi_dnhm_phong = round( (nvl(tien_dnhm,0)+nvl(tien_sodep,0)) *nvl(tyle_huongdt,1) * heso_dichvu_dnhm
																	* heso_vtcv_nvptm * heso_hotro_nvptm
																	* nvl(heso_diaban_tinhkhac,1) ,0)
	---	select doanhthu_kpi_dnhm_phong, heso_dichvu_dnhm from ttkd_bsc.ct_bsc_ptm a
		    where (thang_ptm = 202407 --- thang n
								or thang_luong in (1, 2, 3, 4, 87))			---flag 4 file so 5 import dung thu chuyen dung that
					and (loaitb_id not in (20,21) or ma_kh='GTGT rieng' or (loaitb_id=20 and goi_luongtinh is null)) 
					   and (tien_dnhm>0 or tien_sodep>0)
...
--- DTHU KPI TO:
	...
	---	select MA_NGUOIGT, doanhthu_kpi_to from ttkd_bsc.ct_bsc_ptm a
				    where (thang_ptm = 202407 --- thang n
									or thang_luong in (1, 2, 3, 4, 87))			---flag 4 file so 5 import dung thu chuyen dung that
								and (loaitb_id!=21 or ma_kh='GTGT rieng')
...
select thang_luong, thang_ptm,dich_vu, dichvuvt_id, loaitb_id, tien_dnhm, tocdo_id, goi_id, dthu_ps, thuebao_id,ma_tb,dthu_goi,doanhthu_kpi_nvhotro , chuquan_id, heso_hotro_nvhotro
    from ttkd_bsc.ct_bsc_ptm
    where thang_ptm=202407 and (loaitb_id<>21 or ma_kh='GTGT rieng') 
...
select ma_tb, dthu_ps, dthu_ps_n1, dthu_goi, dthu_goi_ngoaimang, heso_dichvu, heso_dichvu_1,
    ...
    from ttkd_bsc.ct_bsc_ptm
    where thang_ptm = 202405 and loaitb_id=131;

-- TB ngan han:
select thang_ptm, ma_pb,manv_ptm,(select ten_nv from ttkd_bsc.nhanvien_202407 where manv_hrm=a.manv_ptm) ten_nv,
    ...
    from ttkd_bsc.ct_bsc_ptm a
    where thang_ptm = 202407 and thoihan_id=1
