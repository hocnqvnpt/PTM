...
--			select * from ttkd_bsc.bang_heso_dthu a 
			...
			update ttkd_bsc.bang_heso_dthu a 
						set heso_qd_tong = (select 
																...
																where ma_kpi in ('HCM_DT_LUYKE_002') and thang = 202408 and ma_nv = a.ma_nv
															)
--			select * from ttkd_bsc.bang_heso_dthu a 															
			where ma_pb in (
											'VNP0702300',
...
--			select * from ttkd_bsc.bang_heso_dthu a
		...
		select sum(tien_thulao) from ttkd_bsc.tonghop_ct_dongia_ptm where thang = 202407 and ma_pb = 'VNP0701500' ;
					and ma_nv in (select ma_nv from ttkd_bsc.bangluong_dongia_202407_xuly where ma_pb = 'VNP0701500' and nvl(CHENHLECH, 0) <> 0);
		
		

delete from ttkd_bsc.tonghop_ct_dongia_ptm where thang = 202408;
select sum(TIEN_THULAO) from ttkd_bsc.tonghop_ct_dongia_ptm where thang = 202408; and thang_ptm = 202408; ma_nv = 'VNP019958'; thang 7 = 6539341849
...
insert into ttkd_bsc.tonghop_ct_dongia_ptm
	...
	----all dvu ngoai tru VNPtt
			select * from ttkd_bsc.tonghop_ct_dongia_ptm where thang = 202408;
			select sum(luong_dongia_cdbr+luong_dongia_vnpts+luong_dongia_cntt) from ttkd_bsc.bangluong_dongia_202408 a
			;
