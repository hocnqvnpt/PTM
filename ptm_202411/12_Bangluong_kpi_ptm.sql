...
--				where MANV_PTM in ('CTV071620','VNP027259')
	...
	insert into ttkd_bsc.tonghop_dthu_ptm
		select cast(202411 as number) thang, DICH_VU, LOAITB_ID, DICHVUVT_ID, MA_NV, DTHU_KPI 
		from ttkd_bsc.temp_trasau_canhan
	;
-- to truong: thieu 0021_ts
	drop table ttkd_bsc.temp_totruong purge;
	
	select sum(dthu_kpi)/1000000 from ttkd_bsc.temp_totruong ---4449.8077501 ---4167.8310628 ---4132.2632568
	...
	select * from ttkd_bsc.temp_totruong where ma_to = 'VNP0702120'; 9 984 154 803.9 (10 999 250 012.15 )
	union all 
	;
...
--			select * from (
			with pgd as ( select distinct MA_PB, ten_to, MA_TO, MA_NV, THANG, dichvu, nhom_dichvu, TEN_NV, PHUTRACH, NHOM
									...
									where thang = 202411 and ma_pb not in ('VNP0702300', 'VNP0702400', 'VNP0702500') and ma_kpi = 'HCM_DT_PTMOI_021'
									)

						select a.*, pgd.ma_nv 
						from ttkd_bsc.temp_021_ldp1 a
									  join pgd
										on a.ma_to = pgd.ma_to and a.PHUTRACH = pgd.PHUTRACH and a.NHOM = pgd.NHOM and pgd.thang=202411 
						...
						select a.*, pgd.ma_nv 
						from ttkd_bsc.temp_021_ldp1 a
									  join pgd on a.ma_to = pgd.ma_to and a.PHUTRACH = pgd.PHUTRACH and a.NHOM = pgd.NHOM  and pgd.thang=202411 
						...
						select a.*, pgd.ma_nv 
						from ttkd_bsc.temp_021_ldp1 a
									  join pgd on a.ma_to = pgd.ma_to and pgd.thang=202411 
...
--				select * from  ttkd_bsc.bangluong_kpi a
		...
		update ttkd_bsc.bangluong_kpi
			set THUCHIEN = case when ma_nv = 'VNP027259' then THUCHIEN + 33600000/1000000
												when ma_nv = 'VNP017190' then THUCHIEN + 33600000/1000000
												end
--		select * from  ttkd_bsc.bangluong_kpi
		...
		delete from ttkd_bsc.bangluong_kpi a where ma_kpi in ('HCM_DT_PTMOI_021') and a.thang = 202411
...
--			select * from ttkd_bsc.bangluong_kpi a 
		...
		update ttkd_bsc.bangluong_kpi a set GIAO = 
																					case 
																							...
																							when ma_vtcv in ('VNP-HNHCM_KDOL_3.1') then 36		---file giao PBHOL T10
																							when ma_nv = 'VNP017740' and ma_vtcv in ('VNP-HNHCM_KDOL_2') then 118.183	--file giao anh Tuyết  PGP  PBHOL 202411
...
--				select * from ttkd_bsc.bangluong_kpi a
				where a.ma_kpi in ('HCM_DT_PTMOI_021') and a.thang = 202411 and ma_pb != 'VNP0703000'
			;
...
--																									when ma_vtcv in ('VNP-HNHCM_BHKV_2') then 80	---fix so theo vb ap dung T8 PGD KV_banhang
																								...
																								when ma_nv in ('VNP017740') and ma_vtcv in ('VNP-HNHCM_KDOL_2') then 40	--PGD OL Tuyet
...
--				select * from ttkd_bsc.bangluong_kpi a
				where ma_kpi in ('HCM_DT_PTMOI_021') and a.thang = 202411 -- and ma_vtcv in ('VNP-HNHCM_KDOL_17')
--							and ma_nv = 'VNP019532'
			;
		...
		update ttkd_bsc.bangluong_kpi a 
						set (giao, thuchien, tyle_thuchien, mucdo_hoanthanh) = (select giao, thuchien, tyle_thuchien, mucdo_hoanthanh 
								...
								, tytrong = 100
		where ma_nv = 'VNP017014' and ma_kpi in ('HCM_DT_PTMOI_021') and thang = 202411
		;
...
--			select * from ttkd_bsc.dinhmuc_giao_dthu_ptm a
		WHERE a.thang = 202411 and loai_kpi = 'KPI_CHT_GDV'
		;
		MERGE INTO ttkd_bsc.dinhmuc_giao_dthu_ptm a
