...
--				select * from ttkd_bsc.ct_ptm_ngoaictr_imp 
				where thang = 202501 --and dichvuvt_id in (14, 15, 16)
--							ma_tb = 'hcm_vbn_00001181'
--							and DICHVU_VT = 'HMIS'
...
--							and ma_tb = 'hcm_vbn_00001078'
							and rowid  in ( 'AAHL0jABeAAJsfgAAG')
--'AAHL0jABkAAC7PDAAC')
...
--							and manv_hotro = 'VNP017373; D?ch v? ch?a c� tr�n oneBSS'--is not null
				...
				select * from ttkd_bsc.ct_bsc_ptm where thang_ptm >= 202407 and  ma_tb = 'hcm_ecabinet_00000051';
				select * from ttkd_bsc.ct_bsc_ptm where thang_ptm >= 202407 and  ma_duan_banhang = '341997';
				...
				from ttkd_bsc.ct_bsc_ptm where thang_ptm >= 202407 and replace(ma_gd, ' ', '') = 'HCM-LD/02045318';'00928721'; 
				...
				select * from ttkd_bsc.ct_ptm_ngoaictr_imp 
								where thang = 202412 and nvl(dichvuvt_id, 0)  in (7, 13, 14, 15, 16, 0) 
...
--											left join css_hcm.phanloai_kh plk on db.plkh_id = plk.PHANLOAIKH_ID
								...
								where a.thang = 202501 and a.dichvuvt_id in (14, 15, 16)-- not in ('VNP tra sau', 'Internet truc tiep')
												and command = 'insert' 
							...
							select 
								...
								from ttkd_bsc.ct_ptm_ngoaictr_imp a
											...
											left join (select x.thuebao_id, plkh_id, sum(dthu) dthu_ps
																...
																group by x.thuebao_id, plkh_id) db on a.thuebao_id = db.thuebao_id
											left join css_hcm.phanloai_kh plk on db.plkh_id = plk.PHANLOAIKH_ID
								where a.thang = 202501 and a.dichvuvt_id in (13, 14, 15, 16, 7)
												and command = 'update'
								) t
...
--												, a.tyle_hotro = nvl(t.heso_hotro_nvhotro, a.heso_hotro_nvhotro)
						...
						WHERE a.thuebao_id in (select thuebao_id from ttkd_bsc.ct_ptm_ngoaictr_imp where thang = 202501 and command = 'update')
					...
					, HESO_QUYDINH_NVPTM, HESO_QUYDINH_DAI, HESO_QUYDINH_NVHOTRO, HESO_DIABAN_TINHKHAC, HESO_HOSO, DONGIA, ghi_chu
					, MA_DUAN_BANHANG, KIEMTRA_DUAN, NGUON, TRANGTHAI_TT_ID)
							select 87 THANG_LUONG, THANG_PTM, a.MA_GD, MA_KH, MA_TB, SOHOPDONG, DICH_VU, TENKIEU_LD, TEN_TB, DIACHI_LD, SO_NHA, SO_GT, MST, MST_TT, SDT_LH, EMAIL_LH, CHUQUAN_ID, ID_447
											, NGAY_BBBG, NGAY_CAT, NGAY_LUUHS_TTKD, NGAY_LUUHS_TTVT, NGAY_SCAN, SCANBOOK_DU, NOP_DU, MIEN_HSGOC, BS_LUUKHO
											, GOI_CUOC, NHANVIEN_NHAN_ID, PBH_NHAN_ID, PBH_NHAN_GOC_ID, DICHVUVT_ID, LOAITB_ID, HDKH_ID, HDTB_ID, KIEUHD_ID, KIEULD_ID, LOAIHD_ID, KIEUTN_ID, KHACHHANG_ID
											...
											, HESO_TRATRUOC, HESO_KHUYENKHICH, HESO_KVDACTHU, HESO_VTCV_NVPTM, HESO_VTCV_DAI, HESO_VTCV_NVHOTRO, HESO_HOTRO_NVPTM, HESO_HOTRO_DAI, HESO_HOTRO_NVHOTRO
											, HESO_QUYDINH_NVPTM, HESO_QUYDINH_DAI, HESO_QUYDINH_NVHOTRO, HESO_DIABAN_TINHKHAC, HESO_HOSO, DONGIA, ghichu
											, MA_DUAN_BANHANG, KIEMTRA_DUAN
											, 'ct_ptm_ngoaictr_imp_'||b.command as nguon
											, 1 trangthai_tt_id
								from ttkd_bsc.ct_bsc_ptm a
											join (select ma_gd, thuebao_id, DTHU_GOI, command from ttkd_bsc.ct_ptm_ngoaictr_imp
																	where thang = 202501 and command = 'insert_dot') b on a.ma_gd = b.ma_gd and a.thuebao_id = b.thuebao_id 
...
--						select * from ttkd_bsc.ct_bsc_ptm a
						where nguon like 'ct_ptm_ngoaictr_imp%' and thang_luong in (86,87, 70)
									and exists (select * from ttkd_bsc.ct_ptm_ngoaictr_imp where thang = 202501 and nvl(GHINHAN_BSC, 0) = 0 and ma_gd = a.ma_gd)
						;
						commit;
						rollback;
