alter table ttkd_bsc.bangluong_dongia_202501 read only;
alter table ttkd_bsc.bangluong_dongia_202501 read write;
create table ttkd_bsc.bangluong_dongia_202501_l1 as select * from ttkd_bsc.bangluong_dongia_202501;

select sum(TONG_THULAO_THUCCHI)
from (
select * from ttkd_bsc.bangluong_dongia_202501
minus
select * from ttkd_bsc.bangluong_dongia_202501_l1
);

select sum(LUONG_DONGIA_DNHM_VNPTT), sum(LUONG_DONGIA_GOI_VNPTT) from ttkd_bsc.bangluong_dongia_202501_dot2; 352473000	445870660

---lan dau tien chay doan Frame nay, lan sau khong chay doan FRAME nay
------BEGIN dot 1---khong chay cho dot 2
drop table ttkd_bsc.bangluong_dongia_202501;
				create table ttkd_bsc.bangluong_dongia_202501 (
				donvi 		varchar(20),
				ma_nv               varchar(10),
				ten_nv               varchar(50),
				ma_vtcv            varchar(20),
	...
	;
			...
			insert into ttkd_bsc.bangluong_dongia_202501
			...
			SELECT donvi, ma_nv, ten_nv, ma_vtcv, ten_vtcv, ma_pb, ten_pb, ma_to, ten_to, loai_ld, nhomld_id
			  ...
			  where thang = 202501 and donvi = 'TTKD' --and  ma_to = 'VNP0700820' and ma_vtcv is null
						and not exists (select 1 from ttkd_bsc.bangluong_dongia_202501 x where a.ma_nv = x.ma_nv)
			  ; 
		commit;

-- Doanh thu dinh muc - 268 /TTr- NS 05/09/2022 
	---Thay the Doanh thu dinh muc theo vb 243 TTr NSU DH KHKT 20250112 eO
				---vb moi 323 TTr NSU DH KHKT 20250112 eO thay the b 243 
			...
			insert into ttkd_bsc.bldg_danhmuc_vtcv_p1 (MA_VTCV, TEN_VTCV, THANG_BD, DINHMUC_1, DINHMUC_2, VANBAN, GHICHU)
				select distinct ma_vtcv, ten_vtcv, thang thang_bd
							...
							, case ma_vtcv 	when 'VNP-HNHCM_BHKV_6' then 10000000
									...
									end dinhmuc_2, '243 TTr NSU DH KHKT 20250112 eO'VANBAN, 'doanh thu dinh muc chuan: vnd' GHICHU
				from ttkd_bsc.nhanvien
				where donvi = 'TTKD' and thang = 202501 and ma_vtcv in ('VNP-HNHCM_BHKV_6', 'VNP-HNHCM_BHKV_15', 'VNP-HNHCM_BHKV_22', 'VNP-HNHCM_BHKV_27', 'VNP-HNHCM_KDOL_4')
		;
		
		select * from ttkd_bsc.bldg_danhmuc_vtcv_p1 where thang = 202501; and DINHMUC_1 is not null
		;
		select * from ttkd_bsc.dinhmuc_giao_dthu_ptm where thang = 202501
		;
		 select * from ttkd_bsc.bangluong_dongia_202501 where ten_nv like '%Nguyện';
...
-- he so quy doi cua tong dthu:     
			...
			select sum(heso_dthu) from ttkd_bsc.bang_heso_dthu where thang = 202501; 726.4 632.25
			
			delete from ttkd_bsc.bang_heso_dthu where thang = 202501 and donvi = 'TTKD';
			insert into ttkd_bsc.bang_heso_dthu  
						select thang, DONVI, MA_NV, TEN_NV, MA_VTCV, TEN_VTCV, MA_PB, TEN_PB, MA_TO, TEN_TO, LOAI_LD, nhomld_id, null heso_dthu
							from ttkd_bsc.nhanvien where thang = 202501 and donvi = 'TTKD'
						;
			...
			delete from ttkd_bsc.bang_heso_dthu where thang = 202501 and donvi = 'VTTP';
			insert into ttkd_bsc.bang_heso_dthu  
						select thang, DONVI, MA_NV, TEN_NV, MA_VTCV, TEN_VTCV, MA_PB, TEN_PB, MA_TO, TEN_TO, LOAI_LD, nhomld_id, 0.8 heso_dthu
							from ttkd_bsc.nhanvien where thang = 202501 and donvi = 'VTTP'
						;
			...
			USING
						(with hs as (
											...
											from ttkd_bsc.ct_bsc_ptm a
														join ttkd_bsc.nhanvien b on b.thang = 202501 and b.donvi = 'VTTP' and a.manv_ptm = b.ma_nv
											where a.thang_ptm = 202501
...
--				select * from ttkd_bsc.bang_heso_dthu x
			WHERE x.thang = 202501 and x.donvi = 'VTTP'
			;
...
--			select heso_dthu from ttkd_bsc.bang_heso_dthu a 
			...
			----END
			select * from ttkd_bsc.bang_heso_dthu where thang = 202501;
			commit;
...
--			select * from ttkd_bsc.bang_heso_dthu a
			where exists (select * from ttkd_bsc.nhanvien where thang = a.thang and tinh_bsc = 0 and ma_nv = a.ma_nv)
						...
						and thang = 202501 and donvi = 'TTKD'
			;
			commit;
			rollback;
		---import VNPtt -- bang GOM 
		select count(*) from ttkd_bsc.ct_bsc_ptm where thang_ptm = 202501 and loaitb_id = 21; --83323
		select count(*) from ttkd_bsc.ct_bsc_ptm where thang_ptm = 202410 and THANG_TLDG_DT_NVHOTRO = 202501 and loaitb_id = 21;  -- 83316
		delete from ttkd_bsc.ct_bsc_ptm where thang_ptm = 202501 and loaitb_id = 21;
...
--				, DOANHTHU_KPI_NVDAI = nvl(DOANHTHU_KPI_DNHM, 0) + nvl(DOANHTHU_KPI_NVHOTRO, 0) , THANG_TLDG_DT_DAI = 202501
--				, DOANHTHU_KPI_DNHM = 0, DOANHTHU_KPI_NVHOTRO = 0
--		--select * from ttkd_bsc.ct_bsc_ptm 
--		where thang_ptm = 202501 and loaitb_id = 21
--					and ghi_chu like '%VB1036,eO-1036142, pbhtb, chuyển doanh thu các STB này qua CTV051560'			
--				
			;		
		  commit;
		create table  ttkd_bsc.tonghop_ct_dongia_ptm_202501_lan1 as select * from ttkd_bsc.tonghop_ct_dongia_ptm where thang = 202501;
		select * from ttkd_bsc.tonghop_ct_dongia_ptm_202501_lan1;
		select * from ttkd_bsc.tonghop_ct_dongia_ptm where thang = 202501 and donvi != 'TTKD';; and ptm_id in (select id from ttkd_bsc.ct_bsc_ptm where vanban_id = 937981);
--		delete from ttkd_bsc.tonghop_ct_dongia_ptm where thang = 202501 and donvi = 'VTTP';
		
		*********luu ý kiem tra vanban_id in (937981) and thang_tldg_dt = 202501 --> tính heso_dongia = 1 trong thang 10
...
--		delete from ttkd_bsc.tonghop_ct_dongia_ptm where thang = 202501 ;

select sum(TIEN_THULAO) from ttkd_bsc.tonghop_ct_dongia_ptm where thang = 202501; and donvi = 'VTTP'; and (thang_ptm <= 202411 or dichvuvt_id =4); and thang_ptm = 202501; ma_nv = 'VNP019958'; thang 01_ 1917294778 --  -- 8405986209-- 7 249 347 650
...
insert into ttkd_bsc.tonghop_ct_dongia_ptm
			with nv as (select thang, donvi, ma_nv, ten_nv, ten_to, ten_pb, ma_to, ma_pb, ma_vtcv, nhomld_id
									from ttkd_bsc.nhanvien where thang = 202501 and donvi in ('TTKD', 'VTTP')
--									union
--									select thang, 'POT_VTTP' donvi, ma_nv, ten_nv, ten_to, ten_pb, ma_to, ma_pb, ma_vtcv, nhomld_id
--									from ttkd_bsc.nhanvien_vttp_potmasco where thang = 202501
									union
									select 202501 thang, 'POT' donvi, MA_NV, TEN_NV, TEN_dv, null, 'POT', null, null, 5 nhomld_id 
			...
			select 
					...
					(
					    ...
					    where manv_ptm is not null and thang_tldg_dt = 202501
		...
		union all     
					    ...
					    from ttkd_bsc.ct_bsc_ptm
					    where manv_ptm is not null and thang_tldg_dnhm = 202501 
		...
		union all			    
					    ...
					    select  id, ma_gd, thuebao_id, ma_tb, loaitb_id, dichvuvt_id, manv_hotro
									 ...
									 , 'dnhm' nguon, thang_tldg_dnhm, thang_ptm
					    from ttkd_bsc.ct_bsc_ptm
					    where manv_hotro is not null and thang_tldg_dnhm = 202501 
		...
		union all     
					    ...
					    from ttkd_bsc.ct_bsc_ptm
					    where manv_tt_dai is not null and thang_tldg_dt_dai = 202501
		...
		union all     
					    ...
					    from ttkd_bsc.ct_bsc_ptm a
					    where manv_hotro is not null and thang_tldg_dt_nvhotro = 202501 
									and not exists (select 1 from ttkd_bsc.nhanvien where thang = 202501 and ma_pb = 'VNP0702600' and ma_nv = a.manv_hotro)
		union all     
					    -- nv ho tro cua PGP
...
--			insert into ttkd_bsc.tonghop_ct_dongia_ptm
--					select 202501 THANG, DONVI, MA_NV, TEN_NV, TEN_TO, TEN_PB, MA_TO, MA_PB, MA_VTCV, NHOMLD_ID
...
select thang, ma_nv, ma_vtcv
	...
	----all dvu ngoai tru VNPtt
			select nguon, sum(LUONG_DONGIA) from ttkd_bsc.tonghop_ct_dongia_ptm where thang = 202501 and loaitb_id = 21 and donvi = 'TTKD' group by nguon;  
		
			---374000000  ---418643750
			----378480000		----457964475
			;
			update ttkd_bsc.bangluong_dongia_202501
...
--								 select * 
								 from ttkd_bsc.tonghop_ct_dongia_ptm
								 where thang = 202501 
--												and (thang_ptm <= 202412 or dichvuvt_id = 4)		---tra tam ung, không chê lại, giam tien, chi thang cu + voi BRCD thang moi
												
								 group by ma_nv, donvi
			;
			update ttkd_bsc.bangluong_dongia_202501 a
...
--											, sum(tien_thulao) --4925059739
			...
			select * from ttkd_bsc.bangluong_dongia_202501 a ;
...
-- Luong don gia ghtt + Dong luc ghtt : Nhu Y
 create table ttkd_bsc.bangluong_dongia_202501_l1 as select * from  ttkd_bsc.bangluong_dongia_202501;
		...
		---KIEMTRA
				...
				from ttkd_bsc.bangluong_dongia_202501 a 
				union all
				select 'ghtt_cong', sum(tien) luong_dongia_ghtt, 0,0, 0, 0, 0
					from ttkd_bsc.tl_giahan_tratruoc 
                        where thang = 202501 and  ma_kpi = 'DONGIA' and loai_tinh in ('DONGIATRA_OB' ,'DONGIA_TS_TP_TT')
					and  tien <> 0
				union all
				select 'ghtt_cong_bs', sum(tien) luong_dongia_ghtt, 0,0, 0, 0, 0
					from ttkd_bsc.tl_giahan_tratruoc 
                        where thang = 202501 and  ma_kpi = 'DONGIA' and loai_tinh in ( 'DONGIATRA_OB_BS')
					and  tien <> 0
				union all
				select 'ghtt_tru_cntt', 0, 0, sum(tien) giamtru_ghtt_cntt, 0, 0, 0
						from ttkd_bsc.tl_giahan_tratruoc 
                        where thang = 202501 and  ma_kpi = 'DONGIA' and loai_tinh = 'DONGIATRU_CA'
						and  tien <> 0
				union all
				select 'thuhoi_ghtt_tru_ghtt', 0,0, 0, 0, sum(tien) luong_dongia_ghtt, 0
					from ttkd_bsc.tl_giahan_tratruoc 
                        where thang = 202501 and  ma_kpi = 'DONGIA' and loai_tinh in ('THUHOI_DONGIA_GHTT')
					and  tien <> 0
				;	
			

    UPDATE ttkd_bsc.bangluong_dongia_202501 SET thuhoi_dongia_ghtt = NULL, luong_dongia_ghtt = NULL, giamtru_ghtt_cntt = NULL
		; 
--		ROLLBACK;		
		   ;
update ttkd_bsc.bangluong_dongia_202501 a 
...
--												where thang = 202501 and ma_kpi = 'DONGIA' and loai_tinh = 'DONGIATRU_CA' and ma_nv = a.ma_nv
--                                                            group by ma_nv 
--                                                            having  sum(tien)  <> 0) 
...
;
SELECT distinct ten_vtcv, ma_vtcv FROM ttkd_bsc.bangluong_dongia_202501 a where donvi = 'TTKD' and LUONG_DONGIA_GHTT >0 ;
commit;


-- Luong khac: 
			update ttkd_bsc.bangluong_dongia_202501 set luong_khac='', ghichu=' ' 
			    where luong_khac is not null;

 
-- Don gia gia han VNPTS: Chi Nguyen
update ttkd_bsc.bangluong_dongia_202501 a set ghtt_vnpts=''
;
		update ttkd_bsc.bangluong_dongia_202501 a
		    set ghtt_vnpts=(select round(sum(luong_dongia * nvl(hs.heso_dthu, 1)), 0)  
									 ...
									 where x.thang = 202501 and thang_giao is not null and x.ma_nv is not null
										    and x.ma_nv = a.ma_nv
										    )
--		     select ma_nv, ten_vtcv, ten_pb, ghtt_vnpts, ghichu from ttkd_bsc.bangluong_dongia_202501 a
		    ;

---Don gia cho NV thuc hien Nghiep vu lap ho so, giai quyet khieu nai: a Khanh, chi Hoi
			-----BEGIN Bsung tam 1 thang 5
--		select distinct ma_vtcv from khanhtdt_ttkd.TONGHOP_BSC_KPI_2024 b where b.thang = 202501 and TONGTIEN > 0 ;
		select sum(luong_dongia_nghiepvu) from ttkd_bsc.bangluong_dongia_202501 b;

		update ttkd_bsc.bangluong_dongia_202501 Set luong_dongia_nghiepvu = null
		;
			update ttkd_bsc.bangluong_dongia_202501 a
...
---Don gia cho NV thuc hien xu ly noi dung chung tu: c Nhu, Kim Tien, ap dung ca To Thu ngan hang - NVC
		...
		update ttkd_bsc.bangluong_dongia_202501 Set luong_dongia_chungtu = null;
--		select ma_nv, ten_nv, sum(luong_dongia_chungtu) from ttkd_bsc.bangluong_dongia_202501 where luong_dongia_chungtu > 0 group by ma_nv, ten_nv
		;
			update ttkd_bsc.bangluong_dongia_202501 a 
						set
								luong_dongia_chungtu =  (select sum(tien) from ttkd_bsc.tl_giahan_tratruoc
																				  where thang = 202501 and ma_kpi = 'DONGIA' and loai_tinh = 'DONGIA_CHUNG_TU' and ma_nv = a.MA_NV
																				  having  sum(tien)  <> 0
																		)
...
--		select sum(TIEN) from ttkd_bsc.tl_giahan_tratruoc where loai_tinh = 'DONGIA_THUCUOC' and thang = 202501;
		update ttkd_bsc.bangluong_dongia_202501 set luong_dongia_thucuoc = null;
--		select sum(luong_dongia_thucuoc) from ttkd_bsc.bangluong_dongia_202501;
		...
		where thang_thu=202501 and loaids_id=2
		;
		...
		insert into ttkd_bsc.tl_giahan_tratruoc (THANG, LOAI_TINH, MA_KPI, MA_NV, MA_TO, MA_PB, TIEN)
					...
					where thang_thu=202501 and loaids_id=2
		;
		delete from ttkd_bsc.tl_giahan_tratruoc where thang = 202501 and ma_kpi = 'DONGIA' and loai_tinh = 'DONGIA_THUCUOC';
		
			update ttkd_bsc.bangluong_dongia_202501 a 
						set
								luong_dongia_thucuoc =  (select sum(tien) from ttkd_bsc.tl_giahan_tratruoc
																				  where thang = 202501 and ma_kpi = 'DONGIA' and loai_tinh = 'DONGIA_THUCUOC' and ma_nv = a.MA_NV
																				  having  sum(tien)  <> 0)
			;		
 ...
 commit; 
		...
		update ttkd_bsc.bangluong_dongia_202501 a 
						set luong_khac = 23967650
--		select ten_nv, luong_khac from ttkd_bsc.bangluong_dongia_202501 a 
		...
		----*****LUONG DON GIA THAU PGP theo file GGS******----
		update ttkd_bsc.bangluong_dongia_202501 a 
						set luong_khac = 308000
--		select ten_nv, luong_khac from ttkd_bsc.bangluong_dongia_202501 a 
		where ma_nv in ('VNP017611')
		;
		------END THAU----
...
/*		select --nv.donvi, a.ma_nv, a.ten_nv, ten_vtcv, a.ten_pb, 'VNP' ||loaihinh_tb, 
				...
				where a.thang = 202501 group by nv.donvi, a.ma_nv, a.ten_nv, ten_vtcv, a.ten_pb, loaihinh_tb
		;
		select sum(luong_dongia_vnphh)  from ttkd_bsc.bangluong_dongia_202501;
		update ttkd_bsc.bangluong_dongia_202501 a
...
*/		update ttkd_bsc.bangluong_dongia_202501 a
					set luong_dongia_vnphh = ( select sum(TIEN_THULAO)
																...
																where thang = 202501 and ma_nv = a.ma_nv) 
		;
		-----END 384
...
/*		select nv.donvi, a.ma_nv, a.ten_nv, ten_vtcv, a.ten_pb, 'VNP' ||loaihinh_tb, sum(TIEN_THULAO) 
				from vietanhvh.dongia_pttt_202501 a 
							join ttkd_bsc.nhanvien nv on a.ma_nv = nv.ma_nv and a.thang = nv.thang
				where a.thang = 202501 group by nv.donvi, a.ma_nv, a.ten_nv, ten_vtcv, a.ten_pb, loaihinh_tb
		;
		select sum(luong_dongia_vnphh_pttt) from ttkd_bsc.bangluong_dongia_202501 a; 1 870 445 050 -- 1 889 797 797
*/		update ttkd_bsc.bangluong_dongia_202501 a
					set luong_dongia_vnphh_pttt = ( select sum(tien_thulao) tien_luong
																from vietanhvh.dongia_pttt_202501
																where thang = 202501 and ma_nv = a.ma_nv
																)
...
--		select sum(luong_dongia_nghiepvu_vnp) from ttkd_bsc.bangluong_dongia_202501 a
		;
		update ttkd_bsc.bangluong_dongia_202501 a
					set luong_dongia_nghiepvu_vnp = ( select TIEN_THULAO from ttkd_bsc.man_tonghop_KPI 
																				where thang = 202501 and ma_kpi = 'HCM_DT_PTMOI_061'
																								and ma_nv = a.ma_nv
																				)
		;													
		-----END 525
...
--		update ttkd_bsc.bangluong_dongia_202501 set tong_luong_dongia='' where tong_luong_dongia is not null
		;
		...
		update ttkd_bsc.bangluong_dongia_202501 a
		    ...
		    ;
     --    select * from ttkd_bsc.bangluong_dongia_202501 where  ma_nv in ('VNP016902',
'VNP017072',
'VNP017853',
'VNP017014',
'VNP016898',
...
/* -- Kiem tra tong luong don gia: 
			...
			union all
			select sum(tong_luong_dongia) from ttkd_bsc.bangluong_dongia_202501;

*/

select sum(tong_thulao_thucchi) from ttkd_bsc.bangluong_dongia_202501;--1218135446  --
...
-- THUHOI: 
			...
			select *  from ttkd_bsc.bangluong_dongia_202501
			;
--			select loaitb_id, sum(tien_thuhoi) from ttkd_bsc.ct_thuhoi where thang=202501 group by loaitb_id;
			;
--- thu hoi PTM		
				update ttkd_bsc.bangluong_dongia_202501 a  
				   set luong_dongia_ptm_thuhoi = (select sum(tien_thuhoi) from ttkd_bsc.ct_thuhoi where thang=202501 and ma_nv=a.ma_nv)
--				   where exists(select 1 from ttkd_bsc.ct_thuhoi where thang=202501 and ma_nv=a.ma_nv)
				   ;
-- giam tru do thu ho so tai nha: 		
			update ttkd_bsc.bangluong_dongia_202501 a
				   set giamtru_hosotainha =
					   (select round(sum(tien_giam) ,0) from ttkd_bsc.ct_giamtru where thang=202501 and ma_nv=a.ma_nv)
			    --  select giamtru_hosotainha from ttkd_bsc.bangluong_dongia_202501  a
--			    where exists (select 1 from ttkd_bsc.ct_giamtru where thang=202501 and ma_nv=a.ma_nv)
			    ;
...
-- tong_luong_thuhoi:
--		update ttkd_bsc.bangluong_dongia_202501 a set tong_luong_thuhoi='' where tong_luong_thuhoi is not null
--;
			update ttkd_bsc.bangluong_dongia_202501 a
			...
			from ttkd_bsc.bangluong_dongia_202501
			union all
			select 'tien_luong_thuhoi', sum(tong_luong_thuhoi) from ttkd_bsc.bangluong_dongia_202501;
			*/

delete from ttkd_bsc.bangluong_dongia_202501  where ghichu = 'khongtontai';
...
----Backup bang---
create table ttkd_bsc.bangluong_dongia_202501_dot5 as select * from ttkd_bsc.bangluong_dongia_202501; 
select * from ttkd_bsc.bangluong_dongia_202501_dot2;
	
	
--		select * from ttkd_bsc.ct_bsc_ptm where thang_tldg_dt=202501 and manv_ptm='VNP031192'; and loaitb_id = 20;
...
select * from ttkd_bsc.bangluong_dongia_202501;
select * from ttkd_bsc.bangluong_kpi_202501 ;
...
from ttkd_bsc.bangluong_kpi where thang = 202501 and ma_kpi = 'HCM_DT_PTMOI_021' ;
select * from ttkd_bsc.tl_giahan_tratruoc where thang = 202501 and loai_tinh = 'DONGIATRA_OB';
...
--			, HCM_DT_PTMOI_056
--			, HCM_SL_COMBO_006
from ttkd_bsc.bangluong_dongia_202501 where ghichu = 'khongtontai';--ma_nv = 'VNP016902';

SELECT ma_nv, ten_nv, ma_vtcv, ten_vtcv, ma_pb,ma_to, ten_to,ma_kpi, ten_kpi, to_char(giatri) giatri
  FROM ttkd_bsc.bangluong_kpi_202501_tracuu where ma_nv = 'VNP020231';
  ;
delete from ttkd_bsc.bangluong_dongia_202501  where ghichu = 'khongtontai';
		...
		insert into ttkd_bsc.bangluong_dongia_202501 
			...
			from (--x_dongia_temp
					select ma_nv, luong_dongia_cdbr, luong_dongia_vnpts, luong_dongia_dnhm_vnptt, luong_dongia_goi_vnptt, luong_dongia_cntt, tien_thulao, 0 ghtt_vnpts, 0 tien_thuhoi
					  ...
					  where not exists(select 1 from ttkd_bsc.nhanvien where thang = 202501 and ma_nv = a.ma_nv)			--thang n
								 and exists (select 1 from ttkd_bsc.nhanvien where thang < 202501 and donvi = 'TTKD' and ma_nv = a.ma_nv)	--thang n-1
								 and tien_thulao > 0 
							
					union all
							select ma_nv, 0, 0, 0, 0, 0, 0, round(sum(luong_dongia),0), 0
							from ttkd_bsc.ghtt_vnpts a
							where a.thang=202501 and thang_giao is not null and ma_nv is not null
										    and not exists (select 1 from ttkd_bsc.nhanvien where thang = a.thang and ma_nv = a.ma_nv)		
											and exists (select 1 from ttkd_bsc.nhanvien where thang < 202501 and donvi = 'TTKD'  and ma_nv = a.ma_nv)
							group by ma_nv
					union all
					...
					left join (select donvi, ma_nv, ten_nv, ten_to, ten_pb, row_number() over (partition by ma_nv order by thang desc) rnk
										from ttkd_bsc.nhanvien
										where thang <= 202501) nv on a.ma_nv = nv.ma_nv and rnk = 1
...
----NSu tren 100tr gui NSU
		select ma_nv, ten_nv, ten_vtcv, ten_pb, tong_luong_dongia from ttkd_bsc.bangluong_dongia_202501 where tong_luong_dongia>100000000
		;
		select MA_NV, TEN_NV, TEN_TO, TEN_PB, sum(LUONG_DONGIA) LUONG_DONGIA 
		from ttkd_bsc.tonghop_ct_dongia_ptm 
		where thang = 202501 group by MA_NV, TEN_NV, TEN_TO, TEN_PB having sum(LUONG_DONGIA) > 100000000
		;
		
---Anh Nghia tren 100tr/thang
select manv_ptm, ''''||manv_ptm||''',' mnv_nv, 'AM' vitri, sum(dg_goi) tong_dongia
from (
		select manv_ptm, luong_dongia_nvptm dg_goi from ttkd_bsc.ct_bsc_ptm where loaitb_id <> 21 and ((thang_ptm = 202501 and  manv_ptm is not null) or thang_tldg_dt=202501)
		union all
		select manv_hotro, luong_dongia_nvhotro dg_goi from ttkd_bsc.ct_bsc_ptm where loaitb_id = 21 and thang_tldg_dt_nvhotro =202501
		union all
		select manv_ptm, luong_dongia_dnhm_nvptm dg_dnhm from ttkd_bsc.ct_bsc_ptm where (thang_ptm = 202501 and manv_ptm is not null) or thang_tldg_dnhm=202501
)
group by manv_ptm
having sum(dg_goi) > 100000000
union all
		select manv_hotro, ''''||manv_hotro||''',' mnv_nv, 'PS' vitri, sum(luong_dongia_nvhotro) tong_dongia
		from ttkd_bsc.ct_bsc_ptm_pgp where (thang_ptm = 202501 and MANV_HOTRO is not null) or thang_tldg_dt_nvhotro = 202501
		group by manv_hotro
...
-- chi tiet gui anh Nghia:
 ...
 from ttkd_bsc.ct_bsc_ptm 
where( (thang_ptm >= 202410 and (thang_tldg_dt=202501 or thang_tldg_dt is null)) or (thang_ptm < 202410 and thang_tldg_dt=202501)
			    )
						and manv_ptm in ('CTV082572',
'VNP016957',
'CTV073755',
'VNP017599',
'CTV078296',
...
'CTV021976')
			...
			where (thang_ptm = 202501 or thang_tldg_dt_nvhotro = 202501)
...
'VNP017373')
			...
			select * from ttkd_bsc.ct_bsc_ptm_pgp 
			    where (thang_ptm>=202402 and (thang_tldg_dt_nvhotro=202501 or thang_tldg_dt_nvhotro is null)) 
					  or (thang_ptm<202402 and (thang_tlkpi_hotro=202501 or thang_tldg_dt_nvhotro=202501)  )
					  ; 
...
from ttkd_bsc.ct_bsc_ptm a
 where thang_ptm=202501 and dichvuvt_id not in (2,13,14,15,16) and tien_dnhm>0 and ngay_tt is not null and thang_tldg_dnhm is null;
...
from ttkd_bsc.bangluong_dongia_202501 a, ttkd_bsc.bangluong_dongia_202501_l3 b 
...
---Chi tiêt thue bao PTM dot tam ung gửi NSU
	select *
	from ttkd_bsc.bangluong_dongia_202501; where 'khongtontai' = ghichu
	;
	---VNPts ghtt
...
--					sum(TIEN_THUHOI) --24277103
	from (
							...
							from ttkd_bsc.ghtt_vnpts a
							where a.thang=202501 and thang_giao is not null and ma_nv is not null and LUONG_DONGIA > 0 
										    and not exists (select 1 from ttkd_bsc.nhanvien where thang = a.thang and ma_nv = a.ma_nv)		
											and exists (select 1 from ttkd_bsc.nhanvien where thang < 202501 and donvi = 'TTKD'  and ma_nv = a.ma_nv)
						) a
					left join (select donvi, ma_nv, ten_nv, ten_to, ten_pb, row_number() over (partition by ma_nv order by thang desc) rnk
