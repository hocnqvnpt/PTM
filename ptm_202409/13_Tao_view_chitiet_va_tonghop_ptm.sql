-- nv ptm: xong
select count(*) from ttkd_bsc.v_ct_bsc_ptm_202409;
select * from ttkd_bsc.v_bangluong_dongia_ptm_202409;
drop view ttkd_bsc.v_ct_bsc_ptm_202409;
create OR REPLACE VIEW ttkd_bsc.v_ct_bsc_ptm_202409 as
...
from ttkd_bsc.ct_bsc_ptm a
 where (loaitb_id not in (20,21) or (loaitb_id=20 and goi_luongtinh is null) or loaitb_id is null)
              and   thang_ptm between 202406 and 202409
		    and (nvl(thang_tldg_dt, 999999) >= 202409 or thang_tlkpi >= 202409 or thang_tlkpi_dnhm = 202409
                              )
  ;      
  -- Ho tro cua Phong Giai phap:  xong
 drop view ttkd_bsc.v_ct_bsc_ptm_phongGP_202409;
 create OR REPLACE VIEW ttkd_bsc.v_ct_bsc_ptm_phongGP_202409 as
 select 
			   ...
			   (select ma_to from ttkd_bsc.nhanvien where thang = 202409 and manv_hrm=a.manv_hotro) ma_to,
			   (select ten_to from ttkd_bsc.nhanvien where thang = 202409 and  manv_hrm=a.manv_hotro) ten_to,
			   (select ma_pb from ttkd_bsc.nhanvien where thang = 202409 and  manv_hrm=a.manv_hotro) ma_pb,
			   (select ma_pb from ttkd_bsc.nhanvien where thang = 202409 and  manv_hrm=a.manv_hotro) ten_pb,
 ...
 --view chi tieu 044
 -- nv ptm:
drop view ttkd_bsc.v_ct_bsc_cntt_202409;

select * from ttkd_bsc.v_ct_bsc_cntt_202409
;
create OR REPLACE VIEW ttkd_bsc.v_ct_bsc_cntt_202409 as
			 select thang_ptm,ma_gd, ma_gd_gt, tenkieu_ld,ma_tb,dich_vu,ten_tb,diachi_ld--,sdt_lh, email_lh
							,sothang_dc,ma_da,
							   to_char(ngay_bbbg,'dd/mm/yyyy') ngay_bbbg, to_char(ngay_luuhs_ttkd,'dd/mm/yyyy') ngay_luuhs_ttkd, to_char(ngay_luuhs_ttvt,'dd/mm/yyyy') ngay_luuhs_ttvt,
							   to_char(ngay_scan,'dd/mm/yyyy') ngay_scan,goi_cuoc
...
drop view ttkd_bsc.v_bangluong_dongia_ptm_202409;
create OR REPLACE VIEW ttkd_bsc.v_bangluong_dongia_ptm_202409 as
