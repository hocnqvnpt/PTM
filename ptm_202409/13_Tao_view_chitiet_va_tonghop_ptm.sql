-- nv ptm: xong
select * from ttkd_bsc.v_ct_bsc_ptm_202408;
select * from ttkd_bsc.v_bangluong_dongia_ptm_202408;
drop view ttkd_bsc.v_ct_bsc_ptm_202408;
create OR REPLACE VIEW ttkd_bsc.v_ct_bsc_ptm_202408 as
...
from ttkd_bsc.ct_bsc_ptm a
 where (loaitb_id not in (20,21) or (loaitb_id=20 and goi_luongtinh is null) or loaitb_id is null)
              and   (thang_tldg_dt=202408 or thang_tlkpi=202408 or thang_tlkpi_dnhm=202408
                              or (thang_ptm between 202404 and 202408 and thang_tldg_dt is null) 
                              )
  ;          
union all  -- VNPTT
;
 select thang_ptm, tenkieu_ld,ma_tb,dich_vu, datcoc_csd, thang_bddc, thang_ktdc, sothang_dc,
       ...
       ,dthu_goi_goc,dthu_goi
        ...
        doanhthu_kpi_dnhm, thang_tlkpi_dnhm
 from ttkd_bsc.ct_bsc_ptm a
 where loaitb_id=21 and thang_ptm=202408
 ;
                    
select * from ttkd_bsc.v_ct_bsc_ptm_dai_202408;
 
-- CT Thuê bao PTM (vnpts do PBHOL ptm + thuê bao h? tr? kh?i gián ti?p)
 drop view ttkd_bsc.v_ct_bsc_ptm_dai_202408;
 create OR REPLACE VIEW ttkd_bsc.v_ct_bsc_ptm_dai_202408 as
 select 
 thang_ptm,ma_gd,ma_gd_gt,ma_tb,dich_vu,tenkieu_ld,ten_tb,diachi_ld,sothang_dc,ma_da,
        ...
        (select ma_to from ttkd_bsc.nhanvien where thang = 202408 and  ma_nv = a.manv_tt_dai) ma_to,
        (select ten_to from ttkd_bsc.nhanvien where thang = 202408 and  ma_nv=a.manv_tt_dai) ten_to,
        (select ma_pb from ttkd_bsc.nhanvien where thang = 202408 and  ma_nv=a.manv_tt_dai) ma_pb,
        (select ma_pb from ttkd_bsc.nhanvien where thang = 202408 and  ma_nv=a.manv_tt_dai) ten_pb,
 ...
 from ttkd_bsc.ct_bsc_ptm a
 where  thang_ptm between 202405 and 202408 
                and (thang_tldg_dt=202408 or thang_tldg_dt is null or thang_tldg_dt_dai=202408 )
                and manv_tt_dai is not null
;

  -- Ho tro cua Phong Giai phap:  xong
 drop view ttkd_bsc.v_ct_bsc_ptm_phongGP_202408;
 create OR REPLACE VIEW ttkd_bsc.v_ct_bsc_ptm_phongGP_202408 as
 select 
			   ...
			   (select ma_to from ttkd_bsc.nhanvien where thang = 202408 and manv_hrm=a.manv_hotro) ma_to,
			   (select ten_to from ttkd_bsc.nhanvien where thang = 202408 and  manv_hrm=a.manv_hotro) ten_to,
			   (select ma_pb from ttkd_bsc.nhanvien where thang = 202408 and  manv_hrm=a.manv_hotro) ma_pb,
			   (select ma_pb from ttkd_bsc.nhanvien where thang = 202408 and  manv_hrm=a.manv_hotro) ten_pb,
 ...
 where (thang_ptm=202408 
              or ((thang_ptm between 202405 and 202408) and (thang_tldg_dt_nvhotro is null or thang_tldg_dt_nvhotro = 202408 ))
              or thang_tlkpi_hotro = 202408) 
              and exists (select 1 from ttkd_bsc.nhanvien where thang = 202408 and  ma_pb='VNP0702600' and ma_nv = a.manv_hotro)
		    ;
              
 --view chi tieu 044
 -- nv ptm:
drop view ttkd_bsc.v_ct_bsc_cntt_202408;

select * from ttkd_bsc.v_ct_bsc_cntt_202408
;
create OR REPLACE VIEW ttkd_bsc.v_ct_bsc_cntt_202408 as
			 select thang_ptm,ma_gd, ma_gd_gt, tenkieu_ld,ma_tb,dich_vu,ten_tb,diachi_ld--,sdt_lh, email_lh
							,sothang_dc,ma_da,
							   to_char(ngay_bbbg,'dd/mm/yyyy') ngay_bbbg, to_char(ngay_luuhs_ttkd,'dd/mm/yyyy') ngay_luuhs_ttkd, to_char(ngay_luuhs_ttvt,'dd/mm/yyyy') ngay_luuhs_ttvt,
							   to_char(ngay_scan,'dd/mm/yyyy') ngay_scan,goi_cuoc
...
drop view ttkd_bsc.v_bangluong_dongia_ptm_202408;
create OR REPLACE VIEW ttkd_bsc.v_bangluong_dongia_ptm_202408 as
select ma_nv, ten_nv,ma_vtcv,ten_vtcv,	ma_pb,	ten_pb,	ma_to,ten_to,	loai_ld,
				 ...
				 , LUONG_DONGIA_PTM_BS_T7 , tong_luong_dongia,	ghichu	,
				  ...
				  , giamtru_hosotainha,	giamtru_phathuy_qldb, giamtru_ghtt_cntt, LUONG_DONGIA_PTM_THUHOI_T7, tong_luong_thuhoi
				  , TONG_THULAO_THUCCHI
