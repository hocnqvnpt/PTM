...
--		   select thang_luong, thang_ptm, ma_tb, lydo_khongtinh_dongia, thang_tldg_dt, thang_tldg_dt_nvhotro, thang_tlkpi_phong from ttkd_bsc.ct_bsc_ptm a
		    where thang_ptm >= 202405 and lydo_khongtinh_dongia is not null---thang n-3
						and (nvl(thang_tldg_dt, 999999) >= 202408 and nvl(thang_tlkpi_phong, 999999) >= 202408)
...
--				select thang_ptm, nocuoc_ptm from ttkd_bsc.ct_bsc_ptm a 
				    where ma_kh<>'GTGT rieng' and loaitb_id<>21 and thang_tldg_dt is null 
								   ...
								   and ((thang_ptm = 202408 and nocuoc_ptm is not null)           ---thang n     
											 or (thang_ptm = 202407 and nocuoc_n1 is not null)				---thang n-1
											 or (thang_ptm = 202406 and nocuoc_n2 is not null)				--thang n -2
											 or (thang_ptm = 202405 and nocuoc_n3 is not null))			---thang n-3
							 ;
				commit;
...
--			     select thang_ptm, ma_tb, thang_tldg_dt, lydo_khongtinh_luong, lydo_khongtinh_dongia, nguon from ttkd_bsc.ct_bsc_ptm a
			    where thang_ptm >= 202405 and loaitb_id=358 and nvl(dthu_goi,0)=0
...
--				select thang_ptm, ma_tb, thang_tldg_dt, lydo_khongtinh_luong, lydo_khongtinh_dongia, nguon from ttkd_bsc.ct_bsc_ptm a 
			    where thoihan_id=1 and loaitb_id not in (90,146) 
					  ...
					  and ((thang_ptm = 202405 and trangthaitb_id_n3 not in (7,9)) 
							    or (thang_ptm = 202406 and trangthaitb_id_n2 not in (7,9)) 
							    or (thang_ptm = 202407 and trangthaitb_id_n1 not in (7,9))
							    or (thang_ptm = 202408 and trangthaitb_id not in (7,9))
							)
...
-- Thue bao chua phat sinh cuoc
	--check lai truo ckhi update thang
		--cac thue bao khong psc except 13 dvu tinh theo gia han tra truoc
		update ttkd_bsc.ct_bsc_ptm a 
				  set 
--						thang_tldg_dt         = case when thang_tldg_dt = 202408 then null end
--					   ,thang_tlkpi             = case when thang_tlkpi = 202408 then null end
--					   ,thang_tlkpi_to        = case when thang_tlkpi_to=202408 then null end
--					   ,thang_tlkpi_phong = case when thang_tlkpi_phong=202408 then null end
--					   ,thang_tldg_dnhm  = case when thang_tldg_dnhm=202408 then null end
--					   ,thang_tlkpi_dnhm = case when thang_tlkpi_dnhm=202408 then null end
--					   ,thang_tlkpi_dnhm_to = case when thang_tlkpi_dnhm_to=202408 then null end
--					   ,thang_tlkpi_dnhm_phong = case when thang_tlkpi_dnhm_phong=202408 then null end
--					   ,thang_tldg_dt_nvhotro = case when thang_tldg_dt_nvhotro=202408 then null end
--					   ,thang_tlkpi_hotro = case when thang_tlkpi_hotro=202408 then null end
--					   , thang_tldg_dt_dai = case when thang_tldg_dt_dai=202408 then null end
...
--		     select thang_luong, thang_ptm, thang_tldg_dt, ma_gd, ma_tb, trangthai_tt_id, lydo_khongtinh_dongia from ttkd_bsc.ct_bsc_ptm a
		    where thang_ptm >= 202405 and chuquan_id<>264 and thang_tldg_dt is null and loaitb_id not in (20,21,149) 
				  and not exists(select 1 from ttkd_bsc.dm_loaihinh_hsqd where loaitru_tinhluong=1 and loaitb_id=a.loaitb_id)
...
--		     select thang_ptm, thang_tldg_dt, lydo_khongtinh_dongia from ttkd_bsc.ct_bsc_ptm a
		    where thang_ptm >= 202405 and thang_tldg_dt is null and loaitb_id not in (20,21,149) 
				  and not exists(select 1 from ttkd_bsc.dm_loaihinh_hsqd where loaitru_tinhluong=1 and loaitb_id=a.loaitb_id)
...
--			select thang_ptm, ma_tb, thuebao_id, thuebao_cha_id, doanhthu_dongia_nvptm, thang_tldg_dt, lydo_khongtinh_dongia, (select 'sochu: ' ||ma_tb|| lydo_khongtinh_dongia from ttkd_bsc.ct_bsc_ptm where thuebao_id = a.thuebao_cha_id) lydo from ttkd_bsc.ct_bsc_ptm a
			where loaitb_id = 17 and thang_ptm >= 202405
...
--		from ttkd_bsc.ct_bsc_ptm a
		where (thuebao_cha_id is not null and loaitb_id = 17)		---bsung 22/6
					...
					and nvl(thang_tldg_dt, 999999) >= 202408			--thang n
					and lydo_khongtinh_dongia not like '%Trang thai tbao%'
...
--				select lydo_khongtinh_dongia from ttkd_bsc.ct_bsc_ptm
				where lydo_khongtinh_dongia like '%chua thu tien%' and thang_ptm >= 202405 and nvl(thang_tldg_dt, 999999) >= 202408 and trangthai_tt_id = 1;
...
'hcm_smartca_00100214'
				...
				where lydo_khongtinh_dongia like '%Chua nop du ho so%' and thang_ptm >= 202405 and nvl(thang_tldg_dt, 999999) >= 202408 and nop_du = 1
				group by ten_pb;
				commit;
				rollback;