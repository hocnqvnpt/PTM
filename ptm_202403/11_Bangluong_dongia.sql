...
-- XUAT GUI BANG LUONG DON GIA: gui NSU
	select sum(tong_luong_dongia), sum(tong_luong_thuhoi) from ttkd_bsc.bangluong_dongia_202404; 
...
from ttkd_bsc.bangluong_kpi_202404;
			...
			from (
					select ma_nv, dthu_ptm_cdbr, dthu_ptm_vnpts, dthu_ptm_vnptt, dthu_ptm_cntt, luong_ptm_cdbr, luong_ptm_vnpts, luong_ptm_vnptt, luong_ptm_cntt 
					  ...
					  where ma_to is null 
								 and not exists(select 1 from ttkd_bsc.nhanvien_202404 where ma_nv=a.ma_nv)
								 and not exists(select 1 from ttkd_bsc.nhanvien_vttp where ma_nv=a.ma_nv)
					...
					union all
							select ma_nv, 0,0,0,0,0,0,  sum(nvl(luong_dongia_ctv_xhh,0)) + sum(nvl(luong_dongia_vnptt_tong,0))  ,0
							  ...
							  where not exists(select 1 from ttkd_bsc.bangluong_dongia_202404 where ma_nv=a.ma_nv)
											and not exists(select 1 from ttkd_bsc.nhanvien_vttp where ma_nv=a.ma_nv)
							  group by ma_nv)
			group by ma_nv
			  ;
