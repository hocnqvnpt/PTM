select * from ptm_codinh_202403;
drop table ptm_codinh_202403 purge;
create table ptm_codinh_202403 as
        with goi_dadv as (select b.thuebao_id,b.goi_id, b.nhomtb_id, c.ten_goi, row_number() over (partition by thuebao_id order by  nhomtb_id desc)rnk
                                                  ...
                                                  where b.goi_id=c.goi_id and b.trangthai=1 
                                                        --and b.ngay_dk = (select max(nhomtb_id) from css.v_bd_goi_dadv where thuebao_id = b.thuebao_id)
                                                        and b.thang_bd>= 202403  and (c.nhomgoi_dadv_id<>2 or c.nhomgoi_dadv_id is null) 
					)
		, km_sd as (select a.thuebao_id, max(a.tyle_sd) tyle_sd from css.v_khuyenmai_dbtb a, css.v_ct_khuyenmai b
                      where a.chitietkm_id = b.chitietkm_id and a.thang_bd>='202403' and a.hieuluc=1 and a.tyle_sd>0 and a.tyle_sd<>100
                            and a.khuyenmai_id not in (1977, 2056, 2998, 2999) and (b.nhom_km in (1,12) or b.nhom_km is null) 
                      group by thuebao_id
					)
		, km_tb as (select a.thuebao_id, max(a.tyle_tb) tyle_tb 
                      from css.v_khuyenmai_dbtb a, css.v_ct_khuyenmai b
                      where a.chitietkm_id = b.chitietkm_id and a.thang_bd>='202403' and a.hieuluc=1
                            and (b.nhom_km in (1,12) or b.nhom_km is null)
                            and a.tyle_tb>0 and a.tyle_tb<>100 and a.khuyenmai_id not in (1977, 2056, 2998, 2999)                                     
                     group by thuebao_id
					)
		, datcoc_goc as (select hd.hdtb_id, dc.thang_bd thang_bddc, dc.thang_kt thang_ktdc, kmb.DATCOC_CSD, kmb.tien_td, kmb.huong_dc
            --                                                                , sum(cuoc_dc) datcoc_csd, sum(tien_td) tien_td 
                                                            from css.v_hd_thuebao hd, css.v_hdtb_datcoc dc, css.v_ct_khuyenmai kmb
                                                            where hd.hdtb_id=dc.hdtb_id and hd.tthd_id = 6 and to_number(to_char(hd.ngay_ht,'yyyymm')) = 202403
                                                                            and dc.cuoc_dc>0 and (dc.nhom_datcoc_id not in (15,19,20,22,24,40) or dc.nhom_datcoc_id is null)
                                                                            and kmb.khuyenmai_id in (1977, 2056, 2998, 2999) and kmb.chitietkm_id = dc.chitietkm_id
            --                                                group by hd.hdtb_id, dc.thang_bd,dc.thang_kt
                                            union all
                                                    select a.hdtb_id, a.thang_bddc, a.thang_ktdc, b.datcoc_csd, b.tien_td, b.huong_dc
            --                                                    , sum(a.datcoc_csd) datcoc_csd,sum(a.tien_td) tien_td
                                                     from css.v_khuyenmai_dbtb a, css.v_ct_khuyenmai b, css.v_hd_thuebao c
                                                     where c.hdtb_id=a.hdtb_id and a.chitietkm_id = b.chitietkm_id       
                                                                        and a.datcoc_csd>0 and a.khuyenmai_id not in (1977, 2056, 2998, 2999) 
                                                                        and to_number(to_char(c.ngay_ht,'yyyymm')) = 202403
		...
		, pbh_nhan as	(select k.donvi_id, k.ten_dv, k.donvi_cha_id, h.ten_dv dv_cha from admin.v_donvi k, admin.v_donvi h 
		...
		, dvgt as (select thuebao_id, sum(cuoc_sd) cuoc_sd from css.v_sudung_dvgt 
  ...
  select lhtb.loaihinh_tb dich_vu
                ...
                , a.ma_kh, b.ma_tb, pbh_nhan.ten_dv tobh_nhan, a.donvi_id donvi_nhan_id
                ...
                , a.loaihd_id, a.kieuhd_id, a.kieutn_id, b.kieuld_id, b.dichvuvt_id, b.loaitb_id loaitb_id_hdtb, b.loaitb_id
                , (case when b.dichvuvt_id in (8,9) then to_char(tocdo_kenh.tocdo)||tocdo_kenh.donvi else tocdo_adsl.ma_td end) ma_td
                , v_hdtb.tocdo_id, (case when a.loaihd_id=7 then v_db.mucuoctb_id else b.mucuoctb_id end) mucuoctb_id                    
                , v_db.trangthaitb_id, v_db.doituong_id, dt.ten_dt ten_doituong 
                , b.doituong_ct_id, dc.ap_id, dc.khu_id, dc.pho_id, dc.phuong_id, dc.quan_id, dc.sonha, dc.tinh_id 
                , b.ten_tb, b.diachi_ld, a.so_gt, v_db.mst, v_db.mst_tt, v_db.chuquan_id, trim(a.ma_duan) ma_duan_banhang
...
--                , round(datcoc.datcoc_csd/1.1,0) tien_td
   ...
   from  css.v_hd_khachhang a, css.v_hd_thuebao b, v_db, v_hdtb, tinhcuoc.v_dbtb db_old
         ...
         where a.hdkh_id = b.hdkh_id and b.thuebao_id = v_db.thuebao_id  and db_old.ky_cuoc (+) = 20240201 and b.thuebao_id = db_old.thuebao_id (+)
                ...
                and ( (a.loaihd_id=1 and b.kieuld_id not in (540,541,557,249,13130,13222,13224,71,280,550,551,13235, 14069) )  
                 ...
                 and trunc(b.ngay_ht) >= to_date('01/03/2024','dd/mm/yyyy') 
                and trunc(b.ngay_ht) <= to_date('31/03/2024','dd/mm/yyyy')
             ;
                          
             select * from ptm_codinh_202403
                where (ma_tb, ma_gd, hdtb_id) in 
                                                (select ma_tb, ma_gd, hdtb_id
                                                from ptm_codinh_202403 a 
                                                group by ma_tb, ma_gd, hdtb_id having count(*)>1 )
            ;
--Them cot------ tien_dnhm, tien_camket, tien_sodep:
                    ALTER TABLE ptm_codinh_202403 ADD (tien_dnhm NUMBER, tien_sodep NUMBER, tien_camket NUMBER);
                    UPDATE ptm_codinh_202403 a 
                        SET tien_camket = (SELECT tien_ck FROM css.v_camket_hdtb@dataguard WHERE hdtb_id=a.hdtb_id)
                        WHERE dichvuvt_id in (1,10,11)
                        ;
                    
                     
                    MERGE INTO ptm_codinh_202403 a
                    USING (with dnhm as (SELECT dnhm1.khoanmuctt_id loai,dnhm1.hdtb_id, dnhm1.phieutt_id
                                                                          FROM css.v_ct_phieutt@dataguard dnhm1, css.v_hd_thuebao@dataguard hd
                                                                          WHERE hd.hdtb_id=dnhm1.hdtb_id AND dnhm1.khoanmuctt_id IN(1,2,13,17)
                                                                                AND TO_CHAR(hd.ngay_ht,'yyyymm')='202403'
                                                                       )
                                                    , thietbi as (SELECT thietbi1.khoanmuctt_id loai,thietbi1.hdtb_id, thietbi1.phieutt_id
                                                                                            FROM css.v_ct_phieutt@dataguard thietbi1, css.v_hd_thuebao@dataguard hd
                                                                                            WHERE hd.hdtb_id=thietbi1.hdtb_id AND thietbi1.khoanmuctt_id IN(5)
                                                                                                    AND TO_CHAR(hd.ngay_ht,'yyyymm')='202403'
                                                                                           ) 
                                    SELECT hdtb_id, SUM(ct.tien+ct.tienkm) AS tien_dnhm
                                    FROM css.v_phieutt_hd@dataguard hd1,       
                                                (SELECT decode(ct.khoanmuctt_id,19,dnhm.loai,20, thietbi.loai,ct.khoanmuctt_id) loai,     
                                                    ...
                                                    FROM css.v_ct_phieutt@dataguard ct                     
                                                                LEFT JOIN
                                                    ...
                                                    WHERE EXISTS(SELECT 1 FROM css.v_hd_thuebao@dataguard WHERE TO_CHAR(ngay_ht,'yyyymm')='202403' AND hdtb_id=ct.hdtb_id)
                                                ) ct      
                    ...
                    MERGE INTO ptm_codinh_202403 a
                                ...
                                UPDATE ttkd_bct.ptm_codinh_202403 a
                                        SET tien_sodep = (SELECT cuoc_sd FROM css.v_hdtb_cntt@dataguard where cuoc_sd>0 and hdtb_id=a.hdtb_id)
                                    -- SELECT ma_tb, tien_sodep FROM ttkd_bct.ptm_codinh_202402 a
                                    WHERE dichvuvt_id IN (13,14,15,16) AND EXISTS(SELECT 1 FROM css.v_hdtb_cntt@dataguard where cuoc_sd>0 and hdtb_id=a.hdtb_id);
                                COMMIT;
            
            ----move to ONEBSS
            create table ptm_codinh_202403 as
                        select * from ttkd_hcm.ptm_codinh_202403@dataguard
            ;
...
-- Mucuoc_tb:
        drop table temp_muccuoctb_thuebao;
        create table temp_muccuoctb_thuebao as
                with km_dbtb as (select thuebao_id, tyle_tb, cuoc_tb, tien_tb  from css.v_khuyenmai_dbtb@dataguard 
                                                          where phanvung_id=28 and hieuluc=1 and (tyle_tb>0 or cuoc_tb>0 or tien_tb>0) and thang_bd>=202403          
                                                       ) 
            ...
            from css.v_hd_khachhang@dataguard hdkh, css.v_hd_thuebao@dataguard hdtb
                      ,css.v_db_thuebao@dataguard db, css.v_db_cntt@dataguard dbc, css.v_muccuoc_tb@dataguard mc
                      , km_dbtb
                                
            where hdkh.phanvung_id=28 and hdtb.phanvung_id=28 and dbc.phanvung_id=28 and mc.phanvung_id=28 
                and hdkh.hdkh_id=hdtb.hdkh_id and hdtb.thuebao_id=db.thuebao_id and db.thuebao_id=dbc.thuebao_id and db.mucuoctb_id=mc.mucuoctb_id      
                and dbc.thuebao_id=km_dbtb.thuebao_id(+) 
                and hdtb.dichvuvt_id in (13,14,15,16) and hdtb.tthd_id=6
                and hdkh.loaihd_id in (1,6,31,41,49) and to_number(to_char(hdtb.ngay_ht,'yyyymm'))=202403
                ;
                
                drop table temp_muccuoctb_hdtb;
                create table temp_muccuoctb_hdtb as
                    with km_hdtb as (select hdtb1.hdtb_id, km_hdtb1.tyle_tb, km_hdtb1.cuoc_tb,  km_hdtb1.tien_tb
                                                            from css.v_hd_thuebao@dataguard hdtb1, css.v_khuyenmai_hdtb@dataguard km_hdtb1
                                                            where hdtb1.phanvung_id=28 and km_hdtb1.phanvung_id=28
                                                                and hdtb1.hdtb_id=km_hdtb1.hdtb_id
                                                                and hdtb1.tthd_id=6 and to_number(to_char(hdtb1.ngay_ht,'yyyymm'))=202403    
                ...
                from css.v_hd_khachhang@dataguard hdkh, css.v_hd_thuebao@dataguard hdtb, css.v_hdtb_cntt@dataguard hdtbc, css.v_muccuoc_tb@dataguard mc
                          , km_hdtb
                    ...
                    where hdkh.phanvung_id=28 and hdtb.phanvung_id=28 and hdtbc.phanvung_id=28 and mc.phanvung_id=28
                        ...
                        and hdtb.tthd_id=6 and hdkh.loaihd_id in (1,6,31,41,49) and to_number(to_char(hdtb.ngay_ht,'yyyymm'))=202403    
                        and hdtb.dichvuvt_id in (13,14,15,16)
                ;
                select * from ptm_codinh_202403 a where muccuoc_tb is not null;
                update ptm_codinh_202403 a set muccuoc_tb ='';
                update ptm_codinh_202403 a 
                    set muccuoc_tb = (case when loaitb_id=39 then (select muccuoc_tb from temp_muccuoctb_thuebao where thuebao_id=a.thuebao_id ) 
                                                                else (select muccuoc_tb from temp_muccuoctb_hdtb where hdtb_id=a.hdtb_id ) end) 
                    where dichvuvt_id in (13,14,15,16) 
                    ;
                    commit;
--                        
--                select thuebao_id, hdtb_id, loaitb_id, ma_tb, tocdo_id, mucuoctb_id, sl_mailing,muccuoc_tb_old, muccuoc_tb, goi_id, tocdo_id, datcoc_csd, tien_td from  ptm_codinh_202403 
--                    where dichvuvt_id in (13,14,15,16) and muccuoc_tb>0;
--                        
--                select thuebao_id, hdtb_id, loaitb_id, ma_tb, muccuoc_tb_old, muccuoc_tb, goi_id, tocdo_id, datcoc_csd, tien_td from  ptm_codinh_202403 
--                    where dichvuvt_id in (13,14,15,16);
                    
-- tien_dvgt theo hdtb_id lay thoi gian n-1
            drop table temp_tien_dvgt_theohdtb ;
            create table temp_tien_dvgt_theohdtb as
                with b as (select hd.hdtb_id, km.chitietkm_id, km.cuoc_sd, km.tyle_sd, km.tien_sd, ctkm.dichvugt_id 
                                                from css.v_khuyenmai_dbtb@dataguard km, css.v_ctkm_dvgt@dataguard ctkm,  css.v_hd_thuebao@dataguard hd
                                                where hd.hdtb_id=km.hdtb_id and km.chitietkm_id=ctkm.chitietkm_id and km.hieuluc=1 --and thang_bd>=202403
                                                    and nvl(km.tyle_tb,0)=0 and nvl(km.cuoc_tb,0)=0 and nvl(km.tien_tb,0)=0
                                                    and to_number(to_char(hd.ngay_ht,'yyyymm'))>=202402 )
                    select hdtb_id, sum(cuoc_sd) cuoc_sd
                        from (select a.hdtb_id, a.ma_tb, b.chitietkm_id, d.noidung soluong, d.cuoc_sd cuoc_sd_goc, b.cuoc_sd cuoc_giam_1donvi, b.tyle_sd, b.tien_sd                        
                                            ,(case when b.tyle_sd>0 then round(d.cuoc_sd * nvl(to_number(regexp_replace (d.noidung, '\D', '')),1)  *(100-nvl(b.tyle_sd,0))/100,0)  -- giam tyle
                                                        ...
                                                        else d.cuoc_sd * nvl(to_number(regexp_replace (d.noidung, '\D', '')),1) end) cuoc_sd                                   
                                   from css.v_hd_thuebao@dataguard a, css.v_dangky_dvgt@dataguard d, css.dichvu_gt@dataguard e                                     
                                            , b
                        where e.phanvung_id=28 and a.hdtb_id=d.hdtb_id and d.dichvugt_id=e.dichvugt_id                                              
                                ...
                                and to_number(to_char(a.ngay_ht,'yyyymm'))>=202402
                        ) group by hdtb_id
                ;     
            create index temp_tien_dvgt_theohdtb_hdtb on temp_tien_dvgt_theohdtb (hdtb_id);
            
            drop table temp_tien_dvgt_theothuebao ;
                ...
                ;
                    ...
                    update ptm_codinh_202403 set tien_dvgt='';
                    update ptm_codinh_202403 a 
                        set tien_dvgt = case when loaitb_id=39 
                                                            then (select cuoc_sd from temp_tien_dvgt_theothuebao where hdtb_id=a.hdtb_id) 
                                                            else (select cuoc_sd from temp_tien_dvgt_theohdtb where hdtb_id=a.hdtb_id)  end 
                        -- select tien_dvgt from ptm_codinh_202403 a
                        where dichvuvt_id in (13,14,15,16) and loaitb_id=39
                        ;
                        commit;
                        
-- tien_tbi: 
                    UPDATE ptm_codinh_202403 SET tien_tbi='';
                    MERGE INTO ptm_codinh_202403 a
                    USING (SELECT cttbi.hdtb_id, tbi_lhtb.loaitb_id, SUM(CASE WHEN tien_tratruoc>0 THEN cttbi.soluong*cttbi.tien_tratruoc ELSE cttbi.soluong*cttbi.tien END) tien_tbi
                                    FROM css.v_ct_mua_tbi@dataguard cttbi, css.v_loai_tbi@dataguard tbi, css.v_loai_tbi_lhtb@dataguard tbi_lhtb , css.loaihinh_tb@dataguard lhtb
                                    WHERE cttbi.loaitbi_id=tbi.loaitbi_id AND tbi.loaitbi_id=tbi_lhtb.loaitbi_id AND lhtb.loaitb_id=tbi_lhtb.loaitb_id
                                                        ...
                                                        AND EXISTS(SELECT 1 FROM ttkd_bct.ptm_codinh_202403 WHERE hdtb_id=cttbi.hdtb_id AND loaitb_id=tbi_lhtb.loaitb_id)
               ...
               with datcoc_goc as (select b.hdtb_id, a.thuebao_id , a.thang_bd thang_bddc, a.thang_kt thang_ktdc, a.cuoc_dc datcoc_csd, a.tien_td, ctkm.huong_dc
                                                                   from css_hcm.db_datcoc a, css_hcm.hdtb_datcoc b, css_hcm.ct_khuyenmai ctkm
                                                                    where a.thuebao_dc_id=b.thuebao_dc_id and a.chitietkm_id = ctkm.chitietkm_id and ctkm.khuyenmai_id not in (1977, 2056, 2998, 2999)
                                                                                and a.cuoc_dc > 0 and a.hieuluc = 1 and a.ttdc_id = 0 and b.thang_bd >= 202403
...
--                                                                    group by a.hdtb_id, a.thuebao_id, a.thang_bddc,a.thang_ktdc     
                                ...
                                , datcoc as (select min(hdtb_id) hdtb_id, thuebao_id, min(thang_bddc) thang_bddc, max(thang_ktdc) thang_ktdc, max(huong_dc) huong_dc, sum(datcoc_csd) datcoc_csd, sum(tien_td) tien_td
                                                        from datcoc_goc where datcoc_csd > 0
                                                        group by  thuebao_id
                                                        )
--                                                        select * from datcoc where thuebao_id = 12154288
...
--                                ,(case when b.loaitb_id in (80,140) then datcoc.tien_td else round(datcoc.tien_td/1.1,0) end) tien_td  ,
                ...
                from css_hcm.hd_khachhang a, css_hcm.hd_thuebao b, admin_hcm.nhanvien c
                             , datcoc
                where a.hdkh_id=b.hdkh_id and a.ctv_id=c.nhanvien_id and b.hdtb_id = datcoc.hdtb_id
                        and a.loaihd_id=31 and b.kieuld_id=550
                        and to_number(to_char(b.ngay_ht,'yyyymm')) >= 202403
                        and exists(select 1 from ptm_codinh_202403 where datcoc_csd is null and thuebao_id=b.thuebao_id and ma_tiepthi=c.ma_nv )
                        ;
                         
       
   -- Kiem tra trung truoc khi update bs datcoc:
            select * from temp_datcoc
            where thuebao_id in 
                (select thuebao_id from temp_datcoc  a
                where exists(select 1 from ptm_codinh_202403 where datcoc_csd is null and thuebao_id=a.thuebao_id and ma_tiepthi=a.ma_nv)  
                group by  thuebao_id having count(*)>1)
                ;
 
            --them cot hdtb_id_datcoc
            alter table ptm_codinh_202403 add hdtb_id_datcoc number;
            update ptm_codinh_202403 a set (hdtb_id_datcoc, datcoc_csd, tien_td, thang_bddc, thang_ktdc, sothang_dc)=
                        (select hdtb_id, round(datcoc_csd/1.1,0) datcoc_csd, round(tien_td/1.1,0) tien_td, thang_bddc, thang_ktdc, sothang_dc
                            from temp_datcoc where datcoc_csd>0 and thuebao_id=a.thuebao_id and ma_nv=a.ma_tiepthi)
                -- select thuebao_id, ma_tb, sothang_dc, datcoc_csd, tien_td from ptm_codinh_202403 a
                where datcoc_csd is null 
                    ...
                    and ngay_bbbg=(select max(ngay_bbbg) from ptm_codinh_202403 where thuebao_id=a.thuebao_id )
                    ;
                    
            commit;
            
            select a.*, b.ma_tiepthi , b.DATCOC_CSD, b.SOTHANG_DC
            from ttkd_bct.temp_datcoc a, ttkd_bct.ptm_codinh_202403 b
            where a.thuebao_id=b.thuebao_id
            ;

/*

select a.*, b.ma_tiepthi, b.ma_tiepthi , b.DATCOC_CSD, b.SOTHANG_DC
from ttkd_bct.temp_datcoc a, ttkd_bct.ptm_codinh_202403 b
where a.thuebao_id=b.thuebao_id;


-- Kiem tra thuebao_id trung :         
        select ma_gd, hdtb_id,thuebao_id, ma_tb,ngay_bbbg, loaihd_id, kieuld_id, tenkieu_ld, thang_bddc, datcoc_csd, goikhoan, ma_duan, 
                phongbh_nhan,loaitb_id,ma_tiepthi 
            from ptm_codinh_202403
            where (ma_gd, thuebao_id) in (select ma_gd, thuebao_id from ptm_codinh_202403 group by ma_gd, thuebao_id having count(*)>1) 
            order by thuebao_id, ngay_bbbg;
                                      
                                                
-- kiem tra vua lap moi vua chuyen sang Siptrunk trong thang va cung 1 nhan vien tiep thi:
        select * from ptm_codinh_202403 a
            where loaihd_id<>6                 
                         and exists (select 1 from ptm_codinh_202403
                                                where thuebao_id=a.thuebao_id and ma_tiepthi=a.ma_tiepthi and loaihd_id=6);
            

-- Kiem tra thue bao test: luu y xem cac tb test la dich vu gi de kiem tra lai dthu goi
        select ma_kh,thuebao_id, ma_tb,loaitb_id,dich_vu,donvi_nhan_id,cua_hang,phongbh_nhan,pbh_nhan_id,ten_tb,diachi_ld, ma_tiepthi, donvi_tt
            from ptm_codinh_202403 
            where upper(ten_tb) like '%TEST%' and pbh_nhan_id in (122,14); 


-- Kiem tra kieu lap dat:
        select loaihd_id, kieuld_id,tenkieu_ld, count(*) from ptm_codinh_202403
                where bo_dau(tenkieu_ld) not like 'Dat moi%' and bo_dau(tenkieu_ld) not like 'Ban cheo%'
                            and bo_dau(tenkieu_ld) not like 'Hoa mang moi%'
                            and bo_dau(tenkieu_ld) not like 'Lap moi%'
                            and bo_dau(tenkieu_ld) not like 'Dang ky%'
                            and bo_dau(tenkieu_ld) not like 'Chia tach VLAN%'
                            and bo_dau(tenkieu_ld) not like 'Chuyen doi Co dinh sang IMS%'
                            and bo_dau(tenkieu_ld) not like 'Chuyen doi VFONE, BFONE sang Sip Trunking%'
                group by loaihd_id, kieuld_id,tenkieu_ld order by tenkieu_ld;

-- Kiem tra chuquan bi null khong:
update ptm_codinh_202403 a
    set chuquan_id=(select chuquan_id from css_hcm.db_cntt where thuebao_id=a.thuebao_id)  
    --  select * from ptm_codinh_202403 a
    where chuquan_id is null
            and exists (select chuquan_id from css_hcm.db_cntt where thuebao_id=a.thuebao_id);         
*/
/*
-- GDVP, Home Combo:
alter table ptm_codinh_202403 add (chu_nhom varchar2(30), vnp_moi varchar2(100),nhomtb_id number(10)); 
update ptm_codinh_202403 a set chu_nhom='', nhomtb_id='', vnp_moi='';
update ptm_codinh_202403 a set 
        (chu_nhom,nhomtb_id) =(select b.ma_tb chu_nhom, nhomtb_id from css.v_bd_goi_dadv@dataguard b 
                                                     where b.ma_tb is not null 
                                                           and b.chunhom=1 and b.trangthai=1 and b.thang_bd>='202403'
                                                           and b.thuebao_id=a.thuebao_id and b.goi_id=a.goi_id
                                                           and rownum=1)
    where a.loaitb_id in (58,59,61,171) and a.goi_id is not null;

*/
 -- thoihan_id
update ptm_codinh_202403 set thoihan_id=1 
    where (thoihan_id<>1 or thoihan_id is null) and (tg_thue_tu is not null and tg_thue_den is not null );


-- lydo_khongtinh_luong:
alter table ptm_codinh_202403 add lydo_khongtinh_luong varchar2(200);
update ptm_codinh_202403 a
set lydo_khongtinh_luong = (
        with x as (
            select hdtb_id
                            ,case when chuquan_id is null or chuquan_id not in (145,266,264) then ';Chu quan khong thuoc TTKD-HCM' end kq1
                            ,case when doituong_id in (14,15,16,17,19,32,33,34,35,36,48,51,52,54,55,56,57,66) then ';Doi tuong nghiep vu' end kq2
                            ,case when doituong_id in (47,49,50) and dichvuvt_id in (7,8,9) then ';TSL doi tuong VMS, STTTT, Bo CA' end kq3
                            ,case when exists(select 1 from ttkd_bsc.dm_loaihinh_hsqd where loaitru_tinhluong=1 and loaitb_id=b.loaitb_id) then ';Dich vu khong tinh luong' end kq4
                            ,case when (upper(ma_tiepthi_new)='DL_CNT' or upper(ma_tiepthi_new)like'GTGT_%' 
                                                  or upper(ma_tiepthi_new)like'DAILY_%' or upper(ma_tiepthi_new)like'DL_%'
                                                  or upper(ma_nguoigt)like'DL_CNT' or upper(ma_nguoigt)like'GTGT_%' 
                                                  or upper(ma_nguoigt)like'DAILY_%' or upper(ma_nguoigt)like'DL_%')
                                    then '; Phat trien qua Dai ly, chi tinh bsc cho Phong' end kq5
                            ,case when thoihan_id=1 and ((loaitb_id in (58,59) and nvl(tien_dnhm,0)<2000000) or (dichvuvt_id in (7,8,9) and nvl(tien_dnhm,0)<3000000))
                                    then '; TB ngan han nhung khong thu tien DNHM dung qui dinh' end kq6
                           ,case when loaihd_id=41 and loaitb_id=153 and nvl(sothang_dc,0)<6 then '; Ghi nhan dthu khi gia han goi>=6 thang' end kq7       
            from ptm_codinh_202403 b
          )                                        
...
-- nv ptm:   
alter table ptm_codinh_202403 
    add (manv_ptm varchar2(20), tennv_ptm varchar2(100), 
            pbh_ptm_id number,ma_pb varchar2(20),ten_pb varchar2(100),ma_to varchar2(20),ten_to varchar2(100),
...
update ptm_codinh_202403 a 
    set ma_tiepthi_new=(select ma_nv from admin_hcm.nhanvien where nhanvien_id=a.nhanvien_id)
...
update ptm_codinh_202403 a 
    set (pbh_ptm_id,manv_ptm,tennv_ptm, ma_to,ten_to,ma_pb,ten_pb,ma_vtcv,loai_ld, nhom_tiepthi)=
          (select pb.pbh_id,b.manv_hrm,b.ten_nv,b.ma_to,b.ten_to,b.ma_pb,b.ten_pb,b.ma_vtcv,b.loai_ld,b.nhomld_id
            from ttkd_bsc.nhanvien_202403 b, ttkd_bsc.dm_phongban pb 
            where b.ma_pb=pb.ma_pb and b.manv_hrm=a.ma_tiepthi_new) 
    where chuquan_id in (145,266,264) 
        and exists (select 1 from ttkd_bsc.nhanvien_202403 b where b.manv_hrm=a.ma_tiepthi_new) ;

-- NV TTVT
update ptm_codinh_202403 a
   set (pbh_ptm_id,manv_ptm,tennv_ptm, ma_to,ten_to,ma_pb,ten_pb,ma_vtcv, loai_ld, nhom_tiepthi)=
            (select 15, b.ma_nv, b.ten_nv, b.ma_to, b.ten_to, b.ma_pb, b.ten_pb, b.ma_vtcv, b.loai_ld, 2
                from ttkd_bsc.nhanvien_vttp b 
                where thang=(select max(thang) from ttkd_bsc.nhanvien_vttp where ma_nv=b.ma_nv)
                            and ma_nv=a.ma_tiepthi_new)
   where loaihd_id=1 and exists(select 1 from ttkd_bsc.nhanvien_vttp b
                            where thang=(select max(thang) from ttkd_bsc.nhanvien_vttp where ma_nv=b.ma_nv)
                                        and ma_nv=a.ma_tiepthi_new);
...
update ptm_codinh_202403 a
    set pbh_ptm_id=(select pbh_id from ttkd_bsc.dm_phongban where active=1 and ma_pb_pttb=a.pbh_nhan_goc_id),
    ...
    -- select * from ptm_codinh_202308 a
...
update ptm_codinh_202403 a 
    set  pbh_ptm_id=(select pbh_id from ttkd_bsc.dm_phongban where active=1 and ma_pb_pttb=a.pbh_nhan_id),
            ma_pb=(select ma_pb from ttkd_bsc.dm_phongban where active=1 and ma_pb_pttb=a.pbh_nhan_id),
            ten_pb=(select ten_pb from ttkd_bsc.dm_phongban where active=1 and ma_pb_pttb=a.pbh_nhan_id)
...
update ptm_codinh_202403 a 
    set  pbh_ptm_id=16, 
            ma_pb=(select ma_pb from ttkd_bsc.dm_phongban where active=1 and pbh_id=16),
            ten_pb=(select ten_pb from ttkd_bsc.dm_phongban where active=1 and pbh_id=16)
    -- select * from ptm_codinh_202403
    where chuquan_id in (145,266) and ma_pb is null 
            and ((pbh_nhan_id=11298 and pbh_nhan_goc_id is null)
                     or (pbh_nhan_id=11298 and pbh_nhan_goc_id not in (11352,11563,11564,11578,10831,284316,284317,2941,2942,2943,2944,2945,2946,2947,2948)));


-- KTNV:
update ptm_codinh_202403 
    set pbh_ptm_id=null,ma_pb=null,ten_pb=null,ma_to=null, ten_to=null, manv_ptm=null, ma_vtcv=null,loainv_id=null,ten_loainv=null,loai_ld=null
    -- select * from ptm_codinh_202403
    where chuquan_id in (145,266) and ( (ma_pb='VNP0700700' and donvi_nhan_id<>283530) or upper(ten_tb) like '%ERP TEST%');

    
-- TTVT ptm nhung ko co ma_tiepthi hoac ma_tiepthi sai/nghi viec: 
update ptm_codinh_202403 a 
    set pbh_ptm_id=15    
    where pbh_ptm_id is null and chuquan_id in (145,266)
...
update ptm_codinh_202403 a 
    set  pbh_ptm_id=(select pbh_id from ttkd_bsc.dm_phongban where active=1 and ma_pb_pttb=a.pbh_nhan_id),
            ma_pb=(select ma_pb from ttkd_bsc.dm_phongban where active=1 and ma_pb_pttb=a.pbh_nhan_id),
            ten_pb=(select ten_pb from ttkd_bsc.dm_phongban where active=1 and ma_pb_pttb=a.pbh_nhan_id)
    where pbh_ptm_id is null and chuquan_id in (145,266,264)
            and pbh_nhan_id in (2948,11563,11564,284316,2943,11352,2942,2947,2944,2945,284317,2946);
       

-- manv_hotro: lay thong tin PS truong lam dai dien
update ptm_codinh_202403 a set manv_hotro='', tyle_hotro='', tyle_am='';
update ptm_codinh_202403 a set (manv_hotro, tyle_hotro, tyle_am)=
    ...
    -- select ma_gd, ma_tb, loaitb_id, ma_duan_banhang, manv_hotro, tyle_hotro,tyle_am from ptm_codinh_202403 a
...
select a.* from bcss_hcm.ct_no a
    where phanvung_id=28 and ky_cuoc=20240301 
        and khoanmuctt_id not in (441,520,521,527,3126,3127,3421,3953) 
        and exists(select 1 from ptm_codinh_202403 where thuebao_id=a.thuebao_id);
create index temp_ps_tbid on temp_ps (thuebao_id);


alter table ptm_codinh_202403 add dthu_ps number(12);
update ptm_codinh_202403 a 
    set dthu_ps = (select sum(nogoc) from temp_ps where thuebao_id=a.thuebao_id);
              

update ptm_codinh_202403 a 
        set doituong_id=doituong_ct_id
    where chuquan_id in (145,264,266) and (doituong_id=190 and dthu_ps>0) ;
...
update ptm_codinh_202403 bc set dthu_goi_goc='', dthu_goi='', dthu_goi_ngoaimang='';
update ptm_codinh_202403 bc set dthu_goi_goc=
    (case
          -- 1800,1900: phi duy tri + phi tai nguyen + phi so dep
          when bc.loaitb_id in (38,127) and bc.thang_bddc is null 
                    then (select nvl(cuoc_dt,0)+nvl(cuoc_tn,0)+nvl(cuoc_sd,0) from css_hcm.db_cntt where thuebao_id=bc.thuebao_id)
          ...
          -- CA,CA-TAX,CA-IVAN,VNPT CA Sign Server,IVAN: tong tien hop dong tru tien token
          when bc.loaitb_id in (116,55,117,140,132,288,154) and goi_id in (15596,15599,15600,15601,15602,15603,15604,15605) then bc.datcoc_csd-250000
          when bc.loaitb_id in (116,55,117,140,132,288,154) and (goi_id not in (15596,15599,15600,15601,15602,15603,15604,15605) or goi_id is null) then bc.datcoc_csd
          
    
         -- VNPT CA Plugin
         when bc.loaitb_id=80 and bc.thang_bddc is not null then bc.datcoc_csd
         when bc.loaitb_id=80 and bc.thang_bddc is null then (select distinct mc.cuoc_tg from css.v_muccuoc_tb@dataguard mc where phanvung_id=28 and mc.mucuoctb_id=bc.mucuoctb_id) 
       ...
       -- VNPT eKYC (He thong xac thuc va dinh danh dien tu) - theo thang
          when bc.loaitb_id=276 and thang_bddc is null then nvl(muccuoc_tb,0)+nvl(tien_dvgt,0)
          when bc.loaitb_id=276 and thang_bddc is not null then tien_td  
   
        
         -- HDDT, HDDT May tinh tien, VNPT Invoice - Inbot: gia tri hop dong
          when bc.loaitb_id in (122, 2116,373) and tocdo_id in (4784,5153) then nvl(bc.cuoc_dt,0)+nvl(bc.tien_tbi,0) 
          ...
          when bc.loaitb_id=153 and thang_bddc is not null then tien_td 
          
          
          -- Mail Hosting:
          when bc.loaitb_id=159 and thang_bddc is null then nvl(muccuoc_tb,0)+nvl(tien_dvgt,0)
          when bc.loaitb_id=159 and thang_bddc is not null then datcoc_csd  
          ...
          when bc.loaitb_id=12 and thang_bddc is not null then datcoc_csd 
          ...
          -- VNPT Check: 1 lan
          when bc.loaitb_id=52 then tien_tbi 
          
          
          --  Ten mien VN, Ten mien quoc te:
          when bc.loaitb_id in (147,148) and bc.thang_bddc is null then nvl(muccuoc_tb,0)+nvl(tien_dvgt,0)+nvl(tien_tbi,0)
          when bc.loaitb_id in (147,148) and bc.thang_bddc is not null then bc.datcoc_csd+nvl(tien_dvgt,0)
          ...
          when bc.loaitb_id=118 then tien_tbi  
          ...
          -- Tong dai ao 3CX: theo thang,  VB 283/TTKD HCM-DH (goi 12,24,36... /thang)
                -- Trong hop dong hop tac voi Sao Bac Dau. ve phi khoi tao: Sao Bac dau khong phan chi doanh thu cho TTKD nen khong ghi nhan phi khoi tao
        ...
        -- Hoi nghi truyen hinh NGN:
         ...
         -- VNPT HIS: theo thang
         when bc.loaitb_id=136 then nvl(muccuoc_tb,0)+nvl(tien_dvgt,0)+nvl(tien_tbi,0)        
         ...
         -- VNPT GoMeet: 322 (Phan mem hop truc tuyen)
          when bc.loaitb_id=302 then nvl(bc.datcoc_csd,0)+nvl(bc.tien_tbi,0)  
      ...
      */  
         ...
         -- HTVC:
          when bc.dichvuvt_id=4 and bc.loaitb_id=18 
                then (select cuoc_tb from css.v_muccuoc_tb@dataguard mc where phanvung_id=28 and bc.dichvuvt_id=mc.dichvuvt_id and bc.tocdo_id=mc.tocdo_id)

         -- Mega, Fiber tham gia goi tich hop:
          when bc.loaitb_id in (58,11) and bc.goi_id in (select goi_id from css.v_goi_dadv_lhtb@dataguard b where phanvung_id=28 and b.goi_id=bc.goi_id and b.loaitb_id=bc.loaitb_id and b.tocdo_id=bc.tocdo_id and (giamcuoc_tb=100 or giamcuoc_sd=100) ) 
                then
                        case when (select soluong_ip from css.tocdo_adsl@dataguard where soluong_ip>0 and (sl_ip_mp is null or sl_ip_mp<soluong_ip) and tocdo_id=bc.tocdo_id)>=8                                                
                                        then (select distinct tien_goi from css.v_goi_dadv_lhtb@dataguard b where phanvung_id=28 and b.goi_id=bc.goi_id and b.loaitb_id=bc.loaitb_id and b.tocdo_id=bc.tocdo_id)+500000
                                 when (select soluong_ip from css.tocdo_adsl@dataguard where soluong_ip>0 and (sl_ip_mp is null or sl_ip_mp<soluong_ip) and tocdo_id=bc.tocdo_id)=1
                                        then (select distinct tien_goi from css.v_goi_dadv_lhtb@dataguard b where phanvung_id=28 and b.goi_id=bc.goi_id and b.loaitb_id=bc.loaitb_id and b.tocdo_id=bc.tocdo_id )+200000 
                                 else (select distinct tien_goi from css.v_goi_dadv_lhtb@dataguard b where phanvung_id=28 and b.goi_id=bc.goi_id and b.loaitb_id=bc.loaitb_id and b.tocdo_id=bc.tocdo_id)
                        end  
              
          -- Mega, Fiber khong tham gia goi tich hop
          when bc.loaitb_id in (58,11) and (bc.goi_id is null or bc.goi_id not in (select goi_id from css.v_goi_dadv_lhtb@dataguard b where phanvung_id=28 and b.goi_id=bc.goi_id and b.loaitb_id=bc.loaitb_id and b.tocdo_id=bc.tocdo_id and (giamcuoc_tb=100 or giamcuoc_sd=100) ) ) then
                case when tocdo_id in (44171, 44140) then 250000    -- Fiber VT công ích gói FV60
                         when (select soluong_ip from css.tocdo_adsl@dataguard where soluong_ip>0 and (sl_ip_mp is null or sl_ip_mp<soluong_ip) and tocdo_id=bc.tocdo_id)>=8   
                                then (select cuoc_tg from css.v_muccuoc_tb@dataguard mc where phanvung_id=28 and mc.mucuoctb_id=bc.mucuoctb_id)+500000
                         when (select soluong_ip from css.tocdo_adsl@dataguard where soluong_ip=1 and sl_ip_mp is null and tocdo_id=bc.tocdo_id)=1
                                then (select cuoc_tg from css.v_muccuoc_tb@dataguard mc where phanvung_id=28 and mc.mucuoctb_id=bc.mucuoctb_id)+200000 
                        else (select cuoc_tg from css.v_muccuoc_tb@dataguard mc where phanvung_id=28 and mc.dichvuvt_id=bc.dichvuvt_id and mc.mucuoctb_id=bc.mucuoctb_id)
                end  
          ...
          when bc.loaitb_id in (61,171) then
                case 
                    ...
                    when bc.goi_id in (select goi_id from css.v_goi_dadv_lhtb@dataguard b where phanvung_id=28 and b.goi_id=bc.goi_id and b.loaitb_id=bc.loaitb_id and b.tocdo_id=bc.tocdo_id and (giamcuoc_tb=100 or giamcuoc_sd=100) ) 
                            then (select tien_goi from css.v_goi_dadv_lhtb@dataguard b where phanvung_id=28 and b.goi_id=bc.goi_id and b.loaitb_id=bc.loaitb_id and b.tocdo_id=bc.tocdo_id)
                    -- MyTV khong tham gia goi tich hop
                    when (bc.goi_id is null or bc.goi_id not in (select goi_id from css.v_goi_dadv_lhtb@dataguard b where phanvung_id=28 and b.goi_id=bc.goi_id and b.loaitb_id=bc.loaitb_id and b.tocdo_id=bc.tocdo_id and (giamcuoc_tb=100 or giamcuoc_sd=100) ) )            
                            then (select cuoc_tb from css.v_muccuoc_tb@dataguard mc where phanvung_id=28 and mc.mucuoctb_id=bc.mucuoctb_id)                     
                end
          ...
          when bc.dichvuvt_id in (1,10,11) then 
                case 
                        ...
                        when loaitb_id=63 and goi_id in (15648,15589,15590,15649,15591,15650,15592,15651,15593,15652,15594,15653,15595,15654)
                                    then (select tien_goi from css.v_goi_dadv_lhtb@dataguard where phanvung_id=28 and goi_id=bc.goi_id and loaitb_id=bc.loaitb_id)
                       ...
                       -- codinh, codinh tinh khac, vfone, bfone,...
                                ...
                                case
                                        ...
                                        when bc.doituong_id in (1,2,10,12,13,25,40,387) and bc.goi_id=3 then 40000   -- 20K
                                        when bc.doituong_id in (1,2,10,12,13,25,40,387) and bc.goi_id=4 then 80000  -- 66K
                                        when bc.doituong_id in (1,2,10,12,13,25,40,387) and bc.goi_id=5 then 110000  -- 99K
                                        when bc.doituong_id in (1,2,10,12,13,25,40,387) and bc.goi_id=63 then 65455  -- VNPOST
                                        when bc.doituong_id in (1,2,10,12,13,25,40,387) and (bc.goi_id is null or (bc.goi_id not in (3,4,5,63) )) then 20000
...
update ptm_codinh_202403 a set (dthu_goi_goc, ghi_chu)=
        (select 250000-dthu_goi_goc, 'dthu goi=250000-dthu goi cua hs lap moi (lap moi, chuyen doi cung thang)' from ptm_codinh_202403 b
            where loaihd_id=1 and thuebao_id=a.thuebao_id                 
                 and exists (select 1 from ptm_codinh_202403 where thuebao_id=b.thuebao_id and loaihd_id=6)
                 )
    where loaitb_id=77 and loaihd_id=6            
                 and exists (select 1 from ptm_codinh_202403 where thuebao_id=a.thuebao_id and loaihd_id<>6);

update ptm_codinh_202403 set dthu_goi=dthu_goi_goc ;


update ptm_codinh_202403 a 
    set dthu_goi_ngoaimang=
                (select sum(nogoc) from bcss.v_tonghop@dataguard
                    where ky_cuoc=20240301 and khoanmuctc_id in (37,38,39,40,935,936,938,939,943,944,945,4097) and ma_tb=a.ma_tb),
           dthu_goi=(select sum(nogoc) from bcss.v_tonghop@dataguard
                            where ky_cuoc=20240301 and khoanmuctc_id not in (37,38,39,40,935,936,938,939,943,944,945,4097) and ma_tb=a.ma_tb) 
        where loaitb_id=131;
...
update ptm_codinh_202403 a 
    ...
    where loaitb_id in (15,17) 
        and exists (select 1 from (select thuebao_cha_id, count(*) sl_socon from tinhcuoc_hcm.dbtb a
                                                        where ky_cuoc=20240201 and exists (select 1 from ptm_codinh_202403
                                                                                                                              where loaitb_id in (15,17) and thuebao_cha_id=a.thuebao_cha_id)
                                                        group by thuebao_cha_id having count(*)>=29)
                             where thuebao_cha_id=a.thuebao_cha_id);

-- Kiem tra neu co ps thi xet them case nay vi thoi gian chay lenh nay lau
update ptm_codinh_202403 a 
        set dthu_goi_goc = (select cuoc_tggoc+cuoc_ipgoc from bcss.v_thftth@dataguard where phanvung_id=28 and ky_cuoc=20240301 and ma_tb=a.ma_tb),
              dthu_goi = (select cuoc_tggoc+cuoc_ipgoc from bcss.v_thftth@dataguard where phanvung_id=28 and ky_cuoc=20240301 and ma_tb=a.ma_tb)
    where loaitb_id in (58) and (thoihan_id=2 or thoihan_id is null)
        and dthu_goi_goc is null;
...
-- kiem tra trung
    select ma_gd, hdtb_id,thuebao_id, ma_tb, tenkieu_ld, 
    thang_bddc, datcoc_csd, goikhoan, phongbh_nhan, loaitb_id,
...
-- Kiem tra dthu goi:
select a.ma_gd, ma_tb, datcoc_csd, dthu_goi, dthu_ps
    from ptm_codinh_202403 a
    where (thuebao_id, ma_tb, kieuld_id) in (select thuebao_id, ma_tb, kieuld_id from ptm_codinh_202403 group by thuebao_id, ma_tb, kieuld_id having count(*)>1) 
    order by thuebao_id, ngay_bbbg;
...
select  chuquan_id, tenkieu_ld, manv_ptm, dich_vu, thuebao_id, ma_gd, ma_tb, loaitb_id, doituong_id, ten_tb, diachi_ld, ma_td, ngaycn_bbbg, ngay_bbbg, 
            thoihan_id,ma_tiepthi, muccuoc_tb, sl_mailing, goi_id,
            (select ten_goi from css_hcm.goi_dadv where goi_id=a.goi_id) ten_goiddv,   
        ...
        from ptm_codinh_202403 a
        where chuquan_id in (1,9,264) 
            and not exists(select 1 from ttkd_bsc.dm_loaihinh_hsqd where loaitru_tinhluong=1 and loaitb_id=a.loaitb_id)
            and (thoihan_id=2 or thoihan_id is null)
            and dthu_goi is null ; 
                 

                    
select  chuquan_id, tenkieu_ld, dich_vu, ma_tb, loaitb_id, doituong_id, ngaycn_bbbg, ngay_bbbg, muccuoc_tb, goi_id, 
            datcoc_csd, sothang_dc, tien_td, muccuoc_tb, tien_dvgt, tien_tbi,
            dthu_goi_goc, dthu_goi, dthu_ps, lydo_khongtinh_luong, ten_tb
        from ptm_codinh_202403 a
        where chuquan_id in (1,9,264) and loaitb_id not in (61,210) and (thoihan_id=2 or thoihan_id is null)
                    and dthu_goi is null ; 
...
-- Tai lap:
update ptm_codinh_202403 a
    set ngay_td = (select ngay_td from tinhcuoc.v_dbtb@dataguard where ky_cuoc=20240201 and ngay_td is not null and thuebao_id=a.thuebao_id)
    where loaihd_id=7 and kieuld_id in (96,13089) and ngay_td is null;
commit;


update ptm_codinh_202403 a set songay_tamngung = '' ;
update ptm_codinh_202403 a
    set songay_tamngung = trunc(ngay_kh)-trunc(ngay_td),
          duoctinh_ptm = (case when trunc(ngay_kh)-trunc(ngay_td) >=35 then 1 else 0 end)     
    where loaihd_id=7 and kieuld_id in (96,13089) and ngay_td is not null;
commit;

                                     
drop table tailap_202403;
create table tailap_202403 as 
    select a.* from ptm_codinh_202403 a
        where a.kieuld_id in (96,13089) and duoctinh_ptm=1
            and hdtb_id=(select max(hdtb_id) from ptm_codinh_202403 
                                     where kieuld_id in (96,13089) and thuebao_id=a.thuebao_id);
                                         
                                            
create index tailap_202403_ttid on tailap_202403 (thanhtoan_id);
create index tailap_202403_tbid on tailap_202403 (thuebao_id);
create index tailap_202403_hdtbid on tailap_202403 (hdtb_id); 


/* 
-- Tao CCQ
create table ttkd_bct.ccq_202403_goc as 
    select * from ttkd_bct.ptm_codinh_202403
    where loaihd_id=2;  -- ccq  


create index ccq_202403_goc_ttid on ccq_202403_goc (thanhtoan_id);
create index ccq_202403_goc_tbid on ccq_202403_goc (thuebao_id);
create index ccq_202403_goc_hdtbid on ccq_202403_goc (hdtb_id); 
