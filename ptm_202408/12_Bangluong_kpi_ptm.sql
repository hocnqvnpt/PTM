/*
create table bangluong_kpi_202408_l6 as select * from bangluong_kpi_202408;  


ktra sau khi add du lieu thang do Xuan Vinh cung cap
select * from ttkd_bsc.dinhmuc_dthu_ptm where  thang=202408 ;
update ttkd_bsc.dinhmuc_dthu_ptm set  thang=202408 where thang is null;
update ttkd_bsc.dinhmuc_dthu_ptm set dt_giao_bsc='' where thang=202408 and dt_giao_bsc=0;
*/
create table ttkd_bsc.bangluong_kpi_202408_dot2 as select * from ttkd_bsc.bangluong_kpi_202408;  
create table ttkd_bsc.bangluong_kpi_dot2_20240820 as select * from ttkd_bsc.bangluong_kpi where thang = 202408;  


select distinct a.*, b.*, c.ten_vtcv 
		from ttkd_bsc.blkpi_danhmuc_kpi a, ttkd_bsc.blkpi_danhmuc_kpi_vtcv b, ttkd_bsc.nhanvien c
        where a.ma_kpi=b.ma_kpi and b.ma_vtcv=c.ma_vtcv and c.thang= 202408
                    and a.thang = b.thang and a.thang = c.thang
                    and a.ma_kpi='HCM_DT_PTMOI_021' ;
    
                    

drop table ttkd_bsc.bangluong_kpi_202408_ptm purge
	;
		create table ttkd_bsc.bangluong_kpi_202408_ptm as 
			 ...
			 from ttkd_bsc.bangluong_kpi_202408
			;


--		alter table ttkd_bsc.bangluong_kpi_202408_ptm 
...
--      select * from ttkd_bsc.temp_trasau_canhan     
		...
		from ttkd_bsc.temp_trasau_canhan a
							join ttkd_bsc.nhanvien nv on nv.thang = 202408 and a.ma_nv = nv.ma_nv
...
--		insert into ttkd_bsc.temp_trasau_canhan --(MANV_PTM, DTHU_KPI)
				...
				from (
						...
						where thang_tlkpi_dnhm = 202408 and (loaitb_id<>21 or ma_kh='GTGT rieng')
					...
					---DNHM cho cot NVHOTRO kenh ngoai
						...
						from ttkd_bsc.ct_bsc_ptm
						where thang_tlkpi_hotro = 202408 
					...
					---DNHM cho cot DAI
						...
						from ttkd_bsc.ct_bsc_ptm
						where thang_tldg_dt_dai = 202408 and (loaitb_id<>21 or ma_kh='GTGT rieng')
									and doanhthu_kpi_dnhm >0
					union all
...
--									, dthu_goi * heso_dichvu * heso_tratruoc dthu_430
						from ttkd_bsc.ct_bsc_ptm 
						where thang_tlkpi = 202408 and (loaitb_id<>21 or ma_kh='GTGT rieng')
					...
					---Dthu nvhotro PGP
						...
						where thang_tlkpi_hotro=202408 and (loaitb_id<>21 or ma_kh='GTGT rieng')
									and doanhthu_kpi_nvhotro >0
					union all
					---Dthu nv DAI
					...
					----GHTT toBHOL BHKV (Nhu Y)
						...
						where a.thang = 202408 and loai_tinh = 'DOANHTHU'
	...
	create table ttkd_bsc.temp_totruong as
			...
			from(
					...
					select ma_pb, ma_to, ma_tb, loaitb_id, dichvuvt_id, doanhthu_kpi_to * heso_hotro_nvptm as doanhthu_kpi_to
					  ...
					  where thang_tlkpi_to = 202408 and (loaitb_id<>21 or loaitb_id is null) 
				...
				union all
					...
					select b.ma_pb, b.ma_to, ma_tb, loaitb_id, dichvuvt_id, doanhthu_kpi_nvhotro
					 ...
					 where thang_tlkpi_hotro = 202408 and (loaitb_id<>21 or loaitb_id is null)
...
--				 ---MANV_DAI
					union all
						...
						where thang_tlkpi_to = 202408 and (loaitb_id<>21 or ma_kh='GTGT rieng')
									and doanhthu_kpi_to >0
				...
				union all
					...
					select ma_pb, ma_to, ma_tb, loaitb_id, dichvuvt_id, doanhthu_kpi_dnhm_phong * heso_hotro_nvptm as doanhthu_kpi_dnhm
					  from ttkd_bsc.ct_bsc_ptm a
					 where thang_tlkpi_dnhm_to = 202408 and (loaitb_id<>21 or loaitb_id is null)
				...
				union all
					...
					from ttkd_bsc.ghtt_vnpts		----a Nguyen quan ly
					  where thang=202408 and thang_giao is not null and ma_to is not null 
		...
		create table ttkd_bsc.x_temp_phong as
			...
			from(
					...
					select ma_pb, ma_to, ma_tb, loaitb_id, dichvuvt_id, doanhthu_kpi_phong * heso_hotro_nvptm as doanhthu_kpi_phong
					  from ttkd_bsc.ct_bsc_ptm a
					  where thang_tlkpi_phong = 202408 and (loaitb_id<>21 or loaitb_id is null) 
...
--				 ---MANV_DAI
					union all
						...
						where thang_tlkpi_phong = 202408 and (loaitb_id<>21 or ma_kh='GTGT rieng')
									and doanhthu_kpi_phong >0
									and nvl(vanban_id, 0) != 764  ---only T7, 8,9, thang sau xoa
				union all
					...
					select ma_pb, ma_to, ma_tb, loaitb_id, dichvuvt_id, doanhthu_kpi_dnhm_phong * heso_hotro_nvptm as doanhthu_kpi_dnhm_phong
					  from ttkd_bsc.ct_bsc_ptm a
					 where thang_tlkpi_dnhm_phong = 202408 and (loaitb_id<>21 or loaitb_id is null)
									and doanhthu_kpi_dnhm_phong >0
				union all
					---To  Nvien Cot MANV_HOTRO cho dthu DNHM tren DIGISHOP
...
--						NHOM_DICHVU  in ('Dichvu') and
						...
						and ma_to  in (select ma_to from ttkd_bsc.nhanvien where thang = 202408)
						;
...
--			select * from (
			with pgd as ( select distinct MA_PB, ten_to, MA_TO, MA_NV, THANG, dichvu, nhom_dichvu
									...
									where thang = 202408 and nhom_dichvu is not null
									)

						select a.*, pgd.ma_nv 
						from ttkd_bsc.temp_021_ldp1 a
									  join pgd
										on a.ma_to = pgd.ma_to and a.dichvu = pgd.dichvu and a.nhom_dichvu = pgd.nhom_dichvu  and pgd.thang=202408 
						...
						select a.*, pgd.ma_nv 
						from ttkd_bsc.temp_021_ldp1 a
									  join pgd on a.ma_to = pgd.ma_to and a.dichvu = pgd.dichvu and a.nhom_dichvu = pgd.nhom_dichvu  and pgd.thang=202408 
...
--				where ma_nv in ('VNP017072', 'VNP017853')
			...
			---KIEM tra bang sau khi chay
					select sum(dthu_kpi) from
						...
						with pgd as ( select distinct MA_PB, MA_TO, MA_NV, THANG, dichvu, nhom_dichvu
									...
									from ttkd_bsc.temp_021_ldp1 a
												left join pgd on a.ma_to = pgd.ma_to and a.dichvu = pgd.dichvu and a.nhom_dichvu = pgd.nhom_dichvu and pgd.thang=202408
		...
		select ma_nv, ten_vtcv, hcm_dt_ptmoi_021_vnptt, hcm_dt_ptmoi_021_vnpts, hcm_dt_ptmoi_021_cdbr_mytv, hcm_dt_ptmoi_021_cntt, HCM_DT_PTMOI_021 from ttkd_bsc.bangluong_kpi_202408 a;
		select * from ttkd_bsc.bangluong_kpi a where ma_kpi in ('HCM_DT_PTMOI_021') and thang = 202408;
		select * from ttkdhcm_ktnv.ID430_TTDANGKY_CHOTTHANG where thang = 202408 and manv_hrm = 'VNP016902';
--		update ttkd_bsc.dinhmuc_giao_dthu_ptm a
--					set KHDK = (select TONG_DTGIAO from ttkdhcm_ktnv.ID430_TTDANGKY_CHOTTHANG where thang = 202408 and MANV_HRM = a.ma_nv)
--		where thang = 202408-- and ma_nv = 'CTV021946'
--						and exists (select TONG_DTGIAO from ttkdhcm_ktnv.ID430_TTDANGKY_CHOTTHANG where thang = 202408 and MANV_HRM = a.ma_nv)
				;
--		select * from ttkdhcm_ktnv.ID430_TTDANGKY_CHOTTHANG where thang = 202408 and MANV_HRM not in (select ma_nv from ttkd_bsc.dinhmuc_giao_dthu_ptm where thang = 202408)
		;
...
--				select * from  ttkd_bsc.bangluong_kpi a
				where ma_kpi in ('HCM_DT_PTMOI_021') and a.thang = 202408 
				;
...
--		select * from  ttkd_bsc.bangluong_kpi
		where thang = 202408 and ma_kpi = 'HCM_DT_PTMOI_021' and ma_nv in ('VNP027259', 'VNP017190')
		;
...
--				select GIAO from ttkd_bsc.bangluong_kpi a
				where a.ma_kpi in ('HCM_DT_PTMOI_021') and a.thang = 202408 
			;
		update ttkd_bsc.bangluong_kpi a set TYLE_THUCHIEN = case when GIAO = 0 then null
				...
				where ma_kpi in ('HCM_DT_PTMOI_021') and thang = 202408 
			;
		update ttkd_bsc.bangluong_kpi a set MUCDO_HOANTHANH = case 
			...
			;
		select * from ttkd_bsc.bangluong_kpi where ma_kpi in ('HCM_DT_PTMOI_021') and thang = 202408
...
rollback;
		...
		create table ttkd_bsc.bangluong_kpi_202408_l3 as select * from ttkd_bsc.bangluong_kpi_202408;
		select * from  ttkd_bsc.bangluong_kpi where thang = 202408 and ma_kpi = 'HCM_DT_PTMOI_021' and ma_nv = 'CTV051559';
	
		select sum(HCM_DT_PTMOI_021) from  ttkd_bsc.bangluong_kpi_202408 where HCM_DT_PTMOI_021 is not null and ma_nv = 'CTV028802';
		;
		select a.ma_nv, a.ten_nv, a.ten_vtcv, a.ten_donvi, a.HCM_DT_PTMOI_021 HCM_DT_PTMOI_021_newa, b.HCM_DT_PTMOI_021 old
		from ttkd_bsc.bangluong_kpi_202408 a
				join ttkd_bsc.bangluong_kpi_202408_l3 b on a.ma_nv = b.ma_nv
...
--					select * from ttkd_bsc.dinhmuc_giao_dthu_ptm a
					where a.ma_vtcv in ('VNP-HNHCM_BHKV_27') and a.thang = 202408 
			;
			update ttkd_bsc.dinhmuc_giao_dthu_ptm 
					...
					where ma_vtcv not in ('VNP-HNHCM_BHKV_27') and thang = 202408 
;
...
select *
		...
		-- san luong ph�t trien moi brcd, vnp tra sau
