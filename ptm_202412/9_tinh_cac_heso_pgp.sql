...
rollback;
commit;
select * from ttkd_bsc.ct_bsc_ptm_pgp where trunc(ngay_ins) >= '01/01/2025'; 
select * from ttkd_bsc.ct_bsc_ptm_pgp where thang_tldg_dt_nvhotro = 202408; and heso_daily = 0.05;
select * from ttkd_bsc.ct_bsc_ptm_pgp where thang_ptm = 202412; and hdtb_id in (26277035, 26276940); or ngay_ins >= '01/09/2024'; and heso_daily = 0.05; and ma_tb in ('hcm_hddt_00013102', 'hcm_hddt_00023066', 'hcm_hddt_00007104')
...
select * from ttkd_bsc.nhanvien where thang = 202412 and ma_nv = 'VNP001686';

--delete from ttkd_bsc.ct_bsc_ptm_pgp where trunc(ngay_ins) >= '01/12/2024'; 
--delete from ttkd_bsc.ct_bsc_ptm_pgp where thang_ptm = 202412 and ma_tb = 'hcm_ioff_00000672';; and ngay_ins = '04/11/2024 12:27:39'; 
...
select ma_duan_banhang, ptm_id from ttkd_bsc.ct_bsc_ptm_pgp where thang_ptm = 202412 group by ma_duan_banhang, ptm_id having count(TYLE_HOTRO_NV) =2;
----***END check;
select * from ttkd_bsc.ct_bsc_ptm_pgp where ma_tb = 'hcm_hddt_00023152';
select * from ttkd_bsc.ct_bsc_ptm_pgp where ma_duan_banhang = '367200';
select * from ttkd_bsc.ct_bsc_ptm_pgp where ma_gd = '01072084';
...
--		select * from ttkd_bsc.ct_bsc_ptm_pgp a
			where exists (select * from ttkd_bsc.ct_bsc_ptm where ma_duan_banhang is not null and thang_tldg_dt_nvhotro = 202412 and a.ptm_id = id and luong_dongia_nvhotro <> a.LG_DONGIA_PGP)
...
select id, thang_ptm, nguon, ma_gd, ma_kh, ma_tb, dich_vu, dichvuvt_id, loaitb_id, tenkieu_ld, kieuld_id 
    ...
    where thang_ptm = 202412 and manv_hotro is not null and a.ma_duan_banhang is not null and (loaitb_id is null or loaitb_id<>21)			--thang n
...
-- Cap nhat lai cac hop dong dc tinh bs: chay sau khi chay xet_thangtlkpi
		...
		delete from ttkd_bsc.ct_bsc_ptm_pgp a
--		select * from ttkd_bsc.ct_bsc_ptm_pgp a
			where exists (select * from ttkd_bsc.ct_bsc_ptm where ma_duan_banhang is not null and thang_tldg_dt_nvhotro = 202412 and a.ptm_id = id )
...
--						and ma_duan_banhang = '00882474'
		...
		delete from ttkd_bsc.ct_bsc_ptm_pgp a
--		select * from ttkd_bsc.ct_bsc_ptm_pgp a
			where exists (select * from ttkd_bsc.ct_bsc_ptm where ma_duan_banhang is not null and thang_tlkpi_hotro = 202412 and a.ptm_id = id )
...
--											select * from ta where ma_yeucau in ('282078')
					...
					select --a.loaitb_id
					    ...
					    from ttkd_bsc.ct_bsc_ptm a
									left join tt b on to_number(regexp_replace (a.ma_duan_banhang, '\D', ''))  = b.ma_yeucau and a.loaitb_id = b.loaitb_id_obss
					    where (thang_tldg_dt_nvhotro = 202412 or thang_tlkpi_hotro = 202412)  and manv_hotro is not null and a.ma_duan_banhang is not null and (loaitb_id is null or loaitb_id<>21)			--thang n
										  and exists(select 1 from ttkd_bsc.nhanvien where thang = 202412 and (ma_pb='VNP0702600' or ma_nv = 'VNP017718' ) and ma_nv=a.manv_hotro)
