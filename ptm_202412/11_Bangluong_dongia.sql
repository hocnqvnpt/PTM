alter table ttkd_bsc.bangluong_dongia_202412 read only;
alter table ttkd_bsc.bangluong_dongia_202412 read write;
create table ttkd_bsc.bangluong_dongia_202412_l1 as select * from ttkd_bsc.bangluong_dongia_202412;

select sum(TONG_THULAO_THUCCHI)
from (
select * from ttkd_bsc.bangluong_dongia_202412
minus
select * from ttkd_bsc.bangluong_dongia_202412_l1
);

select sum(LUONG_DONGIA_DNHM_VNPTT), sum(LUONG_DONGIA_GOI_VNPTT) from ttkd_bsc.bangluong_dongia_202412_dot2; 352473000	445870660

---lan dau tien chay doan Frame nay, lan sau khong chay doan FRAME nay
------BEGIN dot 1---khong chay cho dot 2
drop table ttkd_bsc.bangluong_dongia_202412;
				create table ttkd_bsc.bangluong_dongia_202412 (
				donvi 		varchar(20),
				ma_nv               varchar(10),
				ten_nv               varchar(50),
				ma_vtcv            varchar(20),
	...
	;
			...
			insert into ttkd_bsc.bangluong_dongia_202412
			...
			SELECT donvi, ma_nv, ten_nv, ma_vtcv, ten_vtcv, ma_pb, ten_pb, ma_to, ten_to, loai_ld, nhomld_id
			  ...
			  where thang = 202412 and donvi = 'TTKD' and  ma_to = 'VNP0700820' and ma_vtcv is null
						and not exists (select 1 from ttkd_bsc.bangluong_dongia_202412 x where a.ma_nv = x.ma_nv)
			  ; 
		commit;

-- Doanh thu dinh muc - 268 /TTr- NS 05/09/2022 
	---Thay the Doanh thu dinh muc theo vb 243 TTr NSU DH KHKT 20241212 eO
				---vb moi 323 TTr NSU DH KHKT 20241212 eO thay the b 243 
			...
			insert into ttkd_bsc.bldg_danhmuc_vtcv_p1 (MA_VTCV, TEN_VTCV, THANG_BD, DINHMUC_1, DINHMUC_2, VANBAN, GHICHU)
				select distinct ma_vtcv, ten_vtcv, thang thang_bd
							...
							, case ma_vtcv 	when 'VNP-HNHCM_BHKV_6' then 10000000
									...
									end dinhmuc_2, '243 TTr NSU DH KHKT 20241212 eO'VANBAN, 'doanh thu dinh muc chuan: vnd' GHICHU
				from ttkd_bsc.nhanvien
				where donvi = 'TTKD' and thang = 202412 and ma_vtcv in ('VNP-HNHCM_BHKV_6', 'VNP-HNHCM_BHKV_15', 'VNP-HNHCM_BHKV_22', 'VNP-HNHCM_BHKV_27', 'VNP-HNHCM_KDOL_4')
		;
		
		select * from ttkd_bsc.bldg_danhmuc_vtcv_p1 where thang = 202412; and DINHMUC_1 is not null
		;
		select * from ttkd_bsc.dinhmuc_giao_dthu_ptm where thang = 202412
		;
		 select * from ttkd_bsc.bangluong_dongia_202412 where ten_nv like '%Nguyện';
...
-- he so quy doi cua tong dthu:     
			...
			select sum(heso_dthu) from ttkd_bsc.bang_heso_dthu where thang = 202412; 726.4 632.25
			
			delete from ttkd_bsc.bang_heso_dthu where thang = 202412 and donvi = 'TTKD';
			insert into ttkd_bsc.bang_heso_dthu  
						select thang, DONVI, MA_NV, TEN_NV, MA_VTCV, TEN_VTCV, MA_PB, TEN_PB, MA_TO, TEN_TO, LOAI_LD, nhomld_id, null heso_dthu
							from ttkd_bsc.nhanvien where thang = 202412 and donvi = 'TTKD'
						;
			...
			delete from ttkd_bsc.bang_heso_dthu where thang = 202412 and donvi = 'VTTP';
			insert into ttkd_bsc.bang_heso_dthu  
						select thang, DONVI, MA_NV, TEN_NV, MA_VTCV, TEN_VTCV, MA_PB, TEN_PB, MA_TO, TEN_TO, LOAI_LD, nhomld_id, 0.8 heso_dthu
							from ttkd_bsc.nhanvien where thang = 202412 and donvi = 'VTTP'
						;
...
--				select * from ttkd_bsc.bang_heso_dthu x
			WHERE x.thang = 202412 and x.donvi = 'VTTP'
			;
...
--			select heso_dthu from ttkd_bsc.bang_heso_dthu a 
			...
			----END
			select * from ttkd_bsc.bang_heso_dthu where thang = 202412;
			commit;
...
--			select * from ttkd_bsc.bang_heso_dthu a
			where exists (select * from ttkd_bsc.nhanvien where thang = a.thang and tinh_bsc = 0 and ma_nv = a.ma_nv)
						...
						and thang = 202412 and donvi = 'TTKD'
			;
			commit;
			rollback;
		---import VNPtt -- bang GOM 
		select count(*) from ttkd_bsc.ct_bsc_ptm where thang_ptm = 202412 and loaitb_id = 21; --83323
		select count(*) from ttkd_bsc.ct_bsc_ptm where thang_ptm = 202410 and THANG_TLDG_DT_NVHOTRO = 202412 and loaitb_id = 21;  -- 83316
		delete from ttkd_bsc.ct_bsc_ptm where thang_ptm = 202412 and loaitb_id = 21;
		...
		insert into ttkd_bsc.ct_bsc_ptm(DICH_VU, DICHVUVT_ID, LOAITB_ID, TENKIEU_LD, KIEULD_ID, LOAIHD_ID
															...
															, MA_TB,GOI_CUOC, SOTHANG_DC, MANV_PTM,TENNV_PTM, MA_VTCV, TEN_VTCV, MA_TO,TEN_TO, MA_PB, TEN_PB, TIEN_DNHM, DOANHTHU_DONGIA_DNHM, LUONG_DONGIA_DNHM_NVPTM, MANV_HOTRO
		...
		select 'VNPTT', 2, 21, 'Hòa mạng mới di động' tenkieu_ld, 2 kieuld_id, 1 loaihd_id,THANG_PTM,THANG_PTM,THANG_PTM,THANG_PTM,THANG_PTM,THANG_PTM,THANG_PTM
					,NGUON,PHAN_LOAI_KENH || '; ' || GHI_CHU,MA_TB,TEN_GOI,CK_GOI_TLDG,MANV_PTM,TENNV_PTM, nv.MA_VTCV, nv.TEN_VTCV, MATO_PTM,TENTO_PTM,MAPB_PTM,TENPB_PTM,TIEN_DNHM,DTHU_DONGIA_DNHM
		...
		set MANV_TT_DAI = 'CTV051560', MA_TO_DAI = 'VNP0701630', MA_VTCV_DAI = 'VNP-HNHCM_BHKV_15'
				, DOANHTHU_KPI_NVDAI = nvl(DOANHTHU_KPI_DNHM, 0) + nvl(DOANHTHU_KPI_NVHOTRO, 0) , THANG_TLDG_DT_DAI = 202412
				, DOANHTHU_KPI_DNHM = 0, DOANHTHU_KPI_NVHOTRO = 0
		--select * from ttkd_bsc.ct_bsc_ptm 
		where thang_ptm = 202412 and loaitb_id = 21
					and ghi_chu like '%VB1036,eO-1036142, pbhtb, chuyển doanh thu các STB này qua CTV051560'			
				
			;		
		  commit;
		create table  ttkd_bsc.tonghop_ct_dongia_ptm_202412_lan1 as select * from ttkd_bsc.tonghop_ct_dongia_ptm where thang = 202412;
		select * from ttkd_bsc.tonghop_ct_dongia_ptm_202412_lan1;
		select * from ttkd_bsc.tonghop_ct_dongia_ptm where thang = 202412 and donvi = 'VTTP';; and ptm_id in (select id from ttkd_bsc.ct_bsc_ptm where vanban_id = 937981);
--		delete from ttkd_bsc.tonghop_ct_dongia_ptm where thang = 202412 and donvi = 'VTTP';
		
		*********luu ý kiem tra vanban_id in (937981) and thang_tldg_dt = 202412 --> tính heso_dongia = 1 trong thang 10
...
--		delete from ttkd_bsc.tonghop_ct_dongia_ptm where thang = 202412 ;

select sum(TIEN_THULAO) from ttkd_bsc.tonghop_ct_dongia_ptm where thang = 202412 and donvi = 'VTTP'; and (thang_ptm <= 202411 or dichvuvt_id =4); and thang_ptm = 202412; ma_nv = 'VNP019958'; thang 12_ 1453835500 --  -- 7074472083 604956859
...
insert into ttkd_bsc.tonghop_ct_dongia_ptm
			with nv as (select thang, donvi, ma_nv, ten_nv, ten_to, ten_pb, ma_to, ma_pb, ma_vtcv, nhomld_id
									from ttkd_bsc.nhanvien where thang = 202412 and donvi in ('TTKD', 'VTTP')
--									union
--									select thang, 'POT_VTTP' donvi, ma_nv, ten_nv, ten_to, ten_pb, ma_to, ma_pb, ma_vtcv, nhomld_id
--									from ttkd_bsc.nhanvien_vttp_potmasco where thang = 202412
									union
									select 202412 thang, 'POT' donvi, MA_NV, TEN_NV, TEN_dv, null, 'POT', null, null, 5 nhomld_id 
			...
			select 
					...
					(
					    ...
					    where manv_ptm is not null and thang_tldg_dt = 202412
		...
		union all     
					    ...
					    from ttkd_bsc.ct_bsc_ptm
					    where manv_ptm is not null and thang_tldg_dnhm = 202412 
		...
		union all			    
					    ...
					    select  id, ma_gd, thuebao_id, ma_tb, loaitb_id, dichvuvt_id, manv_hotro
									 ...
									 , 'dnhm' nguon, thang_tldg_dnhm, thang_ptm
					    from ttkd_bsc.ct_bsc_ptm
					    where manv_hotro is not null and thang_tldg_dnhm = 202412 
		...
		union all     
					    ...
					    from ttkd_bsc.ct_bsc_ptm
					    where manv_tt_dai is not null and thang_tldg_dt_dai = 202412
		...
		union all     
					    ...
					    from ttkd_bsc.ct_bsc_ptm a
					    where manv_hotro is not null and thang_tldg_dt_nvhotro = 202412 
									and not exists (select 1 from ttkd_bsc.nhanvien where thang = 202412 and ma_pb = 'VNP0702600' and ma_nv = a.manv_hotro)
		union all     
					    -- nv ho tro cua PGP
			...
			left join (select donvi, thang, ma_nv, ma_vtcv, ten_vtcv
							...
							else null end heso_dthu from ttkd_bsc.bang_heso_dthu) hs on hs.thang = a.thang_ptm and a.manv_ptm = hs.ma_nv
...
--			insert into ttkd_bsc.tonghop_ct_dongia_ptm
--					select 202412 THANG, DONVI, MA_NV, TEN_NV, TEN_TO, TEN_PB, MA_TO, MA_PB, MA_VTCV, NHOMLD_ID
...
select thang, ma_nv, ma_vtcv
	...
	----all dvu ngoai tru VNPtt
			select nguon, sum(LUONG_DONGIA) from ttkd_bsc.tonghop_ct_dongia_ptm where thang = 202412 and loaitb_id = 21 and donvi = 'TTKD' group by nguon;  
		
			---374000000  ---418643750
			----378480000		----457964475
			;
			update ttkd_bsc.bangluong_dongia_202412
...
--								 select * 
								 from ttkd_bsc.tonghop_ct_dongia_ptm
								 where thang = 202412 
												--and (thang_ptm <= 202411 or dichvuvt_id = 4)		---tra tam ung, không chê lại, giam tien, chi thang cu + voi BRCD + VNP thang moi
												
								 group by ma_nv, donvi
			;
			update ttkd_bsc.bangluong_dongia_202412 a
...
-- Luong don gia ghtt + Dong luc ghtt : Nhu Y
 create table ttkd_bsc.bangluong_dongia_202412_l1 as select * from  ttkd_bsc.bangluong_dongia_202412;
		...
		---KIEMTRA
				...
				from ttkd_bsc.bangluong_dongia_202412 a 
				union all
				select 'ghtt_cong', sum(tien) luong_dongia_ghtt, 0,0, 0, 0, 0
					from ttkd_bsc.tl_giahan_tratruoc 
                        where thang = 202412 and  ma_kpi = 'DONGIA' and loai_tinh in ('DONGIATRA_OB' ,'DONGIA_TS_TP_TT')
					and  tien <> 0
				union all
				select 'ghtt_cong_bs', sum(tien) luong_dongia_ghtt, 0,0, 0, 0, 0
					from ttkd_bsc.tl_giahan_tratruoc 
                        where thang = 202412 and  ma_kpi = 'DONGIA' and loai_tinh in ( 'DONGIATRA_OB_BS')
					and  tien <> 0
				union all
				select 'ghtt_tru_cntt', 0, 0, sum(tien) giamtru_ghtt_cntt, 0, 0, 0
						from ttkd_bsc.tl_giahan_tratruoc 
                        where thang = 202412 and  ma_kpi = 'DONGIA' and loai_tinh = 'DONGIATRU_CA'
						and  tien <> 0
				union all
				select 'thuhoi_ghtt_tru_ghtt', 0,0, 0, 0, sum(tien) luong_dongia_ghtt, 0
					from ttkd_bsc.tl_giahan_tratruoc 
                        where thang = 202412 and  ma_kpi = 'DONGIA' and loai_tinh in ('THUHOI_DONGIA_GHTT')
					and  tien <> 0
				;	
			

    UPDATE ttkd_bsc.bangluong_dongia_202412 SET thuhoi_dongia_ghtt = NULL, luong_dongia_ghtt = NULL, giamtru_ghtt_cntt = NULL
		; 
--		ROLLBACK;		
		   ;
update ttkd_bsc.bangluong_dongia_202412 a 
    set 
		luong_dongia_ghtt = (select round(sum(tien), 0) 
                        from ttkd_bsc.tl_giahan_tratruoc 
                        where thang = 202412 and ma_kpi = 'DONGIA' and loai_tinh in ('DONGIATRA_OB', 'DONGIA_TS_TP_TT', 'DONGIATRA_OB_BS')
                                                     and ma_nv = a.ma_nv
                          group by ma_nv having  sum(tien) <> 0) 
		
--		, giamtru_ghtt_cntt = (select -sum(tien) from ttkd_bsc.tl_giahan_tratruoc 
--												where thang = 202412 and ma_kpi = 'DONGIA' and loai_tinh = 'DONGIATRU_CA' and ma_nv = a.ma_nv
--                                                            group by ma_nv 
--                                                            having  sum(tien)  <> 0) 
...
;
SELECT distinct ten_vtcv, ma_vtcv FROM ttkd_bsc.bangluong_dongia_202412 a where donvi = 'TTKD' and LUONG_DONGIA_GHTT >0 ;
commit;


-- Luong khac: 
			update ttkd_bsc.bangluong_dongia_202412 set luong_khac='', ghichu=' ' 
			    where luong_khac is not null;

 
-- Don gia gia han VNPTS: Chi Nguyen
update ttkd_bsc.bangluong_dongia_202412 a set ghtt_vnpts=''
;
		update ttkd_bsc.bangluong_dongia_202412 a
		    set ghtt_vnpts=(select round(sum(luong_dongia * nvl(hs.heso_dthu, 1)), 0)  
									 ...
									 where x.thang = 202412 and thang_giao is not null and x.ma_nv is not null
										    and x.ma_nv = a.ma_nv
										    )
--		     select ma_nv, ten_vtcv, ten_pb, ghtt_vnpts, ghichu from ttkd_bsc.bangluong_dongia_202412 a
		    ;

---Don gia cho NV thuc hien Nghiep vu lap ho so, giai quyet khieu nai: a Khanh, chi Hoi
			-----BEGIN Bsung tam 1 thang 5
--		select distinct ma_vtcv from khanhtdt_ttkd.TONGHOP_BSC_KPI_2024 b where b.thang = 202412 and TONGTIEN > 0 ;
		select sum(luong_dongia_nghiepvu) from ttkd_bsc.bangluong_dongia_202412 b;

		update ttkd_bsc.bangluong_dongia_202412 Set luong_dongia_nghiepvu = null
		;
			update ttkd_bsc.bangluong_dongia_202412 a
...
---Don gia cho NV thuc hien xu ly noi dung chung tu: c Nhu, Kim Tien, ap dung ca To Thu ngan hang - NVC
		...
		update ttkd_bsc.bangluong_dongia_202412 Set luong_dongia_chungtu = null;
--		select ma_nv, ten_nv, sum(luong_dongia_chungtu) from ttkd_bsc.bangluong_dongia_202412 where luong_dongia_chungtu > 0 group by ma_nv, ten_nv
		;
			update ttkd_bsc.bangluong_dongia_202412 a 
						set
								luong_dongia_chungtu =  (select sum(tien) from ttkd_bsc.tl_giahan_tratruoc
																				  where thang = 202412 and ma_kpi = 'DONGIA' and loai_tinh = 'DONGIA_CHUNG_TU' and ma_nv = a.MA_NV
																				  having  sum(tien)  <> 0
																		)
...
--		select sum(TIEN) from ttkd_bsc.tl_giahan_tratruoc where loai_tinh = 'DONGIA_THUCUOC' and thang = 202412;
		update ttkd_bsc.bangluong_dongia_202412 set luong_dongia_thucuoc = null;
--		select sum(luong_dongia_thucuoc) from ttkd_bsc.bangluong_dongia_202412;
		...
		where thang_thu=202412 and loaids_id=2
		;
			...
			update ttkd_bsc.bangluong_dongia_202412 a 
						set
								luong_dongia_thucuoc =  (select sum(tien) from ttkd_bsc.tl_giahan_tratruoc
																				  where thang = 202412 and ma_kpi = 'DONGIA' and loai_tinh = 'DONGIA_THUCUOC' and ma_nv = a.MA_NV
																				  having  sum(tien)  <> 0)
			;		
 ...
 commit; 
		...
		update ttkd_bsc.bangluong_dongia_202412 a 
						set luong_khac = 26214300
--		select ten_nv, luong_khac from ttkd_bsc.bangluong_dongia_202412 a 
...
/*		select nv.donvi, a.ma_nv, a.ten_nv, ten_vtcv, a.ten_pb, 'VNP' ||loaihinh_tb, sum(TIEN_THULAO) 
				...
				where a.thang = 202412 group by nv.donvi, a.ma_nv, a.ten_nv, ten_vtcv, a.ten_pb, loaihinh_tb
		;
		update ttkd_bsc.bangluong_dongia_202412 a
...
*/		update ttkd_bsc.bangluong_dongia_202412 a
					set luong_dongia_vnphh = ( select sum(TIEN_THULAO)
																...
																where thang = 202412 and ma_nv = a.ma_nv) 
		;
		-----END 384
		
				---VB435---****LUONG DON GIA VB 435 VNP NANG GOI, NANG CHUKY, MUA GOI cho 42 CTV PTTT
/*		select nv.donvi, a.ma_nv, a.ten_nv, ten_vtcv, a.ten_pb, 'VNP' ||loaihinh_tb, sum(TIEN_THULAO) 
				from vietanhvh.dongia_pttt_202412 a 
							join ttkd_bsc.nhanvien nv on a.ma_nv = nv.ma_nv and a.thang = nv.thang
				where a.thang = 202412 group by nv.donvi, a.ma_nv, a.ten_nv, ten_vtcv, a.ten_pb, loaihinh_tb
		;
		select sum(luong_dongia_vnphh_pttt) from ttkd_bsc.bangluong_dongia_202412 a; 1 870 445 050 -- 1 889 797 797
*/		update ttkd_bsc.bangluong_dongia_202412 a
					set luong_dongia_vnphh_pttt = ( select sum(tien_thulao) tien_luong
																from vietanhvh.dongia_pttt_202412
																where thang = 202412 and ma_nv = a.ma_nv
																)
		...
		update ttkd_bsc.bangluong_dongia_202412 a
					set luong_dongia_nghiepvu_vnp = ( select TIEN_THULAO from ttkd_bsc.man_tonghop_KPI 
																				where thang = 202412 and ma_kpi = 'HCM_DT_PTMOI_061'
																								and ma_nv = a.ma_nv
																				)
		;													
		-----END 525
...
--		update ttkd_bsc.bangluong_dongia_202412 set tong_luong_dongia='' where tong_luong_dongia is not null
		;
		...
		update ttkd_bsc.bangluong_dongia_202412 a
		    ...
		    ;
     --    select * from ttkd_bsc.bangluong_dongia_202412 where  ma_nv in ('VNP016902',
'VNP017072',
'VNP017853',
'VNP017014',
'VNP016898',
...
/* -- Kiem tra tong luong don gia: 
			...
			select sum(nvl(luong_dongia_cdbr,0)+nvl(luong_dongia_vnpts,0)+nvl(luong_dongia_cntt,0)
			    ...
			    from  ttkd_bsc.bangluong_dongia_202412
			union all
			select sum(tong_luong_dongia) from ttkd_bsc.bangluong_dongia_202412;

*/

select sum(tong_thulao_thucchi) from ttkd_bsc.bangluong_dongia_202412;--1218135446  --
...
-- THUHOI: 
			...
			select *  from ttkd_bsc.bangluong_dongia_202412
			;
--			select loaitb_id, sum(tien_thuhoi) from ttkd_bsc.ct_thuhoi where thang=202412 group by loaitb_id;
			;
--- thu hoi PTM		
				update ttkd_bsc.bangluong_dongia_202412 a  
				   set luong_dongia_ptm_thuhoi = (select sum(tien_thuhoi) from ttkd_bsc.ct_thuhoi where thang=202412 and ma_nv=a.ma_nv)
--				   where exists(select 1 from ttkd_bsc.ct_thuhoi where thang=202412 and ma_nv=a.ma_nv)
				   ;
-- giam tru do thu ho so tai nha: 		
			update ttkd_bsc.bangluong_dongia_202412 a
				   set giamtru_hosotainha =
					   (select round(sum(tien_giam) ,0) from ttkd_bsc.ct_giamtru where thang=202412 and ma_nv=a.ma_nv)
			    --  select giamtru_hosotainha from ttkd_bsc.bangluong_dongia_202412  a
--			    where exists (select 1 from ttkd_bsc.ct_giamtru where thang=202412 and ma_nv=a.ma_nv)
			    ;
...
-- tong_luong_thuhoi:
--		update ttkd_bsc.bangluong_dongia_202412 a set tong_luong_thuhoi='' where tong_luong_thuhoi is not null
--;
			update ttkd_bsc.bangluong_dongia_202412 a
			...
			from ttkd_bsc.bangluong_dongia_202412
			union all
			select 'tien_luong_thuhoi', sum(tong_luong_thuhoi) from ttkd_bsc.bangluong_dongia_202412;
			*/

delete from ttkd_bsc.bangluong_dongia_202412  where ghichu = 'khongtontai';
...
--	         7,366,025,142	 331,572,581	   7,034,452,561
	...
	---    1,365,197,611	  10,485,000	   1,354,712,611	10g30 4/12/2024
	 ...
	 --      1,226,164,502	 457,043,238	     769,121,264	14g55 04/01/2025
	 --	      1,226,559,105	 481,320,341	     745,238,764
	 
	 --               11,019,914,503	 456,707,068	  10,563,207,435 15g05 16/01/2025
	 --	              11,022,371,328	 480,984,171	  10,541,387,157
	
	   
----Backup bang---
create table ttkd_bsc.bangluong_dongia_202412_dot5 as select * from ttkd_bsc.bangluong_dongia_202412; 
select * from ttkd_bsc.bangluong_dongia_202412_dot2;
	
	
--		select * from ttkd_bsc.ct_bsc_ptm where thang_tldg_dt=202412 and manv_ptm='VNP031192'; and loaitb_id = 20;
	...
	---gui chi tiet NSU
		...
		--		 , dtptm_dongia_cdbr,	dtptm_dongia_vnpts,	dtptm_dongia_cntt, tong_dtptm
				  , luong_dongia_cdbr, luong_dongia_cntt, luong_dongia_vnpts, 
				  luong_dongia_dnhm_vnptt, luong_dongia_goi_vnptt, luong_dongia_vnphh, luong_dongia_vnphh_pttt, luong_dongia_nghiepvu_vnp, ghtt_vnpts,	luong_khac,
...
select * from ttkd_bsc.bangluong_dongia_202412;
select * from ttkd_bsc.bangluong_kpi_202412 ;
...
from ttkd_bsc.bangluong_kpi where thang = 202412 and ma_kpi = 'HCM_DT_PTMOI_021' ;
select * from ttkd_bsc.tl_giahan_tratruoc where thang = 202412 and loai_tinh = 'DONGIATRA_OB';
...
--			, HCM_DT_PTMOI_056
--			, HCM_SL_COMBO_006
from ttkd_bsc.bangluong_dongia_202412 where ghichu = 'khongtontai';--ma_nv = 'VNP016902';

SELECT ma_nv, ten_nv, ma_vtcv, ten_vtcv, ma_pb,ma_to, ten_to,ma_kpi, ten_kpi, to_char(giatri) giatri
  FROM ttkd_bsc.bangluong_kpi_202412_tracuu where ma_nv = 'VNP020231';
  ;
delete from ttkd_bsc.bangluong_dongia_202412  where ghichu = 'khongtontai';
		...
		insert into ttkd_bsc.bangluong_dongia_202412 
			...
			from (--x_dongia_temp
					select ma_nv, luong_dongia_cdbr, luong_dongia_vnpts, luong_dongia_dnhm_vnptt, luong_dongia_goi_vnptt, luong_dongia_cntt, tien_thulao, 0 ghtt_vnpts, 0 tien_thuhoi
					  ...
					  where not exists(select 1 from ttkd_bsc.nhanvien where thang = 202412 and ma_nv = a.ma_nv)			--thang n
								 and exists (select 1 from ttkd_bsc.nhanvien where thang < 202412 and donvi = 'TTKD' and ma_nv = a.ma_nv)	--thang n-1
								 and tien_thulao > 0 
							
					union all
							select ma_nv, 0, 0, 0, 0, 0, 0, round(sum(luong_dongia),0), 0
							from ttkd_bsc.ghtt_vnpts a
							where a.thang=202412 and thang_giao is not null and ma_nv is not null
										    and not exists (select 1 from ttkd_bsc.nhanvien where thang = a.thang and ma_nv = a.ma_nv)		
											and exists (select 1 from ttkd_bsc.nhanvien where thang < 202412 and donvi = 'TTKD'  and ma_nv = a.ma_nv)
							group by ma_nv
					union all
					...
					left join (select donvi, ma_nv, ten_nv, ten_to, ten_pb, row_number() over (partition by ma_nv order by thang desc) rnk
										from ttkd_bsc.nhanvien
										where thang <= 202412) nv on a.ma_nv = nv.ma_nv and rnk = 1
...
----NSu tren 100tr gui NSU
		select ma_nv, ten_nv, ten_vtcv, ten_pb, tong_luong_dongia from ttkd_bsc.bangluong_dongia_202412 where tong_luong_dongia>100000000
		;
		select MA_NV, TEN_NV, TEN_TO, TEN_PB, sum(LUONG_DONGIA) LUONG_DONGIA 
		from ttkd_bsc.tonghop_ct_dongia_ptm 
		where thang = 202412 group by MA_NV, TEN_NV, TEN_TO, TEN_PB having sum(LUONG_DONGIA) > 100000000
		;
		
---Anh Nghia tren 100tr/thang
select manv_ptm, ''''||manv_ptm||''',' mnv_nv, 'AM' vitri, sum(dg_goi) tong_dongia
from (
		select manv_ptm, luong_dongia_nvptm dg_goi from ttkd_bsc.ct_bsc_ptm where loaitb_id <> 21 and ((thang_ptm = 202412 and  manv_ptm is not null) or thang_tldg_dt=202412)
		union all
		select manv_hotro, luong_dongia_nvhotro dg_goi from ttkd_bsc.ct_bsc_ptm where loaitb_id = 21 and thang_tldg_dt_nvhotro =202412
		union all
		select manv_ptm, luong_dongia_dnhm_nvptm dg_dnhm from ttkd_bsc.ct_bsc_ptm where (thang_ptm = 202412 and manv_ptm is not null) or thang_tldg_dnhm=202412
)
group by manv_ptm
having sum(dg_goi) > 100000000
union all
		select manv_hotro, ''''||manv_hotro||''',' mnv_nv, 'PS' vitri, sum(luong_dongia_nvhotro) tong_dongia
		from ttkd_bsc.ct_bsc_ptm_pgp where (thang_ptm = 202412 and MANV_HOTRO is not null) or thang_tldg_dt_nvhotro = 202412
		group by manv_hotro
...
-- chi tiet gui anh Nghia:
 ...
 from ttkd_bsc.ct_bsc_ptm 
where( (thang_ptm >= 202409 and (thang_tldg_dt=202412 or thang_tldg_dt is null)) or (thang_ptm < 202409 and thang_tldg_dt=202412)
			    )
						and manv_ptm in ('VNP017782',
'CTV082572',
'VNP017517',
'VNP017942',
'VNP017784',
'VNP038835')
			...
			where (thang_ptm = 202412 or thang_tldg_dt_nvhotro = 202412)
...
'VNP017373')
			...
			select * from ttkd_bsc.ct_bsc_ptm_pgp 
			    where (thang_ptm>=202402 and (thang_tldg_dt_nvhotro=202412 or thang_tldg_dt_nvhotro is null)) 
					  or (thang_ptm<202402 and (thang_tlkpi_hotro=202412 or thang_tldg_dt_nvhotro=202412)  )
					  ; 
...
from ttkd_bsc.ct_bsc_ptm a
 where thang_ptm=202412 and dichvuvt_id not in (2,13,14,15,16) and tien_dnhm>0 and ngay_tt is not null and thang_tldg_dnhm is null;
...
from ttkd_bsc.bangluong_dongia_202412 a, ttkd_bsc.bangluong_dongia_202412_l3 b 
...
---Chi tiêt thue bao PTM dot tam ung gửi NSU
	select *
	from ttkd_bsc.bangluong_dongia_202412; where 'khongtontai' = ghichu
	;
	---VNPts ghtt
...
--					sum(TIEN_THUHOI) --24277103
	from (
							...
							from ttkd_bsc.ghtt_vnpts a
							where a.thang=202412 and thang_giao is not null and ma_nv is not null and LUONG_DONGIA > 0 
										    and not exists (select 1 from ttkd_bsc.nhanvien where thang = a.thang and ma_nv = a.ma_nv)		
											and exists (select 1 from ttkd_bsc.nhanvien where thang < 202412 and donvi = 'TTKD'  and ma_nv = a.ma_nv)
						) a
					left join (select donvi, ma_nv, ten_nv, ten_to, ten_pb, row_number() over (partition by ma_nv order by thang desc) rnk
