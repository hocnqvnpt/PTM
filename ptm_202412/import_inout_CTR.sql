...
--												doituong_kh = 'KHDN'
--								HESO_HOSO = 0.8
--								,  ghichu = ghichu || '; da thoai hop dong'
...
--							and manv_hotro = 'VNP017373; D?ch v? ch?a c� tr�n oneBSS'--is not null
				...
				select * from ttkd_bsc.ct_bsc_ptm where thang_ptm >= 202407 and  ma_tb = 'hcm_vbn_00001077';
				
				select dthu_goi, luong_dongia_nvptm, thang_tldg_dt, ma_tb, thuebao_id from ttkd_bsc.ct_bsc_ptm where thang_ptm >= 202407 and replace(ma_gd, ' ', '') = '01007346';'00928721'; 
...
--											left join css_hcm.phanloai_kh plk on db.plkh_id = plk.PHANLOAIKH_ID
								...
								where a.thang = 202412 and a.dichvuvt_id in (14, 15, 16)-- not in ('VNP tra sau', 'Internet truc tiep')
												and command = 'insert' 
							...
							select 
								...
								from ttkd_bsc.ct_ptm_ngoaictr_imp a
											...
											left join (select x.thuebao_id, plkh_id, sum(dthu) dthu_ps
																...
																group by x.thuebao_id, plkh_id) db on a.thuebao_id = db.thuebao_id
											left join css_hcm.phanloai_kh plk on db.plkh_id = plk.PHANLOAIKH_ID
								where a.thang = 202412 and a.dichvuvt_id in (13, 14, 15, 16, 7)
												and command = 'update'
								) t
...
--												, a.tyle_hotro = nvl(t.heso_hotro_nvhotro, a.heso_hotro_nvhotro)
						...
						WHERE a.thuebao_id in (select thuebao_id from ttkd_bsc.ct_ptm_ngoaictr_imp where thang = 202412 and command = 'update')
...
--						select * from ttkd_bsc.ct_bsc_ptm a
						where nguon like 'ct_ptm_ngoaictr_imp%' and thang_luong in (86,87, 70)
									and exists (select * from ttkd_bsc.ct_ptm_ngoaictr_imp where thang = 202412 and GHINHAN_BSC = 0 and ma_gd = a.ma_gd)
						;
						commit;
						rollback;
