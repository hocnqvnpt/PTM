-- nv ptm: xong
select * from ttkd_bsc.v_ct_bsc_ptm_202412;
select * from ttkd_bsc.v_bangluong_dongia_ptm_202412;
drop view ttkd_bsc.v_ct_bsc_ptm_202412;
create OR REPLACE VIEW ttkd_bsc.v_ct_bsc_ptm_202412 as
 select thang_ptm,ma_gd, ma_gd_gt, tenkieu_ld,ma_tb,dich_vu,ten_tb,diachi_ld
				,sothang_dc
				  , to_char(ngay_bbbg,'dd/mm/yyyy') ngay_bbbg, to_char(ngay_luuhs_ttkd,'dd/mm/yyyy') ngay_luuhs_ttkd, to_char(ngay_luuhs_ttvt,'dd/mm/yyyy') ngay_luuhs_ttvt,
				   ...
				   manv_ptm ma_nv,ten_pb,ma_to,ten_to,ma_pb, ghi_chu
...
from ttkd_bsc.ct_bsc_ptm a
 where (loaitb_id not in (20,21) or (loaitb_id=20 and goi_luongtinh is null) or loaitb_id is null)
              and (thang_ptm between 202409 and 202412 or thang_tldg_dt = 202412)
		    and (thang_tldg_dt = 0 or nvl(thang_tldg_dt, 999999) >= 202412 or thang_tlkpi >= 202412 or thang_tlkpi_dnhm = 202412 
                              )
  ;      
  -- Ho tro cua Phong Giai phap:  xong
 drop view ttkd_bsc.v_ct_bsc_ptm_phongGP_202412;
 create OR REPLACE VIEW ttkd_bsc.v_ct_bsc_ptm_phongGP_202412 as
 select 
			   ...
			   (select ma_to from ttkd_bsc.nhanvien where thang = 202412 and manv_hrm=a.manv_hotro) ma_to,
			   (select ten_to from ttkd_bsc.nhanvien where thang = 202412 and  manv_hrm=a.manv_hotro) ten_to,
			   (select ma_pb from ttkd_bsc.nhanvien where thang = 202412 and  manv_hrm=a.manv_hotro) ma_pb,
			   (select ma_pb from ttkd_bsc.nhanvien where thang = 202412 and  manv_hrm=a.manv_hotro) ten_pb,
 ...
 where thang_ptm between 202409 and 202412
			and (nvl(thang_tldg_dt_nvhotro, 999999) >= 202412
					    or nvl(thang_tlkpi_hotro, 999999) >= 202412) 
              and exists (select 1 from ttkd_bsc.nhanvien where thang = 202412 and  ma_pb='VNP0702600' and ma_nv = a.manv_hotro)
		    ;
select * from ttkd_bsc.v_ct_bsc_ptm_phongGP_202412;
              

 -- View Tong hop bang luong don gia ptm:
drop view ttkd_bsc.v_bangluong_dongia_ptm_202412;
create OR REPLACE VIEW ttkd_bsc.v_bangluong_dongia_ptm_202412 as
