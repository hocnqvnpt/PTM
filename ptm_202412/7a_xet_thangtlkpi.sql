...
--select thang_luong, thang_ptm, ma_gd, ma_tb, nguon, tenkieu_ld, thang_tldg_dnhm, thang_tlkpi_dnhm, thang_tlkpi_dnhm_to, thang_tlkpi_dnhm_phong, doanhthu_kpi_dnhm, lydo_khongtinh_dongia, nop_du, mien_hsgoc, manv_ptm, manv_hotro from ttkd_bsc.ct_bsc_ptm  a   
		   where (thang_ptm = 202412 )--or nvl(THANG_TLKPI, 999999) >= 202412)
...
-- Ky (n-1)  
		update ttkd_bsc.ct_bsc_ptm a 
			   set thang_tlkpi_dnhm = 202412, thang_tlkpi_dnhm_phong = 202412
						, thang_tlkpi_dnhm_to = case when MA_NGUOIGT  in (select MA_DAILY 
																						from ttkd_bsc.dm_daily_khdn 
																						where ma_pb = 'VNP0702300' and ma_daily not in ('dl_cnt', 'DL_CNT','GTGT00001') and thang = a.thang_ptm)
																					then 0
...
--		     select doanhthu_kpi_dnhm, doanhthu_kpi_dnhm_phong, ma_tb, manv_ptm, ten_pb, dich_vu, heso_daily, thang_tlkpi_dnhm, thang_tlkpi_dnhm_to, thang_tldg_dnhm, tien_dnhm, thang_ptm, trangthai_tt_id, lydo_khongtinh_dongia from ttkd_bsc.ct_bsc_ptm a
		where thang_ptm = 202411 ---thang n-1
						and nvl(thang_tlkpi_dnhm, 999999) >= 202412		 ---thang n
...
--						and ma_tb = 'fvn_lamb808'
		...
		---khong tinh dai ly ca nhan dongia (aNguyen dang tinh), nguoc lai tinh KPI cho dai ly ca nhan
		update ttkd_bsc.ct_bsc_ptm a 
		   set thang_tlkpi = 202412, thang_tlkpi_to = 202412, thang_tlkpi_phong = 202412
				, thang_tlkpi_hotro = 202412
...
--		     select doanhthu_kpi_dnhm, doanhthu_kpi_dnhm_phong, ma_tb, manv_ptm, ten_pb, dich_vu, heso_daily, thang_tlkpi_dnhm, thang_tlkpi_dnhm_to, thang_tldg_dnhm, tien_dnhm, thang_ptm, trangthai_tt_id, lydo_khongtinh_dongia from ttkd_bsc.ct_bsc_ptm a
			where thang_ptm = 202410 	---thang n-2
						and nvl(thang_tlkpi_dnhm, 999999) >= 202412
...
--		     select thang_ptm, doanhthu_kpi_nvptm, doanhthu_kpi_to, doanhthu_kpi_phong, ma_tb, ten_pb, ma_nguoigt, loai_ld , ma_to, heso_daily, thang_tldg_dt, thang_tlkpi, thang_tlkpi_to, thang_tlkpi_phong, lydo_khongtinh_luong, lydo_khongtinh_dongia from ttkd_bsc.ct_bsc_ptm a
			where thang_ptm = 202410 	---thang n-2
							and nvl(thang_tlkpi, 999999) >= 202412  ---thang n
...
select * from ttkd_bsc.ct_bsc_ptm 
    where thang_ptm>=202402 and (loaitb_id<>21 or ma_kh='GTGT rieng')
        and thang_tlkpi_to is null and thang_tlkpi=202410;
...
select * from ttkd_bsc.ct_bsc_ptm 
    where thang_ptm>=202402 
        ...
        and thang_tlkpi_to=202410 and thang_tldg_dt is null
...
select thang_ptm, ten_pb, ten_to,dich_vu, ma_tb, trangthaitb_id_n3, nocuoc_n3, ngay_luuhs_ttkd,nop_du, sothang_dc
    ...
    where thang_ptm>=202402
        ...
        and ((thang_tldg_dt is null and thang_tlkpi=202410)
                or (thang_tldg_dt =202410 and thang_tlkpi is null)) ;   

             
select * from ttkd_bsc.ct_bsc_ptm
where chuquan_id in (145,266)
    and ( thang_tldg_dt=202410 or thang_tlkpi=202410 or thang_tlkpi_to=202410 or thang_tlkpi_phong=202410)
...
where (thang_tlkpi_hotro=202410 and thang_tldg_dt_nvhotro is null)
        or (thang_tldg_dt_nvhotro=202410 and thang_tlkpi_hotro is null );
...
select * from ttkd_bsc.ct_bsc_ptm a
    where chuquan_id in (145,266)
        and ( thang_tldg_dt=202410 or thang_tlkpi=202410 or thang_tlkpi_to=202410 or thang_tlkpi_phong=202410 or thang_tldg_dnhm=202410 or thang_tldg_dt_nvhotro = 202410)
        and loaitb_id not in (20,21,149) and loaihd_id=1
        and (trangthai_tt_id is null or trangthai_tt_id=0);
	   
select * from ttkd_bsc.ct_bsc_ptm a
    where chuquan_id in (145,266)
        and  ((thang_tldg_dt is null and thang_tldg_dt_nvhotro = 202410)
					--or (thang_tldg_dt = 202410 and thang_tldg_dt_nvhotro is null)
					or (thang_tldg_dt is null and THANG_TLKPI = 202410)
				)
	   ;
