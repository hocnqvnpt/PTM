create table ptm_codinh_202412 as
select * from ptm_codinh_202412_cntt	--48s
union all
select * from ptm_codinh_202412_tsl	--39s
union all
select * from ptm_codinh_202412_cd --93s đk dichvt_id tren schema tinhcuoc
union all
select * from ptm_codinh_202412_br	---329s
;
drop table x_v_db purge
;

select * from ptm_codinh_202412;
create table ptm_codinh_202412_cd as
        ...
        , v_db as (select  b.khachhang_id, b.thanhtoan_id, a.thuebao_id, b.ngay_td, b.ngay_cat, b.doituong_id, b.trangthaitb_id
                             ...
                             select  b.khachhang_id, b.thanhtoan_id, a.thuebao_id, b.ngay_td, b.ngay_cat, b.doituong_id, b.trangthaitb_id
                                        , f.mst, e.mst mst_tt
								, b.mucuoctb_id, a.cuoc_tk, a.cuoc_tc, a.cuoc_tbi, a.cuoc_ht, a.cuoc_ip, a.cuoc_nix, a.cuoc_isp
								, a.chuquan_id
								, null cuoc_dt, a.toanha_id, d.ma_duan
                             ...
                             select  b.khachhang_id, b.thanhtoan_id, a.thuebao_id, b.ngay_td, b.ngay_cat, b.doituong_id, b.trangthaitb_id
                                        , f.mst, e.mst mst_tt
                                       , b.mucuoctb_id, a.cuoc_tk, a.cuoc_tc, a.cuoc_tbi, a.cuoc_ht, null cuoc_ip, null cuoc_nix, null cuoc_isp
							    , a.chuquan_id
							    , null cuoc_dt, a.toanha_id, d.ma_duan
...
--                , round(datcoc.datcoc_csd/1.1,0) tien_td
   ...
   from  css.v_hd_khachhang a, css.v_hd_thuebao b, v_db, v_hdtb, tinhcuoc.v_dbtb db_old
         ...
         where a.hdkh_id = b.hdkh_id and b.thuebao_id = v_db.thuebao_id --and v_db.chuquan_id in (145, 264, 266) 
                and db_old.ky_cuoc (+) = 20241101 	---thang n-1
				and db_old.dichvuvt_id in (1, 10,11) and b.dichvuvt_id in (1, 10,11)		---CD, GP
--				and db_old.dichvuvt_id in (4) and b.dichvuvt_id in (4)		---BR
--				and db_old.dichvuvt_id in (7,8,9) and b.dichvuvt_id in (7,8,9)	--TSL
--				and db_old.dichvuvt_id in (12, 13, 14, 15, 16, 26) and b.dichvuvt_id in (12, 13, 14, 15, 16, 26)	--CNTT
...
;
             ...
             select * from ptm_codinh_202412
                where (ma_tb, ma_gd, hdtb_id) in 
                                                ...
                                                from ptm_codinh_202412 a 
                                                group by ma_tb, ma_gd, hdtb_id having count(*)>1 )
            ;
--Them cot------ tien_dnhm, tien_camket, tien_sodep:
                    ALTER TABLE ptm_codinh_202412 ADD (tien_dnhm NUMBER, tien_sodep NUMBER, tien_camket NUMBER)
				;
                    UPDATE ptm_codinh_202412 a 
                ...
                commit;
                    ...
                    MERGE INTO ptm_codinh_202412 a
                    ...
                    MERGE INTO ptm_codinh_202412 a
                                ...
                                UPDATE ptm_codinh_202412 a
                                    ...
                                    -- SELECT ma_tb, tien_sodep FROM ttkd_bct.ptm_codinh_202412 a
...
-- Mucuoc_tb:
	   ...
	   ;
        create table temp_muccuoctb_thuebao as
                with km_dbtb as (select thuebao_id, tyle_tb, cuoc_tb, tien_tb , ngay_sd, tyle_sd
												from css.v_khuyenmai_dbtb
                                                          where phanvung_id=28 and hieuluc=1 and ttdc_id = 0 and (tyle_sd > 0 or tyle_tb>0 or cuoc_tb>0 or tien_tb>0) 
					...
					, hdtb as (select hdtb.thuebao_id, row_number() over (partition by hdtb.thuebao_id order by hdtb_id desc) rnk
									...
									where hdkh.loaihd_id in (8, 1,6,31,41,49) and hdtb.dichvuvt_id in (4, 13,14,15,16) and hdtb.tthd_id = 6
													 and hdtb.loaitb_id in (58, 39) --chi ap dung Key thuebao_id doi voi loaitb_id = 39
                ...
                select db.thuebao_id, db.ma_tb, db.dichvuvt_id, db.loaitb_id, km_dbtb.tyle_tb, km_dbtb.cuoc_tb, km_dbtb.tien_tb, dbc.sl_mailing
							, (case when db.loaitb_id in (58, 9,12,42,43,44,45,46,50,110,124,150,159,183)
										    then 
											   case 
														when db.loaitb_id = 58 and km_dbtb.tyle_sd > 0 then round(mc.cuoc_tg * (100-nvl(km_dbtb.tyle_sd,0))/100, 0)  -- giam ty le sd, ap dung Fiber
														when km_dbtb.tyle_tb>0 then round(mc.cuoc_tg * (100-nvl(km_dbtb.tyle_tb,0))/100, 0)  -- giam ty le, chua nhan so luong
            ...
            where dbc.phanvung_id=28 and mc.phanvung_id=28 
						...
						and db.loaitb_id in (39) --chi ap dung Key thuebao_id doi voi loaitb_id = 39
						 and hdtb.rnk = 1
                ;
...
--                    update ptm_codinh_202412 set tien_dvgt='';
                    ...
                    update ptm_codinh_202412 a 
                        ...
                        -- select tien_dvgt from ptm_codinh_202412 a
...
-- tien_tbi: 
--                    UPDATE ptm_codinh_202412 SET tien_tbi=''
				;
                    MERGE INTO ptm_codinh_202412 a
                    USING (SELECT cttbi.hdtb_id, tbi_lhtb.loaitb_id, SUM(cttbi.soluong*cttbi.tien) tien_tbi
                                    ...
                                    WHERE cttbi.loaitbi_id=tbi.loaitbi_id AND tbi.loaitbi_id=tbi_lhtb.loaitbi_id AND lhtb.loaitb_id=tbi_lhtb.loaitb_id
                                                        ...
                                                        AND EXISTS(SELECT 1 FROM ptm_codinh_202412 
...
--                                , round(datcoc.tien_td/1.1,0) tien_td
		...
		;
            alter table ptm_codinh_202412 add hdtb_id_datcoc number
		  ;
            ...
            update ptm_codinh_202412 a 
                ...
                -- select thuebao_id, ma_tb, sothang_dc, datcoc_csd, tien_td from ptm_codinh_202412 a
                where datcoc_csd is null 
                    and exists(select 1 from temp_datcoc b
                                        where datcoc_csd>0 and thuebao_id=a.thuebao_id and ma_nv=a.ma_tiepthi)
                    and ngay_bbbg=(select max(ngay_bbbg) from ptm_codinh_202412 where thuebao_id=a.thuebao_id )
                    ;
                    
            ---recover datcoc_csd updated
--                update ptm_codinh_202412 set datcoc_csd = null 
----				select * from ttkd_bct.ptm_codinh_202412
--				where hdtb_id_datcoc is not null;
            commit;
            
--            select a.*, b.ma_tiepthi , b.DATCOC_CSD, b.SOTHANG_DC, b.hdtb_id_datcoc
--            from temp_datcoc a, ptm_codinh_202412 b
--            where a.thuebao_id=b.thuebao_id
            ;
            
------ thoihan_id
            update ptm_codinh_202412 set thoihan_id=1 
	...
	-- Cuoc phat sinh:    ---chuyen len dataguard
				...
				select distinct a.* from bcss.v_ct_no a
				    where phanvung_id = 28 and ky_cuoc = 20241201 		--thang n
					   and khoanmuctt_id not in (441,520,521,527,3126,3127,3421,3953) 
					   and exists(select 1 from ptm_codinh_202412 where thuebao_id=a.thuebao_id)
					   ;
				create index tmp_ct_no_tbid on tmp_ct_no (thuebao_id);
				
				alter table ptm_codinh_202412 add dthu_ps number(12)
--				update ptm_codinh_202412 a set dthu_ps=''
				;
				update ptm_codinh_202412 a 
		...
		commit;
				...
				---select dthu_ps from ptm_codinh_202412 a
-- Dthu goi:
				alter table ptm_codinh_202412 
				    add (dthu_goi_goc number(12), dthu_goi number(12), dthu_goi_ngoaimang number(12))
				;
				update ptm_codinh_202412 bc set dthu_goi_goc='', dthu_goi='', dthu_goi_ngoaimang=''
				
				;
				--  select * from ttkd_bct.ptm_codinh_202412;
				
				update ptm_codinh_202412 bc set dthu_goi_goc=
				    (case
...
--												(select soluong_ip 
--													from css.tocdo_adsl
--														where soluong_ip>0 and (sl_ip_mp is null or sl_ip_mp<soluong_ip) and tocdo_id=bc.tocdo_id
--												)>=8   
												soluong_ip >= 8 and (sl_ip_mp is null or sl_ip_mp < soluong_ip)
												  then 500000 + ((select cuoc_tg 
																						from css.v_muccuoc_tb mc 
																						where mc.mucuoctb_id = bc.mucuoctb_id) *  (100 - nvl(bc.tyle_sd, 0))/100
																			)
									when 
--												(select soluong_ip 
--														from css.tocdo_adsl
--															where soluong_ip=1 and sl_ip_mp is null and tocdo_id=bc.tocdo_id)=1
												soluong_ip = 1 and sl_ip_mp is null
										  then 200000 + ((select cuoc_tg 
																						from css.v_muccuoc_tb mc 
																						where mc.mucuoctb_id = bc.mucuoctb_id) *  (100 - nvl(bc.tyle_sd, 0))/100
																			)
								    else ((select cuoc_tg 
						...
						when bc.loaitb_id in (61,171) then
							 case 
								-- MyTV tham gia goi tich hop CCBS (có VNP)
								when bc.goi_dadv_id in (select goi_id 
																				from bcss.v_banggia_goi b 
																				where b.goi_id = bc.goi_dadv_id and NGAY_HIEU_LUC is null
																						 
																		) 
									   then (select MYTV + VTVCAB + SPOTV as dthu_goi_dadv 
															from bcss.v_banggia_goi b 
															where NGAY_HIEU_LUC is null and b.goi_id=bc.goi_dadv_id																		
													)
								-- MyTV tham gia goi tich hop ONEBSS (khong VNP)
								when bc.goi_dadv_id in (select goi_id 
																				from css.v_goi_dadv_lhtb b 
						...
						when bc.dichvuvt_id in (1,10,11) then 
							 case 
								   ...
								   -- codinh, codinh tinh khac, bfone, thue bao SIP (VFONE)...
								   when bc.loaitb_id in (109, 36,32, 34, 1, 63) then 
										  case
												...
												--ca nhan con lai
												when bc.doituong_id in (1,2,10,12,13,25,40,387) then 20000
												--doanh nghiep con lai
												when bc.doituong_id not in (1,2,10,12,13,25,40,387) then 119000
										  end
							 end  -- dichvuvt_id in (1,10,11)                    
						end)
				where loaitb_id not in (131,358)
				and hdtb_id in ( select distinct loaitb_id from ttkd_bct.ptm_codinh_202412 a
				WHERE dichvuvt_id IN (13,14,15,16)
								and a.tien_tbi > 0)
			;
			commit;
select * from ptm_codinh_202412 where tyle_sd >= 50;
...
--															where tien_camket > 250000 then 0
															else greatest(250000 - dthu_goi_goc, 0) end, 'dthu goi=250000-dthu goi cua hs lap moi (lap moi, chuyen doi cung thang)' 
                                            from ptm_codinh_202412 b
										  where loaihd_id=1 and thuebao_id=a.thuebao_id                 
											  and exists (select 1 from ptm_codinh_202412 where thuebao_id=b.thuebao_id and loaihd_id=6)
											  )
		    where loaitb_id=77 and loaihd_id=6
					  and exists (select 1 from ptm_codinh_202412 where thuebao_id=a.thuebao_id and loaihd_id<>6)
								
				;
			
			update ptm_codinh_202412 set dthu_goi=dthu_goi_goc
			...
			update ptm_codinh_202412 a 
					    set dthu_goi_ngoaimang =
											 (select sum(nogoc) from bcss.v_tonghop
												where ky_cuoc = 20241201 --thang n
...
--			    select * from ptm_codinh_202412 a
 ...
 ---move ve TTKDDB
		  create table ttkd_bct.ptm_codinh_202412 as select * from ttkd_hcm.ptm_codinh_202412@dataguard;
		  SELECT * FROM ttkd_bct.ptm_codinh_202412;

--- Rycle Bin DATAGUARD
		drop table ptm_codinh_202412_bs purge;
		drop table ptm_codinh_202412_cd purge;
		drop table ptm_codinh_202412_tsl purge;
		drop table ptm_codinh_202412_cntt purge;
		drop table ptm_codinh_202412_br purge;
		  
		  ----Update ma_nguoigt la daily
		  update ttkd_bct.ptm_codinh_202412 a
						set NHANVIENGT_ID= a.ctv_id
								, ma_nguoigt = upper(a.ma_tiepthi)
								, NGUOI_GT =  a.ten_tiepthi
								, NHOM_GT = a.donvi_tt
--					select * from ttkd_bct.ptm_codinh_202412 a
		  ...
		  create index ttkd_bct.idx_ptm_hdtbid on ttkd_bct.ptm_codinh_202412 (hdtb_id);
		  -- lydo_khongtinh_luong: Update ky luong T3/24
						alter table ttkd_bct.ptm_codinh_202412 add lydo_khongtinh_luong varchar2(200)
						;
						update ttkd_bct.ptm_codinh_202412 a
									set lydo_khongtinh_luong = (
														   with x as (
																			  ...
																			  from ttkd_bct.ptm_codinh_202412 b
																			)                                        
                    ...
                    -- nv ptm:   
                    alter table ttkd_bct.ptm_codinh_202412
				...
				insert into ttkd_bsc.ptm_xuly_50_BHOL 
						 select 202412 thang, hdtb_id, thuebao_id, loaitb_id, ma_tiepthi, pbh_nhan_id
						 ...
						 from ttkd_bct.ptm_codinh_202412 a
...
--				     select ma_tiepthi, manv_ptm from ttkd_bct.ptm_codinh_202412 a
				    where loaitb_id = 288 and ma_tiepthi is null 
					   and pbh_nhan_id=2941 and nhanvien_id=1077 and datcoc_csd>0
					   ; 
...
-- Tao ds dai ly thang va ins dai ly moi:
			--****-- Dai ly hien huu:
--			delete from dm_daily_khdn where thang='202412' ;
			select * from  ttkd_bsc.dm_daily_khdn where thang = 202412;
			...
			insert into ttkd_bsc.dm_daily_khdn --(ma_daily,ten_daily,manv_qldaily,ma_pb,thang,thang_kyhd,ma_to)
			    ...
			    select 202412, ma_daily, ten_daily, manv_qldaily, ma_to, ma_pb, ma_vtcv, ten_vtcv, thang_kyhd, ghichu
			  ...
			  --****-- Dai ly moi:
					insert into ttkd_bsc.dm_daily_khdn (ma_daily,ten_daily,ma_pb, ma_to, manv_qldaily, thang, thang_kyhd, loai_hopdong )
					    select distinct ma_tiepthi ma_daily, (select upper(ten_nv) from admin_hcm.nhanvien_onebss where ma_nv = a.ma_tiepthi) ten_daily,
								 ma_pb, ma_to, ma_nv, 202412, 202412, 'DAI LY MOI' loai_hopdong
					    from 
						   (
...
--					     select MA_GD, pbh_nhan_id, pbh_nhan_goc_id, donvi_tt_id, ma_pb, ten_pb from ttkd_bct.ptm_codinh_202412 a
...
--					     select ma_tiepthi, ma_gd, ma_nguoigt, (select ma_dv from admin_Hcm.donvi where donvi_id = a.pbh_nhan_id)ma_dv, ma_pb, ten_pb, pbh_nhan_id from ttkd_bct.ptm_codinh_202412 a
...
--				    select ma_gd, (select ten_dv from admin_Hcm.donvi where donvi_id = 11298) ten_dv, pbh_nhan_id, pbh_nhan_goc_id from ttkd_bct.ptm_codinh_202412 a
...
--				      select ma_gd, (select ten_dv from admin_Hcm.donvi where donvi_id = 11298) ten_dv, pbh_nhan_id, pbh_nhan_goc_id, ten_tb, ma_pb, manv_ptm from ttkd_bct.ptm_codinh_202412 a
...
--				     select ma_tiepthi, ma_pb, pbh_nhan_id, donvi_tt_id, donviql_tt_id, (select ma_dv from admin_hcm.donvi where donvi_id = a.donvi_tt_id) ma_pb_new from ttkd_bct.ptm_codinh_202412 a
...
--					      select ma_gd, ma_tiepthi, ma_pb, pbh_nhan_id, donvi_tt_id, donviql_tt_id, (select ma_dv ||ten_dv from admin_Hcm.donvi where donvi_id = a.DONVI_TT_ID) from ttkd_bct.ptm_codinh_202412 a
					...
					update ttkd_bct.ptm_codinh_202412 a
					    set  
							  ma_pb = (select ma_dv from admin_Hcm.donvi where donvi_id = a.pbh_nhan_id)
							  , ten_pb = (select ten_dv from admin_Hcm.donvi where donvi_id = a.pbh_nhan_id)
--					      select ma_gd, ma_tiepthi, manv_ptm, ma_pb, pbh_nhan_id, donvi_tt_id, donviql_tt_id, (select ma_dv ||ten_dv from admin_Hcm.donvi where donvi_id = a.pbh_nhan_id) from ttkd_bct.ptm_codinh_202412 a
					    where ma_pb is null and chuquan_id in (145,266,264)
				...
				update ttkd_bct.ptm_codinh_202412 a set manv_hotro='', tyle_hotro='', tyle_am='' where ma_duan_banhang is not null
				;	

				--select * from ttkdhcm_ktnv.amas_booking_presale where ma_yeucau = 228705;
					MERGE into ttkd_bct.ptm_codinh_202412 a
							using (
												with 
...
--					select manv_hotro, tyle_hotro, tyle_am, ma_duan_banhang, loaitb_id, ma_gd, to_number(regexp_replace (a.ma_duan_banhang, '\D', '')) mada from ttkd_bct.ptm_codinh_202412 a
...
---- kiem tra trung
			    ...
			    thang_bddc, datcoc_csd, phongbh_nhan, loaitb_id,
					  ...
					  from ttkd_bct.ptm_codinh_202412
					  where (ma_gd, thuebao_id) in (select ma_gd, thuebao_id from ttkd_bct.ptm_codinh_202412 group by ma_gd, thuebao_id having count(*)>1) 
...
-- Kiem tra dthu goi:
				select a.ma_gd, ma_tb, datcoc_csd, dthu_goi, dthu_ps, manv_ptm
				    from ttkd_bct.ptm_codinh_202412 a
				    where (ma_gd, thuebao_id, ma_tb, kieuld_id) in (select ma_gd, thuebao_id, ma_tb, kieuld_id from ttkd_bct.ptm_codinh_202412 group by ma_gd, thuebao_id, ma_tb, kieuld_id having count(*)>1) 
				    order by thuebao_id, ngay_bbbg;
				
				select * from ttkd_bct.ptm_codinh_202412 
--				 delete from ttkd_bct.ptm_codinh_202412
				...
				select * from bcss.v_thftth@dataguard a 
				    where phanvung_id=28 and ky_cuoc=20241201
					   and exists(select 1 from ttkd_bct.ptm_codinh_202412 
...
-- Tai lap:
--				update ttkd_bct.ptm_codinh_202412 a
--				    set ngay_td_kytruoc = (select ngay_td from tinhcuoc.v_dbtb@dataguard where ky_cuoc=20241201 and ngay_td is not null and thuebao_id=a.thuebao_id)
--				    where loaihd_id=7 and kieuld_id in (96,13089) and ngay_td_kytruoc is null;
--				commit;
--				
--				
--				update ptm_codinh_202412 a set songay_tamngung = '' 
--				;
--				update ptm_codinh_202412 a
...
--				commit;
				...
				drop table ttkd_bct.tailap_202412 purge
				;
				select * from ttkd_bct.tailap_202412;
				create table ttkd_bct.tailap_202412 as 
				    select a.* from ttkd_bct.ptm_codinh_202412 a
					   where a.kieuld_id in (96,13089) and duoctinh_ptm=1
						  and hdtb_id=(select max(hdtb_id) from ttkd_bct.ptm_codinh_202412 
											  where kieuld_id in (96,13089) and thuebao_id=a.thuebao_id)
								;
												 
												    
--				create index ttkd_bct.tailap_202412_ttid on ttkd_bct.tailap_202412 (thanhtoan_id);
--				create index ttkd_bct.tailap_202412_tbid on ttkd_bct.tailap_202412 (thuebao_id);
--				create index ttkd_bct.tailap_202412_hdtbid on ttkd_bct.tailap_202412 (hdtb_id)
				;
