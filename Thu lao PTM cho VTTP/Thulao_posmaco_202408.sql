...
drop table ttkd_bsc.ct_bsc_ptm_potmasco_202408;
            
		  create table ttkd_bsc.ct_bsc_ptm_potmasco_202408 as
                select * from ttkd_bsc.ct_bsc_ptm
                where 
							(manv_ptm in (select ma_nv from ttkd_bsc.nhanvien_vttp_potmasco where thang = 202408)
								or manv_hotro in (select ma_nv from ttkd_bsc.nhanvien_vttp_potmasco where thang = 202408)
							)
                             
						and ((thang_ptm between 202405 and 202408     ---thang n-3
...
select * from ttkd_bsc.nhanvien_vttp_potmasco where thang = 202408;
select * from  ttkd_bsc.bangluong_dongia_potmasco where thang = 202408;
...
-- tong hop:  
		drop table ttkd_bsc.x_th_tlptm_potmasco purge;
		...
		;
					create table ttkd_bsc.x_th_tlptm_potmasco as
					    ...
					    where thang in (202408) and donvi = 'POT_VTTP'          ---thang n---
...
-- sltb + dthu goi chua duoc tinh
		...
		select * from ttkd_bsc.x_th_tlptm_potmasco_chuachitl;
;
			...
			from
			(select 
							nv.thang, 'POT_VTTP' donvi, manv_ptm ma_nv, nv.ten_nv, nv.ten_to, nv.ten_pb, nv.ma_to, nv.ma_pb, nv.ma_vtcv, nv.nhomld_id
							...
							, luong_dongia
							, case 
										when loaitb_id = 21 then 1				---VTTP VNPtt heso = 1
										when thang_ptm < 202407 then 1 	---fix truoc thang 7 theo vb 353 don gia = 858, from thang 7 theo vb 322 thay doi don gia 0.8 * 800 = 640
										when thang_ptm >= 202407 then 0.8 	---fix truoc thang 7 theo vb 353 don gia = 858, from thang 7 theo vb 322 thay doi don gia 0.8 * 800 = 640
										else null end heso_dthu
							, case 
										when loaitb_id = 21 then luong_dongia
										when thang_ptm < 202407 then luong_dongia 	---fix truoc thang 7 theo vb 353 don gia = 858, from thang 7 theo vb 322 thay doi don gia 0.8 * 800 = 640
										when thang_ptm >= 202407 then luong_dongia * 0.8 	---fix truoc thang 7 theo vb 353 don gia = 858, from thang 7 theo vb 322 thay doi don gia 0.8 * 800 = 640
		...
		--			create table x_tpmp as	
					select  id ptm_id, ma_gd, thuebao_id, ma_tb, loaitb_id, dichvuvt_id, manv_ptm
					    ...
					    where manv_ptm is not null and thang_ptm between 202405 and 202408 and thang_tldg_dt is null
				union all     
					    ...
					    from ttkd_bsc.ct_bsc_ptm
					    where manv_ptm is not null and thang_ptm between 202405 and 202408 and thang_tldg_dnhm is null
				union all			    
					    ...
					    select  id, ma_gd, thuebao_id, ma_tb, loaitb_id, dichvuvt_id, manv_hotro
									 ...
									 , 'dnhm' nguon, thang_tldg_dnhm, thang_ptm
					    from ttkd_bsc.ct_bsc_ptm
					    where manv_hotro is not null and thang_ptm between 202405 and 202408 and thang_tldg_dnhm is null 
				...
				union all     
					    ...
					    from ttkd_bsc.ct_bsc_ptm
					    where manv_tt_dai is not null and thang_ptm between 202405 and 202408 and thang_tldg_dt_dai is null
				union all     
					    ...
					    where manv_hotro is not null and thang_ptm between 202405 and 202408 and thang_tldg_dt_nvhotro is null
									and not exists (select 1 from ttkd_bsc.nhanvien where thang = 202408 and ma_pb = 'VNP0702600' and ma_nv = a.manv_hotro)
				 ) a
			    
			join ttkd_bsc.nhanvien_vttp_potmasco nv on nv.thang = 202408 and nv.ma_nv = a.manv_ptm
			)
			group by ma_nv, ten_pb
		;
...
update ttkd_bsc.bangluong_dongia_potmasco set tong_dt_chitl='', tong_tl='' , tong_dt_chuachitl='',  tong_sl_chitl='', tong_sl_chuachitl='' where thang = 202408
;
			update ttkd_bsc.bangluong_dongia_potmasco a
...
-- File excel chitiet_tbao_thulao_thuhoi_XXyyyy: (bao gom da tinh, chua tinh và thu hoi) 
	select 'PTM_chuachi' nguon, thang_ptm
										...
										, (case when dongia = 800 then luong_dongia_nvptm * 0.8
											...
											as tong_tl_goi
											-- select * 
						from ttkd_bsc.ct_bsc_ptm_potmasco_202408 a
									join ttkd_bsc.nhanvien_vttp_potmasco b on b.thang = 202408 and a.manv_ptm = b.ma_nv     --thang n
				...
				union all
						...
						-- select * 
						from ttkd_bsc.ct_bsc_ptm_potmasco_202408 a
									join ttkd_bsc.nhanvien_vttp_potmasco b on b.thang = 202408 and a.manv_hotro = b.ma_nv     --thang n
		...
		union all
				select nguon, thang_ptm, ma_gd, ma_tb, lh.loaihinh_tb, null, null, null, null
					    ...
					    where a.loaitb_id in (58, 61, 20, 11, 21) and thang in (202408) and donvi = 'POT_VTTP'	
		union all
...
-- CT_THUHOI
		select 'Thuhoi' nguon, cast(thang_ptm as number) thang
		 ...
		 where a.thang = 202408 and a.loaitb_id in (58, 61, 20, 21) 
		--   and exists(select * from ttkd_bsc.bangluong_dongia_202408_vttp where ma_nv=a.ma_nv)
   ;

select * from  ttkd_bsc.ct_thuhoi
;
select ma_nv, count(ma_to) from ttkd_bsc.ct_bsc_ptm_202408_vttp where thang=202408 
group by ma_nv;
