...
drop table ttkd_bsc.ct_bsc_ptm_potmasco_202407;

select distinct dich_vu from ttkd_bsc.ct_bsc_ptm_potmasco_202407;
select distinct TENKIEU_LD from ttkd_bsc.ct_bsc_ptm_potmasco_202407;
            
		  create table ttkd_bsc.ct_bsc_ptm_potmasco_202407 as
                select * from ttkd_bsc.ct_bsc_ptm
                where 
							(manv_ptm in (select ma_nv from ttkd_bsc.nhanvien_vttp_potmasco where thang = 202407)
								or manv_hotro in (select ma_nv from ttkd_bsc.nhanvien_vttp_potmasco where thang = 202407)
							)
                             
						and ((thang_ptm between 202404 and 202407     ---thang n-3
								    ...
								    (thang_tldg_dt = 202407 or thang_tldg_dnhm = 202407))
								    )                       ---thang n
    ;
    

select * from ttkd_bsc.nhanvien_vttp_potmasco where thang = 202407;
select * from  ttkd_bsc.bangluong_dongia_potmasco where thang = 202407;
...
-- tong hop:  
		...
		;
					create table ttkd_bsc.x_th_tlptm_potmasco as
					    ...
					    where thang in (202406, 202407) and donvi = 'POT_VTTP'          ---thang n---
...
;
		...
		--			create table x_tpmp as	
					select  id ptm_id, ma_gd, thuebao_id, ma_tb, loaitb_id, dichvuvt_id, manv_ptm
					    ...
					    where manv_ptm is not null and thang_ptm between 202404 and 202407 and thang_tldg_dt is null
				union all     
					    ...
					    from ttkd_bsc.ct_bsc_ptm
					    where manv_ptm is not null and thang_ptm between 202404 and 202407 and thang_tldg_dnhm is null
				union all			    
					    ...
					    select  id, ma_gd, thuebao_id, ma_tb, loaitb_id, dichvuvt_id, manv_hotro
									 ...
									 , 'dnhm' nguon, thang_tldg_dnhm, thang_ptm
					    from ttkd_bsc.ct_bsc_ptm
					    where manv_hotro is not null and thang_ptm between 202404 and 202407 and thang_tldg_dnhm is null 
				...
				union all     
					    ...
					    from ttkd_bsc.ct_bsc_ptm
					    where manv_tt_dai is not null and thang_ptm between 202404 and 202407 and thang_tldg_dt_dai is null
				union all     
					    ...
					    where manv_hotro is not null and thang_ptm between 202404 and 202407 and thang_tldg_dt_nvhotro is null
									and not exists (select 1 from ttkd_bsc.nhanvien where thang = 202407 and ma_pb = 'VNP0702600' and ma_nv = a.manv_hotro)
				 ) a
			    
			join ttkd_bsc.nhanvien_vttp_potmasco nv on nv.thang = 202407 and nv.ma_nv = a.manv_ptm
			)
			group by ma_nv, ten_pb
		;
...
update ttkd_bsc.bangluong_dongia_potmasco set tong_dt_chitl='', tong_tl='' , tong_dt_chuachitl='',  tong_sl_chitl='', tong_sl_chuachitl='' where thang = 202407
;
			update ttkd_bsc.bangluong_dongia_potmasco a
...
-- File excel chitiet_tbao_thulao_thuhoi_XXyyyy: (bao gom da tinh, chua tinh và thu hoi) 
	select 'PTM_chuachi' nguon, thang_ptm
										...
										, (case when dongia = 800 then luong_dongia_nvptm * 0.8
											...
											as tong_tl_goi
											-- select * 
						from ttkd_bsc.ct_bsc_ptm_potmasco_202407 a
									join ttkd_bsc.nhanvien_vttp_potmasco b on b.thang = 202407 and a.manv_ptm = b.ma_nv     --thang n
				...
				union all
						...
						-- select * 
						from ttkd_bsc.ct_bsc_ptm_potmasco_202407 a
									join ttkd_bsc.nhanvien_vttp_potmasco b on b.thang = 202407 and a.manv_hotro = b.ma_nv     --thang n
		...
		union all
				select nguon, thang_ptm, ma_gd, ma_tb, lh.loaihinh_tb, null, null, null, null
					    ...
					    where a.loaitb_id in (58, 61, 20, 11, 21) and thang in (202406, 202407) and donvi = 'POT_VTTP'	
		union all
...
-- CT_THUHOI
		select 'Thuhoi' nguon, cast(thang_ptm as number) thang
		 ...
		 where a.thang = 202407 and a.loaitb_id in (58, 61, 20, 21) 
		--   and exists(select * from ttkd_bsc.bangluong_dongia_202407_vttp where ma_nv=a.ma_nv)
   ;

select * from  ttkd_bsc.ct_thuhoi
;
select ma_nv, count(ma_to) from ttkd_bsc.ct_bsc_ptm_202407_vttp where thang=202407 
group by ma_nv;
