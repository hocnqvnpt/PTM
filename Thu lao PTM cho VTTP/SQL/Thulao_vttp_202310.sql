-- Chi tiet trong ky:      
drop table ct_bsc_ptm_202310_vttp;
create table ct_bsc_ptm_202310_vttp as
    select * from ct_bsc_ptm
    where nhom_tiepthi=2 
            and ((to_number(thang_ptm)>=202307  
                    and ((loaihd_id=1 and loaitb_id in (11,58,61)) or loaitb_id=20)
                    and (thang_tldg_dt='202310' or thang_tldg_dt is null))
                    or (loaitb_id=21 and thang_ptm='202310'));
    

                    
-- Tao bangluong_dongia_202310_vttp
drop table bangluong_dongia_202310_vttp;
create table bangluong_dongia_202310_vttp (
ma_nv               varchar(10),
ten_nv               varchar(50),
ma_vtcv            varchar(20),
...
insert into bangluong_dongia_202310_vttp
      (ma_nv,ten_nv,ma_vtcv,ten_vtcv,ma_pb,ten_pb,ma_to,ten_to,nv_thang )
SELECT ma_nv,ten_nv,ma_vtcv,ten_vtcv,ma_pb,ten_pb,ma_to,ten_to,202310
  FROM nhanvien_vttp a
  where exists(select 1 from ct_bsc_ptm_202310_vttp where manv_ptm=a.ma_nv)
        and thang=(select max(thang) from nhanvien_vttp where ma_nv=a.ma_nv);

/*
select * from nhanvien_vttp where ma_pb is null;
select * from bangluong_dongia_202310_vttp;

select * from nhanvien_vttp;

update nhanvien_vttp a
    set ma_to=(select distinct ma_to from nhanvien_vttp where thang=202310 and ten_pb=a.ten_pb and ten_to=a.ten_to)
    where thang<202310 ;
  
  select * from nhanvien_vttp where ten_to='??i Vi?n Thông B?n Mi?n ?ông';
  update nhanvien_vttp a
    set ma_to='HCM160500' where  ten_to='??i Vi?n Thông B?n Mi?n ?ông';
  update nhanvien_vttp a
    set ma_to='HCM160500', ten_to='??i Vi?n Thông B?n Mi?n ?ông', ma_pb='HCM160000', ten_pb='Trung Tâm Vi?n Thông Sài Gòn'
    where ma_nv='CTV029042' and thang=202307;
    */
    
    
-- tong hop:  
drop table th_tlptm_202310_vttp;
create table th_tlptm_202310_vttp as
select manv_ptm ma_nv, 
            (select ten_pb from nhanvien_vttp where thang=202310 and ma_nv=a.manv_ptm) ten_donvi,            
...
(
    ...
    from ct_bsc_ptm_202310_vttp
    where thang_tldg_dt='202310' and loaitb_id!=21
    group by manv_ptm 
   ...
   union all 
    ...
    -- select *  
    from ct_bsc_ptm_202310_vttp
    where loaitb_id=21 and thang_ptm=202310 and thang_tldg_dt='202310'
    group by manv_ptm  
...
drop table th_tlptm_202310_vttp_chuachitl;
create table th_tlptm_202310_vttp_chuachitl as
select manv_ptm ma_nv, 
            (select ten_pb from nhanvien_vttp where thang=202310 and ma_nv=a.manv_ptm) ten_donvi,
...
-- ptm cdbr + gtgt +vnpts             
update bangluong_dongia_202310_vttp a
      set 
            sl_brcd_chitl='',
            sl_mytv_chitl='',
            sl_vnpts_chitl='',
...
update bangluong_dongia_202310_vttp a
           ...
           =(select sl_brcd_chitl, sl_mytv_chitl, sl_vnpts_chitl, sl_vnptt_chitl, 
                    dt_brcd_chitl, dt_mytv_chitl, dt_vnpts_chitl, dt_vnptt_chitl,
                    tl_brcd, tl_mytv, tl_vnpts, tl_vnptt
             from th_tlptm_202310_vttp
             where ma_nv=a.ma_nv)
    -- select * from bangluong_dongia_202310_vttp a
    where exists(select 1 from th_tlptm_202310_vttp where ma_nv=a.ma_nv);
...
update bangluong_dongia_202310_vttp a
            ...
            =(select sl_brcd_chuachitl, sl_mytv_chuachitl, sl_vnpts_chuachitl, sl_vnptt_chuachitl,
                    dt_brcd_chuachitl, dt_mytv_chuachitl, dt_vnpts_chuachitl, dt_vnptt_chuachitl
              from th_tlptm_202310_vttp_chuachitl
              where ma_nv=a.ma_nv)
    -- select * from bangluong_dongia_202310_vttp a
    where exists(select 1 from th_tlptm_202310_vttp_chuachitl where ma_nv=a.ma_nv);
    
    
-- Tong:
update bangluong_dongia_202310_vttp set tong_dt_chitl='', tong_tl='' , tong_dt_chuachitl='',  tong_sl_chitl='', tong_sl_chuachitl='';
update bangluong_dongia_202310_vttp a
...
update bangluong_dongia_202310_vttp set dt_brcd_chitl='' where dt_brcd_chitl=0;
update bangluong_dongia_202310_vttp set dt_mytv_chitl='' where dt_mytv_chitl=0;
update bangluong_dongia_202310_vttp set dt_vnpts_chitl='' where dt_vnpts_chitl=0;
update bangluong_dongia_202310_vttp set dt_vnptt_chitl='' where dt_vnptt_chitl=0;
update bangluong_dongia_202310_vttp set tong_dt_chitl='' where tong_dt_chitl=0;

update bangluong_dongia_202310_vttp set tl_brcd='' where tl_brcd=0;
update bangluong_dongia_202310_vttp set tl_mytv='' where tl_mytv=0;
update bangluong_dongia_202310_vttp set tl_vnpts='' where tl_vnpts=0;
update bangluong_dongia_202310_vttp set tl_vnptt='' where tl_vnptt=0;
update bangluong_dongia_202310_vttp set tong_tl='' where tong_tl=0;
            
update bangluong_dongia_202310_vttp set dt_brcd_chuachitl='' where dt_brcd_chuachitl=0;
update bangluong_dongia_202310_vttp set dt_mytv_chuachitl='' where dt_mytv_chuachitl=0;
update bangluong_dongia_202310_vttp set dt_vnpts_chuachitl='' where dt_vnpts_chuachitl=0;
update bangluong_dongia_202310_vttp set dt_vnptt_chuachitl='' where dt_vnptt_chuachitl=0;
update bangluong_dongia_202310_vttp set tong_dt_chuachitl='' where tong_dt_chuachitl=0;
            
update bangluong_dongia_202310_vttp set sl_brcd_chitl='' where sl_brcd_chitl=0;
update bangluong_dongia_202310_vttp set sl_mytv_chitl='' where sl_mytv_chitl=0;
update bangluong_dongia_202310_vttp set sl_vnpts_chitl='' where sl_vnpts_chitl=0;
update bangluong_dongia_202310_vttp set sl_vnptt_chitl='' where sl_vnptt_chitl=0;
update bangluong_dongia_202310_vttp set tong_sl_chitl='' where tong_sl_chitl=0;

update bangluong_dongia_202310_vttp set sl_brcd_chuachitl='' where sl_brcd_chuachitl=0;
update bangluong_dongia_202310_vttp set sl_mytv_chuachitl='' where sl_mytv_chuachitl=0;
update bangluong_dongia_202310_vttp set sl_vnpts_chuachitl='' where sl_vnpts_chuachitl=0;
update bangluong_dongia_202310_vttp set sl_vnptt_chuachitl='' where sl_vnptt_chuachitl=0;
update bangluong_dongia_202310_vttp set tong_sl_chuachitl='' where tong_sl_chuachitl=0;


delete bangluong_dongia_202310_vttp
    -- select * from bangluong_dongia_202310_vttp
    where tong_tl is null and tong_dt_chuachitl is null and tong_sl_chitl is null and tong_sl_chuachitl is null ;
    
commit;
    
---- Kiem tra doanh thu 646987674
select 'bangluong_dongia_202310_vttp' , sum(tl_brcd), sum(tl_mytv) , sum(tl_vnpts), sum(tl_vnptt) from bangluong_dongia_202310_vttp 
union all
...
select 'bangluong_dongia_202310_vttp_chi' , sum(tong_sl_chitl) tong_sl
 from bangluong_dongia_202310_vttp 
union all
select 'bangluong_dongia_202310_vttp_chi',  sum(tong_sl_chuachitl) tong_sl
 from bangluong_dongia_202310_vttp
union all 
select 'bangluong_dongia_202310_vttp_tong' , sum(tong_sl_chitl) + sum(tong_sl_chuachitl) tong_sl
 from bangluong_dongia_202310_vttp 
union all
select 'ct_bsc_ptm_202310_vttp', count(*) tong_sl
 from ct_bsc_ptm_202310_vttp;
...
select thang_ptm, ma_gd, ma_tb, dich_vu, ten_tb, diachi_ld, ngay_bbbg, ngay_luuhs_ttkd, ngay_luuhs_ttvt, nop_du, 
         ...
         (case when thang_tldg_dnhm='202310' then doanhthu_dongia_dnhm else null end) dt_dongia_dnhm, 
         (case when thang_tldg_dnhm='202310' then luong_dongia_dnhm_nvptm else null end) tl_dnhm, thang_tldg_dnhm thang_chi_dnhm,
...
-- select * 
from ct_bsc_ptm_202310_vttp a, (select * from nhanvien_vttp where thang=202310) b
...
update bangluong_dongia_202310_vttp a  
   set (dtptm_dongia_cdbr_thuhoi, dtptm_dongia_vnpts_thuhoi, dtptm_dongia_vnptt_thuhoi, dtptm_dongia_khac_thuhoi)=
        (select dthu_ptm_cdbr, dthu_ptm_vnpts, dthu_ptm_vnptt, dthu_ptm_khac
          ...
          where thang='202310' and loai_tinh='dongia_thuhoi' and ma_nv=a.ma_nv_hrm)
    -- select dtptm_dongia_cdbr_thuhoi, dtptm_dongia_vnpts_thuhoi, dtptm_dongia_vnptt_thuhoi, dtptm_dongia_khac_thuhoi from bangluong_dongia_202310_vttp a
...
-- tong_luong_thuhoi:
-- select sum(tong_luong_thuhoi) from bangluong_dongia_202310;  337071317
update bangluong_dongia_202310_vttp a set tong_luong_thuhoi='' where tong_luong_thuhoi is not null;
update bangluong_dongia_202310_vttp a
...
from bangluong_dongia_202310
union all
select 'tien_luong_thuhoi', sum(tong_luong_thuhoi) from bangluong_dongia_202310
union all
...
select 'ct_thuhoi', round(sum(dthu_thu)*0.858,0) luong_thuhoi
    from ct_thuhoi a
    where thang='202310' and exists (select 1 from bangluong_dongia_202310 where ma_nv=a.ma_nv) 
union all
select 'tl_doanhthu_ptm', round(sum(tong_dthu_ptm)*0.858 ,0)
    from tl_doanhthu_ptm a
    where thang='202310'
                and exists (select 1 from bangluong_dongia_202310 where ma_nv=a.ma_nv);


/*
-- XUAT GUI BANG LUONG DON GIA = DS NHAN VIEN KO CO TRONG BANG LUONG DON GIA:
select sum(tong_luong_dongia) from bangluong_dongia_202310; 
