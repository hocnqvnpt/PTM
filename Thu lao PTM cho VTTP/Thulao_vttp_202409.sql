...
drop table ttkd_bsc.ct_bsc_ptm_202409_vttp;

select * from ttkd_bsc.ct_bsc_ptm_202409_vttp;
            
		  create table ttkd_bsc.ct_bsc_ptm_202409_vttp as
                select * from ttkd_bsc.ct_bsc_ptm
                where 
							(manv_ptm in (select ma_nv from ttkd_bsc.nhanvien where thang = 202409 and donvi = 'VTTP')
								or manv_hotro in (select ma_nv from ttkd_bsc.nhanvien where thang = 202409 and donvi = 'VTTP')
							)
                                and ((thang_ptm >= 202406     ---thang n-3
										  and ((loaihd_id = 1 and loaitb_id in (11, 58, 61)) or loaitb_id=20)
										  and (thang_tldg_dt = 202409 or thang_tldg_dnhm = 202409 or thang_tldg_dt is null))          ---thang n
                                            or (loaitb_id = 21 and thang_ptm = 202409)
								    )
								                           ---thang n
    ;
--select * from ttkd_bsc.nhanvien where donvi = 'VTTP' and thang = 202409;
--select * from  ttkd_bsc.bangluong_dongia_vttp where thang = 202409;
...
insert into ttkd_bsc.bangluong_dongia_vttp
      (ma_nv,ten_nv,ma_vtcv,ten_vtcv,ma_pb,ten_pb,ma_to,ten_to,thang )
		SELECT ma_nv,ten_nv,ma_vtcv,ten_vtcv,ma_pb,ten_pb,ma_to,ten_to, thang           --thang n
		  ...
		  where donvi = 'VTTP' and thang = 202409
--					exists(select 1 from ttkd_bsc.ct_bsc_ptm_202409_vttp where manv_ptm=a.ma_nv)
...
-- tong hop:  
		...
		;
					create table ttkd_bsc.x_th_tlptm_vttp as
					    ...
					    where thang = 202409 and donvi = 'VTTP'          ---thang n---
...
;
		...
		--			create table x_tpmp as	
					select  id ptm_id, ma_gd, thuebao_id, ma_tb, loaitb_id, dichvuvt_id, manv_ptm
					    ...
					    where manv_ptm is not null and thang_ptm >= 202406 and thang_tldg_dt is null
				...
				union all     
					    ...
					    from ttkd_bsc.ct_bsc_ptm
					    where manv_ptm is not null and thang_ptm >= 202406 and thang_tldg_dnhm is null
				union all			    
					    ...
					    select  id, ma_gd, thuebao_id, ma_tb, loaitb_id, dichvuvt_id, manv_hotro
									 ...
									 , 'dnhm' nguon, thang_tldg_dnhm, thang_ptm
					    from ttkd_bsc.ct_bsc_ptm
					    where manv_hotro is not null and thang_ptm >= 202406 and thang_tldg_dnhm is null 
				...
				union all     
					    ...
					    from ttkd_bsc.ct_bsc_ptm
					    where manv_tt_dai is not null and thang_ptm >= 202406 and thang_tldg_dt_dai is null
				union all     
					    ...
					    where manv_hotro is not null and thang_ptm >= 202406 and thang_tldg_dt_nvhotro is null
									and not exists (select 1 from ttkd_bsc.nhanvien where thang = 202409 and ma_pb = 'VNP0702600' and ma_nv = a.manv_hotro)				
				 ) a
			    
			join ttkd_bsc.nhanvien nv on nv.donvi = 'VTTP' and nv.thang = 202409 and nv.ma_nv = a.manv_ptm
			)
			group by ma_nv, ten_pb
		;
...
update ttkd_bsc.bangluong_dongia_vttp set tong_dt_chitl='', tong_tl='' , tong_dt_chuachitl='',  tong_sl_chitl='', tong_sl_chuachitl='' where thang = 202409;
update ttkd_bsc.bangluong_dongia_vttp a
...
-- Thu hoi:
				update ttkd_bsc.bangluong_dongia_vttp a  
				   ...
				   where tong_tl_thuhoi is not null
								and thang = 202409
				   ;
--   select * from ttkd_bsc.bangluong_dongia_202409_vttp a
				update ttkd_bsc.bangluong_dongia_vttp a  
				   set (tl_thuhoi_brcd, tl_thuhoi_mytv, tl_thuhoi_vnpts, tl_thuhoi_vnptt, tong_tl_thuhoi, sl_thuhoi_brcd, sl_thuhoi_mytv, sl_thuhoi_vnpts, sl_thuhoi_vnptt, tong_sl_thuhoi) 
							 = (select sum(case when loaitb_id=58 then tien_thuhoi else 0 end)
								  ...
								  where thang = 202409 and ma_nv =a.ma_nv)             ---thang n
			...
			delete from ttkd_bsc.bangluong_dongia_vttp where thang = 202409
...
-- File excel chitiet_tbao_thulao_thuhoi_xx2024:
					...
					select 'PTM_chuachi' nguon, thang_ptm
										...
										, (case when dongia = 800 then luong_dongia_nvptm * 0.8
											...
											as tong_tl_goi
						-- select * 
						from ttkd_bsc.ct_bsc_ptm_202409_vttp a
									join ttkd_bsc.nhanvien b on b.thang = 202409 and donvi = 'VTTP' and a.manv_ptm = b.ma_nv     --thang n
		...
		union all
				select nguon, thang_ptm, ma_gd, ma_tb, lh.loaihinh_tb, null, null, null, null
					    ...
					    where a.loaitb_id in (58, 61, 20, 11, 21) and thang = 202409 and donvi = 'VTTP'
...
--		select *
		 ...
		 where a.thang=202409 and a.loaitb_id in (58, 61, 20, 11, 21) 
						and a.ma_nv=b.ma_nv and a.thang = b.thang and b.donvi = 'VTTP'
		--   and exists(select * from ttkd_bsc.bangluong_dongia_202409_vttp where ma_nv=a.ma_nv)
;
