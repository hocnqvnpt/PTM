update ttkd_bsc.ct_bsc_ptm set nocuoc_n1 = null
where ma_tb = 'tigroup_bdg';
...
-- ID447: cap nhat ktoan kh xac nhan thu tien
	update ttkd_bsc.ct_bsc_ptm a 
		   set (thang_luong, xacnhan_khkt, thang_xacnhan_khkt, thang_tldg_dt, thang_tlkpi, thang_tlkpi_to, thang_tlkpi_phong, thang_tldg_dt_nvhotro, thang_tlkpi_hotro) =       
				 (select 7, tienthu_khkt, to_char(ngaycapnhat_khkt,'yyyymm') , 202405, 202405, 202405, 202405, 
					    case when manv_hotro is not null then 202405 end ,
					    case when manv_hotro is not null then 202405 end
					from ttkdhcm_ktnv.amas_duan_ngoai_doanhthu where id=a.id_447)
	    -- select * from ttkd_bsc.ct_bsc_ptm a
	    where  thang_ptm>=202402 and thang_xacnhan_khkt is null ---thang n-3
...
-- Ptm qua Digishop:
	...
	---  select thang_tldg_dt, thang_tldg_dnhm, heso_hotro_nvptm, heso_hotro_nvhotro from ttkd_bsc.ct_bsc_ptm a 
		    where thang_ptm = 202405 and ungdung_id=17 and (nhom_tiepthi<>2 or nhom_tiepthi is null)
		    ;
...
---3		luot (neu khong co yeu cau)
    -- Dot 1:    dot 2 khong chay
		update ttkd_bsc.ct_bsc_ptm a 
			set thang_tldg_dt=202405, lydo_khongtinh_dongia=null
		    ...
		    where thang_ptm = 202405 and (loaitb_id not in (20, 21,131, 358) or ma_kh='GTGT rieng')  and (chuquan_id is null or chuquan_id <> 264)
						  ...
						  and (lydo_khongtinh_luong is null or (lydo_khongtinh_luong like '%Phat trien qua Dai ly%' and heso_daily=0.05 and MA_NGUOIGT is not null)
						    ...
						    and (dthu_ps > 0 
									or (nvl(dthu_ps, 0) =0 and loaitb_id in (55,80,116,117,140,132,122,288,181,290,292,175,302) 
													and dthu_goi >0 and thang_bddc > 202405)
							) 
						and nocuoc_ptm is null 		
						  ...
						  and not exists (select 1 from ttkd_bct.ptm_codinh_202405
...
create index ttkd_bsc.idx_thangptm on ttkd_bsc.ct_bsc_ptm (thang_ptm);
    ...
    -- Dot 2:    
		update ttkd_bsc.ct_bsc_ptm a 
		    set thang_tldg_dt = 202405
--		    select thang_ptm, ma_gd, thuebao_id, ma_tb, manv_ptm, loai_ld, ma_pb, lydo_khongtinh_luong, lydo_khongtinh_dongia, luong_dongia_nvptm, dthu_goi, dthu_ps, nocuoc_ptm, ten_pb, trangthaitb_id, loaitb_id, thoihan_id, ngay_luuhs_ttkd, ngay_luuhs_ttvt, nop_du, mien_hsgoc, thang_tldg_dt from ttkd_bsc.ct_bsc_ptm a  
...
--							thang_luong = 87
				 ...
				 and (lydo_khongtinh_luong is null or (lydo_khongtinh_luong like '%Phat trien qua Dai ly%' and heso_daily=0.05 and MA_NGUOIGT is not null)
				  ...
				  and nvl(thang_tldg_dt, 999999)  >= 202405 
--				  and thang_tldg_dt is null
...
--		    ;
		...
		update ttkd_bsc.ct_bsc_ptm a 
			   set thang_tldg_dt = 202405, thang_tlkpi=202405, thang_tlkpi_to=202405, thang_tlkpi_phong=202405
				    ,thang_tldg_dnhm=case when tien_dnhm>0 and ngay_tt is not null then 202405 end
				    ,thang_tlkpi_dnhm=case when tien_dnhm>0 and ngay_tt is not null then 202405 end
...
--					, dthu_goi, dthu_ps, nocuoc_ptm, ten_pb, trangthaitb_id, loaitb_id, thoihan_id, ngay_luuhs_ttkd, ngay_luuhs_ttvt, nop_du, mien_hsgoc, thang_tldg_dt  from ttkd_bsc.ct_bsc_ptm a
		    where thang_ptm>=202402 and chuquan_id=264 and manv_ptm is not null
...
--		    ;
		--8
		update ttkd_bsc.ct_bsc_ptm a 
			set thang_tldg_dt_dai = 202405
		    -- select * from ttkd_bsc.ct_bsc_ptm a
		    where thang_ptm = 202405 and loaitb_id<>21
...
--					  ;
    ...
    commit;
		...
		update ttkd_bsc.ct_bsc_ptm a 
			   set thang_tldg_dt_nvhotro = 202405
--		    select  thang_luong, ma_tb, manv_ptm, manv_hotro, thang_tldg_dt, thang_tldg_dt_nvhotro, luong_dongia_nvptm, lydo_khongtinh_luong, lydo_khongtinh_dongia, nguon, ma_gd_gt, tyle_hotro, nop_du, mien_hsgoc, heso_daily  from ttkd_bsc.ct_bsc_ptm a
		    where thang_ptm = 202405 --and ma_tb = 'hcm_econtract_00000999'
--and ma_gd = '00844282'
								and (loaitb_id not in (210, 21,131) or ma_kh='GTGT rieng') and (chuquan_id is null or chuquan_id<>264) 
							  and not exists(select 1 from ttkd_bsc.dm_loaihinh_hsqd where loaitru_tinhluong=1 and loaitb_id=a.loaitb_id) 
							...
							and ( (thang_tldg_dt is null and lydo_khongtinh_luong like '%Phat trien qua Dai ly%')			---PGP dc tinh khi ban qua dai ly
									...
									-- and a.ma_tb in ('hcm_econtract_00000999')
							  and nvl(thang_tldg_dt_nvhotro, 999999)  >= 202405 
--							  and thang_tldg_dt_nvhotro is null
							...
							and nocuoc_ptm is null 
							  ...
							  and  (trangthaitb_id=1 or (loaitb_id=20 and trangthaitb_id=10) or (thoihan_id=1 and (dichvuvt_id in (7,8,9) or loaitb_id in (1,58,59,39,146))) or loaitb_id in (89,90,146) )            
							  and (nop_du=1 or mien_hsgoc is not null) 
--							and (nop_du is null and mien_hsgoc is null)
							--  and (loai_ld not like '_LCN' or loai_ld is null)	---khong tinh dai ly ca nhan
...
--			    where thang_ptm = 202404 and thang_tldg_dnhm = 202404
 ...
 -- dnhm vnpts: 
	...
	-- select chuquan_id, ma_tb, manv_ptm, ten_pb, thang_tldg_dt, thang_tldg_dnhm, tien_dnhm, tien_tt, thang_ptm,  to_char(ngay_tt,'yyyymm'), ngay_tt, soseri, trangthai_tt_id from ttkd_bsc.ct_bsc_ptm a
			    ...
			    where thang_ptm = 202405 and loaitb_id=20
				   and thang_tldg_dt = a.thang_ptm
				   and nvl(thang_tldg_dnhm, 999999) >= 202405   
				   and lydo_khongtinh_luong is null
...
-- Dot 1: dot 2 khong chay
			update ttkd_bsc.ct_bsc_ptm a 
			    set thang_tldg_dt = 202405		--thang n
			    ...
			    where thang_ptm = 202404		--thang n-1
					  ...
					  and (nvl(dthu_ps,0)+nvl(dthu_ps_n1,0)>0 
									or (nvl(dthu_ps_n1, 0) =0 and loaitb_id in (55,80,116,117,140,132,122,288,181,290,292,175,302) 
													and dthu_goi >0 and thang_bddc > 202405---thang n
											)
							) 
...
--				;
			...
			update ttkd_bsc.ct_bsc_ptm a 
			    set thang_tldg_dt_dai = 202405		---thang n
			-- select * from ttkd_bsc.ct_bsc_ptm a 
			   where thang_ptm=202404 	---thang n-1
					  and thang_tldg_dt = 202405		---thang n
					and manv_tt_dai is not null and loaitb_id<>21 
...
--						;
			...
			update ttkd_bsc.ct_bsc_ptm a 
			    set thang_tldg_dt_nvhotro = 202405			--thang n
--			    select ma_tb, thang_tldg_dt, thang_tldg_dt_nvhotro, lydo_khongtinh_dongia, luong_dongia_nvptm, luong_dongia_nvhotro from ttkd_bsc.ct_bsc_ptm a
			    where thang_ptm = 202404 		--thang n-1
					and (loaitb_id not in (210, 21,131) or ma_kh='GTGT rieng') and (chuquan_id is null or chuquan_id<>264)
					  and not exists(select 1 from ttkd_bsc.dm_loaihinh_hsqd where loaitru_tinhluong=1 and loaitb_id=a.loaitb_id)
					...
					and ( (thang_tldg_dt is null and lydo_khongtinh_luong like '%Phat trien qua Dai ly%')			---PGP dc tinh khi ban qua dai ly
								or thang_tldg_dt = 202405				---tam thoi PTM < T5, sau do chuyen ve is not null sau >= T5
							)
					  and nvl(thang_tldg_dt_nvhotro, 999999) >= 202405         
					  and ( trangthaitb_id_n1=1 or (loaitb_id=20 and trangthaitb_id_n1=10) or (thoihan_id=1 and (dichvuvt_id in (7,8,9) or loaitb_id in (1,58,59,39,146))) or loaitb_id in (89,90,146) )
...
--						and (loaitb_id<>21 or ma_kh='GTGT rieng')
 ...
 -- Dot 1:  dot 2 khong chay
			 update ttkd_bsc.ct_bsc_ptm a 
			    set thang_tldg_dt = 202405		---thang n
			    -- select * from ttkd_bsc.ct_bsc_ptm a
			    where thang_ptm = 202403 		--thang n-2
						and (loaitb_id not in (21,131) or ma_kh='GTGT rieng') and chuquan_id in (145,266) 
					  and (loaitb_id<>21 or ma_kh='GTGT rieng') 
...
-- PGP
			update ttkd_bsc.ct_bsc_ptm a 
			    set thang_tldg_dt_nvhotro = 202405		---thang n
--			    select thang_ptm, thang_tldg_dt_nvhotro, lydo_khongtinh_dongia, luong_dongia_nvptm, luong_dongia_nvhotro from ttkd_bsc.ct_bsc_ptm a
			    where thang_ptm = 202402 	---thang n-3
						and (loaitb_id not in (21,131) or ma_kh='GTGT rieng') and chuquan_id in (145,266) 
					 and (lydo_khongtinh_luong is null or (lydo_khongtinh_luong like '%Phat trien qua Dai ly%')		---PGP dc tinh khi ban qua dai ly
										)
...
-- GTGT rieng thang n
			update ttkd_bsc.ct_bsc_ptm a 
				   set  thang_tldg_dt = 202405, thang_tlkpi= 202405, thang_tlkpi_to =202405, thang_tlkpi_phong =202405,
						 thang_tldg_dt_nvhotro=case when manv_hotro is not null and thang_tldg_dt_nvhotro is null then 202405 end
			    -- select * from ttkd_bsc.ct_bsc_ptm a 
			    where thang_ptm >= 202402 ---thang n-3
								and lydo_khongtinh_luong is null and thang_xacnhan_khkt is not null 
...
--		select thuebao_cha_id, case when exists (select 1 from ttkd_bsc.ct_bsc_ptm where thuebao_id = a.thuebao_cha_id and thang_tldg_dt is null)
--								then null
--								else 202405
--							end kq
--		from ttkd_bsc.ct_bsc_ptm a
		where (thuebao_cha_id is not null and loaitb_id = 17)		---bsung 22/6
										and thang_ptm >= 202402
										and nvl(thang_tldg_dt, 999999) >= 202402
;
...
select thang_ptm, ma_gd, ma_tb, soseri, ngay_tt, tien_tt, tien_dnhm, trangthai_tt_id, chuquan_id, manv_ptm, MA_NGUOIGT
  ...
  from ttkd_bsc.ct_bsc_ptm a
  where thang_tldg_dt=202405 and trangthai_tt_id=1 and (tien_dnhm>0 or tien_sodep >0) and ngay_tt is not null
    and exists(select 1 from ttkd_bsc.dm_loaihinh_hsqd where kieutinh_bsc_ptm='thang' and loaitb_id=a.loaitb_id)
    and thang_tldg_dnhm is null
    ;
