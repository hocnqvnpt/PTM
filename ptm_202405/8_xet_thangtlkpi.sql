update ttkd_bsc.ct_bsc_ptm 
		    set thang_tlkpi = thang_tldg_dt, thang_tlkpi_to = thang_tldg_dt, thang_tlkpi_phong = thang_tldg_dt
		    ...
		    where (loaitb_id not in (21) or ma_kh='GTGT rieng') 
						and nvl(thang_tlkpi, 0) <> 999999
						and nvl(thang_tlkpi, 999999) >= 202405
						and thang_tldg_dt = 202405 
--						and thang_luong = 17
...
--     select ma_tb, nguon, thang_tlkpi, thang_tlkpi_to, thang_tlkpi_phong, thang_tldg_dt, thang_tldg_dt_nvhotro, thang_tlkpi_hotro, lydo_khongtinh_dongia, nop_du, mien_hsgoc, manv_ptm, manv_hotro from ttkd_bsc.ct_bsc_ptm  a
    where (loaitb_id not in (21) or ma_kh='GTGT rieng') 
				and nvl(thang_tlkpi_hotro, 0) <> 999999
				and nvl(thang_tlkpi_hotro, 999999) >= 202405
				and thang_tldg_dt_nvhotro = 202405
--				and thang_luong = 26
--				and thang_tlkpi_hotro = 202405 and thang_tldg_dt_nvhotro is null
--				and nop_du is null and mien_hsgoc is  null
				--and LYDO_KHONGTINH_DONGIA like '%Chua nop du ho so%'
...
update ttkd_bsc.ct_bsc_ptm a
   set thang_tlkpi_dnhm = thang_tldg_dnhm, thang_tlkpi_dnhm_to=thang_tldg_dnhm, thang_tlkpi_dnhm_phong=thang_tldg_dnhm
--     select ma_tb, nguon, thang_tldg_dnhm, thang_tlkpi, thang_tlkpi_dnhm, thang_tlkpi_dnhm_to, thang_tlkpi_dnhm_phong, lydo_khongtinh_dongia, nop_du, mien_hsgoc, manv_ptm, manv_hotro from ttkd_bsc.ct_bsc_ptm  a   
		   where (loaitb_id not in (21) or ma_kh='GTGT rieng') 
						and nvl(thang_tlkpi_dnhm, 0) <> 999999
						and nvl(thang_tlkpi_dnhm, 999999) >= 202405
						and thang_tldg_dnhm = 202405
		;
   rollback;
   commit;
-- Ky n 
...
--;
			...
			update ttkd_bsc.ct_bsc_ptm a  
			 ...
			 -- select ma_tb, thang_tldg_dt, thang_tlkpi_dnhm, thang_tlkpi_dnhm_to, thang_tlkpi_dnhm_phong, lydo_khongtinh_dongia from ttkd_bsc.ct_bsc_ptm a
			  where thang_ptm=202405 and thang_tlkpi_dnhm_to is null and loaitb_id<>21
				  and not exists(select 1 from ttkd_bsc.dm_loaihinh_hsqd where loaitru_tinhluong=1 and loaitb_id=a.loaitb_id)
				  and  (upper(ma_tiepthi_new) in ('DL_CNT','GTGT00001') or upper(nguoi_gt) in ('DL_CNT','GTGT00001') or upper(ma_nguoigt) in ('DL_CNT','GTGT00001'))   
...
-- nv, to, phong - kenh CTVXHH:    ---tinh theo nvien hotro
		update ttkd_bsc.ct_bsc_ptm a 
		   set thang_tlkpi=202405, thang_tlkpi_to=202405, thang_tlkpi_phong=202405
		    -- select thang_ptm, ungdung_id, ma_tb, ten_pb, ma_tiepthi, ma_tiepthi_new , ma_to, thang_tldg_dt, thang_tlkpi, thang_tlkpi_to, lydo_khongtinh_luong, lydo_khongtinh_dongia from ttkd_bsc.ct_bsc_ptm a
			where thang_ptm=202405 and thang_tlkpi is null 
						   and (loaitb_id not in (21,131) or ma_kh='GTGT rieng') and chuquan_id in (145,266)
						   and not exists(select 1 from ttkd_bsc.dm_loaihinh_hsqd where loaitru_tinhluong=1 and loaitb_id=a.loaitb_id)
						   and (nop_du=1 or mien_hsgoc is not null) 
						   and (trangthaitb_id=1 or (loaitb_id=20 and trangthaitb_id=10) or (thoihan_id=1 and (dichvuvt_id in (7,8,9) or loaitb_id in (1,58,59,39,146)) and trangthaitb_id=7) or loaitb_id in (89,90,146) )
...
-- dnhm - phong:  dai ly 
			update ttkd_bsc.ct_bsc_ptm a
			 ...
			 where thang_ptm=202405 and thang_tlkpi_dnhm_phong is null
			   and not exists(select 1 from ttkd_bsc.dm_loaihinh_hsqd where loaitru_tinhluong=1 and loaitb_id=a.loaitb_id)
			   and lydo_khongtinh_luong like '%Phat trien qua Dai ly%' 
			   and (nop_du=1 or mien_hsgoc is not null) 
			   and (tien_dnhm>0 or tien_sodep>0) 
...
-- dnhm - nv/to/phong - kenh CTVXHH
				update ttkd_bsc.ct_bsc_ptm a
				   set thang_tlkpi_dnhm=202405, thang_tlkpi_dnhm_to=202405, thang_tlkpi_dnhm_phong=202405
--				  select ma_tb, thang_tlkpi_dnhm_phong, thang_tlkpi_dnhm, lydo_khongtinh_dongia from ttkd_bsc.ct_bsc_ptm a
				 where thang_ptm=202405 and thang_tlkpi_dnhm_phong is null and thang_tlkpi=202405
							   and not exists(select 1 from ttkd_bsc.dm_loaihinh_hsqd where loaitru_tinhluong=1 and loaitb_id=a.loaitb_id) 
							   and (nop_du=1 or mien_hsgoc is not null) 
							   and (tien_dnhm>0 or tien_sodep>0) 
							   and dthu_ps is not null and nocuoc_ptm is null
...
update ttkd_bsc.ct_bsc_ptm a
 ...
 where thang_ptm=202403 and chuquan_id in (145,266) 
				and thang_tldg_dt_nvhotro = 202405			----thang_tlkpi NVPTM co thi Hotro co
				and thang_tlkpi_hotro is null
			    and (loaitb_id not in (21,131) or ma_kh='GTGT rieng')
			    and not exists(select 1 from ttkd_bsc.dm_loaihinh_hsqd where loaitru_tinhluong=1 and loaitb_id=a.loaitb_id)
			    and ((loaitb_id not in (20,149) and trangthai_tt_id=1) or loaitb_id in (20,149))
...
select * from ttkd_bsc.ct_bsc_ptm 
    where thang_ptm>=202402 and (loaitb_id<>21 or ma_kh='GTGT rieng')
        and thang_tlkpi_to is null and thang_tlkpi=202405;
...
select * from ttkd_bsc.ct_bsc_ptm 
    where thang_ptm>=202402 
        ...
        and thang_tlkpi_to=202405 and thang_tldg_dt is null
...
select thang_ptm, ten_pb, ten_to,dich_vu, ma_tb, trangthaitb_id_n3, nocuoc_n3, ngay_luuhs_ttkd,nop_du, sothang_dc
    ...
    where thang_ptm>=202402
        ...
        and ((thang_tldg_dt is null and thang_tlkpi=202405)
                or (thang_tldg_dt =202405 and thang_tlkpi is null)) ;   

             
select * from ttkd_bsc.ct_bsc_ptm
where chuquan_id in (145,266)
    and ( thang_tldg_dt=202405 or thang_tlkpi=202405 or thang_tlkpi_to=202405 or thang_tlkpi_phong=202405)
...
where (thang_tlkpi_hotro=202405 and thang_tldg_dt_nvhotro is null)
        or (thang_tldg_dt_nvhotro=202405 and thang_tlkpi_hotro is null );
...
select * from ttkd_bsc.ct_bsc_ptm a
    where chuquan_id in (145,266)
        and ( thang_tldg_dt=202405 or thang_tlkpi=202405 or thang_tlkpi_to=202405 or thang_tlkpi_phong=202405 or thang_tldg_dnhm=202405 or thang_tldg_dt_nvhotro = 202405)
        and loaitb_id not in (20,21,149) and loaihd_id=1
        and (trangthai_tt_id is null or trangthai_tt_id=0);
	   
select * from ttkd_bsc.ct_bsc_ptm a
    where chuquan_id in (145,266)
        and  ((thang_tldg_dt is null and thang_tldg_dt_nvhotro = 202405)
					--or (thang_tldg_dt = 202405 and thang_tldg_dt_nvhotro is null)
					or (thang_tldg_dt is null and THANG_TLKPI = 202405)
				)
	   ;
