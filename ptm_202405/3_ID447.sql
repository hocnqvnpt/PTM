---ngay 4/5 anh Nghia chua duyet xong, doi khi nào xong moi chay
		select * from ttkd_bct.ptm_gtgt where thang = 202405 and id in (5611427);
			select * from ttkdhcm_ktnv.amas_duan_ngoai_doanhthu ptm  where  ma_yeucau = '214147';'183656'; 
		--KTRA DU LIEU 447
...
--													, substr(trim(b.so_nha)||', '||nvl(b.ten_duong,'')||', '||nvl(b.ten_phuong,'')||', '||nvl(b.ten_quan,'') ,1,500) diachi_kh
--													, a.masothue mst, a.masothue so_gt
...
--											select * from ps_truong where MA_YEUCAU in (214147)
							, t as	 (select c.manv_presale_hrm, b.tyle_hotro/100 tyle_hotro, decode(c.tyle_am, 0, 1, c.tyle_am/100) tyle_am, b.ma_yeucau, b.ma_dichvu, c.tyle_nhom
												...
												, b.ten_kh
										 from ps_truong b
														left join ttkdhcm_ktnv.amas_booking_presale c on b.ma_yeucau = c.ma_yeucau and b.id_ycdv = c.id_ycdv and c.xacnhan = 1 and c.ps_truong = 1
...
--						select * from t where ma_yeucau in (252260) and ma_dichvu = 986
							, d as (select ptm.id, ptm.ma_yeucau, ptm.ngaycapnhat, ptm.ngaycapnhat_tl, ptm.ma_gd, ptm.ma_tb, ptm.sohopdong
												...
												, ptm.manv_am manv_ptm, t.manv_presale_hrm manv_hotro, t.tyle_hotro, t.tyle_hotro_nv, t.tyle_am
										...
										from ttkdhcm_ktnv.amas_duan_ngoai_doanhthu ptm
												 ...
												 where ptm.trangthai_tl=10 and ptm.dexuat=7 and ptm.ma_dichvu not in (20,149) --and id=4946446
																and ptm.tienthu_khkt > 0 --and id in (6448426, 6448425, 6387652)
															    and trunc(ptm.ngaycapnhat) <= to_date('31/05/2024','dd/mm/yyyy') 			---ngay 31 thang n
															    and trunc(ngaycapnhat_tl) <= to_date('08/06/2024','dd/mm/yyyy') 			---ngay 8 thang n+ 1
															    and trunc(ngaycapnhat_khkt) <= to_date('08/06/2024','dd/mm/yyyy') 			---ngay 8 thang n+ 1
															    and not exists(select 1 from ttkd_bct.ptm_gtgt where id=ptm.id)                 
												 )
--												 	select * from d where id in (5879427)

			    select 202405 thang, d.id, d.ma_yeucau, d.ngaycapnhat, d.ngaycapnhat_tl, d.loaihinh_tb, d.dichvuvt_id, d.loaitb_id_obss, d.ma_gd
			    ...
			    from d
							left join ttkd_bsc.nhanvien e on d.manv_ptm = e.ma_nv and e.thang = 202405
			    order by id
			    ;
			    rollback;
...
delete from ttkd_bsc.ct_bsc_ptm; 
 ...
 and ma_kh='GTGT rieng';
			...
			insert into ttkd_bsc.ct_bsc_ptm 
								   ...
								   ma_tiepthi,ma_tiepthi_new,donvi_tt,ghi_chu,manv_hotro, tyle_hotro, doituong_kh, ma_dt_kh,manv_ptm,tennv_ptm
			    ...
			    select 202405 thang_ins, 202405 thang_ptm, id, case when ma_gd is not null then ma_gd else sohopdong end ma_gd, 'GTGT rieng' ma_kh, ma_tb, 1 loaihd_id
			    ...
			    where thang = 202405 and not exists(select 1 from ttkd_bsc.ct_bsc_ptm where id_447 = a.id) 
			    ;
