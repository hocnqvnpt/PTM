update ttkd_bsc.ct_bsc_ptm a set lydo_khongtinh_dongia = null
		---   select ma_tb, lydo_khongtinh_dongia, thang_tldg_dt from ttkd_bsc.ct_bsc_ptm a
		    where thang_ptm>=202402 and lydo_khongtinh_dongia is not null---thang n-3 
...
-- CTVXHH:
			update ttkd_bsc.ct_bsc_ptm a 
				   set lydo_khongtinh_dongia=lydo_khongtinh_luong||'; ptm kenh CTVXHH'
--			    select * from ttkd_bsc.ct_bsc_ptm a 
			    where thang_ptm>=202402 and thang_ptm <= 202404 and ungdung_id=17
			    ;
...
-- No cuoc:    
				update ttkd_bsc.ct_bsc_ptm a 
				    ...
				    where ma_kh<>'GTGT rieng' and loaitb_id<>21 and thang_tldg_dt is null 
					   ...
					   and ((thang_ptm = 202405 and nocuoc_ptm is not null)           ---thang n     
								 or (thang_ptm = 202404 and nocuoc_n1 is not null)				---thang n-1
								 or (thang_ptm = 202403 and nocuoc_n2 is not null)				--thang n -2
								 or (thang_ptm = 202402 and nocuoc_n3 is not null))			---thang n-3
							 ;
				commit;                 
  ...
  -- Trang thai
			update ttkd_bsc.ct_bsc_ptm a 
					set lydo_khongtinh_dongia=lydo_khongtinh_dongia||';Trang thai'
			    where ma_kh<>'GTGT rieng' and loaitb_id not in (21,89,90,146) and thang_tldg_dt is null 
						 and (lydo_khongtinh_dongia not like '%Trang thai%' or lydo_khongtinh_dongia is null)
						 and ( -- thang n:   
								   (thang_ptm=202405  
								   and not exists (select 1 from ttkd_bsc.ct_bsc_ptm 
															where thang_ptm=202405 and (trangthaitb_id=1 or (thoihan_id=1 and (dichvuvt_id in (8,9) or loaitb_id in (1,58,59,39) ) ) ) 
																   and ma_tb=a.ma_tb and ma_kh=a.ma_kh and loaitb_id=a.loaitb_id and tenkieu_ld=a.tenkieu_ld))
								   or
								   (thang_ptm=202404  
								   and not exists (select 1 from ttkd_bsc.ct_bsc_ptm 
															where thang_ptm=202404 and ( trangthaitb_id_n1=1 or (thoihan_id=1 and (dichvuvt_id in (8,9) or loaitb_id in (1,58,59,39) ) ) )   
																   and ma_tb=a.ma_tb and ma_kh=a.ma_kh and loaitb_id=a.loaitb_id and tenkieu_ld=a.tenkieu_ld))
								   or 
								   (thang_ptm=202403  
								   and not exists (select 1 from ttkd_bsc.ct_bsc_ptm 
															where thang_ptm=202403 and ( trangthaitb_id_n2=1 or (thoihan_id=1 and (dichvuvt_id in (8,9) or loaitb_id in (1,58,59,39) ) ) )   
																   and ma_tb=a.ma_tb and ma_kh=a.ma_kh and loaitb_id=a.loaitb_id and tenkieu_ld=a.tenkieu_ld))
								   or
								   (thang_ptm=202402  
								   and not exists (select 1 from ttkd_bsc.ct_bsc_ptm 
															where thang_ptm=202402 and ( trangthaitb_id_n3=1 or (thoihan_id=1 and (dichvuvt_id in (8,9) or loaitb_id in (1,58,59,39) ) ) )   
																   and ma_tb=a.ma_tb and ma_kh=a.ma_kh and loaitb_id=a.loaitb_id and tenkieu_ld=a.tenkieu_ld))
							    );   
...
-- Ho so:
		...
		update ttkd_bsc.ct_bsc_ptm a 
		    ...
		    where thang_ptm>=202402
					 ...
					 and manv_ptm is not null and thang_xacnhan_khkt is null
...
-- Sms Brandname:
				update ttkd_bsc.ct_bsc_ptm 
				    ...
				    where thang_ptm=202405 and loaitb_id=131 and lydo_khongtinh_luong is null ;
--							   and thang_tldg_dt is null
				commit;
rollback;
...
-- Voice Brandname: 
			update ttkd_bsc.ct_bsc_ptm a
			    ...
			    -- select thang_ptm, ma_tb, thang_tldg_dt, lydo_khongtinh_luong, lydo_khongtinh_dongia, nguon from ttkd_bsc.ct_bsc_ptm a
			    where thang_ptm>=202402 and loaitb_id=358 and nvl(dthu_goi,0)=0
...
-- Thue bao ngan han chua thanh ly:
		...
		---		select thang_ptm, ma_tb, thang_tldg_dt, lydo_khongtinh_luong, lydo_khongtinh_dongia, nguon from ttkd_bsc.ct_bsc_ptm a 
			    where thoihan_id=1 and loaitb_id not in (90,146) 
					  ...
					  and ((thang_ptm=202402 and trangthaitb_id_n3 not in (7,9)) 
							    or (thang_ptm=202403 and trangthaitb_id_n2 not in (7,9)) 
							    or (thang_ptm=202404 and trangthaitb_id_n1 not in (7,9))
							    or (thang_ptm=202405 and trangthaitb_id not in (7,9))
							)
...
-- Thue bao chua phat sinh cuoc, ly do: thang_bddc lon hon thang hien tai, nen chua tinh cuoc phan ky vao cuoc ps
		update ttkd_bsc.ct_bsc_ptm a 
				  set thang_tldg_dt         = case when thang_tldg_dt=202405 then null end
					   ,thang_tlkpi             = case when thang_tlkpi=202405 then null end
					   ,thang_tlkpi_to        = case when thang_tlkpi_to=202405 then null end
					   ,thang_tlkpi_phong = case when thang_tlkpi_phong=202405 then null end
					   ,thang_tldg_dnhm  = case when thang_tldg_dnhm=202405 then null end
					   ,thang_tlkpi_dnhm = case when thang_tlkpi_dnhm=202405 then null end
					   ,thang_tlkpi_dnhm_to = case when thang_tlkpi_dnhm_to=202405 then null end
					   ,thang_tlkpi_dnhm_phong = case when thang_tlkpi_dnhm_phong=202405 then null end
					   ,thang_tldg_dt_nvhotro = case when thang_tldg_dt_nvhotro=202405 then null end
					   ,thang_tlkpi_hotro = case when thang_tlkpi_hotro=202405 then null end
...
-- select ma_tb, nguon, thang_ptm, thang_tldg_dt, thang_bddc, lydo_khongtinh_luong, lydo_khongtinh_dongia from ttkd_bsc.ct_bsc_ptm a
		    where thang_ptm>=202402 and (loaitb_id<>21 or ma_kh = 'GTGT rieng') 
			   ...
			   and (thang_tldg_dt=202405 or lydo_khongtinh_dongia is null) 
			   and nvl(dthu_ps,0)+nvl(dthu_ps_n1,0)+nvl(dthu_ps_n2,0)+nvl(dthu_ps_n3,0)=0
			   and nvl(thang_bddc, 0) <= 202405		---thang n
			   ;
...
--		     select lydo_khongtinh_dongia from ttkd_bsc.ct_bsc_ptm a
		    where thang_ptm>=202402 and chuquan_id<>264 and thang_tldg_dt is null and loaitb_id not in (20,21,149) 
				  and not exists(select 1 from ttkd_bsc.dm_loaihinh_hsqd where loaitru_tinhluong=1 and loaitb_id=a.loaitb_id)
...
--		     select lydo_khongtinh_dongia from ttkd_bsc.ct_bsc_ptm a
		    where thang_ptm>=202402 and thang_tldg_dt is null and loaitb_id not in (20,21,149) 
				  and not exists(select 1 from ttkd_bsc.dm_loaihinh_hsqd where loaitru_tinhluong=1 and loaitb_id=a.loaitb_id)
--				  and (lydo_khongtinh_dongia is null or lydo_khongtinh_dongia not like '%Chu quan khong thuoc TTKDHCM%')
				  and lydo_khongtinh_dongia is null ---check lai dieu kien nay co trung hay khong?
				and chuquan_id not in  (145,266)
				  ;
				  
			----Thue bao 30B + D , cuoc ps only so chu, cac so con dc tinh can cu so chu
			update ttkd_bsc.ct_bsc_ptm a set lydo_khongtinh_dongia = (select 'sochu: ' ||ma_tb|| lydo_khongtinh_dongia from ttkd_bsc.ct_bsc_ptm where thuebao_id = a.thuebao_cha_id)
		--	select (select 'sochu: ' ||ma_tb|| lydo_khongtinh_dongia from ttkd_bsc.ct_bsc_ptm where thuebao_id = a.thuebao_cha_id) from ttkd_bsc.ct_bsc_ptm a
			where loaitb_id = 17 and thang_ptm = 202401
							and thuebao_cha_id is not null and thang_tldg_dt is null
							and ma_gd = 'HCM-LD/01585462'
;
            commit;
-- Kiem tra thue bao chua cap nhat ly do khong tinh don gia:
select thang_ptm, trangthaitb_id, nocuoc_ptm,ten_pb, dich_vu, ma_gd, ma_tb, 
                manv_ptm, loai_ld, dthu_goi,lydo_khongtinh_luong, lydo_khongtinh_dongia, thoihan_id,ghi_chu,
                ...
                luong_dongia_nvptm, trangthai_tt_id, nguon
        from ttkd_bsc.ct_bsc_ptm a           
        where thang_ptm=202402 and chuquan_id in (145,264,266)
...
select thang_ptm,ma_to, ma_gd, ma_tb, dich_vu, ten_tb, dthu_goi_goc, dthu_goi, ngay_bbbg,a.ngay_luuhs_ttkd, 
                a.ngay_luuhs_ttvt, a.mien_hsgoc, a.thoihan_id, a.nocuoc_ptm, a.mien_hsgoc,a.nop_du,a.trangthaitb_id, 
                lydo_khongtinh_luong, lydo_khongtinh_dongia, a.thoihan_id
        from ttkd_bsc.ct_bsc_ptm a
        where thang_ptm>=202402
                    and lydo_khongtinh_luong is null and loaitb_id<>21 and ma_kh<>'GTGT rieng'
