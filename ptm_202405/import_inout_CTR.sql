...
----imp ds anh Nghia trong/ngoai ctr
				select * from ttkd_bsc.ct_ptm_ngoaictr_imp where thang = 202405 and ma_tb = 'hcm_colo_00010493'; and dichvu_vt = 'VNP tra sau' and TENKIEU_LD != 'Gia han dich vu';
				...
				update ttkd_bsc.ct_ptm_ngoaictr_imp set
										command = 'insert' 
--													loaitb_id = 358, dichvuvt_id = 15 
--												, dthu_goi = 3540000
--												ma_gd = 'HCM-LD/01586833'
--												ma_tb = 'hcm_vbn_00001181'
--												doituong_kh = 'KHDN'
--				select * from ttkd_bsc.ct_ptm_ngoaictr_imp 
				where thang = 202405 and dichvuvt_id in (14, 15 16)
--							ma_tb = 'hcm_vbn_00001181'
--							and DICHVU_VT= 'Vinaphone tra sau' 
				;		
				---update VNP
				...
				WHEN MATCHED THEN
					UPDATE
					SET a.thuebao_id = b.thuebao_id
					WHERE a.thang = 202405 and a.ma_tb = 'hcm_vbn_00001181'
					;
				
				commit;
				rollback;
				
				update ttkd_bsc.ct_ptm_ngoaictr_imp 
							set command = 'update'--, dthu_goi = 1000000
									, ghichu = 'hop dong lap moi, qua han, hok co dthu_goi, bsung bq dthu_ps Obss, CCBS'
--									, TENKIEU_LD = 'Mua them'
--				select * from ttkd_bsc.ct_ptm_ngoaictr_imp 
				where thang = 202405 and 
--										loaitb_id = 39
											thuebao_id = 12015274 --chu upda, ?ang kiem tra vi sao ti?n hok có trong ctr
				;
				HDHCM1121092114/4
				--ktra ma_gd da tinh tien
				select * from ttkd_bsc.ct_bsc_ptm where thang_ptm >= 202312 and  thuebao_id in (12170099,
12216805,
12015274,
12121098,
11918954);
				and thuebao_id in (select thuebao_id from ttkd_bsc.ct_bsc_ptm where thang_ptm <= 202405 and replace(ma_gd, ' ', '') = 'HCM-DV/10694247');
				
				select * from ttkd_bsc.ct_bsc_ptm where thang_ptm <= 202405 and replace(ma_gd, ' ', '') = 'HCM-DV/10694247';

				select * from ttkd_bsc.ct_ptm_ngoaictr_imp where thang = 202405 and dichvuvt_id in (14, 15, 16) and command is not null and ghichu is not null;= 'update'; and ma_tb= 'hcm_vbn_00001065';
				...
				insert into ttkd_bsc.ct_bsc_ptm (thang_luong, thang_ptm, ten_pb, ma_pb, ten_to, ma_to, manv_ptm, tennv_ptm, ma_vtcv, loai_ld, NHOM_TIEPTHI, TENKIEU_LD, ma_gd, ma_tb, ma_kh
							...
							select 86, a.THANG, b.ten_pb, b.ma_pb, b.ten_to, b.ma_to, a.MANV_PTM, a.TENNV_PTM, b.ma_vtcv, b.loai_ld, b.NHOMLD_ID NHOM_TIEPTHI, kld.TEN_KIEULD, a.MA_GD, nvl(c.MA_TB, 'khongco'||rownum) ma_tb, kh.MA_KH, a.SO_HD
											...
											, 'ct_ptm_ngoaictr_imp_'||command as nguon, plk.MA_PLKH, nvl(kh.mst, 1) mst, nvl(db.dthu_ps, a.DTHU_GOI) dthu_ps, nvl(c.trangthaitb_id, 1), 145 chuquan_id, 858 dongia, lh.loaihinh_tb
											, 1 nop_du, 1 mien_hsgoc, 1 trangthai_tt_id
											, hdkh.hdkh_id, hdtb.hdtb_id, hdkh.loaihd_id, hdtb.kieuld_id
								...
								where a.thang = 202405 and a.dichvuvt_id in (14, 15, 16)-- not in ('VNP tra sau', 'Internet truc tiep')
												and command = 'insert'
							...
							select 
								...
								from ttkd_bsc.ct_ptm_ngoaictr_imp a
											...
											left join (select x.thuebao_id, plkh_id, sum(dthu) dthu_ps
																...
																group by x.thuebao_id, plkh_id) db on a.thuebao_id = db.thuebao_id
											left join css_hcm.phanloai_kh plk on db.plkh_id = plk.PHANLOAIKH_ID
								where a.thang = 202405 and a.dichvuvt_id in (14, 15, 16)
												and command = 'update'
								)
								select * from t
						;		
					...
					USING (
							select 
										a.THANG, a.MA_GD, a.SO_HD
											...
											, 'ct_ptm_ngoaictr_imp_'||command as nguon, dthu_ps, c.trangthaitb_id, command
											, 1 nop_du, 1 mien_hsgoc, 1 trangthai_tt_id
								from ttkd_bsc.ct_ptm_ngoaictr_imp a
...
--												, a.tyle_hotro = nvl(t.heso_hotro_nvhotro, a.heso_hotro_nvhotro)
						...
						WHERE a.thuebao_id in (select thuebao_id from ttkd_bsc.ct_ptm_ngoaictr_imp where thang = 202405 and command = 'update')
					;
					commit;
					rollback;
					---check cot de UPDATE
