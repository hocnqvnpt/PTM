select * from ptm_codinh_202405;  
drop table ptm_codinh_202405 purge;
create table ptm_codinh_202405 as
		...
		, km_sd as (select a.thuebao_id, max(a.tyle_sd) tyle_sd 
								from css.v_khuyenmai_dbtb a, css.v_ct_khuyenmai b
								  where a.chitietkm_id = b.chitietkm_id and a.thang_bd >= 202405		---thang n
		...
		, datcoc_goc as (select hd.hdtb_id, dc.thang_bd thang_bddc, dc.thang_kt thang_ktdc, dc.CUOC_DC DATCOC_CSD
            ...
            --                                                                , sum(cuoc_dc) datcoc_csd, sum(tien_td) tien_td 
                                                            ...
                                                            where hd.hdtb_id=dc.hdtb_id and hd.tthd_id = 6 and to_number(to_char(hd.ngay_ht,'yyyymm')) = 202405	--thang n
            ...
            --                                                    , sum(a.datcoc_csd) datcoc_csd,sum(a.tien_td) tien_td
                                                     ...
                                                     where c.hdtb_id = a.hdtb_id and a.chitietkm_id = b.chitietkm_id-- and dc.nhom_datcoc_id =  ndc.nhom_datcoc_id (+)
														  ...
														  and a.datcoc_csd>0 and a.khuyenmai_id not in (1977, 2056, 2998, 2999) 
                                                                        and c.tthd_id = 6 and to_number(to_char(c.ngay_ht,'yyyymm')) = 202405	--thang n
...
--                , round(datcoc.datcoc_csd/1.1,0) tien_td
   ...
   from  css.v_hd_khachhang a, css.v_hd_thuebao b, v_db, v_hdtb, tinhcuoc.v_dbtb db_old
         ...
         where a.hdkh_id = b.hdkh_id and b.thuebao_id = v_db.thuebao_id  and db_old.ky_cuoc (+) = 20240401 	---thang n-1
...
--                 and trunc(b.ngay_ht) >= to_date('01/05/2024','dd/mm/yyyy') 
--                and trunc(b.ngay_ht) <= to_date('31/05/2024','dd/mm/yyyy')
			 and to_number(to_char(b.ngay_ht, 'yyyymm')) = 202405 		--thang n
--                and b.ma_tb = 'LL001075589'
             ...
             select * from ptm_codinh_202405
                where (ma_tb, ma_gd, hdtb_id) in 
                                                ...
                                                from ptm_codinh_202405 a 
                                                group by ma_tb, ma_gd, hdtb_id having count(*)>1 )
            ;
--Them cot------ tien_dnhm, tien_camket, tien_sodep:
                    ALTER TABLE ptm_codinh_202405 ADD (tien_dnhm NUMBER, tien_sodep NUMBER, tien_camket NUMBER)
				;
                    UPDATE ptm_codinh_202405 a 
                ...
                commit;
                    ...
                    MERGE INTO ptm_codinh_202405 a
                    USING (with dnhm as (SELECT dnhm1.khoanmuctt_id loai,dnhm1.hdtb_id, dnhm1.phieutt_id
																...
																and hd.tthd_id = 6			---update 1/6
                                                                                AND to_number(TO_CHAR(hd.ngay_ht,'yyyymm')) = 202405			---thang n
                                                                       )
                                    ...
                                    FROM css.v_phieutt_hd hd1
                                                , (SELECT decode(ct.khoanmuctt_id,19,dnhm.loai,20, thietbi.loai,ct.khoanmuctt_id) loai
                                                    ...
                                                    WHERE EXISTS(SELECT 1 FROM css.v_hd_thuebao 
																		WHERE to_number(TO_CHAR(ngay_ht,'yyyymm')) = 202405		--thang n
        ...
        create table temp_muccuoctb_thuebao as
                with km_dbtb as (select thuebao_id, tyle_tb, cuoc_tb, tien_tb , ngay_sd
					...
					, hdtb as (select hdtb.thuebao_id, row_number() over (partition by hdtb.thuebao_id order by hdtb_id desc) rnk
									...
									where hdkh.loaihd_id in (1,6,31,41,49) and hdtb.dichvuvt_id in (13,14,15,16) and hdtb.tthd_id = 6
													...
													and to_number(to_char(hdtb.ngay_ht,'yyyymm')) = 202405  -- thang n
									)
            ...
            where dbc.phanvung_id=28 and mc.phanvung_id=28 
                ...
                create table temp_muccuoctb_hdtb as
                    with km_hdtb as (select hdtb1.hdtb_id, km_hdtb1.tyle_tb, km_hdtb1.cuoc_tb,  km_hdtb1.tien_tb
                                                            ...
                                                            where hdtb1.hdtb_id = km_hdtb1.hdtb_id
                                                                and hdtb1.tthd_id = 6 and to_number(to_char(hdtb1.ngay_ht,'yyyymm')) = 202405 		--thang n
			 ...
			 select hdtb_id,   count(*) from temp_muccuoctb_hdtb group by hdtb_id having count(*)>1;
                ...
                update ptm_codinh_202405 a set muccuoc_tb ='';
                ...
                update ptm_codinh_202405 a 
                    ...
                    commit;
--                        
				select * from ptm_codinh_202405 where thuebao_id = 8278567;
            ...
            create table temp_tien_dvgt_theohdtb as
                with b as (select hd.hdtb_id, km.chitietkm_id, km.cuoc_sd, km.tyle_sd, km.tien_sd, ctkm.dichvugt_id 
                                                ...
                                                where hd.hdtb_id=km.hdtb_id and km.chitietkm_id=ctkm.chitietkm_id and km.hieuluc=1 and ttdc_id = 0 --and thang_bd>=202403
                                                    ...
                                                    and to_number(to_char(hd.ngay_ht,'yyyymm')) >= 202404		--thang n-1
									)
						, c as (select a.hdtb_id, a.ma_tb, b.chitietkm_id, d.noidung soluong, d.cuoc_sd cuoc_sd_goc, b.cuoc_sd cuoc_giam_1donvi, b.tyle_sd, b.tien_sd                        
						    ...
						    where e.phanvung_id=28 and a.hdtb_id = d.hdtb_id and d.dichvugt_id = e.dichvugt_id                                              
								  ...
								  and to_number(to_char(a.ngay_ht,'yyyymm')) >= 202404		--thang n-1
						    )
                    select hdtb_id, sum(cuoc_sd) cuoc_sd
                        from  c 
--				    where hdtb_id = 25286500
				    group by hdtb_id
                ;     
		  ...
		  ;
                    create table temp_tien_dvgt_theothuebao as
                            with b as (select hd.thuebao_id, km.chitietkm_id, km.cuoc_sd, km.tyle_sd, km.tien_sd, ctkm.dichvugt_id 
                                                        from css.v_khuyenmai_dbtb km, css.v_ctkm_dvgt ctkm,  css.v_hd_thuebao hd
                                                        where hd.hdtb_id=km.hdtb_id and km.chitietkm_id=ctkm.chitietkm_id and km.hieuluc=1 and km.ttdc_id = 0--and thang_bd>=202403
                                                            and nvl(km.tyle_tb,0)=0 and nvl(km.cuoc_tb,0)=0 and nvl(km.tien_tb,0)=0
                                                            and to_number(to_char(hd.ngay_ht,'yyyymm')) >= 202404	--thang n-1
										)
								, c as (select a.hdtb_id, a.ma_tb, b.chitietkm_id, d.noidung soluong, d.cuoc_sd cuoc_sd_goc, b.cuoc_sd cuoc_giam_1donvi, b.tyle_sd, b.tien_sd                        
...
--                    update ptm_codinh_202405 set tien_dvgt=''
				;
                    update ptm_codinh_202405 a 
...
-- tien_tbi: 
--                    UPDATE ptm_codinh_202405 SET tien_tbi=''
				;
                    MERGE INTO ptm_codinh_202405 a
                    USING (SELECT cttbi.hdtb_id, tbi_lhtb.loaitb_id, SUM(CASE WHEN tien_tratruoc>0 THEN cttbi.soluong*cttbi.tien_tratruoc ELSE cttbi.soluong*cttbi.tien END) tien_tbi
                                    ...
                                    WHERE cttbi.loaitbi_id=tbi.loaitbi_id AND tbi.loaitbi_id=tbi_lhtb.loaitbi_id AND lhtb.loaitb_id=tbi_lhtb.loaitb_id
                                                        ...
                                                        AND EXISTS(SELECT 1 FROM ptm_codinh_202405 
...
-- tien_dnhm, tien_camket, tien_sodep: da di chuyen thuc hien truc tiep tren DATAGUARD line 414
 -- Dat coc sau lap moi trong thang, cung ma_tiepthi voi hs lap moi:
		...
		select * from ttkd_hcm.temp_datcoc@dataguard;
...
--                                                                    group by a.hdtb_id, a.thuebao_id, a.thang_bddc,a.thang_ktdc     
                                ...
                                , datcoc as (select max(hdtb_id) hdtb_id, thuebao_id, min(thang_bddc) thang_bddc, min(thang_ktdc) thang_ktdc
                                                        ...
                                                        from datcoc_goc 
											 where datcoc_csd > 0 and thang_bddc >= 202405
                                                        group by  thuebao_id
                                                        )
...
--                where exists(select 1 from ptm_codinh_202405 where datcoc_csd is null and thuebao_id=a.thuebao_id and ma_tiepthi=a.ma_nv)  
--                group by  thuebao_id having count(*)>1)
--                ;
 
            --them cot hdtb_id_datcoc ---> rui ro --> thu check theo tien hop dong khoan muc tra truoc
            alter table ptm_codinh_202405 add hdtb_id_datcoc number
		  ;
            ...
            update ptm_codinh_202405 a 
                ...
                where datcoc_csd is null 
                    and exists(select 1 from temp_datcoc b
                                        where datcoc_csd>0 and thuebao_id=a.thuebao_id and ma_nv=a.ma_tiepthi)
                    and ngay_bbbg=(select max(ngay_bbbg) from ptm_codinh_202405 where thuebao_id=a.thuebao_id )
                    ;
                    
            ---recover datcoc_csd updated
--                update ptm_codinh_202405 set datcoc_csd = null 
----				select * from ttkd_bct.ptm_codinh_202405
--				where hdtb_id_datcoc is not null;
            commit;
            
--            select a.*, b.ma_tiepthi , b.DATCOC_CSD, b.SOTHANG_DC, b.hdtb_id_datcoc
--            from temp_datcoc a, ptm_codinh_202405 b
--            where a.thuebao_id=b.thuebao_id
            ;
            
------ thoihan_id
            update ptm_codinh_202405 set thoihan_id=1 
	...
	-- Cuoc phat sinh:    ---chuyen len dataguard
				...
				select distinct a.* from bcss.v_ct_no a
				    where phanvung_id = 28 and ky_cuoc = 20240501 
					   ...
					   and exists(select 1 from ptm_codinh_202405 where thuebao_id=a.thuebao_id)
					   ;
--				create index temp_ps_tbid on temp_ps (thuebao_id);
				
				alter table ptm_codinh_202405 add dthu_ps number(12);
--				update ptm_codinh_202405 a set dthu_ps=''
				;
				update ptm_codinh_202405 a 
...
-- Dthu goi:
				alter table ptm_codinh_202405 
				    add (dthu_goi_goc number(12), dthu_goi number(12), dthu_goi_ngoaimang number(12))
				;
				update ptm_codinh_202405 bc set dthu_goi_goc='', dthu_goi='', dthu_goi_ngoaimang=''
				
				;
				--  select * from ttkd_bct.ptm_codinh_202404;
				
				update ptm_codinh_202405 bc set dthu_goi_goc=
				    (case
...
------- Tinh lai dthu_goi_goc cho thue bao vua lap moi co dinh vua chuyen sang Siptrunk trong thang:
			...
			update ptm_codinh_202405 a 
					set (dthu_goi_goc, ghi_chu)=
									   (select 250000-dthu_goi_goc, 'dthu goi=250000-dthu goi cua hs lap moi (lap moi, chuyen doi cung thang)' 
                                            from ptm_codinh_202405 b
										  where loaihd_id=1 and thuebao_id=a.thuebao_id                 
											  and exists (select 1 from ptm_codinh_202405 where thuebao_id=b.thuebao_id and loaihd_id=6)
											  )
		    where loaitb_id=77 and loaihd_id=6
					  and exists (select 1 from ptm_codinh_202405 where thuebao_id=a.thuebao_id and loaihd_id<>6)
								
				;
			
			update ptm_codinh_202405 set dthu_goi=dthu_goi_goc
			...
			update ptm_codinh_202405 a 
					    set dthu_goi_ngoaimang =
											 (select sum(nogoc) from bcss.v_tonghop
												where ky_cuoc = 20240501 --thang n
...
-- ISDN 30B+D lon hon hoac bang 30 kenh/1 luong tu thang truoc => dthu goi =10K
			update ptm_codinh_202405 a 
			    ...
			    where loaitb_id in (15,17) 
				   and exists (select 1 from (select thuebao_cha_id, count(*) sl_socon from tinhcuoc.v_dbtb a
																 where ky_cuoc = 20240401 ---thang n-1
																				and exists (select 1 from ptm_codinh_202405
			...
			update ptm_codinh_202405 a 
				   set dthu_goi_goc = (select cuoc_tggoc + cuoc_ipgoc 
                                                from bcss.v_thftth 
                                                where phanvung_id=28 and ky_cuoc = 20240501 --thang n
                                                        and ma_tb = a.ma_tb)
...
---da xong 2/6 20g15
 ...
 ---move ve TTKDDB
		  create table ttkd_bct.ptm_codinh_202405 as select * from ttkd_hcm.ptm_codinh_202405@dataguard;
		  create index ttkd_bct.idx_ptm_hdtbid on ttkd_bct.ptm_codinh_202405 (hdtb_id);
		  -- lydo_khongtinh_luong: Update ky luong T3/24
						alter table ttkd_bct.ptm_codinh_202405 add lydo_khongtinh_luong varchar2(200)
						;
						update ttkd_bct.ptm_codinh_202405 a
																			  ...
																			  select hdtb_id
																						   ...
																						   ,case when (upper(ma_tiepthi)='DL_CNT' or upper(ma_tiepthi)like'GTGT_%' 
																													...
																													or upper(ma_nguoigt)like'DAILY_%' or upper(ma_nguoigt)like'DL_%')
																															then '; Phat trien qua Dai ly, chi tinh bsc cho Phong' 
																								end kq5
																						   ,case when thoihan_id=1 and ((loaitb_id in (58,59) and nvl(tien_dnhm,0)<2000000) or (dichvuvt_id in (7,8,9) and nvl(tien_dnhm,0)<3000000))
																												then '; TB ngan han nhung khong thu tien DNHM dung qui dinh' 
																								 end kq6
																						  ,case when loaihd_id=41 and loaitb_id=153 and nvl(sothang_dc,0)<6 
																											then '; Ghi nhan dthu khi gia han goi>=6 thang' 
																									end kq7       
																			  from ttkd_bct.ptm_codinh_202405 b
...
--									where (thuebao_id, hdtb_id) in (select thuebao_id, hdtb_id from hocnq_ttkd.ptm_xuly_khieunai_202404)
                    ...
                    -- nv ptm:   
                    alter table ttkd_bct.ptm_codinh_202405
                        add (manv_ptm varchar2(20), tennv_ptm varchar2(100)
							, ma_pb varchar2(20), ten_pb varchar2(100), ma_to varchar2(20), ten_to varchar2(100)
							, ma_vtcv varchar2(20), loainv_id number, ten_loainv varchar2(100), loai_ld varchar2(100)
							, manv_hotro varchar2(20), tyle_hotro number(5,2), tyle_am number(5,2),nhom_tiepthi number )
				...
				-- select loaitb_id thang, hdtb_id, thuebao_id, loaitb_id, ma_tiepthi, pbh_nhan_id, nhanvien_id, datcoc_csd, ghi_chu  from hocnq_ttkd.ptm_xuly_khieunai_BHOL_202404
						 select 202405 thang, hdtb_id, thuebao_id, loaitb_id, ma_tiepthi, pbh_nhan_id
				...
				update ttkd_bct.ptm_codinh_202405 a 
				    ...
				    -- select ma_tiepthi from ttkd_bct.ptm_codinh_202405 a
				    where loaitb_id = 288 and ma_tiepthi is null 
					   and pbh_nhan_id=2941 and nhanvien_id=1077 and datcoc_csd>0
					   ; 
...
--			delete from dm_daily_khdn where thang='202403' ;
			select * from  ttkd_bsc.dm_daily_khdn where thang = 202405;
			...
			insert into ttkd_bsc.dm_daily_khdn --(ma_daily,ten_daily,manv_qldaily,ma_pb,thang,thang_kyhd,ma_to)
			    ...
			    select 202405, ma_daily, ten_daily, manv_qldaily, ma_to, ma_pb, ma_vtcv, ten_vtcv, thang_kyhd, ghichu
			  ...
			  --****-- Dai ly moi:
					insert into ttkd_bsc.dm_daily_khdn (ma_daily,ten_daily,ma_pb, ma_to, manv_qldaily, thang, thang_kyhd, loai_hopdong )
					    select distinct ma_tiepthi ma_daily, (select upper(ten_nv) from admin_hcm.nhanvien_onebss where ma_nv = a.ma_tiepthi) ten_daily,
								 ma_pb, ma_to, ma_nv, 202405, 202405, 'DAI LY MOI' loai_hopdong
					    from 
						   (
				...
				-- Gan ma pb doi voi thue bao phat trien qua Dai ly :
					...
					update ttkd_bct.ptm_codinh_202405 a
					    ...
					    --- select pbh_nhan_id, pbh_nhan_goc_id, donvi_tt_id, ma_pb, ten_pb from ttkd_bct.ptm_codinh_202405 a
					...
					;
						   ...
						   select * from admin_Hcm.donvi where donvi_id in (284316,2943,11352,2942,2947,2944,2945,284317,2946, 2948,11563,11564, 283427, 11298);
					...
					update ttkd_bct.ptm_codinh_202405 a
					    ...
					    --- select ma_nguoigt, (select ma_dv from admin_Hcm.donvi where donvi_id = a.pbh_nhan_id) from ttkd_bct.ptm_codinh_202405 a
			...
			-- PTTT:
				update ttkd_bct.ptm_codinh_202405 a 
				    ...
				    -- select (select ten_dv from admin_Hcm.donvi where donvi_id = 11298) from ttkd_bct.ptm_codinh_202405 a
				    where chuquan_id in (145,266) and ma_pb is null 
			...
			commit;
				...
				update ttkd_bct.ptm_codinh_202405 a 
				    ...
				    ;
				    select manv_hotro, ma_duan_banhang, tyle_hotro, tyle_am, hdtb_id, thuebao_id from ttkd_bct.ptm_codinh_202405 a where ma_duan_banhang is not null and manv_hotro is not null 
			;
				-- TTVT ptm nhung ko co ma_tiepthi hoac ma_tiepthi sai/nghi viec: 
				update ttkd_bct.ptm_codinh_202405 a
				    set ma_pb = 'HCM15'
				    -- select ma_tiepthi, ma_pb, pbh_nhan_id, donvi_tt_id, donviql_tt_id from ttkd_bct.ptm_codinh_202405 a
					...
					update ttkd_bct.ptm_codinh_202405 a
					    set  
							  ma_pb = (select ma_dv from admin_Hcm.donvi where donvi_id = a.pbh_nhan_id)
							  , ten_pb = (select ten_dv from admin_Hcm.donvi where donvi_id = a.pbh_nhan_id)
					     -- select (select ten_dv from admin_Hcm.donvi where donvi_id = a.pbh_nhan_id) from ttkd_bct.ptm_codinh_202405 a
...
-- manv_hotro:  chua chay
				...
				*/
				...
				update ttkd_bct.ptm_codinh_202405 a set manv_hotro='', tyle_hotro='', tyle_am='' where ma_duan_banhang is not null
				;	
...
---Test ma_da --> book 1 PS lam nhieu cong doan
				...
				--  select thang_luong, ma_duan_banhang from ttkd_bsc.ct_bsc_ptm a where thang_ptm = 202404 and  ma_duan_banhang is not null and manv_hotro is null ;
					MERGE into ttkd_bct.ptm_codinh_202405 a
							using (
												with 
													...
													, t as	 (select c.manv_presale_hrm, c.tyle/100 tyle_hotro, decode(tyle_am,0,1,c.tyle_am/100) tyle_am, d.loaitb_id_obss, b.ma_yeucau, b.ma_dichvu, c.tyle_nhom
																 ...
																 from yc_dv b, ttkdhcm_ktnv.amas_booking_presale c, ttkdhcm_ktnv.amas_loaihinh_tb d
																					where rnk = 1 and b.ma_yeucau=c.ma_yeucau and b.id_ycdv=c.id_ycdv and b.ma_dichvu = d.loaitb_id
					...
					---  select manv_hotro, tyle_hotro, tyle_am, ma_duan_banhang, loaitb_id, ma_gd from ttkd_bct.ptm_codinh_202405 a
...
---- kiem tra trung
			    ...
			    thang_bddc, datcoc_csd, phongbh_nhan, loaitb_id,
					  ...
					  from ttkd_bct.ptm_codinh_202405
					  where (ma_gd, thuebao_id) in (select ma_gd, thuebao_id from ttkd_bct.ptm_codinh_202405 group by ma_gd, thuebao_id having count(*)>1) 
...
-- Kiem tra dthu goi:
				select a.ma_gd, ma_tb, datcoc_csd, dthu_goi, dthu_ps
				    from ttkd_bct.ptm_codinh_202405 a
				    where (thuebao_id, ma_tb, kieuld_id) in (select thuebao_id, ma_tb, kieuld_id from ttkd_bct.ptm_codinh_202405 group by thuebao_id, ma_tb, kieuld_id having count(*)>1) 
				    order by thuebao_id, ngay_bbbg;
				...
				select * from ttkd_bct.ptm_codinh_202405  a
...
--				    set ngay_td_kytruoc = (select ngay_td from tinhcuoc.v_dbtb@dataguard where ky_cuoc=20240301 and ngay_td is not null and thuebao_id=a.thuebao_id)
...
--				commit;
				...
				drop table tailap_202405 purge
				;
				select * from ttkd_bct.tailap_202405;
				create table ttkd_bct.tailap_202405 as 
				    select a.* from ttkd_bct.ptm_codinh_202405 a
					   where a.kieuld_id in (96,13089) and duoctinh_ptm=1
						  and hdtb_id=(select max(hdtb_id) from ttkd_bct.ptm_codinh_202405 
