...
;
drop table hocnq_ttkd.x_br_ck_202405;
create table hocnq_ttkd.x_br_ck_202410 as
with tbl as
			(select thuebao_id, tien_td, 'km_dbtb' nguon, ngay_sd 
					from css_hcm.khuyenmai_dbtb where hieuluc = 1 and ttdc_id = 0 and tien_td > 0 and to_number(to_char(ngay_sd, 'yyyymm')) >= 202409
				union all
				select thuebao_id, tien_td, 'db_datcoc', ngay_cn from css_hcm.db_datcoc where hieuluc = 1 and ttdc_id = 0 and tien_td > 0 and to_number(to_char(ngay_cn, 'yyyymm')) >= 202409
			)
		, dthu_skm as
			(
...
from ttkd_bsc.ct_bsc_ptm db
			left join ttkd_bct.ptm_codinh_202410 a on db.hdtb_id = a.hdtb_id
...
where db.thang_ptm = 202410 and db.dichvuvt_id = 4 and db.dthu_goi > 0 and db.thang_tldg_dt > 0; and db.ma_tb in ('plan0324','nvm24','460phuoc')
;
